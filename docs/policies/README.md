...
- [`citation-cordon.md`](citation-cordon.md) — the machine-enforced, forget-proof Proved-vs-Cited invariant: `@[cited]` axioms in located `…Cited.lean` files, `collectAxioms − foundational − @[cited] = ∅` as the per-root `#assert_banked_clean` gate (in `AxCheck.lean`) + the `scripts/cordon` source grep (location + `native_decide` ban), `#audit_cited` in-file, the adversarial fixtures.
