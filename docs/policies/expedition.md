...
- the **bedrock check is a decorrelated reviewer function** ([`review.md`](review.md); the dedicated `hardener` seat was retired 2026-07-15 — its function folded into the reviewer + the elder's right-extension channel) — a principles/taste pass (distinct from the correctness audit) at each gate and at step-back, surfacing overclaims and holes-vs-extensions; *right-extension* findings (a gap that is the *right* extension → Just-Do-It if within reach, else roadmap) route to the elder's compass. The controller integrates the findings and holds precedence; a critical taste finding floors a review-to-equilibrium loop like any reviewer finding.
