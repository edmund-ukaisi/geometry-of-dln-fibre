...
-- LIVE-leaf atom above): the E-block re-pivot chart `eDeepRank0NodeChartGen` bundle, its box
-- divergence `routeMCore_box_diverges_eDeepRank0Gen`, and the `∀ _ : 2 ≤ L`-shaped consumer
-- `interiorLiveGen_hInterior_of_deepRank_zero`. Chart boundary `k` aligned to the InteriorDrop
-- pivot `p* = k+1` (the carrier of the row/col drops). Axiom footprint
...
-- [merge dev 2026-07-20] dev-unique imports unioned in (determinantal/Core/cites); dev cordon-module imports excluded (retired stage 2):
...
-- [expedition aoyagi-engine 2026-07-23] the e-abstraction kill-witness (Fbad: he_lin provably
-- insufficient for the base atom) + the reusable submult interval-composition + innermost peel
...
-- Rung 5b (L6), fork #138 (B) decomposed — TWO modules:
-- GWrapDecomp (STAGE 1): the decomposition identity gWrap = sigmaPiv∘shearH∘permP∘blowA0∘blowA1
--   (`gFaithful_decomp` funext — crux transfers free, banked hideal untouched).
-- ChartJac (STAGE 2/3, imports GWrapDecomp): the chart Jacobian |jacDet gWrap| = |u0|⁷|u1|³|u20|⁸ via
...
-- NodeCover334 (R3, COVER-ONLY de-risk): keptSubsetBornFan_cover_334 = the born-fan over the KEPT center
--   {0,1,2,3,20} (cleared 4..7 spectators) covers closedBall via CoverFold.bornSiblings_union_covers_closed +
--   Corank2FanCover334.shearH_covers. W3-clean (born, NOT the K-orbit skeleton); reviewer SURVIVED (sound,
--   axiom-clean, full, honest). RECONTEXTUALIZED (#183, elder-ruled): this is NOT buildTree's node — buildTree
