...
-- Forced axiom gate: the V-lower wire rests only on `[propext, Classical.choice, Quot.sound]`
-- (hideal_bwd-free / exists_coreResolution-free — the honest F10/(A) bypass; #184 F1-gate).
#assert_banked_clean_batch [rlctAt_ge_iInf_threshold_of_sandwich_cover,
  mem_localAdmissible_of_sandwich_lt, integrableAtFilter_of_sandwich]
