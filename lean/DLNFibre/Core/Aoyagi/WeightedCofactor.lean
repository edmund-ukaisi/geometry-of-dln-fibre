...
def weightedCofactor {ι X : Type*} [Fintype ι] (c : X → ι → ι → ℝ) (R : X → Matrix ι ι ℝ) :
    X → Matrix ι ι ℝ :=
  fun x ↦ Matrix.of (fun a d ↦ c x a d * R x a d)

@[simp] theorem weightedCofactor_apply {ι X : Type*} [Fintype ι] (c : X → ι → ι → ℝ)
    (R : X → Matrix ι ι ℝ) (x : X) (a d : ι) :
...
theorem diagonal_mul_eq_weightedCofactor_mul_diagonal {ι X : Type*} [Fintype ι] [DecidableEq ι]
    (b : X → ι → ℝ) (c : X → ι → ι → ℝ) (R : X → Matrix ι ι ℝ) {V : Set X}
...
theorem weightedCofactor_transport {ι κ X : Type*} [Fintype ι] [DecidableEq ι]
    (b : X → ι → ℝ) (c : X → ι → ι → ℝ) (R : X → Matrix ι ι ℝ) (Hpre Hnext : X → Matrix ι κ ℝ)
...
def UnitLower {ι : Type*} [LinearOrder ι] (A : Matrix ι ι ℝ) : Prop :=
  (∀ a, A a a = 1) ∧ (∀ a d, a < d → A a d = 0)

/-- **`Q̂` is unipotent** (unit-lower) when `R` is and the ratio is `1` on the diagonal. -/
theorem weightedCofactor_unitLower {ι X : Type*} [Fintype ι] [LinearOrder ι]
    (c : X → ι → ι → ℝ) (R : X → Matrix ι ι ℝ) (x : X)
...
theorem weightedCofactor_eq_one {ι X : Type*} [Fintype ι] [DecidableEq ι]
    (c : X → ι → ι → ℝ) (R : X → Matrix ι ι ℝ) (x₀ : X)
