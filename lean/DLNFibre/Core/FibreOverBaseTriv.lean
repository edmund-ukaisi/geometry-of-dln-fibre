...
*geometric* base — additionally needs the GLOBAL gluing of the per-chart data into one fibration
morphism (R1); the LOCAL target-side cocycle is landed (P2.c/f/g) and projection compatibility is
now CLOSED in `Core.FibreProjectionCompat`. See Scope.)
...
* `chartDsig_flat_over_schurLoc` / `chartDsigAt_flat_over_schurLoc` — the top-left / per-pivot
  **flatness over the in-chart base direction `SchurLoc`** `Module.Flat SchurLoc (Away (chartDsig[At]))`,
  ...
  payoff additionally needs the GLOBAL gluing into one fibration morphism (R1) — the LOCAL
  target-side cocycle is landed P2.c/f/g and projection compatibility is now CLOSED in
  `Core.FibreProjectionCompat` — see Scope).

## Scope (honest)

This is the CHARTWISE over-base trivialization + flatness, at every pivot, over the **named**
`SchurLoc`-algebra structure (`schurToDsigAt`).
(i) **projection compatibility** — that `schurToDsigAt : SchurLoc → Total` agrees with the pullback of
`mult`'s projection from the target/base rank-chart (after precomposition with `localizeSchur`) — is now
CLOSED (R5, `Core.FibreProjectionCompat`). So `chartDsigAt_flat_over_schurLoc` is flatness over a
`SchurLoc` algebra now certified to agree with the geometric `mult`-projection (chartwise).
(`Spec(sweepSigmaRing)` is the source/total; `SchurLoc` is only the base direction.)
(ii) R1 — the GLOBAL gluing of the per-chart data into a single `Flat π` / `FiberBundle` morphism
over all of `rankROpen` — stays open. The LOCAL target-side overlap cocycle (pairwise round-trip
P2.c, triple cocycle P2.f, restricted-2-fold naturality/cocycle P2.g) is landed
(`Core.FibreTargetOverlap`); the GLOBAL assembly is the ONLY remaining bundle residual, NOT claimed
here.
...
`SchurLoc` (the S3 `Flat π` payoff additionally needs the GLOBAL gluing into one fibration morphism,
R1; the LOCAL target-side cocycle is landed P2.c/f/g and projection compatibility is now CLOSED in
`Core.FibreProjectionCompat`). -/
...
/-- **The top-left chart total ring is FLAT over `SchurLoc` (chartwise).**
...
`FibreFlatness.standardFibreModel_flat`. The S3 `Flat π` payoff additionally needs the GLOBAL gluing
into one fibration morphism (R1) — the LOCAL target-side cocycle is landed (P2.c/f/g) and projection
compatibility is CLOSED (R5, `Core.FibreProjectionCompat`). -/
...
/-- **The per-pivot chart total ring is FLAT over `SchurLoc` (chartwise, every pivot).**
...
`SchurLoc` (the structure map `schurToDsigAt`). The S3 `Flat π` payoff additionally needs the GLOBAL
gluing into one fibration morphism (R1) — the LOCAL target-side cocycle is landed (P2.c/f/g) and
projection compatibility is CLOSED (R5, `Core.FibreProjectionCompat`). -/
