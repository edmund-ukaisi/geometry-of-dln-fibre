...
`sorry` or a global axiom. The deep trivialization `e` itself is **not** constructed in this module;
this is the route-(b) *reducedness* chain (chart ring reduced ⟹ fibre ideal radical), which fed the
abandoned route-(b) reducedness wall.

**Off the critical path (route-β superseded this).** The final unconditional
`codim (fibre d B) = C + δ` (`Core.FibreCodimFinal`) was reached by **route-β**: the localized chart
`AlgEquiv` `e` is BUILT in `Core.ChartLocalizedAlgEquiv`, and the codimension identity rides on a
`ringKrullDim` chart equality (`Core.ChartSweepWiring`) plus the closure bridge
(`Core.ClosureBridge`) — it does **not** route through the reducedness chain in this file. So the
deep product iso / `IsReduced S` reducedness wall this module's conditional theorems were built
against is no longer on the critical path; these theorems remain a self-contained, clearly-labelled
*conditional* result about that route. (Codex's recommended build order for the route-(b) `e` is
recorded in the thread's `codex/algequiv-answer.md`.)
...
/-! ## The route-(b) chain through the trivialization (the trivialization `e` as a hypothesis) -/
...
content is the *shape* of the contract; the genuine localized chart `e` is built by route-β in
`Core.ChartLocalizedAlgEquiv`, off this route-(b) chain's critical path.) -/
