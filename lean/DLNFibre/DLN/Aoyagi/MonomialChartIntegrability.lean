...
strict one-dimensional inequalities `2*t*k_i < h_i + 1`, and comparison
theorems that consume supplied positive-box loss and density/prior bounds.

It does not construct Aoyagi charts, prove the supplied loss or density/prior
bounds for those charts, handle signed boxes, handle endpoints or divergent
sides, produce a normal-crossing certificate, or extract pole order/RLCT.
-/
