...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- This theorem is a narrow post-processing wrapper around the preceding
-- p.13 finite-integral bridge and the coordinate-prior chart-piece
-- restriction cleanup.
/-- Pushed original coordinate priors feed into the p.13 readback finite
integral socket with the coordinate prior restricted directly to the chart-piece
preimage.

This is the direct chart-piece form of
...
It first applies that theorem on the larger set `sourceChart '' V`, then uses
the finite-dimensional restriction cleanup to replace the final restricted
image measure by the pushforward of the coordinate prior restricted to
`toEdge ⁻¹' chartPiece`.
...
This theorem does not prove source coverage, Haar transport, normal crossings,
pole order, or RLCT extraction. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_p13RegularCoordinates_lt_top_of_case2PassiveThetaEndpointSourceChart_map_originalCoordinatePrior_restrict_chartPiece_preimage_of_detHaar_restrict_le_smul_endpointTopologyTuple_eventually_sourceImageDensity_comp_sourceChart_lower_priorDensity_comp_sourceChart_upper
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
      ...
      (by
        simpa [center, pivotNext, signedBox, weightedBox, passiveSource,
          ...
          exists_open_lintegral_ofReal_loss_rpow_neg_p13RegularCoordinates_lt_top_of_case2PassiveThetaEndpointSourceChart_map_originalCoordinatePrior_restrict_sourceChart_preimage_chartPiece_of_detHaar_restrict_le_smul_endpointTopologyTuple_eventually_sourceImageDensity_comp_sourceChart_lower_priorDensity_comp_sourceChart_upper
            W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
            z₀ hdet₀ hpivot₀ sourceData sourceImageDensity passiveMeasure
            hpassive_lt_top (ν := ν) (loss := loss) Rres hR hc ht hRres
            hcrit (ε := ε) coordDensity (Kprior := Kprior)
            hsource_eventually hprior_eventually hloss) with
    ⟨W, hWopen, hz₀W, hpackage⟩
  refine ⟨W, hWopen, hz₀W, ?_⟩
  intro sourceImageBase sourceImageMeasure hsourceChart hsourceImageDensity
    Csrc hsourceImageDensity_le hCsrc
  rcases
      hpackage hsourceChart hsourceImageDensity (Csrc := Csrc)
        hsourceImageDensity_le hCsrc with
    ⟨U, hUopen, hbaseU, V, hVopen, hz₀V, hleftV, hsource_injV,
      hsource_contOnV, hsource_imageV, himage_p13V, hfinal⟩
  have hleftV' : ∀ z ∈ V, readback (sourceChart z) = z := by
    intro z hz
    simpa [sourceChart, readback, case2PassiveThetaEndpointSourceChart,
      Case2PassiveTheta.A1passive, Case2PassiveTheta.F2,
  ...
  have hpiece_sourceStratum : chartPiece ⊆ sourceStratum := fun E hE =>
    (hchartPiece_sub_local hE).2
  have hfinite_big :=
    hfinal.2 rawHaar (Cdet := Cdet) hdet_dom hCdet hε_ne_zero
      hε_ne_top hdensity (chartPiece := chartPiece) hchartPiece_meas
      hpiece_U hpiece_sourceStratum hchartPiece_sub_imageVW
  have hchartPiece_sub_imageV : chartPiece ⊆ sourceChart '' V := by
    intro E hE
    rcases hchartPiece_sub_imageVW hE with ⟨theta, htheta, rfl⟩
    exact ⟨theta, htheta.1, rfl⟩
  simpa [d, b, toEdge] using
    lintegral_prod_map_canonicalCoord_symm_tupleToEdgeFamily_originalCoordinatePrior_restrict_preimage_chartPiece_lt_top_of_lintegral_prod_restrict_preimage_superset_restrict_chartPiece_lt_top
      (V := reverseVertex W₂) (d := d) b (density := coordDensity)
      (Cset := sourceChart '' V) (chartPiece := chartPiece)
      hsource_imageV hchartPiece_meas hchartPiece_sub_imageV hdensity ν
      hfinite_big
