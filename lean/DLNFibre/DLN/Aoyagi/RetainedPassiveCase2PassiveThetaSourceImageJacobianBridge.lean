...
set_option linter.style.longLine false in
/-- Source-level chart-piece handoff from readback-side domination.

In addition to the existing measurability and local boundedness hypotheses for
the chart-produced density `sourceImageDensity`, it is enough on an arbitrary
measurable piece inside the returned local source to prove readback-side
domination by `(baseJ.withDensity (sourceImageDensity ∘ sourceChart)).restrict W`.
The source-side domination required by the finite-product socket is then
recovered from the pointwise right-inverse identity on the piece.

This is still a consumer theorem: the readback domination and right-inverse
hypothesis are explicit.  It does not prove that a natural chart image is
measurable or contained in the returned `sourceLocal`; `chartPiece` and its
containment are supplied by the caller.  It does not identify an original DLN
prior, prove chart-image equality, source-rank coverage, Haar transport,
normal crossings, pole order, or RLCT. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_p13RegularCoordinates_lt_top_of_case2PassiveThetaEndpointSourceChart_sourceImage_withDensity_externalSourceMeasure_restrict_chartPiece_map_readback_le_smul_coordinateSourceMeasure_jacobian_passiveProductMeasure_finiteMass_sourceStratum_bounds
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  have hsourceLocal_meas : MeasurableSet sourceLocal := by
    exact hUopen.measurableSet.inter hsourceStratum_meas
  have hcoordinate_restrict_eq :
      coordinateSourceMeasure.restrict W =
        (baseJ.restrict W).withDensity
          (fun z ↦ sourceImageDensity (sourceChart z)) := by
    simp [coordinateSourceMeasure, sourceDensity,
      restrict_withDensity hWopen.measurableSet]
  have hsourceChart_coordinate :
      AEMeasurable sourceChart (coordinateSourceMeasure.restrict W) := by
    rw [hcoordinate_restrict_eq]
    exact hsourceChart.mono_ac (withDensity_absolutelyContinuous _ _)
  have hmap_coordinate :
      Measure.map sourceChart (coordinateSourceMeasure.restrict W) =
        sourceImageMeasure := by
    have hmap :
        Measure.map sourceChart
            ((baseJ.restrict W).withDensity
              (fun z ↦ sourceImageDensity (sourceChart z))) =
  ...
  refine ⟨?_, ?_⟩
  · simpa [sourceLocal]
  · intro chartPiece hchartPiece_meas hchartPiece_sub hright externalSourceMeasure
      Cpull hreadback hreadback_dom hCpull
    have hsource_le :
        externalSourceMeasure.restrict chartPiece ≤
          Cpull • sourceImageMeasure.restrict sourceLocal := by
      have hpiece_le :
          externalSourceMeasure.restrict chartPiece ≤
            Cpull •
              (Measure.map sourceChart
                (coordinateSourceMeasure.restrict W)).restrict sourceLocal :=
        measure_restrict_piece_le_smul_map_restrict_of_map_readback_restrict_piece_le_smul
          sourceChart readback externalSourceMeasure coordinateSourceMeasure W
          chartPiece sourceLocal Cpull hchartPiece_meas hchartPiece_sub
          hsourceChart_coordinate hreadback
          (fun E hE ↦ (hright E hE).2) hreadback_dom
      rw [hmap_coordinate] at hpiece_le
      exact hpiece_le
    have hprod_le :
        (externalSourceMeasure.restrict chartPiece).prod ν ≤
          Cpull • (sourceImageMeasure.restrict sourceLocal).prod ν :=
      prod_le_smul_prod_of_le_smul_left (η := ν) hsource_le
    exact
      (by
        simpa [sourceLocal] using
