...
/-!
# `DLN.Aoyagi.Corank2OverVanishB_20_7_6` — pattern-B over-vanishing leaf `(20,7,6)`

The pattern-B per-type deliverables for the over-vanishing leaf `(p1,p2,p3) = (20,7,6)`, mirroring
...
`(20,7,6)` is NON-coinciding, `vm = u6 · u7 · u20`; its `φ` is genuinely QUADRATIC (max degree 2), quadratic cover atom. The 8
...
namespace DLNFibre.DLN.Aoyagi.OverVanishB_20_7_6

/-! ## §0 — the leaf and its composite -/

/-- The over-vanishing leaf index `(p1,p2,p3) = (20,7,6)`. -/
def idxCanon : Idx := ⟨⟨20, by decide⟩, ⟨7, by decide⟩, ⟨6, by decide⟩⟩

/-- The leaf composite `gFlat idxCanon`. -/
noncomputable def gCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun w =>
  blockBlowupMap S1 20 (nativeChart1 20
    (blockBlowupMap (sigmaC1Fs 20) 7 (blockBlowupMap (sigmaC2Fs 20) 6 w)))
...
def phiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun u i =>
  ...
  else if i = 17 then -(u 11 * u 2 + u 13 * u 6)
  else if i = 18 then -(u 11 * u 3 + u 14 * u 6)
  else if i = 19 then -(u 11 * u 4 + u 15 * u 6)
  else 0
...
theorem phiCanon_read (u v : Fin 21 → ℝ) (h : ∀ i, keepCanon i → u i = v i) :
  ...
  funext i
  simp only [phiCanon]
  split_ifs <;> simp only [h0, h2, h3, h4, h6, h11, h12, h13, h14, h15]
...
/-- `∏_d (u d)^(vmExpCanon d) = u 6 * u 7 * u 20`. -/
theorem prod_vmExpCanon (u : Fin 21 → ℝ) : (∏ d, (u d) ^ vmExpCanon d) = u 6 * u 7 * u 20 := by
  ...
  rw [Finset.prod_ite, Finset.prod_const_one, mul_one,
    show Finset.filter (fun d : Fin 21 => d = 6 ∨ d = 7 ∨ d = 20) Finset.univ = {6, 7, 20} from by decide,
...
theorem phiCanon_norm_bound {x : Fin 21 → ℝ} {r : ℝ} (hr : 0 ≤ r) (hx : ‖x‖ ≤ r) :
  ...
  rw [Real.norm_eq_abs]
  by_cases h16 : i = 16
  · rw [h16, show phiCanon x 16 = -(x 0 * x 11 + x 12 * x 6) from rfl]; exact slot 0 11 12 6
  by_cases h17 : i = 17
  · rw [h17, show phiCanon x 17 = -(x 11 * x 2 + x 13 * x 6) from rfl]; exact slot 11 2 13 6
  by_cases h18 : i = 18
  · rw [h18, show phiCanon x 18 = -(x 11 * x 3 + x 14 * x 6) from rfl]; exact slot 11 3 14 6
  by_cases h19 : i = 19
  · rw [h19, show phiCanon x 19 = -(x 11 * x 4 + x 15 * x 6) from rfl]; exact slot 11 4 15 6
...
end DLNFibre.DLN.Aoyagi.OverVanishB_20_7_6
