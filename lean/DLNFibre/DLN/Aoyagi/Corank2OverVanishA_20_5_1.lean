...
/-!
# `DLN.Aoyagi.Corank2OverVanishA_20_5_1` — the per-type CONCRETE facts, leaf `(20,5,1)`

Pattern-A over-vanishing leaf `(p1,p2,p3) = (20,5,1)`. Mirrors the proven `(20,1,1)` template
...
namespace DLNFibre.DLN.Aoyagi.OverVanishA_20_5_1

/-! ## §0 — the leaf and the composite -/

def idxCanon : Idx := ⟨⟨20, by decide⟩, ⟨5, by decide⟩, ⟨1, by decide⟩⟩

noncomputable def gCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun w =>
  blockBlowupMap S1 20 (nativeChart1 20
    (blockBlowupMap (sigmaC1Fs 20) 5 (blockBlowupMap (sigmaC2Fs 20) 1 w)))
...
def phiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun u i =>
  ...
  else if i = 17 then -(u 1 * u 13 + u 10 * u 2)
  else if i = 18 then -(u 1 * u 14 + u 10 * u 3)
  else if i = 19 then -(u 1 * u 15 + u 10 * u 4)
  else 0
...
theorem phiCanon_read (u v : Fin 21 → ℝ) (h : ∀ i, keepCanon i → u i = v i) :
  ...
  funext i
  simp only [phiCanon]
  split_ifs <;> simp only [h0, h1, h2, h3, h4, h10, h12, h13, h14, h15]
...
theorem phiCanon_norm_bound {x : Fin 21 → ℝ} {r : ℝ} (hx : ‖x‖ ≤ r) :
  ...
  rw [Real.norm_eq_abs]
  by_cases h16 : i = 16
  · rw [h16, show phiCanon x 16 = -(x 0 * x 10 + x 1 * x 12) from rfl]; exact slot 0 10 1 12
  by_cases h17 : i = 17
  · rw [h17, show phiCanon x 17 = -(x 1 * x 13 + x 10 * x 2) from rfl]; exact slot 1 13 10 2
  by_cases h18 : i = 18
  · rw [h18, show phiCanon x 18 = -(x 1 * x 14 + x 10 * x 3) from rfl]; exact slot 1 14 10 3
  by_cases h19 : i = 19
  · rw [h19, show phiCanon x 19 = -(x 1 * x 15 + x 10 * x 4) from rfl]; exact slot 1 15 10 4
...
end DLNFibre.DLN.Aoyagi.OverVanishA_20_5_1
