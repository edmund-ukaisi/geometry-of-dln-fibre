...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Produces theta-side product-residual positivity before the source-cylinder bridge.
/-- Rank-cut residual source hypotheses from source-cylinder support, source-density
lower bound, and prior-density upper bound, with theta-side product-residual
positivity produced internally.

This is the source-cylinder analogue of the active-containment produced-product
wrapper.  It constructs an open positivity shrink for `coordinateSourceMeasure`,
runs the source-cylinder rank-cut bridge inside that shrink, and transfers
a.e. positivity to the returned inner rank-cut shrink by absolute continuity of
restricted measures.

It does not prove source-cylinder support, C-one signed-box support,
determinant/raw Haar transport, source-prior transport, density bounds,
source-rank coverage, normal crossings, pole order, or RLCT extraction. -/
theorem exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_rankCutSource_subset_sourceChart_image_inter_sourceCylinder_sourceDensity_lower_priorDensity_upper_of_sourceDensity_continuousAt_lt_top_of_continuousAt_priorDensity_of_subset_detSector
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  refine
    ⟨W, hWopen, hz₀W, (fun z hzW ↦ hVposG (hWVpos hzW)),
      ...
      hpiece_meas, hpiece_open, himageV, hrankPiece_meas, hiffV',
      himage_rank, ?_⟩
  intro rawHaar hrawHaar rankCutSource μprior fixedResidual ε
    hrankCut_subset_sourceCylinder hsource_lower hε_ne_zero hε_ne_top
    Kprior hprior_upper
  let productResidual : Θ → ℝ := fun z ↦
    aoyagiCoordinateSquareSum
  ...
  simpa [rankCutSource, μprior, fixedResidual, productResidual] using
    hpackage rawHaar (ε := ε) hrankCut_subset_sourceCylinder hsource_lower
      hε_ne_zero hε_ne_top (Kprior := Kprior) hprior_upper hproduct_pos_V
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Derives source-cylinder support from C-one support before the produced-product bridge.
/-- Rank-cut residual source hypotheses from C-one signed-box support, source-density
lower bound, and prior-density upper bound, with theta-side product-residual
positivity produced internally.

The pointwise C-one signed-box condition is converted to source-cylinder
support over the inner rank-cut shrink and then widened to the outer shrink.
The source-cylinder produced-product bridge supplies the residual-source
conclusion without asking the caller for theta-side product-residual positivity.

It does not prove the C-one signed-box support itself, source-density
positivity, prior-density bounds, determinant/raw Haar transport, source-prior
transport, source-rank coverage, normal crossings, pole order, or RLCT
extraction. -/
theorem exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_rankCutSource_cOneReadout_mem_signedBox_sourceDensity_lower_priorDensity_upper_of_sourceDensity_continuousAt_lt_top_of_continuousAt_priorDensity_of_subset_detSector
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Produces local density bounds before the source-cylinder bridge.
/-- Rank-cut residual source hypotheses from source-cylinder support, with
...
strict basepoint inequalities into pointwise bounds on a neighborhood, and the
returned `W` and `V` lie inside that neighborhood.  The theorem still does not
prove source-cylinder support, C-one signed-box support, determinant/raw Haar
transport, source-prior transport, source-rank coverage, normal crossings, pole
order, or RLCT extraction. -/
theorem exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_rankCutSource_subset_sourceChart_image_inter_sourceCylinder_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_continuousAt_priorDensity_of_subset_detSector
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  intro rawHaar hrawHaar rankCutSource μprior fixedResidual
    hrankCut_subset_sourceCylinder
  have hsource_lower :
      ∀ᵐ z ∂baseJ.restrict W, ε ≤ sourceDensity z := by
    filter_upwards [ae_restrict_mem hWopen.measurableSet] with z hzW
    exact (hGbounds z (hWbounds hzW)).1
  ...
  simpa [rankCutSource, μprior, fixedResidual] using
    hpackage rawHaar (ε := ε) hrankCut_subset_sourceCylinder hsource_lower
      hε_ne_zero hε_ne_top (Kprior := Kprior) hprior_upper
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Chooses a signed-box radius before using the basepoint source-cylinder shrink.
/-- Rank-cut residual source hypotheses after choosing a signed box around the
basepoint selected-entry coordinate.

The selected-entry signed-box lemma supplies positive radii `Rres` with
`z₀.1.yNext` in the signed box.  The fixed-radius source-cylinder shrink then
removes the source-cylinder support socket.  This theorem still does not prove
source-density positivity, prior-density bounds, determinant/raw Haar
transport, source-prior transport, source-rank coverage, normal crossings, pole
order, or RLCT extraction. -/
theorem exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_continuousAt_priorDensity_of_subset_detSector
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
                                    ...
                                    Cprior < ∞ ∧
                                      (∀ᵐ E ∂μprior.restrict rankCutSource,
                                        ...
                                        residualNegPowerIntegrableOn
                                          ...
                                          (fun E : EdgeFamily ↦ E)
                                          rankCutSource μprior t := by
  intro Θ RawTuple EdgeFamily sourceChart readback p13SourceSet d sourceStratum
    rankEq
  rcases
  ...
  exact
    exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_z0_yNext_mem_signedBox_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_continuousAt_priorDensity_of_subset_detSector
      ...
      (t := t) ht hcrit (density := density) hprior_cont
      sourceImageDensity Rres hRres hsource_cont hsource_finite
      hsource_lt hε_ne_zero (Kprior := Kprior) hprior_lt hz₀_signedBox
      G hGopen hz₀G hGdet (r := r) (rEdge := rEdge) hprod
...
The C-one condition is converted to source-cylinder support over the inner
rank-cut shrink and then widened to the outer shrink before applying the
source-cylinder strict-density bridge.  This theorem still does not prove the
C-one signed-box condition, source-density positivity, prior-density bounds,
determinant/raw Haar transport, source-prior transport, source-rank coverage,
normal crossings, pole order, or RLCT extraction. -/
theorem exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_rankCutSource_cOneReadout_mem_signedBox_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_continuousAt_priorDensity_of_subset_detSector
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
      ...
      (by
        simpa [Θ, RawTuple, EdgeFamily, Y, referenceSource, jacobianDensity,
          ...
          exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_rankCutSource_subset_sourceChart_image_inter_sourceCylinder_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_continuousAt_priorDensity_of_subset_detSector
            ...
            (t := t) ht hcrit (density := density) hprior_cont
            sourceImageDensity Rres hRres hsource_cont hsource_finite
            hsource_lt hε_ne_zero hprior_lt G hGopen hz₀G hGdet
            (r := r) (rEdge := rEdge) hprod) with
    ⟨W, hWopen, hz₀W, hWG, V, hVopen, hz₀V, hVW, hleftV,
      hsource_injV, hsource_contOnV, hpiece_meas, hpiece_open,
      himageV, hrankPiece_meas, hiffV, himage_rank, hsourceCylinder_package⟩
  ...
  refine
    ⟨W, hWopen, hz₀W, hWG, V, hVopen, hz₀V, hVW, hleftV',
      hsource_injV, hsource_contOnV, hpiece_meas, hpiece_open,
      himageV, hrankPiece_meas, hiffV', himage_rank, ?_⟩
  intro rawHaar hrawHaar rankCutSource μprior fixedResidual hcOne_support
  have hrankCut_subset_V : rankCutSource ⊆ sourceChart '' V := by
    intro E hE
    rw [himageV]
    exact hE.1
