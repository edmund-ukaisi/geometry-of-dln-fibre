...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- This packages the determinant-to-`baseJ` handoff with the local upper
-- Jacobian bound, keeping the determinant reverse-domination hypothesis live.
/-- Conditional reverse determinant-Haar to raw-Haar domination for the
with-following Case 2 reference source.

The existing determinant handoff gives domination by the raw image of the
Jacobian-weighted source `baseJ = referenceSource.withDensity jacobianDensity`.
The local upper bound for `jacobianDensity` converts that image to the
unweighted reference source, paying the finite scalar `CJ`.  The
determinant-chart reverse domination hypothesis remains explicit. -/
theorem exists_open_subset_rawHaar_restrict_rawSource_le_smul_measure_map_case2PassiveThetaWithFollowingFactor_rawMap_referenceSource_restrict_of_detHaar_restrict_le_smul_endpointTopologyTuple
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  intro RawTuple Y referenceSource rawMap rawDetChart rawSourceSet
  let jacobianDensity :
      Case2PassiveThetaWithFollowingFactor
          (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J →
        ℝ≥0∞ :=
  ...
  rcases
      exists_open_subset_measure_map_case2PassiveThetaWithFollowingFactorEndpointTopologyTuple_rawOrderMap_twoStage_eq_sourceChart_readback_leftInverse
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
        z₀ hdet₀ hpivot₀ G hGopen hz₀G with
    ⟨Vstage, hVstageopen, hz₀Vstage, hVstageG, hpoint_stage, _hmaps⟩
  rcases
      (by
        simpa [RawTuple, Y, jacobianDensity] using
    ...
    ⟨CJ, hCJ, hJevent⟩
  rcases eventually_nhds_iff.mp hJevent with
    ⟨HJ, hHJ_bound, hHJopen, hz₀HJ⟩
  let Graw :
      Set
        (Case2PassiveThetaWithFollowingFactor
          (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J) :=
    Vstage ∩ HJ
  have hGrawopen : IsOpen Graw := hVstageopen.inter hHJopen
  have hz₀Graw : z₀ ∈ Graw := ⟨hz₀Vstage, hz₀HJ⟩
  rcases
      (by
        simpa [RawTuple, Y, referenceSource, jacobianDensity, baseJ, rawMap,
          rawDetChart, rawSourceSet, Graw] using
          exists_open_subset_rawHaar_restrict_rawSource_le_smul_measure_map_case2PassiveThetaWithFollowingFactor_rawMap_baseJ_restrict_of_detHaar_restrict_le_smul_endpointTopologyTuple
            W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
            z₀ hdet₀ hpivot₀ referenceSource Graw hGrawopen hz₀Graw) with
    ⟨V, hVopen, hz₀V, hVGraw, hrawMap_baseJ, hbase_dom⟩
  have hVG : V ⊆ G := fun z hz ↦ hVstageG (hVGraw.1 hz)
  refine ⟨V, hVopen, hz₀V, hVG, CJ, hCJ, ?_⟩
  intro rawHaar _instRawHaar Cdet hdet_dom hCdet
  let ρ := Fin (Module.finrank ℝ U₀)
  let κ' : Fin 3 → Type :=
    throughSubspaceEndpointComplementIndex
      (reverseVertex W₂) (reverseEdge W₂ B₂) U₀
  have hdet_of_mem :
      ∀ z ∈ V,
        Y z ∈ topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ') := by
    intro z hz
    simpa [Y, RawTuple, ρ, κ'] using (hpoint_stage z (hVGraw.1 hz)).1
  have hrawMapContOn : ContinuousOn rawMap V := by
    rw [continuousOn_iff_continuous_restrict]
    let Sdet : Set RawTuple :=
      topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ')
  ...
  have hrawMap_reference :
    ...
    ContinuousOn.aemeasurable₀ hrawMapContOn
      hVopen.measurableSet.nullMeasurableSet
  have hJ_ae :
      ∀ᵐ z ∂referenceSource.restrict V, jacobianDensity z ≤ CJ := by
    filter_upwards [ae_restrict_mem hVopen.measurableSet] with z hz
    exact hHJ_bound z (hVGraw.2 hz)
  have hbase_le_reference :
      baseJ.restrict V ≤ CJ • referenceSource.restrict V := by
    change (referenceSource.withDensity jacobianDensity).restrict V ≤
      CJ • referenceSource.restrict V
    exact restrict_withDensity_le_smul_restrict_of_ae_le
      hVopen.measurableSet hJ_ae
  have hmap_reference :
      Measure.map rawMap (baseJ.restrict V) ≤
        CJ • Measure.map rawMap (referenceSource.restrict V) :=
    map_le_smul_map_of_le_smul_aemeasurable hrawMap_reference
      hbase_le_reference
  have hraw_base :
      rawHaar.restrict rawSourceSet ≤
        Cdet • Measure.map rawMap (baseJ.restrict V) :=
    hbase_dom rawHaar hdet_dom
  have hraw_reference :
      rawHaar.restrict rawSourceSet ≤
        (Cdet * CJ) • Measure.map rawMap (referenceSource.restrict V) :=
    measure_le_smul_of_le_smul_of_le_smul hraw_base hmap_reference
  exact ⟨ENNReal.mul_lt_top hCdet hCJ, hraw_reference⟩
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Shrinks by eventual upper-density neighborhoods before invoking the passive-self wrapper.
/-- Open-patch coordinate-source finite-integral handoff from eventual upper
density bounds near the base point.

This is the same-shrink topological wrapper around
`exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_density_bounds`.
It does not prove the density bounds; it only converts eventual upper bounds
for `jacobianDensity` and `sourceDensity` into the a.e. restricted-measure
bounds consumed by that theorem. -/
theorem exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_eventually_density_bounds
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
    ...
    let passiveRef :=
      case2PassiveThetaPassiveFieldReferenceMeasure
        (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J
    (∀ᶠ z in nhds z₀, jacobianDensity z ≤ CJ) →
      (∀ᶠ z in nhds z₀, sourceDensity z ≤ CS) →
        ∃ passiveLocalSet :
          Set
            (Case2PassiveTheta.PassiveFields
              (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J),
  ...
  intro RawTuple Y referenceSource jacobianDensity baseJ EdgeFamily sourceChart
    sourceDensity coordinateSourceMeasure retainedData readback passiveRef
    hJ_eventually hsource_eventually
  classical
  rcases eventually_nhds_iff.mp hJ_eventually with
    ⟨GJ, hGJ_bound, hGJopen, hz₀GJ⟩
  rcases eventually_nhds_iff.mp hsource_eventually with
    ⟨GS, hGS_bound, hGSopen, hz₀GS⟩
  let Gdens :
      Set
        (Case2PassiveThetaWithFollowingFactor
          (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J) :=
    (G ∩ GJ) ∩ GS
  have hGdens_open : IsOpen Gdens := (hGopen.inter hGJopen).inter hGSopen
  have hz₀Gdens : z₀ ∈ Gdens := ⟨⟨hz₀G, hz₀GJ⟩, hz₀GS⟩
  have hmain :
      let RawTuple :=
        TopologyTuple (Fin (Module.finrank ℝ U₀))
          (throughSubspaceEndpointComplementIndex
    ...
    simpa [RawTuple, Y, referenceSource, jacobianDensity, baseJ,
      ...
      retainedData, readback, passiveRef, Gdens] using
      exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_density_bounds
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀)
        hDomainOpens hDomainBorel hDomainPolish eNext e z₀ hdet₀ hpivot₀
        hF₀det sourceImageDensity (t := t) Rres ht hRres hcrit
        (CJ := CJ) (CS := CS) hCJ hCS Gdens hGdens_open hz₀Gdens
  rcases hmain with
    ⟨passiveLocalSet, passiveMeasure, followingPatch, K, V, hz₀_passive,
      hpassive_open, hpassive_meas, hpassive_eq, hpassive_lt_top,
      hpassive_dom, hK_pos, hz₀_following, hpatch_open, hpatch_meas,
      hpatch_lt_top, hpatch_det, hpatch_inv_bound, hVopen, hz₀V,
      hVGdens, hV_passive, hV_following, hdetV, hleftV, hsource_inj,
      hsource_contOn, hsource_image, hfinite⟩
  have hVG : V ⊆ G := fun z hz ↦ (hVGdens hz).1.1
  have hJ :
      ∀ᵐ z ∂ referenceSource.restrict V, jacobianDensity z ≤ CJ := by
    filter_upwards [ae_restrict_mem hVopen.measurableSet] with z hzV
    exact hGJ_bound z ((hVGdens hzV).1.2)
  have hsource :
      ∀ᵐ z ∂ baseJ.restrict V, sourceDensity z ≤ CS := by
    filter_upwards [ae_restrict_mem hVopen.measurableSet] with z hzV
    exact hGS_bound z ((hVGdens hzV).2)
  refine
    ⟨passiveLocalSet, passiveMeasure, followingPatch, K, V, hz₀_passive,
      hpassive_open, hpassive_meas, hpassive_eq, hpassive_lt_top,
      hpassive_dom, hK_pos, hz₀_following, hpatch_open, hpatch_meas,
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Composes two long expanded coordinate-source handoff theorem types.
/-- Open-patch coordinate-source finite-integral handoff from local
source-density continuity and finite base value.

This composes the density package theorem with the eventual-density
coordinate-source handoff.  The Jacobian bound is chosen internally from the
determinant-sector endpoint theorem; the source bound is chosen internally
from the explicit assumptions that `sourceImageDensity` composed with the
endpoint source chart is continuous at `z₀` and finite at `z₀`. -/
theorem exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_sourceDensity_continuousAt_lt_top
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  have hbounds :
      ∃ CJ CS : ℝ≥0∞, CJ < ∞ ∧ CS < ∞ ∧
        (∀ᶠ z in nhds z₀, jacobianDensity z ≤ CJ) ∧
          (∀ᶠ z in nhds z₀, sourceDensity z ≤ CS) := by
    simpa [RawTuple, Y, jacobianDensity, EdgeFamily, sourceChart, sourceDensity] using
      exists_finite_eventually_jacobianDensity_sourceDensity_bounds_case2PassiveThetaWithFollowingFactorEndpointSourceChart_of_sourceDensity_continuousAt_lt_top
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀)
        eNext e z₀ hdet₀ sourceImageDensity hsource_cont hsource_finite
  rcases hbounds with
    ⟨CJ, CS, hCJ, hCS, hJ_eventually, hsource_eventually⟩
  exact
    exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_eventually_density_bounds
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀)
        hDomainOpens hDomainBorel hDomainPolish eNext e z₀ hdet₀ hpivot₀
        hF₀det sourceImageDensity (t := t) Rres ht hRres hcrit
        (CJ := CJ) (CS := CS) hCJ hCS G hGopen hz₀G
        hJ_eventually hsource_eventually
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- Specializes the source-density continuity wrapper to the unit source-image density.
/-- Open-patch coordinate-source finite-integral handoff for the unweighted
chart-produced source-image measure.

This is the unit-density specialization of
`..._of_sourceDensity_continuousAt_lt_top`: the composed source density is the
constant `1`, so the continuity and finite-base-value assumptions are
discharged internally.  The theorem is still only about the chart-produced
coordinate-source measure, not an original-prior or Haar-transport
identification. -/
theorem exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_sourceImageDensity_one
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  intro RawTuple Y referenceSource jacobianDensity baseJ EdgeFamily sourceChart
    sourceDensity coordinateSourceMeasure retainedData readback passiveRef
  have hsource_cont : ContinuousAt sourceDensity z₀ := by
        ...
        Continuous
          (fun _ :
            Case2PassiveThetaWithFollowingFactor
              (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J =>
            (1 : ℝ≥0∞))).continuousAt
  have hsource_finite : sourceDensity z₀ < ∞ := by
    simp [sourceDensity]
  exact
    exists_open_passiveLocalSet_matrixEntryReference_open_followingPatch_open_subset_case2PassiveThetaWithFollowingFactor_coordinateSourceMeasure_productResidual_pos_ae_and_lintegral_rpow_neg_of_sourceDensity_continuousAt_lt_top
      W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀)
      hDomainOpens hDomainBorel hDomainPolish eNext e z₀ hdet₀ hpivot₀
      hF₀det (fun _ : EdgeFamily ↦ (1 : ℝ≥0∞)) (t := t) Rres ht hRres
      hcrit G hGopen hz₀G hsource_cont hsource_finite
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- This localized wrapper obtains the endpoint scalar from active endpoint
-- Haar domination, then feeds it to the source-density handoff above.
/-- Localized reverse raw-patch domination from active endpoint-image
containment and a source-density lower bound.

The proof internally shrinks the neighborhood inside the
selected-pivot-nonzero locus.  For each raw patch `P ⊆ rawSourceSet`, containment of
`rawDetChart ∩ rawOrderOnEndpoint ⁻¹' P` in the active writeback image of the
selected-entry source cylinder produces a finite endpoint scalar, and the
existing source-density handoff turns that endpoint domination into raw-patch
domination by the coordinate source measure.

This theorem still assumes null-measurability of the raw endpoint patch and
the source-density lower bound.  It does not prove determinant-chart Haar
...
theorem exists_open_subset_rawHaar_restrict_patch_le_smul_measure_map_case2PassiveThetaWithFollowingFactor_rawMap_coordinateSourceMeasure_restrict_of_endpointPatch_subset_activeWriteback_activeSelectedEntryImage_sourceDensity_lower
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  intro RawTuple Y referenceSource jacobianDensity baseJ EdgeFamily
    ...
    rawDetChart rawSourceSet pivotNext signedBox sourceCylinder activeChart
    activeWriteback
  let Θ :=
    Case2PassiveThetaWithFollowingFactor
      (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J
  ...
  rcases
      (by
        simpa [referenceSource, Y, jacobianDensity, baseJ, EdgeFamily,
          ...
          rawOrderOnEndpoint, rawDetChart, rawSourceSet, Θ] using
          exists_open_subset_rawHaar_restrict_patch_le_smul_measure_map_case2PassiveThetaWithFollowingFactor_rawMap_coordinateSourceMeasure_restrict_of_endpointPatch_restrict_le_smul_endpointTopologyTuple_sourceDensity_lower
            W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
            z₀ hdet₀ hpivot₀ sourceImageDensity Rres Gpivot
            hGpivot_open hz₀Gpivot) with
    ⟨V, hVopen, hz₀V, hVGpivot, hraw_dom⟩
  have hVG : V ⊆ G := by
    intro z hz
    exact (hVGpivot hz).1
  have hVpivot : V ⊆ pivotSet := by
    ...
    exact (hVGpivot hz).2
  refine ⟨V, hVopen, hz₀V, hVG, ?_⟩
  intro rawHaar _instRawHaar P ε hP hendpoint_nm hpatch hlower hε0 hεtop
  have hVpivot' :
      V ⊆ {z |
        case2PassiveThetaPivotNonzero
  ...
  have hactive :
    ...
    simpa [RawTuple, Y, referenceSource, pivotNext, signedBox,
      ...
      (_root_.DLNFibre.DLN.Aoyagi.rawHaar_restrict_endpointPatch_le_smul_case2PassiveThetaWithFollowingFactorEndpointReferenceImage_of_subset_activeWriteback_activeSelectedEntryImage_inter_signedBox_of_subset_pivotNonzero
        ...
        hVpivot' rawHaar (rawDetChart ∩ rawOrderOnEndpoint ⁻¹' P) hpatch)
  rcases hactive with ⟨Cdet, hCdet, hdet_dom⟩
  rcases
      hraw_dom rawHaar (P := P) (Cdet := Cdet) (ε := ε)
        hP hendpoint_nm hdet_dom hCdet hlower hε0 hεtop with
    ⟨hfinite, hdom⟩
  exact ⟨Cdet, hCdet, hfinite, hdom⟩
...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- This wrapper converts a topological source-density lower bound to the
-- a.e. lower bound consumed by the active-containment raw-patch theorem.
/-- Localized reverse raw-patch domination from active endpoint-image
containment and an eventual source-density lower bound.

This is the topological wrapper around
`..._endpointPatch_subset_activeWriteback_activeSelectedEntryImage_sourceDensity_lower`:
an eventual lower bound for `sourceImageDensity (sourceChart z)` near the base
point is converted, after shrinking, into the a.e. lower bound required by the
active-containment source-density handoff.  Endpoint-patch null-measurability
and active endpoint containment remain explicit hypotheses. -/
theorem exists_open_subset_rawHaar_restrict_patch_le_smul_measure_map_case2PassiveThetaWithFollowingFactor_rawMap_coordinateSourceMeasure_restrict_of_endpointPatch_subset_activeWriteback_activeSelectedEntryImage_eventually_sourceDensity_lower
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  intro RawTuple Y referenceSource jacobianDensity baseJ EdgeFamily
    sourceChart sourceDensity coordinateSourceMeasure rawMap rawOrderOnEndpoint
    rawDetChart rawSourceSet pivotNext signedBox sourceCylinder activeChart
    activeWriteback hsource_eventually
  rcases eventually_nhds_iff.mp hsource_eventually with
  ...
  rcases
      (by
        simpa [referenceSource, Y, jacobianDensity, baseJ, EdgeFamily,
          sourceChart, sourceDensity, coordinateSourceMeasure, rawMap,
          rawOrderOnEndpoint, rawDetChart, rawSourceSet, pivotNext, signedBox,
          sourceCylinder, activeChart, activeWriteback] using
  ...
  have hVG : V ⊆ G := hVGH.1
  refine ⟨V, hVopen, hz₀V, hVG, ?_⟩
  intro rawHaar _instRawHaar P hP hendpoint_nm hpatch hε0 hεtop
  have hlower :
      ∀ᵐ z ∂baseJ.restrict V, ε ≤ sourceDensity z := by
    filter_upwards [ae_restrict_mem hVopen.measurableSet] with z hzV
    exact hHlower z (hVGH.2 hzV)
  exact
    hraw_dom rawHaar (P := P) (ε := ε) hP hendpoint_nm hpatch
      hlower hε0 hεtop
