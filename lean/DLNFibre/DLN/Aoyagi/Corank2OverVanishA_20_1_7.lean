...
/-!
# `DLN.Aoyagi.Corank2OverVanishA_20_1_7` — the per-type CONCRETE facts, leaf `(20,1,7)`

Pattern-A over-vanishing leaf `(p1,p2,p3) = (20,1,7)`. Mirrors the proven `(20,1,1)` template
...
namespace DLNFibre.DLN.Aoyagi.OverVanishA_20_1_7

/-! ## §0 — the leaf and the composite -/

def idxCanon : Idx := ⟨⟨20, by decide⟩, ⟨1, by decide⟩, ⟨7, by decide⟩⟩

noncomputable def gCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun w =>
  blockBlowupMap S1 20 (nativeChart1 20
    (blockBlowupMap (sigmaC1Fs 20) 1 (blockBlowupMap (sigmaC2Fs 20) 7 w)))
...
def phiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun u i =>
  ...
  else if i = 13 then -(u 10 * u 2 + u 17 * u 5 * u 7)
  else if i = 14 then -(u 10 * u 3 + u 18 * u 5 * u 7)
  else if i = 15 then -(u 10 * u 4 + u 19 * u 5 * u 7)
  else 0
...
theorem phiCanon_read (u v : Fin 21 → ℝ) (h : ∀ i, keepCanon i → u i = v i) :
  ...
  funext i
  simp only [phiCanon]
  split_ifs <;> simp only [h0, h2, h3, h4, h5, h7, h10, h16, h17, h18, h19]
...
theorem prod_vmExpCanon (u : Fin 21 → ℝ) : (∏ d, (u d) ^ vmExpCanon d) = u 1 * u 7 * u 20 := by
  have h1 : ∀ d : Fin 21, u d ^ vmExpCanon d
      = if d = 1 ∨ d = 7 ∨ d = 20 then u d else 1 := fun d => by
    simp only [vmExpCanon]; split_ifs <;> simp
  simp_rw [h1]
  rw [Finset.prod_ite, Finset.prod_const_one, mul_one,
    show Finset.filter (fun d : Fin 21 => d = 1 ∨ d = 7 ∨ d = 20) Finset.univ
        = {1, 7, 20} from by decide,
...
theorem phiCanon_norm_bound {x : Fin 21 → ℝ} {r : ℝ} (hr1 : 1 ≤ r) (hx : ‖x‖ ≤ r) :
  ...
  rw [Real.norm_eq_abs]
  by_cases h12 : i = 12
  · rw [h12, show phiCanon x 12 = -(x 0 * x 10 + x 16 * x 5 * x 7) from rfl]
    rw [abs_neg]
    calc |x 0 * x 10 + x 16 * x 5 * x 7| ≤ |x 0 * x 10| + |x 16 * x 5 * x 7| := abs_add_le _ _
      _ ≤ r ^ 2 + r ^ 3 := add_le_add (hq 0 10) (hcube 16 5 7)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
  by_cases h13 : i = 13
  · rw [h13, show phiCanon x 13 = -(x 10 * x 2 + x 17 * x 5 * x 7) from rfl]
    rw [abs_neg]
    calc |x 10 * x 2 + x 17 * x 5 * x 7| ≤ |x 10 * x 2| + |x 17 * x 5 * x 7| := abs_add_le _ _
      _ ≤ r ^ 2 + r ^ 3 := add_le_add (hq 10 2) (hcube 17 5 7)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
  by_cases h14 : i = 14
  · rw [h14, show phiCanon x 14 = -(x 10 * x 3 + x 18 * x 5 * x 7) from rfl]
    rw [abs_neg]
    calc |x 10 * x 3 + x 18 * x 5 * x 7| ≤ |x 10 * x 3| + |x 18 * x 5 * x 7| := abs_add_le _ _
      _ ≤ r ^ 2 + r ^ 3 := add_le_add (hq 10 3) (hcube 18 5 7)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
  by_cases h15 : i = 15
  · rw [h15, show phiCanon x 15 = -(x 10 * x 4 + x 19 * x 5 * x 7) from rfl]
    rw [abs_neg]
    calc |x 10 * x 4 + x 19 * x 5 * x 7| ≤ |x 10 * x 4| + |x 19 * x 5 * x 7| := abs_add_le _ _
      _ ≤ r ^ 2 + r ^ 3 := add_le_add (hq 10 4) (hcube 19 5 7)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
...
end DLNFibre.DLN.Aoyagi.OverVanishA_20_1_7
