...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- On the open punctured sector for the passive Case 2 selected-entry chart,
the topology-tuple raw-order p.13 source chart agrees with the direct
chart-produced source edge family, and the selected-entry inverse readout
recovers the residual coordinates.

This is a pointwise topology/source-chart bridge.  It is not a measure
pushforward, determinant-chart Haar transport, source-prior comparison,
source-image equality, source-rank coverage, normal crossings, pole order, or
RLCT extraction. -/
theorem exists_open_case2EndpointTransport_withPassive_topologyTuple_rawOrderSourceChart_eq_sourceChart_puncturedSector_inverseReadout_eq
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  let raw : TopologyTuple ρ κ' ℝ :=
    topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ') y
  have hy :
      y ∈ topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ') :=
    (topologyTuple_mem_topologyTupleDetChartSet
      (K := ℝ) (ρ := ρ) (κ' := κ') (retainedData z)).2 hdet
  have hraw :
      raw ∈ topologyTupleRawOrderSourceRecursiveDetChartSet
        (K := ℝ) (ρ := ρ) (κ' := κ') :=
    mapsTo_topologyTupleEdgeRawOrder_detChartSet_rawOrderSourceRecursiveDetChartSet
      (K := ℝ) (ρ := ρ) (κ' := κ') hy
  have hraw_eq :
      paperEndpointFixedBaseRetainedPassiveP13RawOrderSourceChart W₂ B₂ U₀ hU₀ raw =
        sourceChart z := by
    simpa [raw, y, sourceChart, ρ, κ'] using
  ...
  have hinv : inverseReadout (sourceChart z) = z.2 := by
    ...
    rw [hcoord]
    exact hpre
  exact ⟨hy, hraw, hraw_eq, hlocal, by simpa [E, ρ] using hread, hinv⟩
