...
private abbrev retainedPassiveRawCtopRestFamily
    {M : ℕ} (ρ : Type*) (κ' : Fin (M + 2) → Type*) :=
  retainedPassiveRawA1PassiveFamily (M := M) ρ ×
    (retainedPassiveRawF2Family (M := M) ρ κ' ×
      (retainedPassiveRawA3PassiveFamily (M := M) ρ κ' ×
...
/-- Regroup the raw tuple with `Ctop` as the first product factor. -/
private def retainedPassiveRawCtopFocusLinearEquiv
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*} :
    RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ ≃ₗ[ℝ]
      (retainedPassiveRawCtopFamily ρ ×
        retainedPassiveRawCtopRestFamily (M := M) ρ κ') where
  toFun w :=
    (w.2.2.2.2.1,
  ...
  invFun x :=
    (x.2.1,
      (x.2.2.1,
        (x.2.2.2.1, (x.2.2.2.2.1, (x.1, x.2.2.2.2.2)))))
  left_inv w := by
    rcases w with ⟨A1passive, F2, A3passive, C, Ctop, F3⟩
    rfl
  right_inv x := by
    rcases x with ⟨Ctop, A1passive, F2, A3passive, C, F3⟩
    rfl
  map_add' _ _ := rfl
  map_smul' _ _ := rfl

@[simp]
private theorem retainedPassiveRawCtopFocusLinearEquiv_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (w : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ) :
...
@[simp]
private theorem retainedPassiveRawCtopFocusLinearEquiv_symm_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (x : retainedPassiveRawCtopFamily ρ ×
        retainedPassiveRawCtopRestFamily (M := M) ρ κ') :
    (retainedPassiveRawCtopFocusLinearEquiv
        (ρ := ρ) (κ' := κ')).symm x =
      (x.2.1,
        (x.2.2.1,
...
private def retainedPassiveRawCtopRestA1passiveLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (q : Fin M) :
    retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
      Matrix ρ ρ ℝ where
  toFun w := w.1 q
  map_add' _ _ := rfl
  map_smul' _ _ := rfl

private def retainedPassiveRawCtopRestF2CProjectionLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*} :
    retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
      retainedPassiveRawF2CFamily (M := M) ρ κ' where
  toFun w := (w.2.1, w.2.2.2.1)
  map_add' _ _ := rfl
  map_smul' _ _ := rfl

private def retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (q : Fin (M + 1)) :
    retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
      Matrix (κ' q.succ) ρ ℝ where
  toFun w := Fin.lastCases w.2.2.2.2 (fun p => w.2.2.1 p) q
  map_add' w w' := by
    induction q using Fin.lastCases with
    | last =>
        simp only [Fin.lastCases_last]
...
private theorem retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt_apply_rawTupleRest
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (w : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
    (q : Fin (M + 1)) :
    retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt
        (ρ := ρ) (κ' := κ') q
        (retainedPassiveRawCtopFocusLinearEquiv
          (ρ := ρ) (κ' := κ') w).2 =
      rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') w q := by
  induction q using Fin.lastCases with
  | last =>
      unfold retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt
      change
        Fin.lastCases
          ((retainedPassiveRawCtopFocusLinearEquiv
            (ρ := ρ) (κ' := κ') w).2).2.2.2.2
          (fun p => ((retainedPassiveRawCtopFocusLinearEquiv
            (ρ := ρ) (κ' := κ') w).2).2.2.1 p)
          (Fin.last M) =
          rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') w (Fin.last M)
      simp [rawEdgeTupleA3, ofTopologyTuple]
  | cast p =>
      simp [retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt,
        rawEdgeTupleA3, ofTopologyTuple]

private theorem retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt_apply_rawTupleFields
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (w : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
    (q : Fin (M + 1)) :
    retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt
      ...
      rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') w q := by
  simpa using
    retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt_apply_rawTupleRest
      (ρ := ρ) (κ' := κ') w q

set_option linter.style.longLine false in
private def retainedPassivePostA1passiveTailFDerivLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
    ...
    (fun n hns acc ↦
      ...
      let Psucc :=
        ChartLocalSuffixState.residualFactorProduct (K := ℝ)
          (κ := fun _ : Fin (M + 2) ↦ ρ)
          data.A1seed (Fin.last (M + 1)) p.succ p.succ.le_last
      (retainedPassiveMatrixMulRightLinearMap (data.A1seed p)).comp acc +
        (retainedPassiveMatrixMulLeftLinearMap Psucc).comp
          (retainedPassiveRawCtopRestA1passiveLinearMapAt
            (ρ := ρ) (κ' := κ') q))
    0
    hm

@[simp]
private theorem retainedPassivePostA1passiveTailFDerivLinearMapAt_self
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ) :
    retainedPassivePostA1passiveTailFDerivLinearMapAt
        (ρ := ρ) (κ' := κ') z M le_rfl =
      (0 : retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
        Matrix ρ ρ ℝ) := by
  simp [retainedPassivePostA1passiveTailFDerivLinearMapAt]

set_option linter.style.longLine false in
private theorem retainedPassivePostA1passiveTailFDerivLinearMapAt_step_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
    (r : retainedPassiveRawCtopRestFamily (M := M) ρ κ')
    (m : ℕ) (hm : m < M) :
    retainedPassivePostA1passiveTailFDerivLinearMapAt
      ...
      retainedPassivePostA1passiveTailFDerivLinearMapAt
        ...
        data.A1seed p + Psucc * r.1 q := by
  unfold retainedPassivePostA1passiveTailFDerivLinearMapAt
...
set_option linter.style.longLine false in
private theorem retainedPassivePostA1passiveTailFDerivLinearMapAt_fderiv_after_T12_eq_targetStaged
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  let r :=
    (retainedPassiveRawCtopFocusLinearEquiv
      (ρ := ρ) (κ' := κ') u).2
  let formal := retainedPassiveFormalRawOrderJacobianAt
      (ρ := ρ) (κ' := κ') z
  have hT12A1 :
      u.1 = (formal v).1 := by
    simpa [raw, Dzv, u, formal] using
  ...
  have hstep : ∀ n (hns : n + 1 ≤ M),
    ...
    have hpost :=
      retainedPassivePostA1passiveTailFDerivLinearMapAt_step_apply
        (ρ := ρ) (κ' := κ') z r n (Nat.lt_of_succ_le hns)
    have htarget :=
      retainedPassiveA1TailTargetStagedFDerivAt_step
        (ρ := ρ) (κ' := κ') z Dzv n (Nat.lt_of_succ_le hns)
    have hstage :=
    ...
    have hstage' :
      ...
      change retainedPassiveTargetStagedA1passiveTangentAt
          (ρ := ρ) (κ' := κ') z Dzv q =
        (formal v).1 q
      simpa [raw, Dzv, formal] using hstage
    have hA1 :
        r.1 q =
          retainedPassiveTargetStagedA1passiveTangentAt
            (ρ := ρ) (κ' := κ') z Dzv q := by
    ...
    calc
      retainedPassivePostA1passiveTailFDerivLinearMapAt
          (ρ := ρ) (κ' := κ') z n (Nat.le_of_succ_le hns) r
          =
        retainedPassivePostA1passiveTailFDerivLinearMapAt
            (ρ := ρ) (κ' := κ') z (n + 1) hns r *
          data.A1seed p + Psucc * r.1 q := by
            simpa [data, q, p, Psucc] using hpost
      _ =
        retainedPassiveA1TailTargetStagedFDerivAt
            (ρ := ρ) (κ' := κ') z Dzv (n + 1) hns *
          data.A1seed p +
      ...
      _ =
        retainedPassiveA1TailTargetStagedFDerivAt
          (ρ := ρ) (κ' := κ') z Dzv n (Nat.le_of_succ_le hns) := by
            simpa [data, q, p, Psucc] using htarget.symm
  simpa [motive, raw, Dzv, u, r] using
    Nat.decreasingInduction (motive := motive) hstep hbase hm

set_option linter.style.longLine false in
private def retainedPassivePostA1passiveCtopCorrectionLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  let sourcePair :
      retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
        retainedPassiveRawF2CFamily (M := M) ρ κ' :=
    (retainedPassiveFormalRawF2CLinearEquivAt (ρ := ρ) (κ' := κ') hz).symm.toLinearMap.comp
      retainedPassiveRawCtopRestF2CProjectionLinearMapAt
  let X :
      retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
        retainedPassiveRawF2Family (M := M) ρ κ' :=
    (LinearMap.fst ℝ
  ...
  let dTail :=
    retainedPassivePostA1passiveTailFDerivLinearMapAt
      (ρ := ρ) (κ' := κ') z 0 (Nat.zero_le M)
  let tailTerm :
      retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
        Matrix ρ ρ ℝ :=
    (retainedPassiveMatrixMulRightLinearMap coord.Ctop).comp
      ((retainedPassiveMatrixMulRightLinearMap Tail⁻¹).comp
        ((retainedPassiveMatrixMulLeftLinearMap Tail⁻¹).comp dTail))
  show retainedPassiveRawCtopRestFamily (M := M) ρ κ' →ₗ[ℝ]
      retainedPassiveRawCtopFamily ρ from
    -((retainedPassiveMatrixMulRightLinearMap (coord.solvedA3 0)).comp
        ((LinearMap.proj (0 : Fin (M + 1))).comp Xsucc)) -
      (retainedPassiveMatrixMulLeftLinearMap (coord.F2 (0 : Fin (M + 1)).succ)).comp
        (retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt
          (ρ := ρ) (κ' := κ') 0) +
      tailTerm

set_option linter.style.longLine false in
private theorem retainedPassivePostA1passiveCtopCorrectionLinearMapAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    let dTail :=
      retainedPassivePostA1passiveTailFDerivLinearMapAt
        (ρ := ρ) (κ' := κ') z 0 (Nat.zero_le M) r
    retainedPassivePostA1passiveCtopCorrectionLinearMapAt
        (ρ := ρ) (κ' := κ') hz r =
      -(Xsucc 0 * coord.solvedA3 0)
        - coord.F2 (0 : Fin (M + 1)).succ *
            retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt
              (ρ := ρ) (κ' := κ') 0 r
        + Tail⁻¹ * dTail * Tail⁻¹ * coord.Ctop := by
  dsimp [retainedPassivePostA1passiveCtopCorrectionLinearMapAt,
    retainedPassiveRawCtopRestF2CProjectionLinearMapAt,
    retainedPassiveMatrixMulRightLinearMap,
    retainedPassiveMatrixMulLeftLinearMap]
  rfl
...
set_option linter.style.longLine false in
/-- After the edge-pair and passive `A1` stages, change only `Ctop` by the
target-staged first top-left correction. -/
def retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ ≃ₗ[ℝ]
      RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ :=
  retainedPassiveRawCtopFocusLinearEquiv.trans
    ((linearEquivUpperShear
      (R := ℝ)
      (M := retainedPassiveRawCtopFamily ρ)
      (N := retainedPassiveRawCtopRestFamily (M := M) ρ κ')
      (retainedPassivePostA1passiveCtopCorrectionLinearMapAt
        (ρ := ρ) (κ' := κ') hz)).trans
      retainedPassiveRawCtopFocusLinearEquiv.symm)

set_option linter.style.longLine false in
/-- Formula for the post-`A1passive` `Ctop` raw-tuple shear. -/
theorem retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
          ...
          (w.2.2.1,
            (w.2.2.2.1,
              (w.2.2.2.2.1
                ...
                + Tail⁻¹ * dTail * Tail⁻¹ * coord.Ctop,
                w.2.2.2.2.2))))) := by
  rw [retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt]
  simp [retainedPassivePostA1passiveCtopCorrectionLinearMapAt_apply,
    sub_eq_add_neg, add_assoc]
  have hA3 :=
    retainedPassiveRawCtopRestEdgeTupleA3LinearMapAt_apply_rawTupleFields
      (ρ := ρ) (κ' := κ') w (0 : Fin (M + 1))
  simpa [Fin.succ_zero_eq_one] using
    congrArg
      (fun A : Matrix (κ' (1 : Fin (M + 2))) ρ ℝ =>
        -((ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z).toCoordinateData.F2
          (1 : Fin (M + 2)) * A)) hA3

/-- The post-`A1passive` `Ctop` raw-tuple shear has determinant one. -/
theorem retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_det_eq_one
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
        ...
        (retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt
          (ρ := ρ) (κ' := κ') hz :
            RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ →ₗ[ℝ]
              RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ) =
        LinearMap.det
          (S : (retainedPassiveRawCtopFamily ρ ×
              retainedPassiveRawCtopRestFamily (M := M) ρ κ') →ₗ[ℝ]
              (retainedPassiveRawCtopFamily ρ ×
                retainedPassiveRawCtopRestFamily (M := M) ρ κ')) := by
    simpa [retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt,
      E, S] using
      (LinearMap.det_conj
        (S : (retainedPassiveRawCtopFamily ρ ×
            retainedPassiveRawCtopRestFamily (M := M) ρ κ') →ₗ[ℝ]
  ...
  exact
    linearEquivUpperShear_det_eq_one
      ...
      (M := retainedPassiveRawCtopFamily ρ)
      (N := retainedPassiveRawCtopRestFamily (M := M) ρ κ')
      (f := retainedPassivePostA1passiveCtopCorrectionLinearMapAt
        (ρ := ρ) (κ' := κ') hz)

/-- The post-`A1passive` `Ctop` raw-tuple shear has absolute determinant one. -/
theorem retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_abs_det_eq_one
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    |LinearMap.det
      (retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt
        (ρ := ρ) (κ' := κ') hz :
          RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ →ₗ[ℝ]
            RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)| = 1 := by
  rw [retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_det_eq_one
    (ρ := ρ) (κ' := κ') hz, abs_one]

set_option linter.style.longLine false in
/-- The target edge-pair shear, the post-edge-pair passive `A1` shear, and the
post-`A1passive` first top-left shear.  This is still only a partial
target-side normaliser: the `F3` component is not staged here. -/
def retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
/-- Formula for the first three target-side raw-tuple normalisation stages. -/
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
/-- The first three target-side raw-tuple normalisation stages have absolute
determinant one. -/
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_abs_det_eq_one
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
private theorem retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt_Ctop
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
private theorem retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt_F2C
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
private theorem rawEdgeTupleA3_retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  rw [retainedPassivePostEdgePairA1passiveShearRawTupleLinearEquivAt_apply]
  rw [retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt_apply]
  simpa [rawEdgeTupleA3, ofTopologyTuple] using
    rawEdgeTupleA3_targetEdgePairShearRawTupleLinearMapAt
      (ρ := ρ) (κ' := κ') z w q

private theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_F2C
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
set_option linter.style.longLine false in
private theorem rawEdgeTupleA3_retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
set_option linter.style.longLine false in
/-- After the first three target-side normalisation stages, the first top-left
component of the actual raw-order derivative agrees with the formal raw-order
Jacobian. -/
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopShear_fderiv_Ctop_eq_formalRawOrderJacobianAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
    (retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt
        (ρ := ρ) (κ' := κ') hz ((fderiv ℝ raw z) v)).2.2.2.2.1 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.1 := by
  let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
  let Dzv := (fderiv ℝ raw z) v
  ...
  have hCtop :=
    Ctop_tail_recursive_target_staged_shear_fderiv_topologyTupleEdgeRawOrder_eq_formalRawOrderJacobianAt
      (ρ := ρ) (κ' := κ') hz v
  have huCtop :
      u.2.2.2.2.1 = Dzv.2.2.2.2.1 := by
    simpa [u] using
      retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt_Ctop
        (ρ := ρ) (κ' := κ') hz Dzv
  have huA3 :
      rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') u 0 =
        rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') Dzv 0 := by
    simpa [u] using
      rawEdgeTupleA3_retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt
        (ρ := ρ) (κ' := κ') hz Dzv (0 : Fin (M + 1))
  have huF2C :
      (u.2.1, u.2.2.2.1) =
        retainedPassiveTargetEdgePairShearAt (ρ := ρ) (κ' := κ') z Dzv := by
    simpa [u] using
      retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt_F2C
        (ρ := ρ) (κ' := κ') hz Dzv
  have huXsucc :
      retainedPassiveF2SuccessorFamilyLinearMap (ρ := ρ) (κ' := κ')
          (((retainedPassiveFormalRawF2CLinearEquivAt
              (ρ := ρ) (κ' := κ') hz).symm (u.2.1, u.2.2.2.1)).1) =
        XsuccF2 := by
    rw [huF2C]
    simpa [XsuccF2, Dzv] using
      retainedPassiveF2SuccessorFamilyLinearMap_formalRawF2CLinearEquivAt_symm_targetEdgePairShearAt
        (ρ := ρ) (κ' := κ') hz Dzv
  have huTail :
      retainedPassivePostA1passiveTailFDerivLinearMapAt
  ...
  change u.2.2.2.2.1
      - retainedPassiveF2SuccessorFamilyLinearMap (ρ := ρ) (κ' := κ')
          (((retainedPassiveFormalRawF2CLinearEquivAt
              (ρ := ρ) (κ' := κ') hz).symm (u.2.1, u.2.2.2.1)).1) 0 *
          coord.solvedA3 0
      - coord.F2 (0 : Fin (M + 1)).succ *
          rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') u 0
      + Tail⁻¹ *
      ...
      ((retainedPassiveFormalRawOrderJacobianAt
          (ρ := ρ) (κ' := κ') z) v).2.2.2.2.1
  rw [huCtop, huXsucc, huA3, huTail]
  exact hCtop

private abbrev retainedPassiveRawF3RestFamily
    {M : ℕ} (ρ : Type*) (κ' : Fin (M + 2) → Type*) :=
  retainedPassiveRawA1PassiveFamily (M := M) ρ ×
    (retainedPassiveRawF2Family (M := M) ρ κ' ×
      (retainedPassiveRawA3PassiveFamily (M := M) ρ κ' ×
...
/-- Regroup the raw tuple with `F3` as the first product factor. -/
private def retainedPassiveRawF3FocusLinearEquiv
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*} :
    RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ ≃ₗ[ℝ]
      (retainedPassiveRawF3Family (M := M) ρ κ' ×
        retainedPassiveRawF3RestFamily (M := M) ρ κ') where
  toFun w :=
    (w.2.2.2.2.2,
      (w.1, (w.2.1, (w.2.2.1, (w.2.2.2.1, w.2.2.2.2.1)))))
  invFun x :=
    (x.2.1,
      (x.2.2.1,
        (x.2.2.2.1, (x.2.2.2.2.1, (x.2.2.2.2.2, x.1)))))
  left_inv w := by
    rcases w with ⟨A1passive, F2, A3passive, C, Ctop, F3⟩
    rfl
  right_inv x := by
    rcases x with ⟨F3, A1passive, F2, A3passive, C, Ctop⟩
    rfl
  map_add' _ _ := rfl
  map_smul' _ _ := rfl

@[simp]
private theorem retainedPassiveRawF3FocusLinearEquiv_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (w : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ) :
...
@[simp]
private theorem retainedPassiveRawF3FocusLinearEquiv_symm_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (x : retainedPassiveRawF3Family (M := M) ρ κ' ×
        retainedPassiveRawF3RestFamily (M := M) ρ κ') :
    (retainedPassiveRawF3FocusLinearEquiv
        (ρ := ρ) (κ' := κ')).symm x =
      (x.2.1,
        (x.2.2.1,
...
private def retainedPassiveRawF3RestCtopLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*} :
...
private def retainedPassiveRawF3RestA1passiveLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (q : Fin M) :
    retainedPassiveRawF3RestFamily (M := M) ρ κ' →ₗ[ℝ]
      Matrix ρ ρ ℝ where
...
private def retainedPassiveRawF3RestF2CProjectionLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*} :
    retainedPassiveRawF3RestFamily (M := M) ρ κ' →ₗ[ℝ]
      retainedPassiveRawF2CFamily (M := M) ρ κ' where
  toFun w := (w.2.1, w.2.2.2.1)
...
private def retainedPassiveRawF3RestA3passiveLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (q : Fin M) :
    retainedPassiveRawF3RestFamily (M := M) ρ κ' →ₗ[ℝ]
      Matrix (κ' q.castSucc.succ) ρ ℝ where
  toFun w := w.2.2.1 q
  map_add' _ _ := rfl
  map_smul' _ _ := rfl

private theorem retainedPassiveRawF3RestA3passiveLinearMapAt_apply_rawTupleRest
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    (w : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
    (q : Fin M) :
    retainedPassiveRawF3RestA3passiveLinearMapAt
        ...
        (retainedPassiveRawF3FocusLinearEquiv
          (ρ := ρ) (κ' := κ') w).2 =
      rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') w q.castSucc := by
  simp [retainedPassiveRawF3RestA3passiveLinearMapAt,
    rawEdgeTupleA3, ofTopologyTuple]

set_option linter.style.longLine false in
private def retainedPassivePostCtopTailFDerivLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
...
@[simp]
private theorem retainedPassivePostCtopTailFDerivLinearMapAt_self
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ) :
    retainedPassivePostCtopTailFDerivLinearMapAt
      ...
      (0 : retainedPassiveRawF3RestFamily (M := M) ρ κ' →ₗ[ℝ]
        Matrix ρ ρ ℝ) := by
  simp [retainedPassivePostCtopTailFDerivLinearMapAt]

set_option linter.style.longLine false in
private theorem retainedPassivePostCtopTailFDerivLinearMapAt_step_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ)
    ...
    retainedPassivePostCtopTailFDerivLinearMapAt
      ...
      retainedPassivePostCtopTailFDerivLinearMapAt
          (ρ := ρ) (κ' := κ') z (m + 1) (Nat.succ_le_of_lt hm) r *
        data.A1seed p + Psucc * r.1 q := by
  unfold retainedPassivePostCtopTailFDerivLinearMapAt
  rw [Nat.decreasingInduction_succ_left _ _ (Nat.succ_le_of_lt hm) (Nat.le_of_lt hm)]
  simp [retainedPassiveMatrixMulRightLinearMap,
    retainedPassiveMatrixMulLeftLinearMap,
    retainedPassiveRawF3RestA1passiveLinearMapAt]
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopTailFDerivLinearMapAt_fderiv_after_T123_eq_targetStaged
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  have hT123A1 :
    ...
    rw [retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_apply]
    rw [retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_apply]
    simpa [raw, Dzv, formal] using hT12A1
  let motive : (m : ℕ) → m ≤ M → Prop := fun m hm ↦
    retainedPassivePostCtopTailFDerivLinearMapAt
        (ρ := ρ) (κ' := κ') z m hm r =
      retainedPassiveA1TailTargetStagedFDerivAt
...
private def retainedPassivePostCtopSourcePairLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
private def retainedPassivePostCtopSourceCLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  (LinearMap.snd ℝ
      (retainedPassiveRawF2Family (M := M) ρ κ')
      (retainedPassiveRawCFamily (M := M) κ')).comp
    (retainedPassivePostCtopSourcePairLinearMapAt
      (ρ := ρ) (κ' := κ') hz)

private theorem retainedPassivePostCtopSourceCLinearMapAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    retainedPassivePostCtopSourceCLinearMapAt
        (ρ := ρ) (κ' := κ') hz r =
      ((retainedPassiveFormalRawF2CLinearEquivAt (ρ := ρ) (κ' := κ') hz).symm
        (r.2.1, r.2.2.2.1)).2 := by
  rfl

private def retainedPassivePostCtopSourceCAtLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    retainedPassiveRawF3RestFamily (M := M) ρ κ' →ₗ[ℝ]
      Matrix (κ' q.succ) (κ' q.castSucc) ℝ :=
  (LinearMap.proj q).comp
    (retainedPassivePostCtopSourceCLinearMapAt
      (ρ := ρ) (κ' := κ') hz)

private theorem retainedPassivePostCtopSourceCAtLinearMapAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    retainedPassivePostCtopSourceCAtLinearMapAt
        (ρ := ρ) (κ' := κ') hz q r =
      ((retainedPassiveFormalRawF2CLinearEquivAt (ρ := ρ) (κ' := κ') hz).symm
        (r.2.1, r.2.2.2.1)).2 q := by
  rfl

set_option linter.style.longLine false in
private theorem retainedPassivePostCtopSourceCAtLinearMapAt_after_T123_eq_targetRecovered
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  change
    retainedPassivePostCtopSourceCAtLinearMapAt
      ...
      retainedPassiveTargetRecoveredSourceCAt
        (M := M) (ρ := ρ) (κ' := κ') hz w q
  rw [retainedPassivePostCtopSourceCAtLinearMapAt_apply]
  change
    ((retainedPassiveFormalRawF2CLinearEquivAt
        (ρ := ρ) (κ' := κ') hz).symm (u.2.1, u.2.2.2.1)).2 q =
      retainedPassiveTargetRecoveredSourceCAt
        (M := M) (ρ := ρ) (κ' := κ') hz w q
  rw [hF2C]
  rfl
...
private def retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt
  ...
  let dTail :=
    retainedPassivePostCtopTailFDerivLinearMapAt
      (M := M + 1) (ρ := ρ) (κ' := κ') z 0 (Nat.zero_le (M + 1))
  let tailTerm :
      retainedPassiveRawF3RestFamily (M := M + 1) ρ κ' →ₗ[ℝ]
        Matrix ρ ρ ℝ :=
    (retainedPassiveMatrixMulRightLinearMap data.Ctop).comp
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt_zero_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ)
    (r : retainedPassiveRawF3RestFamily (M := M + 1) ρ κ') :
    let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
    let Tail :=
      ChartLocalSuffixState.retainedPassiveA1TailAfterFirst (K := ℝ) (ρ := ρ)
        data.A1seed
    let dTail :=
      retainedPassivePostCtopTailFDerivLinearMapAt
        (M := M + 1) (ρ := ρ) (κ' := κ') z 0 (Nat.zero_le (M + 1)) r
...
set_option linter.flexible false in
private theorem retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt_succ_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ)
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    (dNext :
      retainedPassiveRawF3RestFamily (M := M + 1) ρ κ' →ₗ[ℝ]
        Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ)
    (rest : retainedPassiveRawF3RestFamily (M := M + 1) ρ κ') :
    retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz q dAcur dPsucc dCnext dNext rest =
      let p : Fin ((M + 1) + 1) := q.castSucc
      let r : Fin ((M + 1) + 1) := q.succ
      let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
      let coord := data.toCoordinateData
      ...
      let Psucc : Matrix ρ ρ ℝ :=
        ChartLocalSuffixState.residualFactorProduct
          (K := ℝ) (κ := fun _ : Fin ((M + 1) + 2) ↦ ρ)
          coord.solvedA1 (Fin.last ((M + 1) + 1)) p.succ p.succ.le_last
      let dCnext' : Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' r.succ) ℝ := (by
        simpa [r] using dCnext rest)
      let dG : Matrix (κ' r.castSucc) ρ ℝ := (by
        simpa [r] using
          retainedPassiveRawF3RestA3passiveLinearMapAt
            (M := M + 1) (ρ := ρ) (κ' := κ') q rest);
      -((dCnext' * data.C r +
            Cnext *
              retainedPassivePostCtopSourceCAtLinearMapAt
                (M := M + 1) (ρ := ρ) (κ' := κ') hz r rest) *
          A3p * Pcast⁻¹)
        - (Cprod * dG * Pcast⁻¹)
        + Cprod * A3p * Pcast⁻¹ *
            (dPsucc rest * coord.solvedA1 p + Psucc * dAcur rest) *
            Pcast⁻¹
        + dNext rest := by
  dsimp [retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt,
    retainedPassiveMatrixMulRightLinearMap, retainedPassiveMatrixMulLeftLinearMap,
    LinearMap.comp_apply, LinearMap.add_apply, LinearMap.neg_apply, LinearMap.sub_apply]
  rfl
...
private def retainedPassivePostCtopF3ZeroCorrectionLinearMapAt
    ...
    retainedPassiveRawF3RestFamily (M := 0) ρ κ' →ₗ[ℝ]
      retainedPassiveRawF3Family (M := 0) ρ κ' :=
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  let coord := data.toCoordinateData
  (retainedPassiveMatrixMulLeftLinearMap coord.F3).comp
    retainedPassiveRawF3RestCtopLinearMapAt

set_option linter.style.longLine false in
private theorem retainedPassivePostCtopF3ZeroCorrectionLinearMapAt_apply
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ)
    (r : retainedPassiveRawF3RestFamily (M := 0) ρ κ') :
    let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
    let coord := data.toCoordinateData
    retainedPassivePostCtopF3ZeroCorrectionLinearMapAt
        (ρ := ρ) (κ' := κ') z r =
      coord.F3 * r.2.2.2.2 := by
  dsimp [retainedPassivePostCtopF3ZeroCorrectionLinearMapAt,
    retainedPassiveRawF3RestCtopLinearMapAt,
    retainedPassiveMatrixMulLeftLinearMap]
  rfl

set_option linter.style.longLine false in
/-- In the single-edge case after the `Ctop` stage, change only `F3` by the
target-staged terminal lower-left correction. -/
def retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ) :
    RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ ≃ₗ[ℝ]
      RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ :=
  retainedPassiveRawF3FocusLinearEquiv.trans
    ((linearEquivUpperShear
      (R := ℝ)
      (M := retainedPassiveRawF3Family (M := 0) ρ κ')
      (N := retainedPassiveRawF3RestFamily (M := 0) ρ κ')
      (retainedPassivePostCtopF3ZeroCorrectionLinearMapAt
        (ρ := ρ) (κ' := κ') z)).trans
      retainedPassiveRawF3FocusLinearEquiv.symm)

set_option linter.style.longLine false in
/-- Formula for the single-edge post-`Ctop` `F3` raw-tuple shear. -/
theorem retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_apply
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ)
    (w : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ) :
    let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
    let coord := data.toCoordinateData
      ...
      (w.1,
        (w.2.1,
          (w.2.2.1,
            (w.2.2.2.1,
              (w.2.2.2.2.1,
                w.2.2.2.2.2 + coord.F3 * w.2.2.2.2.1))))) := by
  rfl

/-- The single-edge post-`Ctop` `F3` raw-tuple shear has determinant one. -/
theorem retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_det_eq_one
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ) :
    LinearMap.det
      (retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt
        (ρ := ρ) (κ' := κ') z :
          RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
            RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ) = 1 := by
  let E :=
    retainedPassiveRawF3FocusLinearEquiv
      (M := 0) (ρ := ρ) (κ' := κ')
  let S :=
        ...
        (retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt
          (ρ := ρ) (κ' := κ') z :
            RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
              RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ) =
        LinearMap.det
          (S : (retainedPassiveRawF3Family (M := 0) ρ κ' ×
              retainedPassiveRawF3RestFamily (M := 0) ρ κ') →ₗ[ℝ]
              (retainedPassiveRawF3Family (M := 0) ρ κ' ×
                retainedPassiveRawF3RestFamily (M := 0) ρ κ')) := by
    simpa [retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt,
      E, S] using
      (LinearMap.det_conj
        (S : (retainedPassiveRawF3Family (M := 0) ρ κ' ×
            retainedPassiveRawF3RestFamily (M := 0) ρ κ') →ₗ[ℝ]
            (retainedPassiveRawF3Family (M := 0) ρ κ' ×
              retainedPassiveRawF3RestFamily (M := 0) ρ κ'))
        E.symm)
  rw [hconj]
  exact
    linearEquivUpperShear_det_eq_one
      ...
      (M := retainedPassiveRawF3Family (M := 0) ρ κ')
      (N := retainedPassiveRawF3RestFamily (M := 0) ρ κ')
      (f := retainedPassivePostCtopF3ZeroCorrectionLinearMapAt
        (ρ := ρ) (κ' := κ') z)

/-- The single-edge post-`Ctop` `F3` raw-tuple shear has absolute determinant
one. -/
theorem retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_abs_det_eq_one
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    (z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ) :
    |LinearMap.det
      (retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt
        (ρ := ρ) (κ' := κ') z :
          RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
            RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ)| = 1 := by
  rw [retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_det_eq_one
    (ρ := ρ) (κ' := κ') z, abs_one]

set_option linter.style.longLine false in
/-- In the single-edge case, the first four target-side normalisation stages:
edge pair, passive `A1`, `Ctop`, and terminal `F3`. -/
def retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ}
...
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt_apply
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ}
...
set_option linter.style.longLine false in
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt_abs_det_eq_one
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ}
  ...
  have hE :
      |LinearMap.det
        (E : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
          RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ)| = 1 := by
    simpa [E] using
  ...
  have hS :
      |LinearMap.det
        (S : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
          RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ)| = 1 := by
    simpa [S] using
      retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_abs_det_eq_one
        (ρ := ρ) (κ' := κ') z
  change |LinearMap.det ((S : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
      RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ).comp
        (E : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ →ₗ[ℝ]
          RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ))| = 1
  rw [LinearMap.det_comp, abs_mul, hS, hE, one_mul]

set_option linter.style.longLine false in
private def retainedPassivePostCtopF3PosCorrectionLinearMapAt
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    (hz : z ∈ topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ')) :
    retainedPassiveRawF3RestFamily (M := M + 1) ρ κ' →ₗ[ℝ]
      retainedPassiveRawF3Family (M := M + 1) ρ κ' :=
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  let coord := data.toCoordinateData
  let qLast : Fin (M + 1) := Fin.last M
  let q0 : Fin (M + 1) := 0
  let p0 : Fin ((M + 1) + 1) := q0.castSucc
  let Early : Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ :=
    ChartLocalSuffixState.retainedPassiveLowerLeftProductTailSum
      (K := ℝ) (ρ := ρ) (κ := κ')
      coord.solvedA1
      (ChartLocalSuffixState.retainedPassiveA3WithoutLast (K := ℝ) (ρ := ρ)
        data.A3seed)
      data.C p0.val (Nat.le_of_lt p0.isLt)
  let dEarly :=
    retainedPassivePostCtopLowerLeftProductTailFDerivLinearMapAt
      (M := M) (ρ := ρ) (κ' := κ') hz 0 (Nat.zero_le (M + 1));
  -((retainedPassiveMatrixMulRightLinearMap
      (coord.solvedA1 (Fin.last (M + 1)))).comp dEarly) +
    (retainedPassiveMatrixMulLeftLinearMap (coord.F3 - Early)).comp
      (retainedPassiveRawF3RestA1passiveLinearMapAt
        (M := M + 1) (ρ := ρ) (κ' := κ') qLast)

set_option linter.style.longLine false in
private theorem retainedPassivePostCtopF3PosCorrectionLinearMapAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    let dEarly :=
      retainedPassivePostCtopLowerLeftProductTailFDerivLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz 0 (Nat.zero_le (M + 1)) r;
    retainedPassivePostCtopF3PosCorrectionLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz r =
      -(dEarly * coord.solvedA1 (Fin.last (M + 1))) +
        (coord.F3 - Early) * r.1 qLast := by
  dsimp [retainedPassivePostCtopF3PosCorrectionLinearMapAt,
    retainedPassiveRawF3RestA1passiveLinearMapAt,
    retainedPassiveMatrixMulRightLinearMap,
    retainedPassiveMatrixMulLeftLinearMap]

set_option linter.style.longLine false in
/-- In the positive-tail case after the `Ctop` stage, change only `F3` by the
target-staged final lower-left correction. -/
def retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    (hz : z ∈ topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ')) :
    RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ ≃ₗ[ℝ]
      RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ :=
  retainedPassiveRawF3FocusLinearEquiv.trans
    ((linearEquivUpperShear
      (R := ℝ)
      (M := retainedPassiveRawF3Family (M := M + 1) ρ κ')
      (N := retainedPassiveRawF3RestFamily (M := M + 1) ρ κ')
      (retainedPassivePostCtopF3PosCorrectionLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz)).trans
      retainedPassiveRawF3FocusLinearEquiv.symm)

set_option linter.style.longLine false in
/-- Formula for the positive-tail post-`Ctop` `F3` raw-tuple shear. -/
theorem retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    let dEarly :=
      retainedPassivePostCtopLowerLeftProductTailFDerivLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz 0 (Nat.zero_le (M + 1)) rest
          ...
          (w.2.2.1,
            (w.2.2.2.1,
              (w.2.2.2.2.1,
                w.2.2.2.2.2
                  - dEarly * coord.solvedA1 (Fin.last (M + 1))
                  + (coord.F3 - Early) * w.1 qLast))))) := by
  rw [retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt]
  simp [retainedPassivePostCtopF3PosCorrectionLinearMapAt_apply,
    sub_eq_add_neg, add_assoc]

set_option linter.style.longLine false in
/-- The positive-tail post-`Ctop` `F3` raw-tuple shear has determinant one. -/
theorem retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_det_eq_one
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    LinearMap.det
      (retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt
        (M := M) (ρ := ρ) (κ' := κ') hz :
          RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
            RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ) = 1 := by
  let E :=
    retainedPassiveRawF3FocusLinearEquiv
      (M := M + 1) (ρ := ρ) (κ' := κ')
  let S :=
        ...
        (retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt
          (M := M) (ρ := ρ) (κ' := κ') hz :
            RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
              RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ) =
        LinearMap.det
          (S : (retainedPassiveRawF3Family (M := M + 1) ρ κ' ×
              retainedPassiveRawF3RestFamily (M := M + 1) ρ κ') →ₗ[ℝ]
              (retainedPassiveRawF3Family (M := M + 1) ρ κ' ×
                retainedPassiveRawF3RestFamily (M := M + 1) ρ κ')) := by
    simpa [retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt,
      E, S] using
      (LinearMap.det_conj
        (S : (retainedPassiveRawF3Family (M := M + 1) ρ κ' ×
            retainedPassiveRawF3RestFamily (M := M + 1) ρ κ') →ₗ[ℝ]
            (retainedPassiveRawF3Family (M := M + 1) ρ κ' ×
              retainedPassiveRawF3RestFamily (M := M + 1) ρ κ'))
        E.symm)
  rw [hconj]
  exact
    linearEquivUpperShear_det_eq_one
      ...
      (M := retainedPassiveRawF3Family (M := M + 1) ρ κ')
      (N := retainedPassiveRawF3RestFamily (M := M + 1) ρ κ')
      (f := retainedPassivePostCtopF3PosCorrectionLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz)

/-- The positive-tail post-`Ctop` `F3` raw-tuple shear has absolute determinant
one. -/
theorem retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_abs_det_eq_one
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    |LinearMap.det
      (retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt
        (M := M) (ρ := ρ) (κ' := κ') hz :
          RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
            RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ)| = 1 := by
  rw [retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_det_eq_one
    (M := M) (ρ := ρ) (κ' := κ') hz, abs_one]

set_option linter.style.longLine false in
/-- In the positive-tail case, the first four target-side normalisation
stages: edge pair, passive `A1`, `Ctop`, and terminal `F3`. -/
def retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
...
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
...
set_option linter.style.longLine false in
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt_abs_det_eq_one
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
  ...
  have hE :
      |LinearMap.det
        (E : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
          RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ)| = 1 := by
    simpa [E] using
      retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_abs_det_eq_one
        (ρ := ρ) (κ' := κ') hz
  have hS :
      |LinearMap.det
        (S : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
          RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ)| = 1 := by
    simpa [S] using
      retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_abs_det_eq_one
        (M := M) (ρ := ρ) (κ' := κ') hz
  change |LinearMap.det ((S : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
      RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ).comp
        (E : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ →ₗ[ℝ]
          RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ))| = 1
  rw [LinearMap.det_comp, abs_mul, hS, hE, one_mul]

private theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_F3
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
set_option linter.style.longLine false in
private theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_A1passive
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopSolvedA1TangentLinearMapAt_after_T123_eq_targetStaged
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
    ...
    let rest :=
      (retainedPassiveRawF3FocusLinearEquiv
        (ρ := ρ) (κ' := κ') u).2
    retainedPassivePostCtopSolvedA1TangentLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') z m hm rest =
      retainedPassiveSolvedA1TargetStagedTangentAt
        (M := M) (ρ := ρ) (κ' := κ') z Dzv m hm := by
  let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
  let Dzv := (fderiv ℝ raw z) v
  let u :=
    retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt
  ...
  · subst m
    ...
    let dTailPost :=
      retainedPassivePostCtopTailFDerivLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') z 0 (Nat.zero_le M) rest
    let dTailTarget :=
      retainedPassiveA1TailTargetStagedFDerivAt
        (M := M) (ρ := ρ) (κ' := κ') z Dzv 0 (Nat.zero_le M)
    have hCtop :=
    ...
    have hRecCtop :=
      retainedPassiveTargetRecoveredSourceCtopAt_fderiv_eq_sourceCtop
        (M := M) (ρ := ρ) (κ' := κ') hz v
    change dTailPost = dTailTarget at hTail
    change retainedPassiveTargetRecoveredSourceCtopAt
        (M := M) (ρ := ρ) (κ' := κ') z Dzv =
      v.2.2.2.2.1 at hRecCtop
    change u.2.2.2.2.1 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.1 at hCtop
    dsimp [retainedPassiveFormalRawOrderJacobianAt] at hCtop
    rw [retainedPassiveFormalRawOrderJacobian_apply] at hCtop
    calc
      retainedPassivePostCtopSolvedA1TangentLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') z 0 (Nat.succ_pos M) rest
          =
        u.2.2.2.2.1 - Tail⁻¹ * dTailPost * Tail⁻¹ * data.Ctop := by
          simp [retainedPassivePostCtopSolvedA1TangentLinearMapAt_zero_apply,
            rest, u, data, Tail, dTailPost]
      _ =
        Tail⁻¹ * retainedPassiveTargetRecoveredSourceCtopAt
            (M := M) (ρ := ρ) (κ' := κ') z Dzv
          - Tail⁻¹ * dTailTarget * Tail⁻¹ * data.Ctop := by
          rw [hTail, hCtop, hRecCtop]
      _ =
        retainedPassiveSolvedA1TargetStagedTangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv 0 (Nat.succ_pos M) := by
          rw [retainedPassiveSolvedA1TargetStagedTangentAt_zero]
  · obtain ⟨k, rfl⟩ := Nat.exists_eq_succ_of_ne_zero h0
    let q : Fin M := ⟨k, Nat.succ_lt_succ_iff.mp hm⟩
    have hpost :=
      retainedPassivePostCtopSolvedA1TangentLinearMapAt_succ_apply
        (M := M) (ρ := ρ) (κ' := κ') z q rest
    have hpost' :
        retainedPassivePostCtopSolvedA1TangentLinearMapAt
            (M := M) (ρ := ρ) (κ' := κ') z (k + 1) hm rest =
          rest.1 q := by
      simpa [q] using hpost
    have htarget :=
    ...
    have htarget' :
        retainedPassiveSolvedA1TargetStagedTangentAt
            (M := M) (ρ := ρ) (κ' := κ') z Dzv (k + 1) hm =
          retainedPassiveTargetStagedA1passiveTangentAt
            (M := M) (ρ := ρ) (κ' := κ') z Dzv q := by
      simpa [q] using htarget
    have hA1formal :=
      retainedPassiveTargetEdgePairThenA1passiveShear_fderiv_A1passive_eq_formalRawOrderJacobianAt
        (M := M) (ρ := ρ) (κ' := κ') hz v
    have hA1formal_q := congrFun hA1formal q
    have hA1source :=
      retainedPassiveTargetStagedA1passiveTangentAt_fderiv_eq_source
        (M := M) (ρ := ρ) (κ' := κ') hz v q
    change retainedPassiveTargetStagedA1passiveTangentAt
    ...
    have hT123A1 :
      ...
      calc
        (retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt
            (ρ := ρ) (κ' := κ') hz Dzv).1 q =
          v.1 q := by
            simpa [raw, Dzv, retainedPassiveFormalRawOrderJacobianAt,
              retainedPassiveFormalRawOrderJacobian_apply] using hA1formal_q
        _ =
          retainedPassiveTargetStagedA1passiveTangentAt
            (M := M) (ρ := ρ) (κ' := κ') z Dzv q := hA1source.symm
    calc
      retainedPassivePostCtopSolvedA1TangentLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') z (k + 1) hm rest
          = rest.1 q := hpost'
      ...
      _ =
        retainedPassiveTargetStagedA1passiveTangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv q := hT123A1
      _ =
        retainedPassiveSolvedA1TargetStagedTangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv (k + 1) hm := htarget'.symm

set_option linter.style.longLine false in
private theorem retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt_after_T123_eq_targetStaged
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  have hbase : motive (M + 1) le_rfl := by
    dsimp [motive]
    rw [retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt_self,
  ...
  have hstep : ∀ n (hns : n + 1 ≤ M + 1),
    ...
    have hpost :=
      retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt_step_apply
        (M := M) (ρ := ρ) (κ' := κ') z rest n (Nat.lt_of_succ_le hns)
    have htarget :=
      retainedPassiveSolvedA1SuffixTargetStagedFDerivAt_step
        (M := M) (ρ := ρ) (κ' := κ') z Dzv n (Nat.lt_of_succ_le hns)
    have htangent :=
    ...
    calc
      retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') z n (Nat.le_of_succ_le hns) rest
          =
        retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt
            (M := M) (ρ := ρ) (κ' := κ') z (n + 1) hns rest *
          coord.solvedA1 p +
            Psucc *
              retainedPassivePostCtopSolvedA1TangentLinearMapAt
                (M := M) (ρ := ρ) (κ' := κ') z n
                (Nat.lt_of_succ_le hns) rest := by
          simpa [data, coord, p, Psucc] using hpost
      _ =
        retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
            (M := M) (ρ := ρ) (κ' := κ') z Dzv (n + 1) hns *
          coord.solvedA1 p +
            Psucc *
              retainedPassiveSolvedA1TargetStagedTangentAt
                ...
                (Nat.lt_of_succ_le hns) := by
          rw [ih]
          simpa [raw, Dzv, u, rest] using congrArg
            (fun X ↦
              retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
                  (M := M) (ρ := ρ) (κ' := κ') z Dzv (n + 1) hns *
                coord.solvedA1 p + Psucc * X) htangent
      _ =
        retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv n
          (Nat.le_of_succ_le hns) := by
          simpa [data, coord, p, Psucc] using htarget.symm
  simpa [motive, raw, Dzv, u, rest] using
    Nat.decreasingInduction (motive := motive) hstep hbase hm

set_option linter.style.longLine false in
private theorem retainedPassivePostCtopCSuffixFDerivLinearMapAt_after_T123_eq_targetStaged
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M) ρ κ' ℝ}
  ...
  have hbase : motive (M + 1) le_rfl := by
    dsimp [motive]
    rw [retainedPassivePostCtopCSuffixFDerivLinearMapAt_self,
  ...
  have hstep : ∀ n (hns : n + 1 ≤ M + 1),
    ...
    have hsource :=
      retainedPassivePostCtopSourceCAtLinearMapAt_after_T123_eq_targetRecovered
        (M := M) (ρ := ρ) (κ' := κ') hz Dzv p
    let dPostSucc : Matrix (κ' (Fin.last (M + 1))) (κ' p.succ) ℝ := by
      simpa [p] using
        retainedPassivePostCtopCSuffixFDerivLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') hz (n + 1) hns rest
    let dTargetSucc : Matrix (κ' (Fin.last (M + 1))) (κ' p.succ) ℝ := by
      simpa [p] using
        retainedPassiveCSuffixTargetStagedFDerivAt
          (M := M) (ρ := ρ) (κ' := κ') hz Dzv (n + 1) hns
    have hpostFormula :
        retainedPassivePostCtopCSuffixFDerivLinearMapAt
            (M := M) (ρ := ρ) (κ' := κ') hz n (Nat.le_of_succ_le hns) rest =
          dPostSucc * data.C p +
            Csucc *
              retainedPassivePostCtopSourceCAtLinearMapAt
                (M := M) (ρ := ρ) (κ' := κ') hz p rest := by
      simpa [data, p, Csucc, dPostSucc, retainedPassiveMatrixMulRightLinearMap,
        retainedPassiveMatrixMulLeftLinearMap, LinearMap.add_apply,
        LinearMap.comp_apply] using hpost
    have hreplace :
        dPostSucc * data.C p +
            Csucc *
              retainedPassivePostCtopSourceCAtLinearMapAt
                (M := M) (ρ := ρ) (κ' := κ') hz p rest =
          dTargetSucc * data.C p +
            Csucc *
              retainedPassiveTargetRecoveredSourceCAt
                (M := M) (ρ := ρ) (κ' := κ') hz Dzv p := by
      have hdSucc : dPostSucc = dTargetSucc := by
        simpa [dPostSucc, dTargetSucc, p] using ih
      rw [hdSucc]
      simpa [raw, Dzv, u, rest] using congrArg
        (fun X ↦ dTargetSucc * data.C p + Csucc * X) hsource
    have hold :
        dTargetSucc * data.C p +
            Csucc *
              retainedPassiveTargetRecoveredSourceCAt
          ...
          retainedPassiveCSuffixTargetStagedFDerivAt
            (M := M) (ρ := ρ) (κ' := κ') hz Dzv n
            (Nat.le_of_succ_le hns) := by
      simpa [retainedPassiveCTailTargetOnlyStepAt, data, p, Csucc,
        dTargetSucc] using htarget.symm
    exact hpostFormula.trans (hreplace.trans hold)
  simpa [motive, raw, Dzv, u, rest] using
    Nat.decreasingInduction (motive := motive) hstep hbase hm
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt_after_T123_eq_targetOnly
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
  ...
  · subst m
    ...
    let dTailPost :=
      retainedPassivePostCtopTailFDerivLinearMapAt
        (M := M + 1) (ρ := ρ) (κ' := κ') z 0 (Nat.zero_le (M + 1)) rest
    let dTailTarget :=
      retainedPassiveA1TailTargetStagedFDerivAt
        (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv 0 (Nat.zero_le (M + 1))
    have hCtop :=
    ...
    have hTail :=
      retainedPassivePostCtopTailFDerivLinearMapAt_fderiv_after_T123_eq_targetStaged
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v 0 (Nat.zero_le (M + 1))
    have hRecCtop :=
      retainedPassiveTargetRecoveredSourceCtopAt_fderiv_eq_sourceCtop
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v
    change dTailPost = dTailTarget at hTail
    ...
    calc
      retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt
          ...
          =
        u.2.2.2.2.1 - Tail⁻¹ * dTailPost * Tail⁻¹ * data.Ctop := by
          simp [retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt_zero_apply,
            rest, u, data, Tail, dTailPost]
      _ =
        Tail⁻¹ * retainedPassiveTargetRecoveredSourceCtopAt
            (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv
      ...
      _ =
        retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv 0 (Nat.succ_pos M) := by
          rw [retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt_zero]
  · obtain ⟨k, rfl⟩ := Nat.exists_eq_succ_of_ne_zero h0
    let q : Fin M := ⟨k, Nat.succ_lt_succ_iff.mp hm⟩
    have hpost :=
    ...
    have htarget' :
        retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt
            (M := M) (ρ := ρ) (κ' := κ') z Dzv (k + 1) hm =
          retainedPassiveTargetStagedA1passiveTangentAt
            (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv q.castSucc := by
      simpa [q] using htarget
    have hA1formal :=
      retainedPassiveTargetEdgePairThenA1passiveShear_fderiv_A1passive_eq_formalRawOrderJacobianAt
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v
    have hA1formal_q :=
      congrFun hA1formal q.castSucc
    have hA1source :=
      retainedPassiveTargetStagedA1passiveTangentAt_fderiv_eq_source
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v q.castSucc
    change retainedPassiveTargetStagedA1passiveTangentAt
        (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv q.castSucc =
      v.1 q.castSucc at hA1source
    have hT123A1 :
        u.1 q.castSucc =
          retainedPassiveTargetStagedA1passiveTangentAt
            (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv q.castSucc := by
      ...
      calc
        ...
        _ =
          retainedPassiveTargetStagedA1passiveTangentAt
            (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv q.castSucc := hA1source.symm
    calc
      retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') z (k + 1) hm rest
          = rest.1 q.castSucc := hpost'
      _ = u.1 q.castSucc := rfl
      _ =
        retainedPassiveTargetStagedA1passiveTangentAt
          (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv q.castSucc := hT123A1
      _ =
        retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv (k + 1) hm := htarget'.symm
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopCnextFDerivLinearMapAt_after_T123_eq_targetStaged
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    let rest :=
      (retainedPassiveRawF3FocusLinearEquiv
        (ρ := ρ) (κ' := κ') u).2
    retainedPassivePostCtopCnextFDerivLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz q rest =
      retainedPassiveCnextTargetStagedFDerivAt
        (M := M) (ρ := ρ) (κ' := κ') hz Dzv q := by
  let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
  let Dzv := (fderiv ℝ raw z) v
  let u :=
    retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt
  ...
  have hsuffix :=
    retainedPassivePostCtopCSuffixFDerivLinearMapAt_after_T123_eq_targetStaged
      (M := M + 1) (ρ := ρ) (κ' := κ') hz v r.succ.val hm
  simpa [retainedPassivePostCtopCnextFDerivLinearMapAt,
    retainedPassiveCnextTargetStagedFDerivAt, raw, Dzv, u, rest, r, hm] using hsuffix
...
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt_after_T123_eq_targetOnly
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    (dNext :
      retainedPassiveRawF3RestFamily (M := M + 1) ρ κ' →ₗ[ℝ]
        Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ)
    (dNextTarget : Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ)
    (hNext :
      let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
      let Dzv := (fderiv ℝ raw z) v
      let u :=
    ...
    retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz q
        (retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') z q.val q.isLt)
        (retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt
          (M := M + 1) (ρ := ρ) (κ' := κ') z p.succ.val
          (Fin.val_fin_le.mp p.succ.le_last))
        (retainedPassivePostCtopCnextFDerivLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') hz q)
        dNext rest =
      retainedPassiveLowerLeftTailTargetOnlyStepCoreWithCnextAt
        ...
        (retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv q.val q.isLt)
        (retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
          (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv p.succ.val
          (Fin.val_fin_le.mp p.succ.le_last))
        dNextTarget := by
  let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
  let Dzv := (fderiv ℝ raw z) v
  ...
  let p : Fin ((M + 1) + 1) := q.castSucc
  let r : Fin ((M + 1) + 1) := q.succ
  change dNext rest = dNextTarget at hNext
  have hAcur :
      retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') z q.val q.isLt rest =
        retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt
          (M := M) (ρ := ρ) (κ' := κ') z Dzv q.val q.isLt := by
    simpa [raw, Dzv, u, rest] using
      retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt_after_T123_eq_targetOnly
        (M := M) (ρ := ρ) (κ' := κ') hz v q.val q.isLt
  have hPsucc :
      retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt
          (M := M + 1) (ρ := ρ) (κ' := κ') z p.succ.val
          (Fin.val_fin_le.mp p.succ.le_last) rest =
        retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
          (M := M + 1) (ρ := ρ) (κ' := κ') z Dzv p.succ.val
          (Fin.val_fin_le.mp p.succ.le_last) := by
    simpa [raw, Dzv, u, rest, p] using
      retainedPassivePostCtopSolvedA1SuffixFDerivLinearMapAt_after_T123_eq_targetStaged
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v p.succ.val
        (Fin.val_fin_le.mp p.succ.le_last)
  have hCnext :
      retainedPassivePostCtopCnextFDerivLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') hz q rest =
  ...
  have hCcur :
      retainedPassivePostCtopSourceCAtLinearMapAt
          (M := M + 1) (ρ := ρ) (κ' := κ') hz r rest =
        retainedPassiveTargetRecoveredSourceCAt
          (M := M + 1) (ρ := ρ) (κ' := κ') hz Dzv r := by
    simpa [raw, Dzv, u, rest, r] using
      retainedPassivePostCtopSourceCAtLinearMapAt_after_T123_eq_targetRecovered
        (M := M + 1) (ρ := ρ) (κ' := κ') hz Dzv r
  have hA3rest :
      retainedPassiveRawF3RestA3passiveLinearMapAt
          (M := M + 1) (ρ := ρ) (κ' := κ') q rest =
        rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') u q.castSucc := by
    simpa [u, rest] using
      retainedPassiveRawF3RestA3passiveLinearMapAt_apply_rawTupleRest
        (M := M + 1) (ρ := ρ) (κ' := κ') u q
  have hA3pres :
      rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') u q.castSucc =
        rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') Dzv q.castSucc := by
    simpa [u] using
      rawEdgeTupleA3_retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt
        (M := M + 1) (ρ := ρ) (κ' := κ') hz Dzv q.castSucc
  have hA3 :
      retainedPassiveRawF3RestA3passiveLinearMapAt
          (M := M + 1) (ρ := ρ) (κ' := κ') q rest =
        rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') Dzv q.castSucc :=
    hA3rest.trans hA3pres
  dsimp [p, r] at hPsucc hCcur
  change
    retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt
        (M := M) (ρ := ρ) (κ' := κ') hz q
        (retainedPassivePostCtopCurrentSolvedA1TangentLinearMapAt
      ...
      retainedPassiveLowerLeftTailTargetOnlyStepCoreWithCnextAt
        ...
        (retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
          ...
          (Fin.val_fin_le.mp p.succ.le_last))
        dNextTarget
  rw [retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt_apply]
  dsimp [p, r]
  rw [hCnext, hCcur, hA3, hPsucc, hAcur, hNext]
  simp [retainedPassiveLowerLeftTailTargetOnlyStepCoreWithCnextAt,
    retainedPassiveLowerLeftTailTargetOnlyStepCoreAt, rawEdgeTupleA3, ofTopologyTuple]
...
set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
private theorem retainedPassivePostCtopLowerLeftProductTailFDerivLinearMapAt_after_T123_eq_targetOnly
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
  ...
  have hstep : ∀ n (hns : n + 1 ≤ M + 1),
    ...
    have hNext :
        ...
        let rest :=
          (retainedPassiveRawF3FocusLinearEquiv
            (ρ := ρ) (κ' := κ') u).2
        postNext rest = targetNext := by
      simpa [raw, Dzv, u, rest, postNext, targetNext, motive] using ih
    have hcore :=
      retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt_after_T123_eq_targetOnly
        (M := M) (ρ := ρ) (κ' := κ') hz v q postNext targetNext hNext
    calc
      retainedPassivePostCtopLowerLeftProductTailFDerivLinearMapAt
          (M := M) (ρ := ρ) (κ' := κ') hz n (Nat.le_of_succ_le hns) rest =
        retainedPassivePostCtopLowerLeftTailStepCoreLinearMapAt
          ...
          (retainedPassivePostCtopCnextFDerivLinearMapAt
            (M := M) (ρ := ρ) (κ' := κ') hz q)
          postNext rest := by
          simpa [q, p, postNext] using congrArg (fun L ↦ L rest) hpostStep
      _ =
        retainedPassiveLowerLeftTailTargetOnlyStepCoreWithCnextAt
          (M := M) (ρ := ρ) (κ' := κ') hz Dzv q
          (retainedPassiveLowerLeftTailCurrentTargetOnlySolvedA1TangentAt
          ...
          (retainedPassiveSolvedA1SuffixTargetStagedFDerivAt
            ...
            (Fin.val_fin_le.mp p.succ.le_last))
          targetNext := by
          simpa [raw, Dzv, u, rest, q, p, postNext, targetNext] using hcore
      _ =
        retainedPassiveLowerLeftProductTailTargetOnlyFDerivAt
          (M := M) (ρ := ρ) (κ' := κ') hz Dzv n (Nat.le_of_succ_le hns) := by
          simpa [q, p, targetNext] using htargetStep.symm
  simpa [motive, raw, Dzv, u, rest] using
    Nat.decreasingInduction (motive := motive) hstep hbase hm
...
set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- In the positive-tail case, after the first four target-side normalisation
stages, the terminal `F3` component of the actual raw-order derivative agrees
with the formal raw-order Jacobian. -/
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShear_fderiv_F3_eq_formalRawOrderJacobianAt
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := M + 1) ρ κ' ℝ}
    ...
    let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
    (retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt
        (M := M) (ρ := ρ) (κ' := κ') hz ((fderiv ℝ raw z) v)).2.2.2.2.2 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.2 := by
  let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  ...
  have hF3 :=
    F3_tail_pos_targetOnly_dEarly_dLast_target_staged_shear_fderiv_topologyTupleEdgeRawOrder_eq_formalRawOrderJacobianAt
      (M := M) (ρ := ρ) (κ' := κ') hz v
  have huF3 :
      u.2.2.2.2.2 = Dzv.2.2.2.2.2 := by
    simpa [u] using
      retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_F3
        (M := M + 1) (ρ := ρ) (κ' := κ') hz Dzv
  have hDEarly : postDEarly = dEarly := by
    simpa [raw, Dzv, u, rest, postDEarly, dEarly] using
      retainedPassivePostCtopLowerLeftProductTailFDerivLinearMapAt_after_T123_eq_targetOnly
        (M := M) (ρ := ρ) (κ' := κ') hz v 0 (Nat.zero_le (M + 1))
  have hT123A1 :
      u.1 qLast =
        (retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt
          (ρ := ρ) (κ' := κ') hz Dzv).1 qLast := by
    have h :=
      retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_A1passive
        (M := M + 1) (ρ := ρ) (κ' := κ') hz Dzv
    simpa [u] using congrArg (fun A : Fin (M + 1) → Matrix ρ ρ ℝ ↦ A qLast) h
  have hT12A1formal :
      (retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt
          (ρ := ρ) (κ' := κ') hz Dzv).1 qLast =
        ((retainedPassiveFormalRawOrderJacobianAt
          (ρ := ρ) (κ' := κ') z) v).1 qLast := by
    have h :=
      retainedPassiveTargetEdgePairThenA1passiveShear_fderiv_A1passive_eq_formalRawOrderJacobianAt
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v
    simpa [raw, Dzv] using congrArg (fun A : Fin (M + 1) → Matrix ρ ρ ℝ ↦ A qLast) h
  have hA1stage :=
    A1passive_target_staged_shear_fderiv_topologyTupleEdgeRawOrder_eq_formalRawOrderJacobianAt
      (M := M + 1) (ρ := ρ) (κ' := κ') hz v qLast
  have hA1 :
      u.1 qLast =
        Dzv.1 qLast
          - XsuccF2 qLast.succ * coord.solvedA3 qLast.succ
          - coord.F2 qLast.succ.succ *
              rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') Dzv qLast.succ := by
    change Dzv.1 qLast
        - XsuccF2 qLast.succ * coord.solvedA3 qLast.succ
        - coord.F2 qLast.succ.succ *
            rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') Dzv qLast.succ =
      ((retainedPassiveFormalRawOrderJacobianAt
          (ρ := ρ) (κ' := κ') z) v).1 qLast at hA1stage
    exact hT123A1.trans (hT12A1formal.trans hA1stage.symm)
  change Dzv.2.2.2.2.2
      - dEarly * coord.solvedA1 (Fin.last (M + 1))
      + (coord.F3 - Earlyfun z) *
          (Dzv.1 qLast
  ...
  change u.2.2.2.2.2
      - postDEarly * coord.solvedA1 (Fin.last (M + 1))
      + (coord.F3 - Earlyfun z) * u.1 qLast =
    ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.2
  rw [huF3, hDEarly, hA1]
  exact hF3
...
set_option linter.style.longLine false in
/-- In the single-edge case, after the first four target-side normalisation
stages, the terminal `F3` component of the actual raw-order derivative agrees
with the formal raw-order Jacobian. -/
theorem retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShear_fderiv_F3_eq_formalRawOrderJacobianAt
    {ρ : Type*} {κ' : Fin 2 → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Fintype (κ' j)]
    [∀ j, DecidableEq (κ' j)]
    {z : RetainedPassiveRawTopologyTuple (M := 0) ρ κ' ℝ}
    ...
    let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
    (retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt
        (ρ := ρ) (κ' := κ') hz ((fderiv ℝ raw z) v)).2.2.2.2.2 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.2 := by
  let raw := topologyTupleEdgeRawOrder (K := ℝ) (ρ := ρ) (κ' := κ')
  let Dzv := (fderiv ℝ raw z) v
  ...
  have hExprU :
    ...
    change Dzv.2.2.2.2.1
        ...
        - coord.F2 (0 : Fin 1).succ *
            rawEdgeTupleA3 (K := ℝ) (ρ := ρ) (κ' := κ') Dzv 0 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.1 at hCtopExpr
    change u.2.2.2.2.1 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.1 at huCtop
    exact hCtopExpr.trans huCtop.symm
  change Dzv.2.2.2.2.2
      + coord.F3 *
          (Dzv.2.2.2.2.1
            - XsuccF2 0 * coord.solvedA3 0
  ...
  change u.2.2.2.2.2 + coord.F3 * u.2.2.2.2.1 =
      ((retainedPassiveFormalRawOrderJacobianAt
        (ρ := ρ) (κ' := κ') z) v).2.2.2.2.2
  rw [huF3, ← hExprU]
  exact hF3
