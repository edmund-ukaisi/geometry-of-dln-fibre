...
set_option linter.style.longLine false in
/-- Theta-side weighted source-chart identities give readback domination after
transporting the weight to the chart-produced source-image reference.

This is the non-topological source-side `withDensity` version of
`aemeasurable_readback_and_measure_map_readback_restrict_piece_le_smul_restrict_superset_of_restrict_eq_withDensity`.
It assumes the restricted external measure is already supplied as the pushforward
of `(thetaReference.withDensity (density ∘ sourceChart)).restrict V`; the theorem
only performs the measure bookkeeping and then invokes the existing local
readback-domination socket.  It does not prove the weighted identity, density
bound, source coverage, Haar transport, normal crossings, pole order, or RLCT
extraction. -/
theorem aemeasurable_readback_and_measure_map_readback_restrict_piece_le_smul_restrict_superset_of_restrict_eq_map_sourceChart_withDensity
    {Θ E : Type*} [MeasurableSpace Θ] [MeasurableSpace E]
    (sourceChart : Θ → E) (readback : E → Θ)
    (externalMeasure : Measure E) (thetaReference : Measure Θ)
    (V W : Set Θ) (chartPiece : Set E) (density : E → ENNReal) (c : ENNReal)
  ...
  have hpush :
    ...
    measure_map_restrict_withDensity_eq_withDensity_map_of_ae_eq
      ...
      (rawDensity := density) hV hsource hdensity hfactor
  have heq_edge :
      externalMeasure.restrict chartPiece =
        ((Measure.map sourceChart (thetaReference.restrict V)).withDensity
  ...
  exact
    aemeasurable_readback_and_measure_map_readback_restrict_piece_le_smul_restrict_superset_of_restrict_eq_withDensity
      sourceChart readback externalMeasure thetaReference V W chartPiece density c
      hV hchartPiece hVW hsource hreadback hleft heq_edge hdensity_le

set_option linter.style.longLine false in
/-- Theta-side weighted source-chart identities give readback domination after
transporting the weight to the chart-produced source-image reference.

This is the source-side `withDensity` version of
...
bound, source coverage, Haar transport, normal crossings, pole order, or RLCT
extraction. -/
theorem aemeasurable_readback_and_measure_map_readback_restrict_piece_le_smul_restrict_superset_of_restrict_eq_map_sourceChart_withDensity_of_continuousOn_injOn
    {Θ E : Type*} [MeasurableSpace Θ] [TopologicalSpace Θ]
    [BorelSpace Θ] [PolishSpace Θ]
    [MeasurableSpace E] [TopologicalSpace E] [BorelSpace E] [T2Space E]
    (sourceChart : Θ → E) (readback : E → Θ)
  ...
  exact
    aemeasurable_readback_and_measure_map_readback_restrict_piece_le_smul_restrict_superset_of_restrict_eq_map_sourceChart_withDensity
      sourceChart readback externalMeasure thetaReference V W chartPiece density c
      hV hchartPiece hVW hsource hreadback hleft hdensity heq hdensity_le
...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- After shrinking inside the local selected-pivot nonzero locus, the enlarged
with-following endpoint source-chart image is exactly the p.13 source set
restricted by the readback preimage of the returned theta neighborhood.

This is a local p.13 chart equality.  It does not prove global p.13 source
coverage, source-rank-stratum coverage, finite atlas coverage, source-prior
transport, Haar/Jacobian transport, normal crossings, pole order, or RLCT
extraction. -/
theorem exists_open_subset_measurableSet_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_eq_p13SourceEdgeFamilySet_inter_readback_preimage
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
      ...
      (by
        simpa [EdgeFamily, sourceChart, readback] using
          exists_open_case2PassiveThetaWithFollowingFactorEndpointSourceChart_readback_leftInverse_pivotNonzero
            W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
            z₀ hdet₀ hpivot₀) with
    ⟨Vpivot, hVpivot_open, hz₀Vpivot, hpivotVpivot, _hleftVpivot⟩
  let Gpivot :
      Set
        (Case2PassiveThetaWithFollowingFactor
          (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n S J) :=
    G ∩ Vpivot
  have hGpivot_open : IsOpen Gpivot := hGopen.inter hVpivot_open
  have hz₀Gpivot : z₀ ∈ Gpivot := ⟨hz₀G, hz₀Vpivot⟩
  rcases
      (by
        simpa [EdgeFamily, retainedData, sourceChart, readback, Gpivot] using
          exists_open_subset_continuousOn_measurableSet_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_readback_leftInverse
            W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
            z₀ hdet₀ hpivot₀ Gpivot hGpivot_open hz₀Gpivot) with
    ⟨V, hVopen, hz₀V, hVGpivot, hdetV, hleftV, hsource_inj,
      hsource_contOn, hsource_image⟩
  have hVG : V ⊆ G := fun z hz ↦ hVGpivot.1 hz
  have hVVpivot : V ⊆ Vpivot := fun z hz ↦ hVGpivot.2 hz
  have hpivotV : ∀ z ∈ V,
      case2PassiveThetaPivotNonzero
    ...
    have hz_fields :
        (Case2PassiveThetaWithFollowingFactor.mk
            (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) (n := n) (S := S) (J := J)
            z.1.A1passive z.1.F2 z.1.A3passive z.1.Ctop z.1.F3
            z.1.yNext z.2) ∈ Vpivot := by
      simpa [Case2PassiveThetaWithFollowingFactor.mk,
        Case2PassiveTheta.A1passive, Case2PassiveTheta.F2,
        Case2PassiveTheta.A3passive, Case2PassiveTheta.Ctop,
        Case2PassiveTheta.F3, Case2PassiveTheta.yNext] using hVVpivot hz
    simpa [Case2PassiveThetaWithFollowingFactor.mk,
      Case2PassiveTheta.A1passive, Case2PassiveTheta.F2,
      Case2PassiveTheta.A3passive, Case2PassiveTheta.Ctop,
      Case2PassiveTheta.F3, Case2PassiveTheta.yNext] using
      hpivotVpivot z.1.A1passive z.1.F2 z.1.A3passive z.1.Ctop z.1.F3
        z.1.yNext z.2 hz_fields
  have hdetV' : ∀ z ∈ V, (retainedData z).detChart := by
    intro z hz
    have hz_fields :
  ...
  have hpoint : ∀ z ∈ V, sourceChart z ∈ p13SourceSet := by
    ...
    simpa [sourceChart, p13SourceSet, retainedData,
      ...
      paperEndpointFixedBaseRetainedPassiveP13SourceChart,
      paperEndpointFixedBaseRetainedPassiveP13SourceEdgeFamilyOfData, detData] using hmem
  have hright_of_p13 : ∀ E ∈ p13SourceSet, readback E ∈ V →
      sourceChart (readback E) = E := by
    intro E hE hreadV
    have hpivotRead := hpivotV (readback E) hreadV
    simpa [sourceChart, readback, p13SourceSet] using
      case2PassiveThetaWithFollowingFactorEndpointSourceChart_rightInverse_of_mem_p13SourceEdgeFamilySet
        W₂ B₂ n hS hcont hnext hU₀ eNext e E hE hpivotRead
  have himage_eq : sourceChart '' V = p13SourceSet ∩ readback ⁻¹' V := by
    ext E
    constructor
    · rintro ⟨z, hzV, rfl⟩
      constructor
      · exact hpoint z hzV
      · change readback (sourceChart z) ∈ V
        rw [hleftV' z hzV]
        exact hzV
    · intro hE
      refine ⟨readback E, hE.2, ?_⟩
      exact hright_of_p13 E hE.1 hE.2
  exact
    ⟨V, hVopen, hz₀V, hVG, hdetV', hpivotV, hleftV', hsource_inj',
      hsource_contOn', hsource_image', himage_eq⟩

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- The local p.13/readback-preimage chart piece for the enlarged
with-following endpoint source chart is open as well as measurable.

This packages the same local image equality as
...
with the topological fact that the source-side patch is an open edge-family
neighborhood.  It still does not prove global source coverage, source-rank
coverage, source-prior transport, Haar/Jacobian transport, normal crossings,
pole order, or RLCT extraction. -/
theorem exists_open_subset_measurableSet_isOpen_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_eq_p13SourceEdgeFamilySet_inter_readback_preimage
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
        ...
        (∀ z ∈ V, (retainedData z).detChart) ∧
          ...
          (∀ z ∈ V, readback (sourceChart z) = z) ∧
            Set.InjOn sourceChart V ∧ ContinuousOn sourceChart V ∧
              MeasurableSet (sourceChart '' V) ∧
                sourceChart '' V = p13SourceSet ∩ readback ⁻¹' V ∧
                  IsOpen (p13SourceSet ∩ readback ⁻¹' V) := by
  intro EdgeFamily retainedData sourceChart readback p13SourceSet
  rcases
      (by
        simpa [EdgeFamily, retainedData, sourceChart, readback, p13SourceSet] using
    ...
    ⟨V, hVopen, hz₀V, hVG, hdetV, hpivotV, hleftV, hsource_inj,
      hsource_contOn, hsource_image_meas, himage_eq⟩
  have hdetV' : ∀ z ∈ V, (retainedData z).detChart := by
    intro z hz
    have hz_fields :
        (Case2PassiveThetaWithFollowingFactor.mk
  ...
  have hpivotV' : ∀ z ∈ V,
    ...
    simpa [Case2PassiveThetaWithFollowingFactor.mk,
      ...
      Case2PassiveTheta.A3passive, Case2PassiveTheta.Ctop,
      Case2PassiveTheta.F3, Case2PassiveTheta.yNext] using
      hpivotV z.1.A1passive z.1.F2 z.1.A3passive z.1.Ctop z.1.F3
        z.1.yNext z.2 hz_fields
  have hleftV' : ∀ z ∈ V, readback (sourceChart z) = z := by
    intro z hz
    have hz_fields :
  ...
  have hpatch_open : IsOpen (p13SourceSet ∩ readback ⁻¹' V) := by
    simpa [EdgeFamily, readback, p13SourceSet] using
      isOpen_paperEndpointFixedBaseRetainedPassiveP13SourceEdgeFamilySet_inter_case2PassiveThetaWithFollowingFactorEndpointSourceChartReadback_preimage_of_subset_pivotNonzero
        W₂ B₂ n hS hnext hU₀ e hVopen hpivotV'
  exact
    ⟨V, hVopen, hz₀V, hVG, hdetV', hpivotV', hleftV', hsource_inj,
      hsource_contOn, hsource_image_meas, himage_eq, hpatch_open⟩

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- The first retained `C` block of the enlarged with-following endpoint datum
has the rank of the independent following-factor coordinate.

This is finite coordinate bookkeeping for the enlarged Case 2 chart.  It does
not impose a full-rank condition on the following factor. -/
theorem rank_case2PassiveThetaWithFollowingFactorEndpointRetainedData_C_zero_eq_followingFactor
    {ρ : Type*} [DecidableEq ρ] {τ : Type} [Fintype τ] [DecidableEq τ]
    (n : ℕ → ℕ) {S J : ℕ} (hS : 1 ≤ S)
    (hcont : J + 1 ≤ prefixMinNat n (S + 1))
    (hnext : J + 2 ≤ prefixMinNat n (S + 1))
    ...
    ((case2PassiveThetaWithFollowingFactorEndpointRetainedData
      (ρ := ρ) n hS hcont hnext z eNext e).C (0 : Fin 2)).rank =
      z.2.rank := by
  let rawData :
      RetainedPassiveNonredundantCoordinateData
  ...
  let data :
    ...
    case2PassiveThetaWithFollowingFactorEndpointRetainedData
      (ρ := ρ) n hS hcont hnext z eNext e
  have hC0raw : (rawData.C (0 : Fin 2)).rank = z.2.rank := by
    have hC0eq : rawData.C (0 : Fin 2) = z.2 := by
      change
      ...
      exact
        case2DisplayedPostPivotFreeFollowingFactor_freeCprimeOfFollowingFactor
          n hS hcont z.2
    simpa using congrArg Matrix.rank hC0eq
  calc
    (data.C (0 : Fin 2)).rank = (rawData.C (0 : Fin 2)).rank := by
      simpa [data, rawData, case2PassiveThetaWithFollowingFactorEndpointRetainedData] using
...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- The second retained `C` block of the enlarged with-following endpoint datum
has the rank of the successor selected-entry matrix read from `yNext`.

This is finite coordinate bookkeeping for the enlarged Case 2 chart. -/
theorem rank_case2PassiveThetaWithFollowingFactorEndpointRetainedData_C_one_eq_successorSelectedEntryMatrix
    {ρ : Type*} [DecidableEq ρ] {τ : Type} [Fintype τ] [DecidableEq τ]
    (n : ℕ → ℕ) {S J : ℕ} (hS : 1 ≤ S)
    (hcont : J + 1 ≤ prefixMinNat n (S + 1))
    (hnext : J + 2 ≤ prefixMinNat n (S + 1))
    ...
    (e : ∀ q : Fin 3, case2PostPivotTwoEdgeDomain n S J τ q ≃ κ' q) :
    ((case2PassiveThetaWithFollowingFactorEndpointRetainedData
      (ρ := ρ) n hS hcont hnext z eNext e).C (1 : Fin 2)).rank =
      (case2SuccessorSelectedEntryMatrix n hS hnext z.1.yNext eNext).rank := by
  let rawData :
      RetainedPassiveNonredundantCoordinateData
        (K := ℝ) (ρ := ρ) (case2PostPivotTwoEdgeDomain n S J τ) :=
    case2PassiveThetaWithFollowingFactorRetainedData
  ...
  have hC1raw :
    ...
    simpa [rawData, case2PassiveThetaWithFollowingFactorRetainedData,
      ...
      rank_case2DisplayedPostPivotResidualBlock_sourceResidualOfMatrix
        n hS hcont
        (case2SuccessorSelectedEntryMatrix n hS hnext z.1.yNext eNext) eNext
  calc
    (data.C (1 : Fin 2)).rank = (rawData.C (1 : Fin 2)).rank := by
      simpa [data, rawData, case2PassiveThetaWithFollowingFactorEndpointRetainedData] using
        ChartLocalSuffixState.RetainedPassiveNonredundantCoordinateData.rank_endpointTransport_C
          (K := ℝ) (ρ := ρ) e rawData (1 : Fin 2)
    _ = (case2SuccessorSelectedEntryMatrix n hS hnext z.1.yNext eNext).rank :=
        hC1raw
...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- A concrete enlarged with-following passive-theta endpoint source-chart
point belongs to the named source-rank stratum when the base-product rank and
the two residual-block rank equations are supplied.

The first residual rank is the rank of the independent following factor `z.2`;
no full-rank or determinant condition is imposed on that factor.  This is
pointwise one-way support for chart-produced source points.  It does not prove
source-rank coverage, source-image equality with a stratum, source-prior
transport, Haar/Jacobian transport, normal crossings, pole order, or RLCT
extraction. -/
theorem case2PassiveThetaWithFollowingFactorEndpointSourceChart_mem_sourceRankStratum
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  have hC1raw :
    ...
    simpa [rawData, case2PassiveThetaWithFollowingFactorRetainedData,
      ...
      rank_case2DisplayedPostPivotResidualBlock_sourceResidualOfMatrix
        n hS hcont
        (case2SuccessorSelectedEntryMatrix n hS hnext z.1.yNext eNext) eNext
  have hC0 : (data.C (0 : Fin 2)).rank = z.2.rank := by
    calc
      (data.C (0 : Fin 2)).rank = (rawData.C (0 : Fin 2)).rank := by
        simpa [data, rawData, case2PassiveThetaWithFollowingFactorEndpointRetainedData] using
          ChartLocalSuffixState.RetainedPassiveNonredundantCoordinateData.rank_endpointTransport_C
  ...
  have hmem :
    ...
    fin_cases p
    · calc
        r + (data.C (0 : Fin 2)).rank = r + z.2.rank := by rw [hC0]
        _ = rEdge 0 := hr0
    · calc
        r + (data.C (1 : Fin 2)).rank =
            r + (case2SuccessorSelectedEntryMatrix n hS hnext z.1.yNext eNext).rank := by
          rw [hC1]
        _ = rEdge 1 := hr1
  simpa [case2PassiveThetaWithFollowingFactorEndpointSourceChart, data, EdgeFamily, ρ, κ'] using hmem

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- Source-rank membership of a p.13 source family reads back to the two
theta-side rank equations for the enlarged with-following chart.

This is the reverse direction of the previous pointwise support theorem, but
only on the retained-passive p.13 source set and under the selected-pivot
nonzero condition needed by the with-following readback.  It does not prove
global source-rank coverage, finite atlas coverage, Haar/Jacobian transport,
normal crossings, pole order, or RLCT extraction. -/
theorem case2PassiveThetaWithFollowingFactorEndpointSourceChartReadback_rank_eq_of_mem_p13SourceEdgeFamilySet_sourceRankStratum
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  let data :=
    sourceReadback (K := ℝ) (ρ := ρ) (κ' := κ') EMat
  have hEMat_mem :
      EMat ∈
        sourceRecursiveDetChartSet
          (K := ℝ) (ρ := ρ) (κ' := κ') := by
    simpa [EMat, ρ, κ',
      paperEndpointFixedBaseRetainedPassiveP13SourceEdgeFamilySet] using hsource
  have hEMat_chart :
      sourceRecursiveDetChart (K := ℝ) (ρ := ρ) EMat :=
    (mem_sourceRecursiveDetChartSet
      (K := ℝ) (ρ := ρ) (κ' := κ') EMat).1 hEMat_mem
  have hdetData : data.detChart := by
    simpa [data, ρ, κ'] using
      sourceReadback_detChart_of_sourceRecursiveDetChart
        (K := ℝ) (ρ := ρ) EMat hEMat_chart
  have hread :
      case2PassiveThetaWithFollowingFactorEndpointRetainedData
          (ρ := ρ) n hS hcont hnext z eNext e =
        data := by
    simpa [z, data, EMat, ρ, κ'] using
      case2PassiveThetaWithFollowingFactorEndpointRetainedData_sourceChartReadback_eq_sourceReadback_of_pivotNonzero
        W₂ B₂ n hS hcont hnext hU₀ eNext e X hpivot
  have hright :
      paperEndpointFixedBaseRetainedPassiveP13SourceEdgeFamilyOfData
          W₂ B₂ U₀ hU₀ data =
        X := by
    simpa [data, EMat, ρ, κ'] using
      ...
      paperEndpointFixedBaseRetainedPassiveP13SourceEdgeFamilyOfData
        ...
        paperEndpointFixedBaseSourceRankStratum
          ...
          (fun E :
            ∀ p : Fin 2, reverseVertex W₂ p.castSucc →L[ℝ] reverseVertex W₂ p.succ ↦
            E) r rEdge := by
    simpa [hright] using hsrc
  have hC0data :
      (data.C (0 : Fin 2)).rank = rEdge 0 - r :=
    rank_C_of_retainedPassiveP13SourceEdgeFamilyOfData_mem_sourceRankStratum
      (K := ℝ) W₂ B₂ (U₀ := U₀) (hU₀ := hU₀) data
      hdetData hsrcData (0 : Fin 2)
  have hC1data :
      (data.C (1 : Fin 2)).rank = rEdge 1 - r :=
    rank_C_of_retainedPassiveP13SourceEdgeFamilyOfData_mem_sourceRankStratum
      (K := ℝ) W₂ B₂ (U₀ := U₀) (hU₀ := hU₀) data
      hdetData hsrcData (1 : Fin 2)
  have hC0z :
      (data.C (0 : Fin 2)).rank = z.2.rank := by
    simpa [hread] using
  ...
  have hC1z :
    ...
    simpa [hread] using
      rank_case2PassiveThetaWithFollowingFactorEndpointRetainedData_C_one_eq_successorSelectedEntryMatrix
        (ρ := ρ) n hS hcont hnext z eNext e
  have hle0 : r ≤ rEdge 0 := hsrc.2.2 (0 : Fin 2)
  have hle1 : r ≤ rEdge 1 := hsrc.2.2 (1 : Fin 2)
  constructor
  · have hzrank : z.2.rank = rEdge 0 - r := hC0z.symm.trans hC0data
    omega
  · have hzrank :
        (case2SuccessorSelectedEntryMatrix n hS hnext z.1.yNext eNext).rank =
          rEdge 1 - r := hC1z.symm.trans hC1data
    omega
...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- The local enlarged with-following endpoint source-chart image is contained
in the named source-rank stratum under explicit rank equations on the chosen
local theta domain.

This is one-way support for chart-produced source points.  The rank of the
free following factor is an explicit hypothesis; no full-rank condition is
silently imposed.  It does not prove source-rank coverage, source-image
equality with a stratum, source-prior transport, Haar/Jacobian transport,
normal crossings, pole order, or RLCT extraction. -/
theorem exists_open_subset_measurableSet_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_subset_sourceRankStratum
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
              ...
              MeasurableSet (sourceChart '' V) ∧
                (∀ z ∈ V,
                  ...
                  r + (case2SuccessorSelectedEntryMatrix
                    n hS hnext z.1.yNext eNext).rank = rEdge 1 →
                  sourceChart z ∈ sourceStratum) ∧
                  ((∀ z ∈ V, r + z.2.rank = rEdge 0) →
                    (∀ z ∈ V,
                      r + (case2SuccessorSelectedEntryMatrix
                        n hS hnext z.1.yNext eNext).rank = rEdge 1) →
  ...
  have hpoint : ∀ z ∈ V,
    ...
    simpa [sourceChart, sourceStratum, retainedData] using
      case2PassiveThetaWithFollowingFactorEndpointSourceChart_mem_sourceRankStratum
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
        z (hdetV' z hzV) hprod hzrank0 hzrank1
  have himage_subset :
      (∀ z ∈ V, r + z.2.rank = rEdge 0) →
        (∀ z ∈ V,
          r + (case2SuccessorSelectedEntryMatrix
            n hS hnext z.1.yNext eNext).rank = rEdge 1) →
        ∀ E ∈ sourceChart '' V, E ∈ sourceStratum := by
    intro hrank0 hrank1 E hE
    rcases hE with ⟨z, hzV, rfl⟩
    exact hpoint z hzV (hrank0 z hzV) (hrank1 z hzV)
  exact
    ⟨V, hVopen, hz₀V, hVG, hdetV', hleftV', hsource_inj, hsource_contOn,
      hsource_image, hpoint, himage_subset⟩
...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- A pushed-forward enlarged with-following endpoint source-chart measure is
supported on the named source-rank stratum under a.e. residual-rank equations
on the restricted theta domain.
...
transport, Haar/Jacobian transport, normal crossings, pole order, or RLCT
extraction. -/
theorem exists_open_subset_measure_map_case2PassiveThetaWithFollowingFactorEndpointSourceChart_restrict_sourceRankStratum_eq_self
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  rcases
      exists_open_subset_measurableSet_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_subset_sourceRankStratum
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
        z₀ hdet₀ hpivot₀ G hGopen hz₀G hprod with
    ⟨V, hVopen, hz₀V, hVG, hdetV, hleftV, hsource_inj, hsource_contOn,
  ...
  have hchart_mem :
    ...
    exact hpoint z hzV hzrank.1 hzrank.2
...
set_option linter.unusedSectionVars false in
set_option linter.style.longLine false in
/-- After shrinking to the local p.13 image patch, the source-rank stratum
cuts the enlarged with-following source-chart image exactly by the theta-side
rank-equation locus.

The domain is explicitly restricted to the two residual-rank equations.  This
does not assert that the whole local image is contained in the source-rank
stratum, nor that the global source-rank stratum is covered. -/
theorem exists_open_subset_measurableSet_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_rankEq_eq_p13SourceEdgeFamilySet_inter_readback_preimage_inter_sourceRankStratum
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  intro Θ EdgeFamily retainedData sourceChart readback p13SourceSet sourceStratum rankEq
  rcases
      exists_open_subset_measurableSet_case2PassiveThetaWithFollowingFactorEndpointSourceChart_image_eq_p13SourceEdgeFamilySet_inter_readback_preimage
        W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
        z₀ hdet₀ hpivot₀ G hGopen hz₀G with
    ⟨V, hVopen, hz₀V, hVG, hdetV, hpivotV, hleftV, hsource_inj,
      hsource_contOn, hsource_image, himage_p13⟩
  have hiff : ∀ z ∈ V, sourceChart z ∈ sourceStratum ↔ z ∈ rankEq := by
    intro z hzV
    constructor
    · intro hzsrc
      have hzpoint :
          sourceChart z ∈ p13SourceSet := by
        have hzimg : sourceChart z ∈ sourceChart '' V := ⟨z, hzV, rfl⟩
        have hzrhs : sourceChart z ∈ p13SourceSet ∩ readback ⁻¹' V := by
          rw [← himage_p13]
          exact hzimg
        exact hzrhs.1
      have hread_alias : readback (sourceChart z) = z := by
        simpa [readback, sourceChart] using hleftV z hzV
      have hpivotRead :
          case2PassiveThetaPivotNonzero
            (ρ := Fin (Module.finrank ℝ U₀)) (τ := τ) n hS hnext
            (readback (sourceChart z)).1 := by
        rw [hread_alias]
        exact hpivotV z hzV
      have hranks :
          let zread := readback (sourceChart z)
          r + zread.2.rank = rEdge 0 ∧
            r + (case2SuccessorSelectedEntryMatrix
              n hS hnext zread.1.yNext eNext).rank = rEdge 1 := by
        exact
          case2PassiveThetaWithFollowingFactorEndpointSourceChartReadback_rank_eq_of_mem_p13SourceEdgeFamilySet_sourceRankStratum
            ...
            (sourceChart z) hzpoint hzsrc (by simpa [readback] using hpivotRead)
      simpa [rankEq, hread_alias] using hranks
    · intro hzrank
      exact
        case2PassiveThetaWithFollowingFactorEndpointSourceChart_mem_sourceRankStratum
          W₂ B₂ n hS hcont hnext (U₀ := U₀) (hU₀ := hU₀) eNext e
          z (hdetV z hzV) hprod hzrank.1 hzrank.2
  have himage_rank :
      sourceChart '' (V ∩ rankEq) =
        (p13SourceSet ∩ readback ⁻¹' V) ∩ sourceStratum := by
    ext E
    ...
    · rintro ⟨z, hz, rfl⟩
      ...
      have hzrhs : sourceChart z ∈ p13SourceSet ∩ readback ⁻¹' V := by
        rw [← himage_p13]
        exact hzimg
      exact ⟨hzrhs, (hiff z hzV).2 hzrank⟩
    · intro hE
      have hEimage : E ∈ sourceChart '' V := by
        rw [himage_p13]
        exact hE.1
      rcases hEimage with ⟨z, hzV, rfl⟩
      exact ⟨z, ⟨hzV, (hiff z hzV).1 hE.2⟩, rfl⟩
  exact
    ⟨V, hVopen, hz₀V, hVG, hdetV, hpivotV, hleftV, hsource_inj,
      hsource_contOn, hsource_image, hiff, himage_rank⟩
