...
/-!
# `DLN.Aoyagi.Corank2OverVanishB_20_6_6` — pattern-B over-vanishing leaf `(20,6,6)`

The pattern-B per-type deliverables for the over-vanishing leaf `(p1,p2,p3) = (20,6,6)`, mirroring
...
`(20,6,6)` is COINCIDING (`p2 = p3`), `vm = u6 · u20`; its `φ` carries a degree-3 term, so the CUBIC cover atom applies. The 8
...
namespace DLNFibre.DLN.Aoyagi.OverVanishB_20_6_6

/-! ## §0 — the leaf and its composite -/

/-- The over-vanishing leaf index `(p1,p2,p3) = (20,6,6)`. -/
def idxCanon : Idx := ⟨⟨20, by decide⟩, ⟨6, by decide⟩, ⟨6, by decide⟩⟩

/-- The leaf composite `gFlat idxCanon`. -/
noncomputable def gCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun w =>
  blockBlowupMap S1 20 (nativeChart1 20
    (blockBlowupMap (sigmaC1Fs 20) 6 (blockBlowupMap (sigmaC2Fs 20) 6 w)))
...
def phiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun u i =>
  ...
  else if i = 13 then -(u 11 * u 2 + u 17 * u 6 * u 7)
  else if i = 14 then -(u 11 * u 3 + u 18 * u 6 * u 7)
  else if i = 15 then -(u 11 * u 4 + u 19 * u 6 * u 7)
  else 0
...
theorem phiCanon_read (u v : Fin 21 → ℝ) (h : ∀ i, keepCanon i → u i = v i) :
  ...
  funext i
  simp only [phiCanon]
  split_ifs <;> simp only [h0, h2, h3, h4, h6, h7, h11, h16, h17, h18, h19]
...
def vmExpCanon : Fin 21 → ℕ := fun d => if d = 6 ∨ d = 20 then 1 else 0

/-- `∏_d (u d)^(vmExpCanon d) = u 6 * u 20`. -/
theorem prod_vmExpCanon (u : Fin 21 → ℝ) : (∏ d, (u d) ^ vmExpCanon d) = u 6 * u 20 := by
  have h1 : ∀ d : Fin 21, u d ^ vmExpCanon d = if d = 6 ∨ d = 20 then u d else 1 :=
    fun d => by simp only [vmExpCanon]; split_ifs <;> simp
  simp_rw [h1]
  rw [Finset.prod_ite, Finset.prod_const_one, mul_one,
    show Finset.filter (fun d : Fin 21 => d = 6 ∨ d = 20) Finset.univ = {6, 20} from by decide,
...
theorem phiCanon_norm_bound {x : Fin 21 → ℝ} {r : ℝ} (hr1 : 1 ≤ r) (hx : ‖x‖ ≤ r) :
  ...
  rw [Real.norm_eq_abs]
  by_cases h12 : i = 12
  · rw [h12, show phiCanon x 12 = -(x 0 * x 11 + x 16 * x 6 * x 7) from rfl]
    exact slot _ _ (hq 0 11) (hcube 16 6 7)
  by_cases h13 : i = 13
  · rw [h13, show phiCanon x 13 = -(x 11 * x 2 + x 17 * x 6 * x 7) from rfl]
    exact slot _ _ (hq 11 2) (hcube 17 6 7)
  by_cases h14 : i = 14
  · rw [h14, show phiCanon x 14 = -(x 11 * x 3 + x 18 * x 6 * x 7) from rfl]
    exact slot _ _ (hq 11 3) (hcube 18 6 7)
  by_cases h15 : i = 15
  · rw [h15, show phiCanon x 15 = -(x 11 * x 4 + x 19 * x 6 * x 7) from rfl]
    exact slot _ _ (hq 11 4) (hcube 19 6 7)
...
end DLNFibre.DLN.Aoyagi.OverVanishB_20_6_6
