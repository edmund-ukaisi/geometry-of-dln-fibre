...
/-- The stored `C` coordinate has Frechet derivative equal to the
corresponding tangent coordinate. -/
theorem fderiv_retainedPassive_C_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Finite ρ] [∀ j, Finite (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (p : Fin (M + 1)) :
    (fderiv ℝ
      (fun y : TopologyTuple ρ κ' ℝ ↦
        (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C p) z) v =
      v.2.2.2.1 p := by
  let _ : Fintype ρ := Fintype.ofFinite ρ
  let _ : ∀ j, Fintype (κ' j) := fun j ↦ Fintype.ofFinite (κ' j)
  ...
  have hLC :
    ...
    LC.fderiv
  change (fderiv ℝ (fun y : TopologyTuple ρ κ' ℝ ↦ y.2.2.2.1 p) z) v =
    v.2.2.2.1 p
  rw [hLC]
  rfl
...
set_option linter.unusedFintypeInType false in
/-- Uniform endpoint split for the Frechet derivative of the zeroed-final
`A3` family: passive source tangent away from the last edge, and zero at the
last edge. -/
theorem fderiv_retainedPassiveA3WithoutLast_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [∀ j, Fintype (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (p : Fin (M + 1)) :
    (fderiv ℝ
      (fun y : TopologyTuple ρ κ' ℝ ↦
        retainedPassiveA3WithoutLast (K := ℝ) (ρ := ρ)
          ...
          p) z) v =
      Fin.lastCases
        (motive := fun p : Fin (M + 1) ↦ Matrix (κ' p.succ) ρ ℝ)
        (0 : Matrix (κ' (Fin.last M).succ) ρ ℝ)
        (fun q : Fin M ↦ v.2.2.1 q) p := by
  induction p using Fin.lastCases with
  | last =>
      simpa using
...
set_option maxRecDepth 2048 in
/-- The Frechet derivative of a stored `C`-block suffix product obeys the
noncommutative product rule, with the current `C` derivative rewritten as the
corresponding source tangent. -/
theorem fderiv_retainedPassive_C_residualFactorProduct_castSucc_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Finite ρ] [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (p : Fin (M + 1)) :
    let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
    let Ccast : TopologyTuple ρ κ' ℝ →
        Matrix (κ' (Fin.last (M + 1))) (κ' p.castSucc) ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := κ')
          (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C
          (Fin.last (M + 1)) p.castSucc p.castSucc.le_last
    let Csucc : TopologyTuple ρ κ' ℝ →
  ...
  let Csucc : TopologyTuple ρ κ' ℝ →
    ...
    fun y ↦
      residualFactorProduct (K := ℝ) (κ := κ')
        (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C
        (Fin.last (M + 1)) p.succ p.succ.le_last
  let Cfactor : TopologyTuple ρ κ' ℝ → Matrix (κ' p.succ) (κ' p.castSucc) ℝ :=
    fun y ↦ (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C p
  let B : Matrix (κ' (Fin.last (M + 1))) (κ' p.succ) ℝ →L[ℝ]
      Matrix (κ' p.succ) (κ' p.castSucc) ℝ →L[ℝ]
        Matrix (κ' (Fin.last (M + 1))) (κ' p.castSucc) ℝ :=
    matrixMulContinuousLinearMap
      (l := κ' (Fin.last (M + 1))) (m := κ' p.succ) (n := κ' p.castSucc)
  have hCsuccdiff : DifferentiableAt ℝ Csucc z := by
    simpa [Csucc] using
      differentiableAt_residualFactorProduct_C
        (ρ := ρ) (κ' := κ') p.succ p.succ.le_last z
  have hCfactordiff : DifferentiableAt ℝ Cfactor z := by
    simpa [Cfactor] using
      differentiableAt_C (ρ := ρ) (κ' := κ') p z
  have hCcast_eq : Ccast = fun y ↦ Csucc y * Cfactor y := by
    funext y
    simpa [Ccast, Csucc, Cfactor, data] using
      residualFactorProduct_castSucc
        (K := ℝ) (κ := κ')
        (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C
        (j := Fin.last (M + 1)) p p.succ.le_last
  have hmulDeriv :
      fderiv ℝ (fun y : TopologyTuple ρ κ' ℝ ↦ Csucc y * Cfactor y) z =
        B.precompR _ (Csucc z) (fderiv ℝ Cfactor z) +
          B.precompL _ (fderiv ℝ Csucc z) (Cfactor z) := by
    simpa [B] using
      (B.hasFDerivAt_of_bilinear hCsuccdiff.hasFDerivAt hCfactordiff.hasFDerivAt).fderiv
  have hCfactor_apply : (fderiv ℝ Cfactor z) v = v.2.2.2.1 p := by
    simpa [Cfactor] using
      fderiv_retainedPassive_C_apply (ρ := ρ) (κ' := κ') z v p
  calc
    (fderiv ℝ Ccast z) v =
        (fderiv ℝ (fun y : TopologyTuple ρ κ' ℝ ↦ Csucc y * Cfactor y) z) v := by
          rw [hCcast_eq]
    _ = Csucc z * (fderiv ℝ Cfactor z) v + (fderiv ℝ Csucc z) v * Cfactor z := by
          rw [hmulDeriv]
          simp [B, matrixMulContinuousLinearMap_apply]
    _ = (fderiv ℝ Csucc z) v * data.C p + Csucc z * v.2.2.2.1 p := by
          rw [hCfactor_apply]
          simp [Cfactor, data]
          abel

/-- The Frechet derivative of an empty stored `C`-block suffix product is
zero. -/
theorem fderiv_retainedPassive_C_residualFactorProduct_self_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (j : Fin (M + 2)) :
    (fderiv ℝ
      (fun y : TopologyTuple ρ κ' ℝ ↦
        residualFactorProduct (K := ℝ) (κ := κ')
          (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C
          j j le_rfl) z) v =
      (0 : Matrix (κ' j) (κ' j) ℝ) := by
  rw [show
      (fun y : TopologyTuple ρ κ' ℝ ↦
        residualFactorProduct (K := ℝ) (κ := κ')
          (ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') y).C
...
set_option maxRecDepth 2048 in
/-- Non-first-factor specialization of the solved-`A1` residual-product
derivative, with the successor solved-`A1` derivative substituted. -/
theorem fderiv_retainedPassive_solvedA1_residualFactorProduct_succ_castSucc_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ] [∀ j, Finite (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    (hz : z ∈ topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ'))
    (v : TopologyTuple ρ κ' ℝ) (q : Fin M) :
    let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
    let p : Fin (M + 1) := q.succ
    let A1fun : TopologyTuple ρ κ' ℝ → Fin (M + 1) → Matrix ρ ρ ℝ :=
      fun y s ↦
    ...
    let dPsucc := (fderiv ℝ Psucc z) v
    (fderiv ℝ Pcast z) v =
      dPsucc * data.toCoordinateData.solvedA1 p + Psucc z * v.1 q := by
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  let _ : ∀ j, Fintype (κ' j) := fun j ↦ Fintype.ofFinite (κ' j)
  let p : Fin (M + 1) := q.succ
  let A1fun : TopologyTuple ρ κ' ℝ → Fin (M + 1) → Matrix ρ ρ ℝ :=
...
set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- Nonterminal source-staging of the `A3` derivative factor in the
retained-passive lower-left tail product-rule recurrence.

This substitutes only the `dA3p` factor in the current summand.  The
derivatives of `Cprod`, `Pcast`, and the successor tail remain explicit. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_castSucc_product_dG_castSucc_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
  ...
  have hprod :=
    fderiv_retainedPassiveLowerLeftProductTailSum_castSucc_product_apply
      (ρ := ρ) (κ' := κ') hz v p
  have hA3 :
      (fderiv ℝ A3p z) v = v.2.2.1 q := by
    simpa [A3p, A3fun, p] using
      fderiv_retainedPassiveA3WithoutLast_castSucc_apply
        (ρ := ρ) (κ' := κ') z v q
  have hprodLocal :
      (fderiv ℝ Tailfun z) v =
        -((fderiv ℝ Cprod z) v * A3p z * (Pcast z)⁻¹)
          ...
          + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v *
              (Pcast z)⁻¹
          + (fderiv ℝ Nextfun z) v := by
    simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, A3p, Pcast, Nextfun, p] using hprod
  rw [hA3] at hprodLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, A3p, Pcast, Nextfun, p] using hprodLocal

set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- Source-stage the `dCprod` factor in the nonterminal retained-passive
`dEarly` product-rule recurrence, after the `dG` factor has already been
source-staged.

The successor `C`-product derivative remains explicit. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_castSucc_product_dCprod_dG_castSucc_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    ...
    let Tailfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last (M + 1))) ρ ℝ :=
      fun y ↦
        retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
          (A1fun y) (A3fun y) (Cfun y) p.val (Nat.le_of_lt p.isLt)
    let Cprod :
        TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last (M + 1))) (κ' r.castSucc) ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := κ') (Cfun y)
          (Fin.last (M + 1)) r.castSucc r.castSucc.le_last
    let Cnext : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last (M + 1))) (κ' r.succ) ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := κ') (Cfun y)
          (Fin.last (M + 1)) r.succ r.succ.le_last
    let A3p : TopologyTuple ρ κ' ℝ → Matrix (κ' r.castSucc) ρ ℝ :=
      fun y ↦ by
        simpa [p, r] using A3fun y p
    let dG : Matrix (κ' r.castSucc) ρ ℝ := by
      simpa [p, r] using v.2.2.1 q
    let Pcast : TopologyTuple ρ κ' ℝ → Matrix ρ ρ ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := fun _ : Fin (M + 2) ↦ ρ)
          (A1fun y) (Fin.last (M + 1)) p.castSucc p.castSucc.le_last
    let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last (M + 1))) ρ ℝ :=
      fun y ↦
        retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
          (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
    (fderiv ℝ Tailfun z) v =
      -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
          A3p z * (Pcast z)⁻¹)
        - (Cprod z * dG * (Pcast z)⁻¹)
        + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v * (Pcast z)⁻¹
        + (fderiv ℝ Nextfun z) v := by
  ...
  have hCprod :
      (fderiv ℝ Cprod z) v =
        (fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r := by
    simpa [Cprod, Cnext, Cfun, data, r, p] using
      ...
      (fderiv ℝ Tailfun z) v =
        -((fderiv ℝ Cprod z) v * A3p z * (Pcast z)⁻¹)
          ...
          + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v *
              (Pcast z)⁻¹
          + (fderiv ℝ Nextfun z) v := by
    simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, A3p, dG, Pcast, Nextfun, r, p] using hprod
  rw [hCprod] at hprodLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Nextfun, data, r, p]
    using hprodLocal

set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- Substitute the solved-`A1` residual-product product rule into the explicit
`dPcast` factor of the retained-passive `dEarly` recurrence, after `dCprod`
and `dG` have already been staged.

The derivative of the current solved-`A1` factor remains explicit. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_castSucc_product_dCprod_dG_dPcast_castSucc_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    ...
    (fderiv ℝ Tailfun z) v =
      -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
        ...
        + Cprod z * A3p z * (Pcast z)⁻¹ *
            ((fderiv ℝ Psucc z) v * data.toCoordinateData.solvedA1 p +
              Psucc z * (fderiv ℝ (fun y : TopologyTuple ρ κ' ℝ ↦ A1fun y p) z) v) *
            (Pcast z)⁻¹
        + (fderiv ℝ Nextfun z) v := by
  let p : Fin (M + 1) := q.castSucc
  let r : Fin (M + 1) := q.succ
      ...
      (fderiv ℝ Tailfun z) v =
        -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
          ...
          + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v *
              (Pcast z)⁻¹
          + (fderiv ℝ Nextfun z) v := by
    simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Nextfun,
      data, r, p] using hrec
  rw [hPcast] at hrecLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Psucc,
    Nextfun, data, r, p] using hrecLocal

set_option linter.style.longLine false in
/-- The named one-step RHS for the retained-passive lower-left tail derivative,
with the current solved-`A1` tangent, suffix-product tangent, and recursive
successor derivative supplied explicitly. -/
def retainedPassiveLowerLeftTailStepCoreAt
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (q : Fin M)
    (dAcur dPsucc : Matrix ρ ρ ℝ)
    (dNext : Matrix (κ' (Fin.last (M + 1))) ρ ℝ) :
    Matrix (κ' (Fin.last (M + 1))) ρ ℝ :=
  let p : Fin (M + 1) := q.castSucc
  let r : Fin (M + 1) := q.succ
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  ...
  let Psucc : TopologyTuple ρ κ' ℝ → Matrix ρ ρ ℝ :=
    fun y ↦
      residualFactorProduct (K := ℝ) (κ := fun _ : Fin (M + 2) ↦ ρ)
        (A1fun y) (Fin.last (M + 1)) p.succ p.succ.le_last
  let step : Matrix (κ' (Fin.last (M + 1))) ρ ℝ :=
    -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
        A3p z * (Pcast z)⁻¹)
      - (Cprod z * dG * (Pcast z)⁻¹)
      + Cprod z * A3p z * (Pcast z)⁻¹ *
...
set_option linter.style.longLine false in
/-- The generic retained-passive lower-left tail derivative recurrence,
restated through `retainedPassiveLowerLeftTailStepCoreAt`. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_castSucc_product_dCprod_dG_dPcast_stepCore_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin (M + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    ...
    let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last (M + 1))) ρ ℝ :=
      fun y ↦
        retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
          (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
    (fderiv ℝ Tailfun z) v =
      retainedPassiveLowerLeftTailStepCoreAt (ρ := ρ) (κ' := κ') z v q
        ((fderiv ℝ (fun y : TopologyTuple ρ κ' ℝ ↦ A1fun y p) z) v)
        ((fderiv ℝ Psucc z) v)
        ((fderiv ℝ Nextfun z) v) := by
  let p : Fin (M + 1) := q.castSucc
  ...
  let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last (M + 1))) ρ ℝ :=
    fun y ↦
      retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
        (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
  simpa [retainedPassiveLowerLeftTailStepCoreAt, A1fun, A3fun, Cfun, Tailfun,
    Psucc, Nextfun, p] using
    fderiv_retainedPassiveLowerLeftProductTailSum_castSucc_product_dCprod_dG_dPcast_castSucc_apply
      (ρ := ρ) (κ' := κ') hz v q

set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- First-index specialization of the retained-passive `dEarly` recurrence
with the zero solved-`A1` branch substituted into the explicit `dPcast`
factor.  The passive tail derivative is left explicit. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_zero_product_dCprod_dG_dPcast_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    ...
    let Tailfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ :=
      fun y ↦
        retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
          (A1fun y) (A3fun y) (Cfun y) p.val (Nat.le_of_lt p.isLt)
    let Cprod :
        TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' r.castSucc) ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := κ') (Cfun y)
          (Fin.last ((M + 1) + 1)) r.castSucc r.castSucc.le_last
    let Cnext : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' r.succ) ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := κ') (Cfun y)
    ...
    let Tail := retainedPassiveA1TailAfterFirst (K := ℝ) (ρ := ρ) data.A1seed
    let dTail := (fderiv ℝ Tfun z) v
    let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ :=
      fun y ↦
        retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
          (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
    (fderiv ℝ Tailfun z) v =
      ...
      (fderiv ℝ Pcast z) v =
        dPsucc * data.toCoordinateData.solvedA1 p +
          Psucc z *
            (Tail⁻¹ * v.2.2.2.2.1 -
              Tail⁻¹ * dTail * Tail⁻¹ * data.Ctop) := by
    simpa [A1fun, Pcast, Psucc, Tfun, Tail, dTail, dPsucc, data, p, q] using
  ...
  have hPsubst :
      (fderiv ℝ Psucc z) v * data.toCoordinateData.solvedA1 p +
          Psucc z *
            (fderiv ℝ
              (fun y : TopologyTuple ρ κ' ℝ ↦ A1fun y p) z) v =
        dPsucc * data.toCoordinateData.solvedA1 p +
          Psucc z *
            (Tail⁻¹ * v.2.2.2.2.1 -
              Tail⁻¹ * dTail * Tail⁻¹ * data.Ctop) := by
  ...
  have hrecLocal :
      (fderiv ℝ Tailfun z) v =
        -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
          ...
          + (fderiv ℝ Nextfun z) v := by
    simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Psucc,
      Nextfun, data, r, p] using hrec
  rw [hPsubst] at hrecLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Psucc,
    Tfun, Tail, dTail, dPsucc, Nextfun, data, r, p] using hrecLocal

set_option linter.style.longLine false in
/-- First-index retained-passive lower-left tail derivative recurrence,
restated through `retainedPassiveLowerLeftTailStepCoreAt`. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_zero_product_dCprod_dG_dPcast_stepCore_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    ...
    let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ :=
      fun y ↦
        retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
          (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
    (fderiv ℝ Tailfun z) v =
      retainedPassiveLowerLeftTailStepCoreAt (M := M + 1) (ρ := ρ) (κ' := κ') z v q
        (Tail⁻¹ * v.2.2.2.2.1 - Tail⁻¹ * dTail * Tail⁻¹ * data.Ctop)
        dPsucc
        ((fderiv ℝ Nextfun z) v) := by
  let q : Fin (M + 1) := 0
  ...
  let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ :=
    fun y ↦
      retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
        (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
  simpa [retainedPassiveLowerLeftTailStepCoreAt, A1fun, A3fun, Cfun, Tailfun,
    Psucc, Tfun, Tail, dTail, dPsucc, Nextfun, data, p, q] using
    fderiv_retainedPassiveLowerLeftProductTailSum_zero_product_dCprod_dG_dPcast_apply
      (M := M) (ρ := ρ) (κ' := κ') hz v

set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- Successor-index specialization of the retained-passive `dEarly`
recurrence with the successor solved-`A1` branch substituted into the explicit
`dPcast` factor. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_succ_product_dCprod_dG_dPcast_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    (hz : z ∈ topologyTupleDetChartSet (K := ℝ) (ρ := ρ) (κ' := κ'))
    (v : TopologyTuple ρ κ' ℝ) (s : Fin M) :
    let q : Fin (M + 1) := s.succ
    let u : Fin (M + 1) := s.castSucc
    let p : Fin ((M + 1) + 1) := q.castSucc
    let r : Fin ((M + 1) + 1) := q.succ
    let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
    let A1fun : TopologyTuple ρ κ' ℝ → Fin ((M + 1) + 1) → Matrix ρ ρ ℝ :=
    ...
    (fderiv ℝ Tailfun z) v =
      -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
        ...
        - (Cprod z * dG * (Pcast z)⁻¹)
        + Cprod z * A3p z * (Pcast z)⁻¹ *
            (dPsucc * data.toCoordinateData.solvedA1 p + Psucc z * v.1 u) *
            (Pcast z)⁻¹
        + (fderiv ℝ Nextfun z) v := by
  let q : Fin (M + 1) := s.succ
  let u : Fin (M + 1) := s.castSucc
  ...
  have hsucc :
      (fderiv ℝ Pcast z) v =
        dPsucc * data.toCoordinateData.solvedA1 p + Psucc z * v.1 u := by
    have hup : u.succ = p := by
      change s.castSucc.succ = s.succ.castSucc
      exact Fin.succ_castSucc s
    simpa [A1fun, Pcast, Psucc, dPsucc, data, hup] using
      fderiv_retainedPassive_solvedA1_residualFactorProduct_succ_castSucc_apply
        (M := M + 1) (ρ := ρ) (κ' := κ') hz v u
  have hPsubst :
      (fderiv ℝ Psucc z) v * data.toCoordinateData.solvedA1 p +
          Psucc z *
            (fderiv ℝ
  ...
  rw [hPsubst] at hrecLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Psucc,
    dPsucc, Nextfun, data, r, p, q, u] using hrecLocal

set_option linter.style.longLine false in
/-- Successor-index retained-passive lower-left tail derivative recurrence,
restated through `retainedPassiveLowerLeftTailStepCoreAt`. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_succ_product_dCprod_dG_dPcast_stepCore_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
  ...
  let Nextfun : TopologyTuple ρ κ' ℝ → Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ :=
    fun y ↦
      retainedPassiveLowerLeftProductTailSum (K := ℝ) (ρ := ρ) (κ := κ')
        (A1fun y) (A3fun y) (Cfun y) (p.val + 1) (Nat.succ_le_of_lt p.isLt)
  simpa [retainedPassiveLowerLeftTailStepCoreAt, A1fun, A3fun, Cfun, Tailfun,
    Psucc, dPsucc, Nextfun, p, q, u] using
    fderiv_retainedPassiveLowerLeftProductTailSum_succ_product_dCprod_dG_dPcast_apply
      (M := M) (ρ := ρ) (κ' := κ') hz v s

set_option linter.style.longLine false in
/-- The current solved-`A1` target tangent used by the recursive lower-left
tail derivative staging.  At the first index this is the solved top-left
branch; at successor indices it is the retained passive `A1` source tangent at
the predecessor index. -/
def retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (m : ℕ) (hm : m < M + 1) :
    Matrix ρ ρ ℝ :=
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  let Tfun : TopologyTuple ρ κ' ℝ → Matrix ρ ρ ℝ :=
    fun y ↦ retainedPassiveA1TailAfterFirst (K := ℝ) (ρ := ρ)
...
@[simp]
theorem retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt_zero
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) :
    ...
    retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt
        (M := M) (ρ := ρ) (κ' := κ') z v 0 (Nat.succ_pos M) =
      Tail⁻¹ * v.2.2.2.2.1 - Tail⁻¹ * dTail * Tail⁻¹ * data.Ctop := by
  simp [retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt]

set_option linter.flexible false in
@[simp]
theorem retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt_succ
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (s : Fin M) :
    retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt
        (M := M) (ρ := ρ) (κ' := κ') z v (s.val + 1)
        (Nat.succ_lt_succ s.isLt) =
      v.1 s.castSucc := by
  simp [retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt]
  exact congrArg (fun i : Fin (M + 1) ↦ v.1 i) (by ext; rfl)

set_option linter.style.longLine false in
/-- Recursive target-staged derivative for the retained-passive lower-left
zeroed-final tail, indexed by the Nat position of the current earlier-tail
coordinate. -/
def retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (m : ℕ) (hm : m ≤ M + 1) :
...
@[simp]
theorem retainedPassiveLowerLeftProductTailTargetStagedFDerivAt_self
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) :
    retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
        (M := M) (ρ := ρ) (κ' := κ') z v (M + 1) le_rfl =
      (0 : Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ) := by
  simp [retainedPassiveLowerLeftProductTailTargetStagedFDerivAt]

set_option linter.style.longLine false in
/-- One-step unfold equation for the recursive target-staged lower-left tail
derivative. -/
theorem retainedPassiveLowerLeftProductTailTargetStagedFDerivAt_step
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (m : ℕ) (hm : m < M + 1) :
    retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
        (M := M) (ρ := ρ) (κ' := κ') z v m (Nat.le_of_lt hm) =
      let q : Fin (M + 1) := ⟨m, hm⟩
      let p : Fin ((M + 1) + 1) := q.castSucc
      let A1fun : TopologyTuple ρ κ' ℝ → Fin ((M + 1) + 1) → Matrix ρ ρ ℝ :=
        fun y t ↦
      ...
      retainedPassiveLowerLeftTailStepCoreAt
        ...
        dPsucc
        (retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
          (M := M) (ρ := ρ) (κ' := κ') z v (m + 1) (Nat.succ_le_of_lt hm)) := by
  unfold retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
  rw [Nat.decreasingInduction_succ_left]

set_option linter.style.longLine false in
/-- First-index unfold equation for the recursive target-staged lower-left
tail derivative. -/
theorem retainedPassiveLowerLeftProductTailTargetStagedFDerivAt_zero
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) :
    ...
    retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
      ...
      retainedPassiveLowerLeftTailStepCoreAt
        ...
        dPsucc
        (retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
          (M := M) (ρ := ρ) (κ' := κ') z v 1 (Nat.succ_le_succ (Nat.zero_le M))) := by
  simpa [retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt_zero] using
    retainedPassiveLowerLeftProductTailTargetStagedFDerivAt_step
      (M := M) (ρ := ρ) (κ' := κ') z v 0 (Nat.succ_pos M)

set_option linter.style.longLine false in
/-- Successor-index unfold equation for the recursive target-staged
lower-left tail derivative. -/
theorem retainedPassiveLowerLeftProductTailTargetStagedFDerivAt_succ
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    (z v : TopologyTuple ρ κ' ℝ) (s : Fin M) :
    ...
    retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
      ...
      retainedPassiveLowerLeftTailStepCoreAt
        ...
        dPsucc
        (retainedPassiveLowerLeftProductTailTargetStagedFDerivAt
          (M := M) (ρ := ρ) (κ' := κ') z v (s.val + 2)
          (Nat.succ_le_of_lt (Nat.succ_lt_succ s.isLt))) := by
  simpa [retainedPassiveLowerLeftTailCurrentTargetSolvedA1TangentAt_succ] using
    retainedPassiveLowerLeftProductTailTargetStagedFDerivAt_step
      (M := M) (ρ := ρ) (κ' := κ') z v (s.val + 1)
      (Nat.succ_lt_succ s.isLt)
...
The theorem collapses the empty successor `C`-product derivative and the
zeroed-final successor tail, while leaving `dPcast` explicit. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_last_product_dCprod_dG_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
    ...
    let Pcast : TopologyTuple ρ κ' ℝ → Matrix ρ ρ ℝ :=
      fun y ↦
        residualFactorProduct (K := ℝ) (κ := fun _ : Fin ((M + 1) + 2) ↦ ρ)
          (A1fun y) (Fin.last ((M + 1) + 1)) p.castSucc p.castSucc.le_last
    (fderiv ℝ Tailfun z) v =
      -(((0 : Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' r.succ) ℝ) *
            data.C r + Cnext z * v.2.2.2.1 r) * A3p z * (Pcast z)⁻¹)
        - (Cprod z * dG * (Pcast z)⁻¹)
        + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v * (Pcast z)⁻¹ := by
  let q : Fin (M + 1) := Fin.last M
  let p : Fin ((M + 1) + 1) := q.castSucc
  let r : Fin ((M + 1) + 1) := q.succ
  let data := ofTopologyTuple (K := ℝ) (ρ := ρ) (κ' := κ') z
  ...
  have hCnext :
      (fderiv ℝ Cnext z) v =
        (0 : Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' r.succ) ℝ) := by
    have hfun :
        Cnext =
          fun _ : TopologyTuple ρ κ' ℝ ↦
            (1 : Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' (Fin.last ((M + 1) + 1))) ℝ) := by
      funext y
      simp [Cnext, Cfun, q, r]
    rw [hfun]
    rw [fderiv_const_apply]
  ...
  have hNext :
      (fderiv ℝ Nextfun z) v =
        (0 : Matrix (κ' (Fin.last ((M + 1) + 1))) ρ ℝ) := by
    simp [A1fun, A3fun, Cfun, Nextfun, p, q]
  have hrecLocal :
      (fderiv ℝ Tailfun z) v =
        -(((fderiv ℝ Cnext z) v * data.C r + Cnext z * v.2.2.2.1 r) *
            A3p z * (Pcast z)⁻¹)
          ...
          + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v *
              (Pcast z)⁻¹
          + (fderiv ℝ Nextfun z) v := by
    simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Nextfun, q, r, p]
      using hrec
  rw [hCnext, hNext] at hrecLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Nextfun,
    data, q, r, p] using hrecLocal

set_option maxRecDepth 2048 in
set_option linter.style.longLine false in
/-- Substitute the solved-`A1` residual-product product rule into the explicit
terminal `dPcast` factor of the retained-passive `dEarly` recurrence, after
the terminal `dCprod` and `dG` simplifications. -/
theorem fderiv_retainedPassiveLowerLeftProductTailSum_last_product_dCprod_dG_dPcast_apply
    {M : ℕ} {ρ : Type*} {κ' : Fin ((M + 1) + 2) → Type*}
    [Fintype ρ] [DecidableEq ρ]
    [∀ j, Fintype (κ' j)] [∀ j, DecidableEq (κ' j)]
    {z : TopologyTuple ρ κ' ℝ}
  ...
  have hrecLocal :
      (fderiv ℝ Tailfun z) v =
        -(((0 : Matrix (κ' (Fin.last ((M + 1) + 1))) (κ' r.succ) ℝ) *
          ...
          + Cprod z * A3p z * (Pcast z)⁻¹ * (fderiv ℝ Pcast z) v *
              (Pcast z)⁻¹ := by
    simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast,
      data, q, r, p] using hrec
  rw [hPcast] at hrecLocal
  simpa [A1fun, A3fun, Cfun, Tailfun, Cprod, Cnext, A3p, dG, Pcast, Psucc,
    data, q, r, p] using hrecLocal
