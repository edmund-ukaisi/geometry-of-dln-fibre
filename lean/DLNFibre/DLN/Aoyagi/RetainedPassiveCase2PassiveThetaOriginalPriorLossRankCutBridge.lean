...
set_option linter.style.longLine false in
set_option maxHeartbeats 900000 in
-- The residual-source package is produced before calling the local loss socket.
/-- Local original-loss finite-integral handoff on the with-following rank-cut
p.13/readback patch, with residual-source positivity and integrability produced
internally by the strict rank-cut residual-source theorem.

This removes the residual zero-locus-nullity continuation from the base
original-loss bridge.  It still assumes the source-image density data and the
strict source/prior density inequalities needed by that residual-source
producer, and it keeps the local source data, adapted-product lower bound, and
regular-coordinate density bounds explicit.  It does not construct source
density, identify an original source-chart prior, prove determinant/raw Haar
transport, prove source-rank coverage, normal crossings, pole order, or RLCT
extraction. -/
theorem exists_open_lintegral_lossDLN_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_localSource_bounds
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
      ...
      (by
        simpa [Θ, RawTuple, EdgeFamily, sourceChart, readback, p13SourceSet,
          ...
          exists_open_residualSourceHypotheses_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_sourceDensity_continuousAt_lt_top_strict_sourceDensity_lower_strict_priorDensity_upper_of_continuousAt_priorDensity_of_subset_detSector
            ...
            G hGopen hz₀G hGdet (r := r) (rEdge := rEdge) hprod) with
    ⟨W, _hWopen, _hz₀W, hWG, V, hVopen, hz₀V, hVW, hleft,
      hsource_inj, hsource_contOn, hpiece_meas, hpiece_open, himage,
      hrankPiece_meas, hiffV, himage_rank, hresidual_source⟩
  have hleft' : ∀ z ∈ V, readback (sourceChart z) = z := by
    intro z hz
    have hz_fields :
        (Case2PassiveThetaWithFollowingFactor.mk
  ...
  have hiffV' : ∀ z ∈ V, sourceChart z ∈ sourceStratum ↔ z ∈ rankEq := by
    ...
    simpa [sourceChart, sourceStratum, rankEq,
      ...
      hiffV z.1.A1passive z.1.F2 z.1.A3passive z.1.Ctop z.1.F3
        z.1.yNext z.2 hz_fields
  refine
    ⟨V, hVopen, hz₀V, fun z hzV ↦ hWG (hVW hzV), hleft',
      hsource_inj, hsource_contOn, hpiece_meas, hpiece_open, himage,
      hrankPiece_meas, hiffV', himage_rank, ?_⟩
  intro rawHaar hrawHaar H sourceData d b ν hν lossDensity R c C hR hc hC
    hadapted_lower hdensity_nonneg hdensity_le
  letI : rawHaar.IsAddHaarMeasure := hrawHaar
  letI : ν.IsAddHaarMeasure := hν
  rcases hresidual_source rawHaar with
    ⟨_Cdet, _hCdet, _hCprior, hpos_source, hbase_source⟩
  simpa [ρreg, CedgeProd, μprior] using
    exists_open_lintegral_ofReal_lossDLN_chainMapMatrixTuple_rpow_neg_mul_density_p13RegularCoordinates_lt_top_of_localSource_adaptedProductDifferenceSquareSum_lower
      (W := W₂) (B := B₂) sourceData b
      (source := (p13SourceSet ∩ readback ⁻¹' V) ∩ sourceStratum)
...
zero-locus nullity, product-zero density hypotheses, and raw Haar data remain
explicit.  It does not prove source-rank coverage, normal crossings, pole
order, or RLCT extraction. -/
theorem exists_open_radius_lintegral_lossDLN_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_zero_set_null_of_rank_eq_of_source_base_of_continuousAt_pos_priorDensity_product_zero_finBasis_volume
    (W₂ : Fin 3 → Type v) [∀ i, AddCommGroup (W₂ i)]
    [∀ i, TopologicalSpace (W₂ i)] [∀ i, IsTopologicalAddGroup (W₂ i)]
    [∀ i, T2Space (W₂ i)] [∀ i, Module ℝ (W₂ i)]
    [∀ i, ContinuousSMul ℝ (W₂ i)]
  ...
  rcases
      (by
        simpa [Θ, RawTuple, EdgeFamily, p13SourceSet,
          sourceStratum, rankEq, μprior, CedgeProd, d, b] using
          exists_open_radius_lintegral_lossDLN_originalEdgeFamilyPrior_rankCutP13Readback_case2PassiveThetaWithFollowingFactor_of_zero_set_null_of_source_base_of_continuousAt_pos_priorDensity_product_zero_finBasis_volume
            (τ := τ) W₂ B₂ n (S := S) (J := J) hS hcont hnext
            (U₀ := U₀) (hU₀ := hU₀) eNext e z₀ hpivot₀ hF₀det
            (t := t) ht hcrit (priorDensity := priorDensity) hprior_cont
            G hGopen hz₀G hGdet (r := r) (rEdge := rEdge) hprod) with
  ...
  refine
    ⟨V, hVopen, hz₀V, hVG, hleft', hsource_inj, hsource_contOn,
      hpiece_meas, hpiece_open, himage, hrankPiece_meas, hiffV',
      himage_rank, ?_⟩
  intro rawHaar hrawHaar hzero H hH hr0 hr1 hsource_base Rmax
    hprior_prod_cont hprior_prod_pos hRmax
  have hdet₀ :
