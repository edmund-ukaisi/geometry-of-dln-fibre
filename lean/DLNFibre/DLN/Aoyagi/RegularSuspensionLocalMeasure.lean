...
set_option linter.unusedSectionVars false in
/-- Source-local two-sided loss/density bounds become product-measure a.e.
bounds after restricting the base to a sufficiently small source neighborhood.

This is only filter-to-measure plumbing for supplied local bounds.  It does not
prove the bounds, compare an original loss, construct a product chart, transport
a density, or prove any integrability statement. -/
theorem exists_open_ae_restrict_localSource_prod_p13RegularCoordinates_two_sided_loss_density_bounds
    [∀ j, FiniteDimensional ℝ (W j)]
    {α : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
          ...
          u ∈ Metric.ball
              (0 : EuclideanSpace ℝ
                (AoyagiRegularBlockCoordinateIndex
                  ...
                  (throughSubspaceEndpointComplementIndex
                    (reverseVertex W) (reverseEdge W B) U₀ 0))) R →
            loss (x, u) ≤ CL * (aoyagiCoordinateSquareSum
                (paperEndpointFixedBaseResidualBlockCoordinateMap
                  (K := ℝ) W B U₀ hU₀ Cedge x) +
              aoyagiCoordinateSquareSum (fun i => u i)))
    (hdensity_lower :
  ...
  have hbounds :
      ∀ᶠ x in nhdsWithin x₀ source,
        ...
        (∀ u : EuclideanSpace ℝ ρ,
          u ∈ Metric.ball (0 : EuclideanSpace ℝ ρ) R →
            density (x, u) ≤ Dρ) := by
    filter_upwards [hloss_lower, hloss_upper, hdensity_lower, hdensity_upper] with
      x hxloss_lower hxloss_upper hxdensity_lower hxdensity_upper
    exact ⟨by simpa [ρ] using hxloss_lower,
      by simpa [ρ] using hxloss_upper,
      by simpa [ρ] using hxdensity_lower,
      by simpa [ρ] using hxdensity_upper⟩
  rcases exists_open_ae_restrict_inter_prod_fst_of_eventually_nhdsWithin
      (μ := μ) (ν := ν) (x₀ := x₀) (s := source) hsource_meas hbounds with
    ⟨U, hUopen, hxU, hbounds_ae⟩
  refine ⟨U, hUopen, hxU, ?_, ?_, ?_, ?_⟩
  · filter_upwards [hbounds_ae] with z hz hzball
    exact hz.1 z.2 hzball
  · filter_upwards [hbounds_ae] with z hz hzball
    exact hz.2.1 z.2 hzball
  · filter_upwards [hbounds_ae] with z hz hzball
    exact hz.2.2.1 z.2 hzball
  · filter_upwards [hbounds_ae] with z hz hzball
    exact hz.2.2.2 z.2 hzball
...
The four comparison bounds are supplied on the source-rank stratum filter. This
theorem only specializes the local-source handoff to that source set; it does
not prove the bounds, chart transport, or any integrability statement. -/
theorem exists_open_ae_restrict_source_prod_p13RegularCoordinates_two_sided_loss_density_bounds
    [∀ j, FiniteDimensional ℝ (W j)]
    {α : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
  ...
  rcases
      exists_open_ae_restrict_localSource_prod_p13RegularCoordinates_two_sided_loss_density_bounds
        (W := W) (B := B) (x₀ := x₀) (U₀ := U₀) (hU₀ := hU₀)
        (Cedge := Cedge) (source := sourceStratum) (μ := μ) (ν := ν)
        (loss := loss) (density := density) (R := R)
        (cL := cL) (CL := CL) (dρ := dρ) (Dρ := Dρ)
        (by simpa [sourceStratum] using hsource_meas)
        (by simpa [sourceStratum] using hloss_lower)
        (by simpa [sourceStratum] using hloss_upper)
        (by simpa [sourceStratum] using hdensity_lower)
        (by simpa [sourceStratum] using hdensity_upper) with
    ⟨U, hUopen, hxU, hloss_lower_ae, hloss_upper_ae, hdensity_lower_ae,
      hdensity_upper_ae⟩
  exact ⟨U, hUopen, hxU,
    by simpa [sourceStratum] using hloss_lower_ae,
    by simpa [sourceStratum] using hloss_upper_ae,
    by simpa [sourceStratum] using hdensity_lower_ae,
    by simpa [sourceStratum] using hdensity_upper_ae⟩
...
set_option linter.unusedSectionVars false in
/-- Source-local two-sided comparison handoff as an open-neighborhood
integrability equivalence.

All analytic inputs remain hypotheses: residual measurability, residual
positivity and local boundedness by `R^2`, the four loss/density comparison
bounds, and the positivity of comparison constants.  The theorem only shrinks
to a source neighborhood where the supplied source-filter bounds are available
as product-measure a.e. facts and then applies the p.13 two-sided comparison
iff. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_two_sided_bounds
    [∀ j, FiniteDimensional ℝ (W j)]
    {α : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
    ...
    {t R cL CL dρ Dρ : ℝ}
    (hsource_meas : MeasurableSet source)
    (hmeas_source : AEMeasurable
      (fun x : α =>
        aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x)) (μ.restrict source))
    (hR : 0 < R)
    (hpos_source :
  ...
  let ρ :=
    AoyagiRegularBlockCoordinateIndex
      ...
      (throughSubspaceEndpointComplementIndex
        (reverseVertex W) (reverseEdge W B) U₀ 0)
  rcases exists_open_ae_restrict_localSource_prod_p13RegularCoordinates_two_sided_loss_density_bounds
      (W := W) (B := B) (x₀ := x₀) (U₀ := U₀) (hU₀ := hU₀)
      (Cedge := Cedge) (source := source) (μ := μ) (ν := ν)
      (loss := loss) (density := density) (R := R)
      (cL := cL) (CL := CL) (dρ := dρ) (Dρ := Dρ)
      hsource_meas hloss_lower hloss_upper hdensity_lower hdensity_upper with
    ⟨U, hUopen, hxU, hloss_lower_ae, hloss_upper_ae, hdensity_lower_ae,
      hdensity_upper_ae⟩
  have hsubset : U ∩ source ⊆ source := Set.inter_subset_right
  have hmeas_U : AEMeasurable
      (fun x : α =>
        aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x)) (μ.restrict (U ∩ source)) := by
    exact AEMeasurable.mono_set hsubset hmeas_source
  have hpos_U :
      ∀ᵐ x ∂ μ.restrict (U ∩ source),
        0 < aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x) := by
    exact ae_mono (Measure.restrict_mono hsubset le_rfl) hpos_source
  have hle_U :
      ∀ᵐ x ∂ μ.restrict (U ∩ source),
        aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x) ≤ R ^ 2 := by
    exact ae_mono (Measure.restrict_mono hsubset le_rfl) hle_source
  have hiff :=
    lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_two_sided_bounds
      ...
      (cL := cL) (CL := CL) (dρ := dρ) (Dρ := Dρ)
      hmeas_U hR hpos_U hle_U hcL hCL hdρ hDρ ht
      (by simpa [ρ] using hloss_lower_ae)
      (by simpa [ρ] using hloss_upper_ae)
      (by simpa [ρ] using hdensity_lower_ae)
      (by simpa [ρ] using hdensity_upper_ae)
  exact ⟨U, hUopen, hxU, by simpa [ρ, residualNegPowerIntegrableOn] using hiff⟩

set_option linter.unusedSectionVars false in
/-- Source-rank-stratum version of the local two-sided comparison handoff as
an open-neighborhood integrability equivalence.

This only specializes the local-source theorem to
`paperEndpointFixedBaseSourceRankStratum`.  All analytic inputs remain
hypotheses: residual measurability, residual positivity and local boundedness
by `R^2`, the four loss/density comparison bounds on the source-rank-stratum
filter, and the positivity of comparison constants. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_sourceStratum_two_sided_bounds
    [∀ j, FiniteDimensional ℝ (W j)]
    {α : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
  ...
  simpa [sourceStratum] using
    exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_two_sided_bounds
      (W := W) (B := B) (x₀ := x₀) (U₀ := U₀) (hU₀ := hU₀)
      (Cedge := Cedge) (H := H) (r := r) (rEdge := rEdge) sourceData
      (source := sourceStratum) (μ := μ) (ν := ν)
      (loss := loss) (density := density) (t := t) (R := R)
      (cL := cL) (CL := CL) (dρ := dρ) (Dρ := Dρ)
      (by simpa [sourceStratum] using hsource_meas)
      (by simpa [sourceStratum] using hmeas_source)
      hR
      (by simpa [sourceStratum] using hpos_source)
      (by simpa [sourceStratum] using hle_source)
      hcL hCL hdρ hDρ ht
      (by simpa [sourceStratum] using hloss_lower)
      (by simpa [sourceStratum] using hloss_upper)
      (by simpa [sourceStratum] using hdensity_lower)
      (by simpa [sourceStratum] using hdensity_upper)

set_option linter.unusedSectionVars false in
/-- Boundary-explicit source-stratum iff from residual hypotheses on a local
chart source.

Inside the supplied open neighborhood `Ulocal`, the source-rank stratum must be
contained in `localSource`; the theorem applies the local-source two-sided iff
to `Ulocal ∩ sourceStratum` and rewrites the final open set.  It does not
prove source-rank coverage, chart image equality, comparison bounds, density
transport, normal crossings, pole order, or RLCT. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_sourceStratum_bounds_locally_subset_localSource_two_sided_bounds
    [∀ j, FiniteDimensional ℝ (W j)]
    {α : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
  ...
  let sourceStratum :=
    paperEndpointFixedBaseSourceRankStratum
      (K := ℝ) W B Cedge r rEdge
  let source := Ulocal ∩ sourceStratum
  have hUlocal_mem : Ulocal ∈ nhds x₀ := hUlocal_open.mem_nhds hx₀Ulocal
  have hfilter : nhdsWithin x₀ source = nhdsWithin x₀ sourceStratum := by
    simpa [source, sourceStratum] using
      nhdsWithin_inter_of_mem (a := x₀) (s := Ulocal) (t := sourceStratum)
        (mem_nhdsWithin_of_mem_nhds hUlocal_mem)
  have hsource_meas' : MeasurableSet source := by
    exact hUlocal_open.measurableSet.inter (by simpa [sourceStratum] using hsource_meas)
  have hsubset_local : source ⊆ localSource := by
    intro x hx
    exact (hcoverage (by simpa [source, sourceStratum] using hx)).2
  have hmeas_source :
      AEMeasurable
        (fun x : α =>
          aoyagiCoordinateSquareSum
            (paperEndpointFixedBaseResidualBlockCoordinateMap
              (K := ℝ) W B U₀ hU₀ Cedge x))
        (μ.restrict source) := by
    exact AEMeasurable.mono_set hsubset_local hmeas_local
  have hpos_source :
      ∀ᵐ x ∂ μ.restrict source,
        0 < aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x) := by
    exact ae_mono (Measure.restrict_mono hsubset_local le_rfl) hpos_local
  have hle_source :
      ∀ᵐ x ∂ μ.restrict source,
        aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x) ≤ R ^ 2 := by
    exact ae_mono (Measure.restrict_mono hsubset_local le_rfl) hle_local
  rcases
      exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_two_sided_bounds
        (W := W) (B := B) (x₀ := x₀) (U₀ := U₀) (hU₀ := hU₀)
        (Cedge := Cedge) (H := H) (r := r) (rEdge := rEdge) sourceData
        ...
        (loss := loss) (density := density) (t := t) (R := R)
        (cL := cL) (CL := CL) (dρ := dρ) (Dρ := Dρ)
        hsource_meas' hmeas_source hR hpos_source hle_source
        hcL hCL hdρ hDρ ht
        (by simpa [hfilter] using hloss_lower)
        (by simpa [hfilter] using hloss_upper)
        (by simpa [hfilter] using hdensity_lower)
        (by simpa [hfilter] using hdensity_upper) with
    ⟨Uchart, hUchart_open, hx₀Uchart, hiff⟩
  let U := Uchart ∩ Ulocal
  refine ⟨U, hUchart_open.inter hUlocal_open, ⟨hx₀Uchart, hx₀Ulocal⟩, ?_⟩
  simpa [U, source, sourceStratum, Set.inter_assoc, Set.inter_left_comm,
    Set.inter_comm] using hiff
...
set_option linter.unusedSectionVars false in
/-- Residual positivity from a weighted signed-box chart pushforward and an
explicit absolute-monomial lower bound on the chart-side residual square.

This is the positivity-only fragment of the weighted signed-box residual
source constructor.  It does not assume the critical inequalities or
source-density upper bounds that would imply residual negative-power
integrability. -/
theorem residualSquareSum_pos_ae_of_measure_map_signedBox_withDensity_monomialLower
    [∀ j, FiniteDimensional ℝ (W j)]
    {α ι : Type*} [MeasurableSpace α] [Fintype ι]
    {U₀ : Submodule ℝ (reverseVertex W 0)}
    {hU₀ : IsCompl U₀ (LinearMap.ker (paperTotalMap W B))}
  ...
  have hpos_map :
      ∀ᵐ x ∂ Measure.map chart weightedBox,
        0 < aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x) := by
    exact
        ...
        (hchart.mono_ac (by
          simpa [signedBox, weightedBox] using
            withDensity_absolutelyContinuous signedBox
              (fun y : ι → ℝ => ENNReal.ofReal (density y))))
        hpos_meas).2 (by simpa [residual] using hpos_weighted)
  simpa [signedBox, weightedBox, hmap] using hpos_map

set_option linter.unusedSectionVars false in
/-- Residual positivity from a weighted signed-box chart pushforward, with
positive-set measurability derived from measurable fixed-basis edge matrices.

This positivity-only helper deliberately avoids the signed-box critical
integrability inequalities. -/
theorem residualSquareSum_pos_ae_of_measure_map_signedBox_withDensity_monomialLower_of_measurable_edgeMatrix
    [∀ j, FiniteDimensional ℝ (W j)]
    {α ι : Type*} [MeasurableSpace α] [Fintype ι]
    {U₀ : Submodule ℝ (reverseVertex W 0)}
    {hU₀ : IsCompl U₀ (LinearMap.ker (paperTotalMap W B))}
  ...
  exact
    residualSquareSum_pos_ae_of_measure_map_signedBox_withDensity_monomialLower
      (W := W) (B := B) (U₀ := U₀) (hU₀ := hU₀)
      (Cedge := Cedge) (source := source) (μ := μ)
      (chart := chart) (density := density) (c := c) (R := R) (k := k)
      hchart hmap
...
set_option linter.unusedSectionVars false in
/-- Boundary-explicit consumer whose loss and density bounds live on the
source-rank stratum, while residual positivity and residual negative-power
integrability are supplied on a local chart source.

Inside the supplied open neighborhood `Ulocal`, the source-rank stratum must be
contained in `localSource`; the theorem then shrinks to that neighborhood and
restricts the local residual hypotheses to the final source-stratum measure.
It does not prove source-rank coverage, chart image equality, density transport,
normal crossings, pole order, or RLCT. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_of_sourceStratum_bounds_locally_subset_localSource
    [∀ j, FiniteDimensional ℝ (W j)]
    {α : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
  ...
  rcases exists_open_ae_restrict_source_prod_p13RegularCoordinates_loss_density_bounds
      ...
      (loss := loss) (density := density) (R := R) (c := c) (C := C)
      hsource_meas hloss hdensity_nonneg hdensity_le with
    ⟨Ubounds, hUbounds_open, hx₀Ubounds, hloss_ae, hdensity_nonneg_ae,
      hdensity_le_ae⟩
  let U := Ubounds ∩ Ulocal
  have hU_open : IsOpen U := hUbounds_open.inter hUlocal_open
  have hx₀U : x₀ ∈ U := ⟨hx₀Ubounds, hx₀Ulocal⟩
  have hsubset_local : U ∩ sourceStratum ⊆ localSource := by
    intro x hx
    have hxlocal : x ∈ Ulocal ∩ sourceStratum := ⟨hx.1.2, hx.2⟩
    exact (hcoverage (by simpa [sourceStratum] using hxlocal)).2
  have hsubset_bounds : U ∩ sourceStratum ⊆ Ubounds ∩ sourceStratum := by
    intro x hx
    exact ⟨hx.1.1, hx.2⟩
  have hmeasure_bounds_le :
      (μ.restrict (U ∩ sourceStratum)).prod ν ≤
        (μ.restrict (Ubounds ∩ sourceStratum)).prod ν := by
    rw [Measure.restrict_prod_eq_prod_univ, Measure.restrict_prod_eq_prod_univ]
    exact Measure.restrict_mono (Set.prod_mono_left hsubset_bounds) le_rfl
  have hloss_U :
      ∀ᵐ z : α × EuclideanSpace ℝ ρ ∂
          (μ.restrict (U ∩ sourceStratum)).prod ν,
        z.2 ∈ Metric.ball (0 : EuclideanSpace ℝ ρ) R →
          c * (aoyagiCoordinateSquareSum
              (paperEndpointFixedBaseResidualBlockCoordinateMap
                (K := ℝ) W B U₀ hU₀ Cedge z.1) +
            aoyagiCoordinateSquareSum (fun i => z.2 i)) ≤ loss z := by
    exact ae_mono hmeasure_bounds_le (by simpa [ρ, sourceStratum] using hloss_ae)
  have hdensity_nonneg_U :
      ∀ᵐ z : α × EuclideanSpace ℝ ρ ∂
          (μ.restrict (U ∩ sourceStratum)).prod ν,
        z.2 ∈ Metric.ball (0 : EuclideanSpace ℝ ρ) R →
          0 ≤ density z := by
    exact ae_mono hmeasure_bounds_le
      (by simpa [ρ, sourceStratum] using hdensity_nonneg_ae)
  have hdensity_le_U :
      ∀ᵐ z : α × EuclideanSpace ℝ ρ ∂
          (μ.restrict (U ∩ sourceStratum)).prod ν,
  ...
  rcases residualSourceHypotheses_mono
      (W := W) (B := B) (U₀ := U₀) (hU₀ := hU₀)
      (Cedge := Cedge) (source := localSource)
      (source' := U ∩ sourceStratum) (μ := μ) (t := t)
      hsubset_local hpos_local
      (by simpa [residualNegPowerIntegrableOn] using hbase_local) with
    ⟨hpos_U, hbase_U⟩
  have hfin :=
    lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_of_residual_power_lt_top
      (W := W) (B := B) sourceData
      ...
      (c := c) (C := C) hR hc hC ht hpos_U
      (by simpa [residualNegPowerIntegrableOn] using hbase_U)
      (by simpa [ρ, sourceStratum] using hloss_U)
      (by simpa [ρ, sourceStratum] using hdensity_nonneg_U)
      (by simpa [ρ, sourceStratum] using hdensity_le_U)
  exact ⟨U, hU_open, hx₀U, by simpa [ρ, sourceStratum] using hfin⟩
...
set_option linter.unusedSectionVars false in
/-- Local-source signed-box handoff to the two-sided p.13 loss-density
integrability equivalence.

The weighted signed-box source chart supplies residual positivity through a
positivity-only monomial-lower helper, while residual measurability is derived
from the measurable fixed-basis edge matrices.  The local boundedness
hypothesis `residual <= Rreg^2` and the four two-sided loss/density comparison
bounds remain explicit.  No source coverage, chart
construction, Jacobian/density transport, normal crossings, pole order, or RLCT
is asserted. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_residualSource_signedBox_withDensity_monomialLower_edgeMatrix
    [∀ j, FiniteDimensional ℝ (W j)]
    {α ι : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    [Fintype ι] {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
    ...
    {loss density :
      α × EuclideanSpace ℝ
        (AoyagiRegularBlockCoordinateIndex
          ...
          (throughSubspaceEndpointComplementIndex
            (reverseVertex W) (reverseEdge W B) U₀ 0)) → ℝ}
    {t Rreg cLreg CLreg dρ Dρ cres : ℝ}
    {Rres : ι → ℝ} {kres : ι → ℕ}
    (hRreg : 0 < Rreg)
  ...
  have hpos_source :
      ...
      residualSquareSum_pos_ae_of_measure_map_signedBox_withDensity_monomialLower_of_measurable_edgeMatrix
        ...
        (Cedge := Cedge) (source := source) (μ := μ)
        (chart := sourceChart) (density := sourceDensity)
        (c := cres) (R := Rres) (k := kres)
        hEdgeMatrix hsourceChart hmap hcres hres_lower
  exact
    exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_two_sided_bounds
      (W := W) (B := B) sourceData
      (source := source) (μ := μ) (ν := ν) (loss := loss) (density := density)
      (t := t) (R := Rreg) (cL := cLreg) (CL := CLreg) (dρ := dρ) (Dρ := Dρ)
      hsource_meas hmeas_source hRreg hpos_source hle_source
...
set_option linter.unusedSectionVars false in
/-- Source-rank-stratum signed-box handoff to the two-sided p.13 loss-density
integrability equivalence.

This is the source-stratum specialization of the local-source signed-box
theorem.  It keeps the residual local boundedness hypothesis and all
loss/density comparison bounds explicit, and it does not assume the signed-box
critical inequalities or source-density upper bounds that would imply residual
integrability on the whole source stratum. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_residualSource_signedBox_withDensity_monomialLower_edgeMatrix
    [∀ j, FiniteDimensional ℝ (W j)]
    {α ι : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    [Fintype ι] {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
      ...
      cres * ∏ i, (|y i|) ^ (2 * (kres i : ℝ)) ≤
        aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge (sourceChart y)))
    (hle_source :
      ∀ᵐ x ∂ μ.restrict (paperEndpointFixedBaseSourceRankStratum
          (K := ℝ) W B Cedge r rEdge),
        aoyagiCoordinateSquareSum
          (paperEndpointFixedBaseResidualBlockCoordinateMap
            (K := ℝ) W B U₀ hU₀ Cedge x) ≤ Rreg ^ 2)
  ...
  simpa [sourceStratum] using
    exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_residualSource_signedBox_withDensity_monomialLower_edgeMatrix
      ...
      (dρ := dρ) (Dρ := Dρ) (cres := cres) (Rres := Rres) (kres := kres)
...
set_option linter.unusedSectionVars false in
/-- Boundary-explicit source-stratum signed-box handoff on an open local piece.

The signed-box pushforward is assumed for the actual local source
`Ulocal ∩ sourceStratum`.  This avoids any claim that a pushforward identity
for a larger chart source automatically restricts to a smaller neighborhood.
The proof applies the local-source signed-box two-sided theorem to that local
piece, then rewrites the resulting open set back as a source-stratum
neighborhood. -/
theorem exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_openSourceStratum_residualSource_signedBox_withDensity_monomialLower_edgeMatrix
    [∀ j, FiniteDimensional ℝ (W j)]
    {α ι : Type*} [TopologicalSpace α] [MeasurableSpace α] [OpensMeasurableSpace α]
    [Fintype ι] {x₀ : α}
    {U₀ : Submodule ℝ (reverseVertex W 0)}
  ...
  rcases
      exists_open_lintegral_ofReal_loss_rpow_neg_mul_density_p13RegularCoordinates_lt_top_iff_residual_power_lt_top_of_localSource_residualSource_signedBox_withDensity_monomialLower_edgeMatrix
        ...
        (by simpa [source, sourceStratum] using hmap)
        hcres hres_lower
        (by simpa [source, sourceStratum] using hle_source)
        (by simpa [hfilter] using hloss_lower)
        (by simpa [hfilter] using hloss_upper)
        (by simpa [hfilter] using hdensity_lower)
        (by simpa [hfilter] using hdensity_upper) with
