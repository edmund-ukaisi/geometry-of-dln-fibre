...
/-!
# `DLN.Aoyagi.Corank2OverVanishA_20_5_7` — the per-type CONCRETE facts, leaf `(20,5,7)`

Pattern-A over-vanishing leaf `(p1,p2,p3) = (20,5,7)`. Mirrors the proven `(20,1,1)` template
...
namespace DLNFibre.DLN.Aoyagi.OverVanishA_20_5_7

/-! ## §0 — the leaf and the composite -/

def idxCanon : Idx := ⟨⟨20, by decide⟩, ⟨5, by decide⟩, ⟨7, by decide⟩⟩

noncomputable def gCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun w =>
  blockBlowupMap S1 20 (nativeChart1 20
    (blockBlowupMap (sigmaC1Fs 20) 5 (blockBlowupMap (sigmaC2Fs 20) 7 w)))
...
def phiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun u i =>
  ...
  else if i = 17 then -(u 1 * u 13 * u 7 + u 10 * u 2)
  else if i = 18 then -(u 1 * u 14 * u 7 + u 10 * u 3)
  else if i = 19 then -(u 1 * u 15 * u 7 + u 10 * u 4)
  else 0
...
theorem phiCanon_read (u v : Fin 21 → ℝ) (h : ∀ i, keepCanon i → u i = v i) :
  ...
  funext i
  simp only [phiCanon]
  split_ifs <;> simp only [h0, h1, h2, h3, h4, h7, h10, h12, h13, h14, h15]
...
theorem prod_vmExpCanon (u : Fin 21 → ℝ) : (∏ d, (u d) ^ vmExpCanon d) = u 5 * u 7 * u 20 := by
  have h1 : ∀ d : Fin 21, u d ^ vmExpCanon d
      = if d = 5 ∨ d = 7 ∨ d = 20 then u d else 1 := fun d => by
    simp only [vmExpCanon]; split_ifs <;> simp
  simp_rw [h1]
  rw [Finset.prod_ite, Finset.prod_const_one, mul_one,
    show Finset.filter (fun d : Fin 21 => d = 5 ∨ d = 7 ∨ d = 20) Finset.univ
        = {5, 7, 20} from by decide,
...
theorem phiCanon_norm_bound {x : Fin 21 → ℝ} {r : ℝ} (hr1 : 1 ≤ r) (hx : ‖x‖ ≤ r) :
  ...
  rw [Real.norm_eq_abs]
  by_cases h16 : i = 16
  · rw [h16, show phiCanon x 16 = -(x 0 * x 10 + x 1 * x 12 * x 7) from rfl]
    rw [abs_neg]
    calc |x 0 * x 10 + x 1 * x 12 * x 7| ≤ |x 0 * x 10| + |x 1 * x 12 * x 7| := abs_add_le _ _
      _ ≤ r ^ 2 + r ^ 3 := add_le_add (hq 0 10) (hcube 1 12 7)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
  by_cases h17 : i = 17
  · rw [h17, show phiCanon x 17 = -(x 1 * x 13 * x 7 + x 10 * x 2) from rfl]
    rw [abs_neg]
    calc |x 1 * x 13 * x 7 + x 10 * x 2| ≤ |x 1 * x 13 * x 7| + |x 10 * x 2| := abs_add_le _ _
      _ ≤ r ^ 3 + r ^ 2 := add_le_add (hcube 1 13 7) (hq 10 2)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
  by_cases h18 : i = 18
  · rw [h18, show phiCanon x 18 = -(x 1 * x 14 * x 7 + x 10 * x 3) from rfl]
    rw [abs_neg]
    calc |x 1 * x 14 * x 7 + x 10 * x 3| ≤ |x 1 * x 14 * x 7| + |x 10 * x 3| := abs_add_le _ _
      _ ≤ r ^ 3 + r ^ 2 := add_le_add (hcube 1 14 7) (hq 10 3)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
  by_cases h19 : i = 19
  · rw [h19, show phiCanon x 19 = -(x 1 * x 15 * x 7 + x 10 * x 4) from rfl]
    rw [abs_neg]
    calc |x 1 * x 15 * x 7 + x 10 * x 4| ≤ |x 1 * x 15 * x 7| + |x 10 * x 4| := abs_add_le _ _
      _ ≤ r ^ 3 + r ^ 2 := add_le_add (hcube 1 15 7) (hq 10 4)
      _ ≤ 2 * r ^ 3 := by linarith [hr23]
...
end DLNFibre.DLN.Aoyagi.OverVanishA_20_5_7
