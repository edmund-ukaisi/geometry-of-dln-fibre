...
/-- The ideal generated by the three regular matrix-entry families on p. 13:
`Ctop - 1`, `F2`, and `F3`. -/
def regularBlockEntryIdeal {a b m n p : Type*}
    (X : Matrix a b R) (F2 : Matrix n p R) (F3 : Matrix m n R) : Ideal R :=
...
/-- The four-block ideal is the supremum of the regular-block ideal and the
residual-block entry ideal. -/
theorem fourMatrixEntryIdeal_eq_regularBlockEntryIdeal_sup_matrixEntryIdeal
    {a b m n p : Type*}
    (X : Matrix a b R) (F2 : Matrix n p R) (F3 : Matrix m n R)
    (D : Matrix m p R) :
...
theorem matrixEntryIdeal_fromBlocks_neg_neg_sub_mul_eq_fourMatrixEntryIdeal
  ...
  calc
    ...
    _ = fourMatrixEntryIdeal X F2 F3 (D - F3 * F2) := by
      unfold fourMatrixEntryIdeal regularBlockEntryIdeal
