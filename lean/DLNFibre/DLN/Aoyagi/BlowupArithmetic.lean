...
For every selected pivot in the center, the data provide a chart-point type,
the selected variable, residual coordinates, and transformed values of all
center variables, with the exact selected-entry formula
`pivot -> u`, `i != pivot -> u * residual i`.

This is a finite coordinate scaffold only.  It does not prove chart coverage,
regularity, transition regularity, Jacobian arithmetic, normal crossings, or
RLCT extraction. -/
structure SelectedEntryChartFamilyData (center : Finset ι) where
  ChartPoint : center → Type*
  selectedVar : ∀ pivot : center, ChartPoint pivot → α
  residualCoord : ∀ pivot : center, ChartPoint pivot → ι → α
  value : ∀ pivot : center, ChartPoint pivot → ι → α
  value_eq_selectedEntry :
    ∀ (pivot : center) (x : ChartPoint pivot) (i : ι),
      value pivot x i =
        selectedEntryChartMap pivot.1 (selectedVar pivot x) (residualCoord pivot x) i

namespace SelectedEntryChartFamilyData

/-- The standard selected-entry chart family whose chart points are explicit
pairs `(u, residual)`.

This constructs the finite coordinate formula itself, not chart coverage or
regularity. -/
def standard (center : Finset ι) :
    SelectedEntryChartFamilyData (α := α) center where
  ChartPoint := fun _ ↦ α × (ι → α)
  selectedVar := fun _ x ↦ x.1
  residualCoord := fun _ x ↦ x.2
  value := fun pivot x i ↦ selectedEntryChartMap pivot.1 x.1 x.2 i
  value_eq_selectedEntry := by
    intro pivot x i
    rfl

@[simp] theorem value_pivot
    {center : Finset ι} (data : SelectedEntryChartFamilyData (α := α) center)
    (pivot : center) (x : data.ChartPoint pivot) :
    data.value pivot x pivot.1 = data.selectedVar pivot x := by
  rw [data.value_eq_selectedEntry]
  simp

theorem value_of_ne
    {center : Finset ι} (data : SelectedEntryChartFamilyData (α := α) center)
    {pivot : center} (x : data.ChartPoint pivot) {i : ι}
    (hi : i ≠ pivot.1) :
    data.value pivot x i = data.selectedVar pivot x * data.residualCoord pivot x i := by
  rw [data.value_eq_selectedEntry]
  exact selectedEntryChartMap_of_ne (data.selectedVar pivot x)
    (data.residualCoord pivot x) hi

theorem selected_dvd_value
    {center : Finset ι} (data : SelectedEntryChartFamilyData (α := α) center)
    (pivot : center) (x : data.ChartPoint pivot) (i : ι) :
    data.selectedVar pivot x ∣ data.value pivot x i := by
  rw [data.value_eq_selectedEntry]
  exact selectedEntryChartMap_pivot_dvd pivot.1
    (data.selectedVar pivot x) (data.residualCoord pivot x) i

/-- In a supplied selected-entry chart family, the selected variable occurs
among the transformed finite-center values. -/
theorem selected_mem_valueSet
    {center : Finset ι} (data : SelectedEntryChartFamilyData (α := α) center)
    (pivot : center) (x : data.ChartPoint pivot) :
...
/-- In a concrete selected-entry chart family, the transformed finite-center
ideal is generated by the selected variable.

This is the family-data version of
`selectedEntryChartMap_centerIdeal_eq_span_singleton`.  It is still finite
ideal algebra only, not chart coverage, regularity, transition regularity, or
normal crossings. -/
theorem centerIdeal_eq_span_singleton
    {ι R : Type*} [DecidableEq ι] [CommSemiring R]
    {center : Finset ι} (data : SelectedEntryChartFamilyData (α := R) center)
    (pivot : center) (x : data.ChartPoint pivot) :
    Ideal.span
        {v : R | ∃ i, i ∈ center ∧ data.value pivot x i = v} =
      Ideal.span ({data.selectedVar pivot x} : Set R) := by
  simpa [data.value_eq_selectedEntry] using
    selectedEntryChartMap_centerIdeal_eq_span_singleton
      pivot.2 (data.selectedVar pivot x) (data.residualCoord pivot x)
...
/-- Concrete selected-entry chart-family data for the finite Case 2
residual-block center.

This carries the affine selected-entry coordinate formula for every residual
block pivot.  It is not chart coverage, transition regularity, Jacobian
arithmetic, or normal crossings. -/
abbrev Case2ResidualBlockSelectedEntryChartFamilyData
    (n : ℕ → ℕ) (S J : ℕ) (R : Type*) [Monoid R] :=
  SelectedEntryChartFamilyData (α := R) (case2ResidualBlockPivotEntries n S J)

namespace Case2ResidualBlockSelectedEntryChartFamilyData

/-- The standard concrete selected-entry chart family for the Case 2 residual
block center. -/
def standard (n : ℕ → ℕ) (S J : ℕ) (R : Type*) [Monoid R] :
    Case2ResidualBlockSelectedEntryChartFamilyData n S J R :=
  SelectedEntryChartFamilyData.standard (α := R)
    (case2ResidualBlockPivotEntries n S J)

/-- The displayed top-left Case 2 pivot as a point of the finite residual
block center. -/
def displayedPivot (n : ℕ → ℕ) {S J : ℕ}
...
@[simp] theorem standard_value_displayedPivot
    ...
    (p : ℕ × ℕ) :
    (standard n S J R).value (displayedPivot n hS hcont) (u, residual) p =
      selectedEntryChartMap (J + 1, J + 1) u residual p :=
  rfl

end Case2ResidualBlockSelectedEntryChartFamilyData
...
/-- The free post-pivot tail of Aoyagi's paper `C' = Q⁻¹ C` is the next
same-stage source following factor.

Only the pivot row of `Q⁻¹ C` changes, so after deleting the pivot column the
tail is the original source following factor restricted to `(S,J+1)`. -/
theorem case2DisplayedPostPivotFreeFollowingFactor_paperCprime_eq_sourceFollowingFactor_succ
    {τ R : Type*} [CommRing R]
    (n : ℕ → ℕ) {S J : ℕ} (hS : 1 ≤ S)
    (hcont : J + 1 ≤ prefixMinNat n (S + 1))
    (residual : ℕ × ℕ → R) (C : ℕ → τ → R) :
  ...
  rw [show
      case2DisplayedPostPivotFreeFollowingFactor n hS hcont
          (case2DisplayedPaperCprime n hS hcont residual C) =
        case2DisplayedPostPivotFollowingFactor n hS hcont residual C from rfl]
  exact case2DisplayedPostPivotFollowingFactor_eq_sourceFollowingFactor_succ
    n hS hcont residual C

/-- Concrete displayed source-chart lower-row handoff for Aoyagi's paper
transported following factor `C' = Q⁻¹ C`, without the chart-family boundary.

This projects the `P`-operated source side of the weighted `Q/P` equality to
the lower rows, rewrites the weighted right side by the post-pivot product,
and then identifies the lower tail of `Q⁻¹ C` with the next same-stage source
following factor.  It is finite displayed-pivot algebra only. -/
theorem sourceChartMap_paperCprimeWeightedLowerRows_withoutChartFamily
    {τ R : Type*} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
  ...
  have hproj :
      (((weightedPivotBlockRowOp q (fun i ↦ pivotFirstX row col A i ()) *
        ...
        (((weightedPivotDiagonal (post.weight (J + 1))
          ...
          case2DisplayedPaperDppp n hS hcont residual) *
          Cprime).submatrix e id) := by
    simpa [row, col, A, upivot, post, Cprime,
      case2DisplayedPaperDchart, case2DisplayedPaperDppp,
      case2DisplayedPaperCprime, case2DisplayedPaperQinv,
      case2DisplayedSourceNormalizedBlock_eq_displayedNormalizedMatrix,
      case2DisplayedSourceSubstitutionBlock_eq_displayedSubstitutionMatrix,
      case2DisplayedSourceTransportedFollowingFactor_eq_displayedTransportedFollowingFactor,
      case2DisplayedSourceFollowingFactor] using congrArg
      (fun M : Matrix (Unit ⊕ pivotComplement row) τ R ↦ M.submatrix e id)
      hqp
  have htail :
      case2DisplayedPostPivotFreeFollowingFactor n hS hcont Cprime =
  ...
  calc
    ...
    _ = diagonal
          ...
          (case2DisplayedPostPivotResidualBlock n hS hcont residual *
            case2SourceFollowingFactor (n := n) (S := S) (J := J + 1) C) := by
      simpa [row, col, A, upivot, post, Cprime, e, htail,
        case2ResidualRowLevel] using
        case2DisplayedWeightedPaperDppp_mul_freeCprime_postPivot_eq_nextSameStageProduct
          n hS hcont
          (post.weight (J + 1))
          (fun i : pivotComplement (case2DisplayedPivotRow n hS hcont) ↦
...
/-- Concrete displayed source-chart lower-row handoff for Aoyagi's paper
transported following factor `C' = Q⁻¹ C`.

This compatibility wrapper keeps the older chart-family-bearing API; the
finite proof delegates to the chart-family-free constructor. -/
theorem sourceChartMap_paperCprimeWeightedLowerRows_withCorrectedPostData
    {τ R : Type*} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
...
multiplication by a supplied following product, without the chart-family
boundary.

This is only congruence of the existing paper-`C'` lower-row handoff.  The
matrix `F` remains supplied; the theorem does not include the pivot row,
construct a successor following product, prove chart coverage, or assert a
transition invariant. -/
theorem sourceChartMap_paperCprimeWeightedLowerRows_mul_followingProduct_withoutChartFamily
      ...
      (((weightedPivotBlockRowOp q (fun i ↦ pivotFirstX row col A i ()) *
        ...
        (diagonal
          (fun i : Case2ResidualRowIndex n S (J + 1) ↦
            post.weight (case2ResidualRowLevel n S (J + 1) i)) *
          (case2DisplayedPostPivotResidualBlock n hS hcont residual *
            case2SourceFollowingFactor (n := n) (S := S) (J := J + 1) C)) * F ∧
      IntroducedLabelExponentCertificates L n S (J + 1)
        (updateSelectedLabelVector S (J + 1) (correctedCase2PivotVector n S J) t)
        (updateSelectedLabelScalar S (J + 1)
          (((prefixMinNat n S : ℤ) - (J : ℤ)) *
  ...
  refine ⟨q, ?_, hexp, hlevel, hgap, hcase2⟩
  simpa using congrArg (fun M : Matrix (Case2ResidualRowIndex n S (J + 1)) τ R ↦ M * F) hq

/-- Continuing displayed source-chart lower-row handoff after right
multiplication by a supplied following product.

This compatibility wrapper keeps the older chart-family-bearing API; the
finite proof delegates to the chart-family-free constructor. -/
theorem sourceChartMap_paperCprimeWeightedLowerRows_mul_followingProduct_withCorrectedPostData
    {υ τ R : Type*} [CommRing R] [Fintype τ]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
  ...
  exact
    sourceChartMap_paperCprimeWeightedLowerRows_mul_followingProduct_withoutChartFamily
      pre u residual hS hSL hcont exponentPre levelInv leastValueGap C F

/-- Continuing displayed source-chart stack boundary with old top rows and the
actual source suffix, without the chart-family boundary.

This lifts the supplied paper `Q/P` identity through the unchanged old top
rows and then multiplies by Aoyagi's raw source suffix.  The right side keeps
the paper transported following factor `C' = Q⁻¹ C` in pivot-first order.  This
is a finite stack identity, not source production of a source-ordered successor
following object, suffix, chart coverage, transition invariant, or RLCT
consequence. -/
theorem sourceChartMap_continuingOldTopSourceSuffixPaperCprimeStack_withoutChartFamily
    {R : Type*} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
     ...
     ∃ q : pivotComplement row → R,
      (((fromBlocks (case2DisplayedSourceOldTopWeight pre) 0 0
        ...
        verticalBlock (case2DisplayedSourceOldTopBlock C)
          (case2DisplayedSourceFollowingFactor n hS hcont C)) * suffix) =
      ((fromBlocks (case2DisplayedSourceOldTopWeight pre) 0 0
          ((weightedPivotDiagonal (post.weight (J + 1))
              (fun i : pivotComplement row ↦
                post.weight (case2ResidualRowLevel n S J i.1)) *
            case2DisplayedPaperDppp n hS hcont residual)) *
        verticalBlock (case2DisplayedSourceOldTopBlock C)
          (case2DisplayedPaperCprime n hS hcont residual C)) * suffix)) ∧
      IntroducedLabelExponentCertificates L n S (J + 1)
        (updateSelectedLabelVector S (J + 1) (correctedCase2PivotVector n S J) t)
        (updateSelectedLabelScalar S (J + 1)
          (((prefixMinNat n S : ℤ) - (J : ℤ)) *
  ...
  refine
    ⟨q, ?_,
      ...
      hpost.case2Gap_of_leastValueGap_of_correctedExponentPostData
        levelInv hexpPost leastValueGap⟩
  exact congrArg
    (fun M ↦ M * sourceSuffixProduct κ Ctail S hSuffix)
    hblock

/-- Continuing displayed source-chart stack boundary with old top rows and the
actual source suffix.
...
theorem sourceChartMap_continuingOldTopSourceSuffixPaperCprimeStack_withCorrectedPostData
    {R : Type*} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
...
The rewrite uses only that the `(S,J+1)` following-factor restriction ignores
the replaced row `J+1`.  This remains a lower-row identity; it does not produce
the supplied successor object from charts, include old top rows or suffixes, or
assert a transition theorem. -/
theorem sourceChartMap_paperCprimeWeightedLowerRows_withSuccFollowingFactorAndCorrectedData
    {τ R : Type*} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
...
The matrix `F` remains supplied, and the theorem is still only a lower-row
identity.  It does not construct a successor following product, chart-family
successor, suffix product, transition invariant, Jacobian, normal crossings,
pole order, or RLCT. -/
theorem sourceChartMap_paperCprimeWeightedLowerRows_mul_F_withSuccFollowingFactorAndCorrectedData
    {υ τ R : Type*} [CommRing R] [Fintype τ]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
...
This keeps the successor lower-row diagonal explicit and projects only the
lower rows.  It is not a full successor product including the pivot row, not
source production of `C'^(S+1)`, and not chart coverage or a transition
theorem. -/
theorem sourceChartMap_continuingWeightedSourceFollowingPayload_withFiniteCenterIdeal
    {τ R : Type*} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t : ℕ → ℕ → ℕ → ℤ}
    {numerator leastValue : ℕ → ℕ → ℤ}
...
paper-`C'` lower-row handoff and finite center principalization. -/
abbrev ContinuingWeightedSourceFollowingFrontierPayload
    {τ R : Type*} [CommRing R]
    (L : ℕ) (n : ℕ → ℕ) (S J : ℕ)
    (t : ℕ → ℕ → ℕ → ℤ)
    (numerator leastValue : ℕ → ℕ → ℤ)
...
/-- The current source-row block of the supplied successor following factor is
the formula-level successor block.

This consumes only the obligation's supplied formula equality for `Csucc`.
It does not construct `Csucc`, a suffix, charts, transitions, normal crossings,
pole order, or RLCT data. -/
theorem continuing_Csucc_currentFollowingBlock_eq_formula
    {R : Type u} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t t' : ℕ → ℕ → ℕ → ℤ}
    {numerator numerator' leastValue leastValue' : ℕ → ℕ → ℤ}
    ...
    case2SourceCurrentFollowingBlock n S Csucc =
      case2SourceSuccessorFollowingBlock n data.stage_pos data.continuation
        residual C := by
  rw [ob.Csucc_eq_formula]
  rfl
...
This is a consumer of `SourceProductionObligation`: the existing continuing
source-current stack identity is rewritten through the supplied equality
`Csucc = case2DisplayedSourceSuccessorFollowingFactor ...`.  It does not
construct `Csucc`, source-produce `C'^(S+1)`, produce the source suffix,
construct successor charts or transitions, prove normal crossings, pole order,
termination, or RLCT data. -/
theorem continuing_sourceCurrentStack_suppliedCsucc
    {R : Type u} [CommRing R]
    {L : ℕ} {n : ℕ → ℕ} {S J : ℕ}
    {t t' : ℕ → ℕ → ℕ → ℤ}
    {numerator numerator' leastValue leastValue' : ℕ → ℕ → ℤ}
  ...
  rcases
      sourceChartMap_continuingOldTopSourceSuffixSuccFollowingBlock_withCorrectedPostData
        pre u residual data.stage_pos data.stage_le data.continuation hnext
  ...
  refine ⟨q, ?_, hexp, hlevel, hgap, hcase2⟩
  have hCsuccBlock :
      (case2SourceCurrentFollowingBlock n S Csucc).submatrix
          (case2SourceOldTopPaperCprimeRowEquiv n data.stage_pos data.continuation) id =
        (case2SourceSuccessorFollowingBlock n data.stage_pos data.continuation
          residual C).submatrix
          (case2SourceOldTopPaperCprimeRowEquiv n data.stage_pos data.continuation) id := by
