...
/-!
# `DLN.Aoyagi.Corank2OverVanishB_20_6_7` — pattern-B over-vanishing leaf `(20,6,7)`

The pattern-B per-type deliverables for the over-vanishing leaf `(p1,p2,p3) = (20,6,7)`, mirroring the
canonical `(20,1,1)` template (`Corank2OverVanishCanon334`). Pattern B = reg-seq DROPS column `c = 1`
(keeps columns `c = 0, 2`), `S = {0,2,3,5,6,8,9,11}`; straighten-block `{12,13,14,15}` (for `p2 = 6`).

`(20,6,7)` is NON-coinciding (`p2 = 6 ≠ 7 = p3`, `vm = u₆·u₇·u₂₀`, degree 3) and its straightening
`φ` is genuinely QUADRATIC (max degree 2), so the quadratic cover atom `image_comp_blockShear_superset`
applies. The 8 reg-seq entry values are `sympy`-verified end-to-end vs the Lean `gFlat`
(`a3f030 @4ceface15`, cross-checked by this seat, max err `1.3e-15`).
-/
...
namespace DLNFibre.DLN.Aoyagi.OverVanishB_20_6_7

/-! ## §0 — the leaf and its composite -/

/-- The over-vanishing leaf index `(p1,p2,p3) = (20,6,7)`. -/
def idxCanon : Idx := ⟨⟨20, by decide⟩, ⟨6, by decide⟩, ⟨7, by decide⟩⟩

/-- The leaf composite `gFlat idxCanon` (`∘id`s and `Idx` projections resolved). -/
noncomputable def gCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun w =>
  blockBlowupMap S1 20 (nativeChart1 20
    (blockBlowupMap (sigmaC1Fs 20) 6 (blockBlowupMap (sigmaC2Fs 20) 7 w)))

/-- `gFlat idxCanon = gCanon`. -/
theorem gFlat_idxCanon : gFlat idxCanon = gCanon := by funext w; rfl

/-! ## §1 — the straightening shear `Ψ = psiCanon` (block `{12,13,14,15}`) -/
...
def phiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := fun u i =>
  ...
  else if i = 13 then -(u 11 * u 2 + u 17 * u 7)
  else if i = 14 then -(u 11 * u 3 + u 18 * u 7)
  else if i = 15 then -(u 11 * u 4 + u 19 * u 7)
  else 0

/-- The straightening `Ψ = blockShear φ`. -/
noncomputable def psiCanon : (Fin 21 → ℝ) → (Fin 21 → ℝ) := blockShear phiCanon

/-- The kept coordinates: everything outside the straightened block `{12,13,14,15}`. -/
...
/-- `φ` reads only kept coordinates (`{0,2,3,4,7,11,16,17,18,19}`, all outside the block). -/
theorem phiCanon_read (u v : Fin 21 → ℝ) (h : ∀ i, keepCanon i → u i = v i) :
    phiCanon u = phiCanon v := by
  have h0 := h 0 (by decide); have h2 := h 2 (by decide); have h3 := h 3 (by decide)
  have h4 := h 4 (by decide); have h7 := h 7 (by decide); have h11 := h 11 (by decide)
  have h16 := h 16 (by decide); have h17 := h 17 (by decide); have h18 := h 18 (by decide)
  have h19 := h 19 (by decide)
  funext i
  simp only [phiCanon]
  split_ifs <;> simp only [h0, h2, h3, h4, h7, h11, h16, h17, h18, h19]
...
/-- The dominant monomial exponent `vm = u₆·u₇·u₂₀`. -/
def vmExpCanon : Fin 21 → ℕ := fun d => if d = 6 ∨ d = 7 ∨ d = 20 then 1 else 0

/-- `∏_d (u d)^(vmExpCanon d) = u₆·u₇·u₂₀`. -/
theorem prod_vmExpCanon (u : Fin 21 → ℝ) : (∏ d, (u d) ^ vmExpCanon d) = u 6 * u 7 * u 20 := by
  have h1 : ∀ d : Fin 21, u d ^ vmExpCanon d = if d = 6 ∨ d = 7 ∨ d = 20 then u d else 1 :=
    fun d => by simp only [vmExpCanon]; split_ifs <;> simp
  simp_rw [h1]
  rw [Finset.prod_ite, Finset.prod_const_one, mul_one,
    show Finset.filter (fun d : Fin 21 => d = 6 ∨ d = 7 ∨ d = 20) Finset.univ = {6, 7, 20} from by
      decide,
...
/-- The 8-element regular-sequence entry-index set `S` (pattern B). -/
...
def zcPair : Fin 4 × Fin 3 → Fin 21 := fun p =>
  if p = (0, 0) then 0 else if p = (0, 2) then 12 else if p = (1, 0) then 2
  else if p = (1, 2) then 13 else if p = (2, 0) then 3 else if p = (2, 2) then 14
...
theorem canon_hentry (u : Fin 21 → ℝ) (k : Fin (dvec (Fin.last 2) * dvec 0)) (hk : k ∈ Scanon) :
  ...
  fin_cases hp <;>
    first
      | exact entry_00 u | exact entry_02 u | exact entry_10 u | exact entry_12 u
      | exact entry_20 u | exact entry_22 u | exact entry_30 u | exact entry_32 u
...
set_option linter.unusedSimpArgs false in
/-- Every binding axis of `jacExp idxCanon` (`⊆ {6,7,20}`) is outside the block `{12,13,14,15}`. -/
...
/-- **The folded Jacobian.** `|jacDet (gFlat idxCanon ∘ Ψ) u| = jacWeight (jacExp idxCanon) u`. -/
...
/-! ## §6 — the QUADRATIC cover-transport (φ has max degree 2) -/
...
theorem phiCanon_norm_bound {x : Fin 21 → ℝ} {r : ℝ} (hr : 0 ≤ r) (hx : ‖x‖ ≤ r) :
  ...
  rw [Real.norm_eq_abs]
  by_cases h12 : i = 12
  · rw [h12, show phiCanon x 12 = -(x 0 * x 11 + x 16 * x 7) from rfl]; exact slot 0 11 16 7
  by_cases h13 : i = 13
  · rw [h13, show phiCanon x 13 = -(x 11 * x 2 + x 17 * x 7) from rfl]; exact slot 11 2 17 7
  by_cases h14 : i = 14
  · rw [h14, show phiCanon x 14 = -(x 11 * x 3 + x 18 * x 7) from rfl]; exact slot 11 3 18 7
  by_cases h15 : i = 15
  · rw [h15, show phiCanon x 15 = -(x 11 * x 4 + x 19 * x 7) from rfl]; exact slot 11 4 19 7
  · rw [phiCanon_keep x i ⟨h12, h13, h14, h15⟩, abs_zero]; positivity

/-- **The folded-chart cover survives the quadratic fold** — the base chart's image over
`closedBall 0 r` sits inside the folded chart `g ∘ Ψ`'s image over the inflated ball `r + 2r²`. -/
...
end DLNFibre.DLN.Aoyagi.OverVanishB_20_6_7
