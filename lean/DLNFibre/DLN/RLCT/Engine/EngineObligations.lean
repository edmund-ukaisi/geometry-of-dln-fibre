...
the CoV `ChartBridge` (Q5 route (b), fork 8) replacing the refuted abstract `ChartsCover`. **Statement
...
`LeafPullback` + `LeafJacobian` + the **chartMap-derived coherence** (each leaf's `chartMap` = the fold
of its root→leaf edge substitutions — so `LeafPullback`/`LeafJacobian` are proved compositionally, not
...
self-map of `Params M` with a derivative `Dφ w` (existential — no derivative field is stored as data);
its determinant is the monomial-Jacobian ledger times a positive-bounded unit:
`|det Dφ| = ∏ (paramsEquivFlat w (divCoord k))^{divExp k − 1} · jacUnit`. Exactly the datum `region_glue`
feeds to `rlctAtOn_boundedUnit_localHomeomorph` (strip the unit) + the scaling bridge (the monomial). -/
...
/-- **The CoV bridge** (fork 8): the atlas covers an UPSTAIRS-open neighbourhood of the zero locus via
...
/-- **The edge-relational per-step invariant** (council: case-keyed identities). case 2: a divisor of
...
`S = J = 0`; the CoV `ChartBridge`; and the exponent hooks — `minAdm M` is an emitted terminal exponent
...
/-- **Reduction layer** (map: `reduction-layer`). The regular peel: the resolution begins at the base
...
/-- **Coverage — THE HARD PART** (map: `coverage-theorem`, owned by coverage-design). The atlas covers
an UPSTAIRS-open box-neighbourhood via chart images, per-leaf InjOn + pullback + Jacobian + the derived
