...
not the degenerate `⊤` over an empty set) and **`r < H s` at every layer** (`hpos`, equivalently the
reduced widths `0 < M⁽ˢ⁾ = H⁽ˢ⁾ − r` — STRICT, all layers including endpoints). `hpos` is load-bearing,
not cosmetic: at `r = H s` (a zero reduced width `M s = 0`, reachable under the non-strict `hr`) the
reduced core `dlnLoss M 0` degenerates — `prod M ≡ 0`, so its local RLCT is `⊤` (`rlctAtOn_zero_eq_top`)
for `L ≥ 2` — while the closed form `aoyagiLambda` is finite; the L2 core reduction routes through
`resolution_charts` (R1), which requires exactly `hMid : 0 < M s`. (`hpos` also subsumes the `L = 1`
