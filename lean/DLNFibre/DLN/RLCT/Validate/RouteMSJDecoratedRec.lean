...
decorated-peel object (architect fit report, near-fit-with-delta→mismatch). Kept for the trail; do
NOT build toward `DecoratedDescent` — one live spine per fork (P6).

*(Original docstring:)* the DECORATED-recursion driver — `(□)` MODULO `DecoratedDescent`: given the
Q2-corrected decorated contract, `RouteMBoxThresholdFinite M` holds for every width vector `M`;
re-points the `(□)` chain off the dead plain `routeMBoxThresholdFinite_of_decoratedPeel` onto the
decorated recursion; the analytic content is entirely in `DecoratedDescent`. -/
