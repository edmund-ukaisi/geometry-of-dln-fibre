...
nondegenerate reduced-width vector `M : Fin (L + 1) → ℕ` (all layers `> 0`) with **no interior
both-drop** (`hNo : NoInteriorBothDrop M`) and the **named box-finiteness** analytic input
(`hbox : RouteMBoxThresholdFinite M`), the deepest DLN core `dlnLoss M 0` at the origin
`(0 : Params M)` has local RLCT `ofReal(lambdaCore M)`.

This is a **wiring** of banked ∀L pieces, swapping the L = 2 achiever
(`routeMCore_box_diverges_achiever_L2`) for the general-`L` achiever
(`routeMCore_box_diverges_achiever_full`, `RouteMAchieverFull.lean`) and carrying `hNo`.

## Two genuine hypotheses that are NOT ∀L-dischargeable (precision discipline)
...
* **`hNo : NoInteriorBothDrop M`** — the achiever value leg lives on the `NoInteriorBothDrop`
  stratum (a parallel hand adjudicates whether `hMid` alone forces it, i.e. whether a
  `StructAdm/hMid → NoInteriorBothDrop` bridge de-conditionalizes it). Carried here.
...
- **Sub-B** (`hdiv`): the general achiever `routeMCore_box_diverges_achiever_full` through Sub-A,
  supplied `hMpos`/`hne`/`hNo`.
...
least one genuine layer `1 ≤ L`) with no interior both-drop (`hNo`) and the named box-finiteness
input (`hbox`), the deepest DLN core `dlnLoss M 0` at the origin has local RLCT
`ofReal(lambdaCore M)`. Assembled from the general-`L` achiever box divergence
(`routeMCore_box_diverges_achiever_full`) through the sorry-free `IsRouteMCover` assembly, the
cover→rlct bridge, the flat↔params transport, and the value lane. Conditional on `hNo` (the achiever
stratum) and `hbox` (the open box-finiteness), both carried as explicit hypotheses. -/
theorem r1_resolution_general (M : Fin (L + 1) → ℕ) (hL : 1 ≤ L) (hMid : ∀ s, 0 < M s)
    (hNo : NoInteriorBothDrop M) (hbox : RouteMBoxThresholdFinite M) :
  ...
  have hdiv : ∀ c' : NNReal,
    ...
    exact routeMCore_box_diverges_achiever_full M hpos hMid hne hNo c' hguard ε hε
