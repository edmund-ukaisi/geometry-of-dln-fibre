...
(`readE (wInt (k+1)) ⟨k⟩ = EfixedReaderGen k`,
...
`eDeepRank0UnitGen x = VvalGen (x p₀) M tach (genBlkFlatEfpGen k (kLDU x)) hle = sqSumHmat0` of the
...
struct `Rmat`, at `k+1` `rmatPad (EfixedReaderGen k (kLDU x))`); `Rfin = 0`. -/
theorem genBlkContinuous_efpGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1)) :
    GenBlkContinuous M (tach M)
      (fun x => genBlkFlatEfpGen M ha k hp1 hp2 (kLDU M (tach M) ha x)) := by
  ...
  · -- Wblk
    ...
    | (jj + 1) =>
      ...
      · simp only [dif_neg hjj]; exact continuous_const
  · -- Rmat = Function.update (genBlkFlatLive 0 (kLDU x)).Rmat (k+1) (rmatPad (EfixedReaderGen k))
    show Continuous (fun x => (genBlkFlatEfpGen M ha k hp1 hp2 (kLDU M (tach M) ha x)).Rmat j)
    show Continuous (fun x => Function.update
      (genBlkFlatLive M (tach M) ha 0 (kLDU M (tach M) ha x)).Rmat (k.val + 1)
      (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderGen M ha k (kLDU M (tach M) ha x))) j)
    by_cases hj : j = k.val + 1
    · subst hj
      have hrw : (fun x => Function.update
            (genBlkFlatLive M (tach M) ha 0 (kLDU M (tach M) ha x)).Rmat (k.val + 1)
            (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderGen M ha k (kLDU M (tach M) ha x)))
            (k.val + 1))
          = fun x => rmatPad M (tach M) (k.val + 1) hp1 hp2
              (EfixedReaderGen M ha k (kLDU M (tach M) ha x)) := by
      ...
      exact continuous_matrix (fun i j => by
        ...
        · simp only [EfixedReaderGen, Matrix.of_apply, if_pos hij]; exact continuous_const
        · simp only [EfixedReaderGen, Matrix.of_apply, if_neg hij]
          exact (hreadE k).matrix_elem i j)
    · have hrw : (fun x => Function.update
            (genBlkFlatLive M (tach M) ha 0 (kLDU M (tach M) ha x)).Rmat (k.val + 1)
            (rmatPad M (tach M) (k.val + 1) hp1 hp2
              (EfixedReaderGen M ha k (kLDU M (tach M) ha x))) j)
  ...
  have hsq : Continuous (fun x => ∑ i, ∑ j,
      ((show Matrix (Fin (Text M (tach M) 0)) (Fin (Wext M L)) ℝ
          from (chainOfMt (x p₀) M (tach M)
            (genBlkFlatEfpGen M ha k hp1 hp2 (kLDU M (tach M) ha x))
...
/-- **The E-fixed chart-params at radial `0`** — `chartParamsGen 0 (genBlkFlatEfpGen k 0) = 0`.
...
theorem chartParamsGen_EfpGen_zero (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1)) :
    chartParamsGen (0 : ℝ) M (tach M) (genBlkFlatEfpGen M ha k hp1 hp2 0) (hleStruct M (tach M) ha)
      = (fun _ => 0 : Params M) := by
  rw [← chartParamsGen_live_zero M (tach M) ha 0]
  funext s
  change Matrix.reindex _ _ (Agen 0 M (tach M) (genBlkFlatEfpGen M ha k hp1 hp2 0) _ s.val)
  ...
  · -- `Cgen 0 (s+1) = Bmat(s+1)·chainQ(Nblk(s+1))` (the `0 • Rmat` term dies); Efp and live share
    ...
    rw [Cgen, Cgen, dif_pos hs, dif_pos hs, zero_smul, zero_smul, add_zero, add_zero,
      genBlkFlatEfpGen_Bmat_succ M ha k hp1 hp2 0 s.val (by omega),
      genBlkFlatEfpGen_Nblk_succ M ha k hp1 hp2 0 s.val (by omega),
...
and `chartParamsGen 0 (genBlkFlatEfpGen k 0) = 0`. -/
theorem eDeepRank0PhiGen_zero (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  change phiGen ((0 : Fin (routeMAmbient M) → ℝ) p₀) M (tach M)
    (genBlkFlatEfpGen M ha k hp1 hp2 0) (hleStruct M (tach M) ha) = 0
...
(`readE (wInt (k+1)) ⟨k⟩ = EfixedReaderGen k`); the surviving `Hmat 0 = 1` entry threads from pivot
...
noncomputable def EfixedReaderGenP (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
...
-/
theorem EfixedReaderGenP_map (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    {𝕜 𝕜' : Type} [CommRing 𝕜] [CommRing 𝕜']
    (vp : Fin (routeMAmbient M) → 𝕜) (vr : Fin (routeMAmbient M) → 𝕜') (f : 𝕜 →+* 𝕜')
    (hv : ∀ q, f (vp q) = vr q) :
    Matrix.map (EfixedReaderGenP M ha k vp) f = EfixedReaderGenP M ha k vr := by
  ext i j
  simp only [Matrix.map_apply, EfixedReaderGenP, Matrix.of_apply]
  split
  · exact map_one f
  · exact hv _

/-- **The generic Efp reader over ℝ is dr0build's `EfixedReaderGen`** (same formula). -/
theorem EfixedReaderGenP_eq (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (x : Fin (routeMAmbient M) → ℝ) :
    EfixedReaderGenP M ha k x = EfixedReaderGen M ha k x := by
  ext i j; simp only [EfixedReaderGenP, EfixedReaderGen, Matrix.of_apply]

/-- **The generic (poly) E-fixed-pivot decoder** `genBlkFlatEfpGenP k x : GenBlk M (tach M) 𝕜` — the
generic live decoder `genBlkFlatLiveGen ha 0 x` with boundary `k+1`'s `Rmat` overridden to
`rmatPad (EfixedReaderGenP k x)`. -/
noncomputable def genBlkFlatEfpGenP (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
  ...
  Rmat := Function.update (genBlkFlatLiveGen M (tach M) ha 0 x).Rmat (k.val + 1)
    (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderGenP M ha k x)
      : Matrix (Fin (Text M (tach M) (k.val + 1))) (Fin (Wext M (k.val + 1))) 𝕜)
  Rfin := (genBlkFlatLiveGen M (tach M) ha 0 x).Rfin

/-- **The generic Efp decoder over ℝ is dr0build's `genBlkFlatEfpGen`** (definitional — same
fields). -/
theorem genBlkFlatEfpGenP_eq_efp (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) :
    genBlkFlatEfpGenP M ha k hp1 hp2 x = genBlkFlatEfpGen M ha k hp1 hp2 x := by
  unfold genBlkFlatEfpGenP genBlkFlatEfpGen
  rw [genBlkFlatLiveGen_eq_live, EfixedReaderGenP_eq]
...
`rmatPad_map` + `EfixedReaderGenP_map`). -/
theorem genBlkFlatEfpGenP_genBlkMap_of (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    {𝕜 𝕜' : Type} [CommRing 𝕜] [CommRing 𝕜']
    (vp : Fin (routeMAmbient M) → 𝕜) (vr : Fin (routeMAmbient M) → 𝕜') (f : 𝕜 →+* 𝕜')
    (hv : ∀ q, f (vp q) = vr q) :
    GenBlkMap M (tach M) (genBlkFlatEfpGenP M ha k hp1 hp2 vp)
      (genBlkFlatEfpGenP M ha k hp1 hp2 vr) f := by
  ...
  change ((Function.update (genBlkFlatLiveGen M (tach M) ha 0 vp).Rmat (k.val + 1)
      (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderGenP M ha k vp))) s).map f
    = (Function.update (genBlkFlatLiveGen M (tach M) ha 0 vr).Rmat (k.val + 1)
      (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderGenP M ha k vr))) s
  by_cases hs : s = k.val + 1
  · subst hs
    rw [Function.update_self, Function.update_self, rmatPad_map,
      EfixedReaderGenP_map M ha k vp vr f hv]
...
noncomputable def UPolyEfpGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  sqSumHmat0 (chainOfMt
    ...
    (genBlkFlatEfpGenP M ha k hp1 hp2 (kLDUGen M (tach M) ha (Xvec (routeMAmbient M))))
...
theorem eval_UPolyEfpGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  have hmap : GenBlkMap M (tach M)
      (genBlkFlatEfpGenP M ha k hp1 hp2 (kLDUGen M (tach M) ha (Xvec (routeMAmbient M))))
      (genBlkFlatEfpGenP M ha k hp1 hp2 (kLDU M (tach M) ha u)) (MvPolynomial.eval u) :=
    genBlkFlatEfpGenP_genBlkMap_of M ha k hp1 hp2
  ...
  rw [eDeepRank0UnitGen, VvalGen_eq_sqSumHmat0,
    ← genBlkFlatEfpGenP_eq_efp M ha k hp1 hp2 (kLDU M (tach M) ha u), UPolyEfpGen,
...
`(0,0)`-pivot indicator at boundary `k` (`EfixedReaderGen` pins `(0,0) ↦ 1`, reads `readE`
elsewhere; `readE (wInt (k+1)) ⟨k⟩` is the pivot indicator at `p = k+1`). General-`L` lift of
`EfixedReader_wInt1`. -/
theorem EfixedReaderGen_wInt (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L) :
    EfixedReaderGen M ha k (wInt M ha (k.val + 1))
      = fun i j => readE M (tach M) ha (wInt M ha (k.val + 1)) k i j := by
  ext i j
  rw [EfixedReaderGen, Matrix.of_apply, readE_wInt]
  by_cases hij : i.val = 0 ∧ j.val = 0
...
since
`EfixedReaderGen k (wInt (k+1)) = readE (wInt (k+1)) ⟨k⟩`). General-`L` lift of
`genBlkFlatEfp_wInt1`. -/
theorem genBlkFlatEfpGen_wInt (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1)) :
    genBlkFlatEfpGen M ha k hp1 hp2 (wInt M ha (k.val + 1))
...
(`genBlkFlatEfpGen_wInt`), `kLDU` fixes `wInt` (`kLDU_wIntGen`), and the surviving
...
theorem eDeepRank0UnitGen_wInt_ne_zero (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  rw [eDeepRank0UnitGen, kLDU_wIntGen M ha (k.val + 1), genBlkFlatEfpGen_wInt M ha k hp1 hp2,
