...
ordinary pointwise IFT chart (`dln_hchart_residual`, sorry-free, clean-three) brings the loss into the
post-chart sum-of-squares form
...
`nReg`-block of the split, valid at EVERY optimal `v` (no rank-`r`-exactness, no deepest-gauge chart,
no Morse-Bott lemma). The quasi-split engine `rlct_quasiSplit_ge` (via
...
(deepest-side value, `= nReg/2 + coreDeepest`) + the `hAtV` half + `hCore : coreDeepest ≤ rlctAtOn R t0`.
The FIRST peel produces `hAtV`; the SOLE remaining per-`v` content is the **residual-core domination**
`coreDeepest ≤ rlctAtOn R t0` (that the first-peel slice residual has RLCT `≥` the deepest core) and
the slice non-vanishing `hRne`.
...
`hDom` is only asked to supply the residual value comparison + non-vanishing. Its honest discharge is
Aoyagi's Theorem-4 homogeneity comparison applied through the SECOND selected-minor peel + the
...
feeds the `hAtV` half through `rlctAt_ge_nReg_add_slice_of_residual`; `deepest_le_of_optimal_via_L2_ge`
then closes with `hDeepest` + `hDom`. `nReg = nRegL2 H r`. -/
theorem d1ge_L2_of_firstPeel_dominates
  ...
  -- FIRST peel: the invertible `nReg`-minor of the flat Jacobian (coverage: `nReg ≤ rank jacFlatL2`).
