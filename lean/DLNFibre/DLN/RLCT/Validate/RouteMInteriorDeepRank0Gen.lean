...
* `eBlockPivotGen` (+ membership) / `EfixedReaderKGen` / `genBlkFlatEfpKGen` — the E-fixed decoder.
...
* `EfixedReaderKGen_pboE` + the `Cgen`/`Nblk`/`Wblk` matches → `hmap_EfpGen` — the E-slot MAP identity
...
/-! The E-scaling at the pivot boundary is handled directly by the E-block match `EfixedReaderKGen_pboE`
below (which gives `(x p₀) • EfixedReaderKGen k x = readE (pbo p₀ x) k` relative to the pivot). At a
...
`EfixedReader`. -/
noncomputable def EfixedReaderKGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (x : Fin (routeMAmbient M) → ℝ) :
...
`(x p₀) • EfixedReaderKGen k x = readE (pbo p₀ x) k` (as matrices), where `p₀ = eBlockPivotGen k`.
...
theorem EfixedReaderKGen_pboE (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hk : k.val ≠ L - 1)
    (hr : 0 < Text M (tach M) (k.val + 1) - Text M (tach M) (k.val + 2))
    (hc : 0 < Wext M (k.val + 1) - Text M (tach M) (k.val + 2))
    (x : Fin (routeMAmbient M) → ℝ) :
    (x (eBlockPivotGen M ha k hr hc)) • EfixedReaderKGen M ha k x
  ...
  funext i j
  rw [Matrix.smul_apply, smul_eq_mul, EfixedReaderKGen, Matrix.of_apply]
...
/-- **The E-fixed-pivot decoder** `genBlkFlatEfpKGen k` — the live decoder (`rfin = 0`, leaf empty at
`deepRank = 0`) with the pivot-boundary `k+1`'s `Rmat` overridden to `rmatPad (EfixedReaderKGen k x)`.
General-`L` lift of `genBlkFlatEfp`. -/
noncomputable def genBlkFlatEfpKGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  Rmat := Function.update (genBlkFlatLive M (tach M) ha 0 x).Rmat (k.val + 1)
    (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderKGen M ha k x))
...
/-- **The E-fixed chart at an arbitrary point** `phiEfpAt k p₀ y := phiGen (y p₀) … (genBlkFlatEfpKGen k y)`.
...
noncomputable def phiEfpAt (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  phiGen (y p₀) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 y) (hleStruct M (tach M) ha)
...
/-- **The deepRank = 0 unit factor** `eDeepRank0UnitGen x := VvalGen (x p₀) … (genBlkFlatEfpKGen k (kLDU x))`.
...
noncomputable def eDeepRank0UnitGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  fun x => VvalGen (x p₀) M (tach M)
    (genBlkFlatEfpKGen M ha k hp1 hp2 (kLDU M (tach M) ha x)) (hleStruct M (tach M) ha)
...
theorem genBlkFlatEfpKGen_Rmat0 (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) :
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Rmat 0
      = (0 : Matrix (Fin (Text M (tach M) 0)) (Fin (Wext M 0)) ℝ) := by
  change Function.update (genBlkFlatLive M (tach M) ha 0 x).Rmat (k.val + 1)
    (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderKGen M ha k x)) 0 = _
...
theorem C0_eq_one_EfpGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    (chainOfMt v M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x) (hleStruct M (tach M) ha)).toChain.C 0
      = (1 : Matrix (Fin (Text M (tach M) 0)) (Fin (Text M (tach M) 0)) ℝ) := by
  rw [chainOfMt_C_zero v M (tach M) _ (hleStruct M (tach M) ha) ha.hL,
    genBlkFlatEfpKGen_Rmat0 M ha k hp1 hp2 x, smul_zero, add_zero]
  have hBmat : (genBlkFlatEfpKGen M ha k hp1 hp2 x).Bmat 0
...
theorem hC0_EfpGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    (v : ℝ) (x : Fin (routeMAmbient M) → ℝ) :
    (chainOfMt v M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x) (hleStruct M (tach M) ha)).toChain.C 0
        * (chainOfMt v M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x)
...
theorem routeMCore_phiEfpAt (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    routeMCore M (phiEfpAt M ha k p₀ hp1 hp2 y)
      = (y p₀) ^ 2
        * VvalGen (y p₀) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 y) (hleStruct M (tach M) ha) :=
  routeMCore_phiGen (y p₀) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 y) (hleStruct M (tach M) ha)
...
reads `EfixedReaderKGen k` (fixed-`1` pivot) which matches the direct `readE (pbo y)` via the radial
identity `EfixedReaderKGen_pboE`; at OTHER interior boundaries the E-block reads live `readE`, matching
...
theorem genBlkFlatEfpKGen_Bmat_succ (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) (s : ℕ) (hs : s < L) :
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Bmat (s + 1)
...
theorem genBlkFlatEfpKGen_Nblk_succ (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) (s : ℕ) (hs : s < L) :
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Nblk (s + 1) = readN M (tach M) ha x ⟨s, hs⟩ :=
  genBlkFlatLive_Nblk_succ M (tach M) ha 0 x s hs

/-- The Efp decoder's `Rmat` at the pivot boundary `k+1` IS `rmatPad (EfixedReaderKGen k x)`. -/
theorem genBlkFlatEfpKGen_Rmat_pivot (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) :
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Rmat (k.val + 1)
      = rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderKGen M ha k x) := by
  change Function.update (genBlkFlatLive M (tach M) ha 0 x).Rmat (k.val + 1)
    (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderKGen M ha k x)) (k.val + 1) = _
  rw [Function.update_self]

/-- The Efp decoder's `Rmat` at a NON-pivot boundary `s ≠ k+1` is the live/struct decoder's. -/
theorem genBlkFlatEfpKGen_Rmat_ne (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) (s : ℕ) (hs : s ≠ k.val + 1) :
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Rmat s = (genBlkFlatLive M (tach M) ha 0 x).Rmat s := by
  change Function.update (genBlkFlatLive M (tach M) ha 0 x).Rmat (k.val + 1)
    (rmatPad M (tach M) (k.val + 1) hp1 hp2 (EfixedReaderKGen M ha k x)) s = _
  rw [Function.update_of_ne hs]

/-- The Efp decoder's `Cgen` at the PIVOT boundary `k+1` (`k+1 < L`) is the Schur frame with
`E = EfixedReaderKGen k x`. -/
theorem Cgen_EfpGen_pivot (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    (hkL : k.val + 1 < L)
    (hp1 : Text M (tach M) (k.val + 1 + 1) ≤ Text M (tach M) (k.val + 1))
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (v : ℝ) (x : Fin (routeMAmbient M) → ℝ) :
    Cgen v M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x) (hleStruct M (tach M) ha) (k.val + 1)
      = schurFrameProd M (tach M) (k.val + 1) (ha.hdesc k.val (by omega)) (ha.hub k.val) v
          ...
          (readN M (tach M) ha x ⟨k.val, by omega⟩) (EfixedReaderKGen M ha k x) := by
  rw [Cgen, dif_pos hkL, genBlkFlatEfpKGen_Bmat_succ M ha k hp1 hp2 x k.val (by omega),
    genBlkFlatEfpKGen_Nblk_succ M ha k hp1 hp2 x k.val (by omega),
    genBlkFlatEfpKGen_Rmat_pivot M ha k hp1 hp2 x, schurFrameProd]
...
theorem Cgen_EfpGen_nonpivot (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    Cgen v M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x) (hleStruct M (tach M) ha) (s + 1)
  ...
  rw [Cgen, dif_pos hs1, genBlkFlatEfpKGen_Bmat_succ M ha k hp1 hp2 x s (by omega),
    genBlkFlatEfpKGen_Nblk_succ M ha k hp1 hp2 x s (by omega),
    genBlkFlatEfpKGen_Rmat_ne M ha k hp1 hp2 x (s + 1) hne,
...
theorem live_Nblk_matchEGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Nblk (j + 1)
  ...
  rw [genBlkFlatEfpKGen_Nblk_succ M ha k hp1 hp2 x j hjL,
...
theorem live_Wblk_matchEGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) (j : ℕ) (hj1 : j + 1 < L) :
    (genBlkFlatEfpKGen M ha k hp1 hp2 x).Wblk (j + 1)
      = (genBlkFlatLive M (tach M) ha (rfinDirectGen M ha
          (pivotBlowupOn (activeMGen M ha) p₀ x))
...
boundary `s+1 = k+1` the E-block is `EfixedReaderKGen` matched via `EfixedReaderKGen_pboE`; at OTHER
...
theorem Cgen_interior_matchEGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    Cgen (x (eBlockPivotGen M ha k hr hc)) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x)
  ...
  · -- the PIVOT boundary: LHS E = EfixedReaderKGen, matched via EfixedReaderKGen_pboE.
    ...
    · exact EfixedReaderKGen_pboE M ha k hk hr hc x
...
theorem Cgen_leaf_matchEGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    Cgen v M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x) (hleStruct M (tach M) ha) L
      = Cgen v' M (tach M)
...
/-- **The chart-parameter match** `chartParamsGen (x p₀) (genBlkFlatEfpKGen k x) = BparamsLeafGen (pbo x)`
...
theorem chartParamsGen_matchEGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
    ...
    (hp2 : Text M (tach M) (k.val + 1 + 1) ≤ Wext M (k.val + 1))
    (x : Fin (routeMAmbient M) → ℝ) :
    chartParamsGen (x (eBlockPivotGen M ha k hr hc)) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x)
  ...
  congr 1
  have hN : (genBlkFlatEfpKGen M ha k hp1 hp2 x).Nblk s.val
  ...
  have hW : (genBlkFlatEfpKGen M ha k hp1 hp2 x).Wblk s.val
  ...
  have hC : Cgen (x p₀) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x)
...
theorem hmap_EfpGen (M : Fin (L + 1) → ℕ) (ha : StructAdm M (tach M)) (k : Fin L)
  ...
  funext x
  show phiGen (x (eBlockPivotGen M ha k hr hc)) M (tach M) (genBlkFlatEfpKGen M ha k hp1 hp2 x)
