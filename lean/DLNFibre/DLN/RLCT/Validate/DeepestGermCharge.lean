...
`Score := frobSqMat (Rcore)` (`Rcore` the (1,1)-block Schur complement of `Mw := reindex(P0·(prod−B)·QL)`)
...
(`schur_gap_le_coreRelative`) bounds the gap by `2√(coreΦ·frobSqMat D) + frobSqMat D` (`D = Rcore − S0·S1`),
the matrix-algebra route to `(♦)` once the leading-order germ residual `frobSqMat D ≤ ⅛(Sreg+coreΦ)`
...
With the global Schur factorization `R = S0·(1−K)·S1` (so `coreΦ = frobSqMat (S0·S1)` is the per-layer
core energy and `D = R − S0·S1` the remainder), the gap between the Schur energy `frobSqMat R` and the
core energy is bounded by `2·√(frobSqMat(S0·S1) · frobSqMat D) + frobSqMat D`. The S5c difference-of-squared
split (`schur_core_germ_comparability` (iii)) gives `frobSqMat R = frobSqMat(S0S1) + 2·cross + frobSqMat D`, and
Cauchy–Schwarz ((iv)) gives `cross² ≤ frobSqMat(S0S1)·frobSqMat D`, so `|cross| ≤ √(frobSqMat(S0S1)·frobSqMat D)`.
This REPLACES the refuted additive `≤ C·Sreg` charge: the gap is charged to the CORE energy `frobSqMat(D)`
...
theorem schur_gap_le_coreRelative {m0 m1 m2 : Type*}
    ...
    (hR : R = S0 * (1 - K) * S1) :
    |frobSqMat R - frobSqMat (S0 * S1)|
      ≤ 2 * Real.sqrt (frobSqMat (S0 * S1) * frobSqMat (R - S0 * S1)) + frobSqMat (R - S0 * S1) := by
  ...
  have hDnn : 0 ≤ frobSqMat D := frobSqMat_nonneg D
  have hBnn : 0 ≤ frobSqMat (S0 * S1) := frobSqMat_nonneg _
  -- `frobSqMat R − frobSqMat(S0S1) = 2·cross + frobSqMat D` (the split (iii)).
  have hdiff : frobSqMat R - frobSqMat (S0 * S1) = 2 * cross + frobSqMat D := by rw [hsplit]; ring
  rw [hdiff]
  -- Triangle then Cauchy–Schwarz: `|2·cross + frobSqMat D| ≤ 2|cross| + frobSqMat D ≤ 2√(B·D) + frobSqMat D`.
  have htri : |2 * cross + frobSqMat D| ≤ 2 * |cross| + frobSqMat D := by
    calc |2 * cross + frobSqMat D| ≤ |2 * cross| + |frobSqMat D| := abs_add_le _ _
      _ = 2 * |cross| + frobSqMat D := by rw [abs_mul, abs_of_nonneg hDnn]; norm_num
  refine le_trans htri ?_
  have hcross_le : |cross| ≤ Real.sqrt (frobSqMat (S0 * S1) * frobSqMat D) := by
    calc |cross| = Real.sqrt (cross ^ 2) := (Real.sqrt_sq_eq_abs cross).symm
      _ ≤ Real.sqrt (frobSqMat (S0 * S1) * frobSqMat D) := Real.sqrt_le_sqrt hcross_sq
...
`coreΦ w = frobSqMat (S0 w · S1 w)` (the exact core match — a special case of `hCore_germ` with
`Ccore = 0`). Then `R w − S0 w · S1 w = −w³` is nonzero for `w ≠ 0`, and the quadratic remainder charge
`frobSqMat (R − S0·S1) = w⁶ ≤ Crem·|w|²` holds near `0` for `Crem = 1` (since `w⁶ ≤ w²` for `|w| ≤ 1`),
