...
decorated peel + well-founded recursion of `RouteMSJDecorated` (contract in the thread notes
`genm-covfinish/notes.md`); `sjJointResolution` (`RouteMSJResolution`) stays its named sorry.
