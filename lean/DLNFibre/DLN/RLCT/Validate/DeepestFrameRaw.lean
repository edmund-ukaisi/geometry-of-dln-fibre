...
The genuinely-new geometric glue joining the BANKED index round-trip (`readX/Y/Z_deepestSplit`,
...
`(paramsEquivFlat.symm (w − wstar))`. The four arms (`readX/Y/Z/T_deepestSplit_raw`) decode each
...
`reindex(fromBlocks readX readY readZ Tcore) = (paramsEquivFlat.symm (w − wstar)) s`. All
...
/-- `readX (deepestSplit w) s i j` decoded to the raw deviation entry of `(paramsEquivFlat.symm
(w − wstar)) s` at the top-left `r × r` block (row `i.castLE`, column `j.castLE`). -/
theorem readX_deepestSplit_raw (H : Fin (L + 1) → ℕ) (r : ℕ)
  ...
  rw [readX_deepestSplit, deepestRoleIndexEquiv_symm_recombine]
...
/-- `readY (deepestSplit w) s i j` decoded to the raw deviation entry at the top-right `r × (H_{s+1}−r)`
block (row `i.castLE`, column `⟨r + j, _⟩`). -/
theorem readY_deepestSplit_raw (H : Fin (L + 1) → ℕ) (r : ℕ)
    (hr : ∀ s : Fin (L + 1), r ≤ H s) (hL : 1 ≤ L) (wstar w : Fin (flatDim H) → ℝ)
  ...
  rw [readY_deepestSplit, deepestRoleIndexEquiv_symm_recombine]
  conv_lhs => rw [← (paramsEquivFlat H).apply_symm_apply (w - wstar)]
  rw [paramsEquivFlat_apply_equivFin]
...
/-- `readZ (deepestSplit w) s i j` decoded to the raw deviation entry at the bottom-left
`(H_s−r) × r` block (row `⟨r + i, _⟩`, column `j.castLE`). -/
theorem readZ_deepestSplit_raw (H : Fin (L + 1) → ℕ) (r : ℕ)
    (hr : ∀ s : Fin (L + 1), r ≤ H s) (hL : 1 ≤ L) (wstar w : Fin (flatDim H) → ℝ)
  ...
  rw [readZ_deepestSplit, deepestRoleIndexEquiv_symm_recombine]
  conv_lhs => rw [← (paramsEquivFlat H).apply_symm_apply (w - wstar)]
  rw [paramsEquivFlat_apply_equivFin]
...
(row `⟨r + a, _⟩`, column `⟨r + b, _⟩`). The T-core analogue of `readX/Y/Z_deepestSplit_raw`. -/
...
The four `fromBlocks` arms decode via `readX/Y/Z/T_deepestSplit_raw` at the recovered split index. -/
theorem reindex_fromBlocks_reads_eq_deviation (H : Fin (L + 1) → ℕ) (r : ℕ)
    ...
    Matrix.reindex (rThresholdSplit r (H s.castSucc) (hr s.castSucc)).symm
        ...
        (Matrix.fromBlocks
          (readX H r hr hL ((deepestSplit H r hr hL wstar w).1, (deepestSplit H r hr hL wstar w).2.2) s)
          (readY H r hr hL ((deepestSplit H r hr hL wstar w).1, (deepestSplit H r hr hL wstar w).2.2) s)
          (readZ H r hr hL ((deepestSplit H r hr hL wstar w).1, (deepestSplit H r hr hL wstar w).2.2) s)
  ...
  · rw [Matrix.fromBlocks_apply₁₁, readX_deepestSplit_raw]
