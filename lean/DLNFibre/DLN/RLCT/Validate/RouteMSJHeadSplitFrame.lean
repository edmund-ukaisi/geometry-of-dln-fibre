...
It is quarantined as one `sorry` here; the separate F2 tide proves it (contract = this statement
verbatim).
-/
...
/-! ## The one isolated primitive (F2 contract) -/

/-- **F2 contract — the isolated primitive (TRUE, Mathlib v4.29-absent): a measurable Hermitian
eigendecomposition of a measurable family.** (i) the canonical SORTED eigenvalue tuple `eigenvalues₀`
is measurable in the parameter; (ii) there is a measurable ORTHOGONAL `U` diagonalizing the family
with those sorted eigenvalues (reindexed `Fin M₂ ≃ Fin (card (Fin M₂))`). Witnessed pointwise by
`IsHermitian.eigenvectorUnitary`/`eigenvalues₀`; only the MEASURABILITY is the absent labour. -/
