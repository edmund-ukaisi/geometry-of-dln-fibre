...
Each is proven by the R1.2 threshold bracket (`Skeleton`): the **binding axis** `j = 1`
...
`≥ 3/2` (`monomialThreshold_ge_of_mult`). So `= 3/2` by antisymmetry — pure S2 arithmetic, no raw
`⨅` evaluation, independent of the `#66` Params↔Fin8 seam. These feed `fm-2`'s `⨅`-over-24-leaves
assembly (`⨅ = 3/2`, `θ = 1`). -/
...
theorem case222_unit_leaf_threshold :
  ...
  · -- multiplicity `m = 3`: `3·kⱼ ≤ hⱼ + 1` on every axis ⟹ threshold `≥ 3/2`.
    have := monomialThreshold_ge_of_mult 2 (![1, 1] : Fin 2 → ℕ) (![3, 2] : Fin 2 → ℕ)
      3 (by intro j; fin_cases j <;> norm_num) (by intro j; fin_cases j <;> norm_num)
    rwa [show ((3 : ℕ) : ℝ≥0∞) / 2 = 3 / 2 by norm_num] at this
