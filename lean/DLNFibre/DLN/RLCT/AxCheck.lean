...
/-! ## The banked-clean gate — ONE batched axiom + blueprint walk over all roots
...
interesting footprints are re-emitted as `#print axioms` after the list:
* the 5 live-frontier DIRTY roots (carry `sorryAx` from unproven skeleton rungs) — these are
  informational `#print axioms`, NOT asserted (a `#assert` on them would redden the build);
* the 4 payoff DELIVERABLES — asserted in the batch AND `#print`ed here, so their clean-three
  `[propext, Classical.choice, Quot.sound]` footprint stays individually in the build log
  (the union summary cannot distinguish clean-three from clean-with-cite).
-/

#assert_banked_clean_batch [
...
-- no analysis axiom), no `sorryAx`.
  prodAux_frontScalarShear_cancel,
...
-- cone is sorry-free; the three analytic per-family facts (hcancel/hUpos/hSpre) are discharged.
  routeMCore_smearedL2_square_uncond,
...
-- permitted `monomial_rlct` (hfin leaf side, NO new axiom — same S2 the headline rides). Item-97 regime (ii).
  rectSchurRecStep_mnp,
  routeMBoxThresholdFinite_mnp,
  routeMLayerCover_coverLe_mnp,
...
-- hard atoms; `deepest_gauge_construction_L2_ofBundle` is the Route-X parameterized gauge body.
  deepest_regular_core_normal_form_L2_front,
  exists_frontPivotFrame_lastBlock_isUnit,
...
-- producer for the DLN loss at a general v) consumes these; reviewer-PASS, non-vacuity tight.
  rlct_quasiSplit_ge,
  rlct_smooth_block_ge,
  coupled_controls_slice,
...
-- witness `routeM231sm_box_diverges_via_smearedChart` confirms the structure is non-vacuous.
  ...
  hSmeared_of_smearedChart,
  routeM231sm_box_diverges_via_smearedChart,
...
-- is the (degenerate) non-vacuity witness (the real nReg>0 instance is the unbuilt analytic existence).
  deepest_le_of_optimal_of_chart_certificate,
  GeneralVChartL2.ofExactGerm,
...
-- R1-LOWER generic achiever then needs only `hInterior` (the interior-det atom) + the spine wiring.
  hSmeared_squareSmeared_L2,
  smeared_deepRank_eq_M0,
  smearedChart_of_square,
...
-- achiever` stays OPEN (#120-gated). `eDeepRank0Unit_ae_pos` is S2-FREE clean-three (nonzero-poly witness).
  routeMCore_box_diverges_achiever_L2,
  routeMCore_box_diverges_eDeepRank0,
  routeMCore_box_diverges_interiorLive,
  eDeepRank0Unit_ae_pos,
...
-- box-divergence atom's threshold, via `monomial_rlct`).
  routeMCore_box_diverges_interiorLiveGen,
  interiorLiveNodeChartGen,
...
-- (L ≥ 3) is the remaining general-`L` interior gap (banked at L = 2 only via `eDeepRank0`).
  interiorLiveGen_hInterior_of_deepRank_pos,
  routeMCore_box_diverges_interiorLiveGen_of_deepRank_pos,
...
-- [propext, Classical.choice, Quot.sound] (historical: pre-retirement this line also expected `monomial_rlct`), no `sorryAx`.
  routeMCore_box_diverges_eDeepRank0Gen,
  interiorLiveGen_hInterior_of_deepRank_zero,
...
-- and the spine adds none of its own.
  routeMCore_box_diverges_achiever_full,
...
-- [propext, Classical.choice, Quot.sound]: NO `monomial_rlct` (no analysis axiom), no `sorryAx`.
  noInteriorBothDrop_of_not_interiorDrop,
...
-- residual). They bank the case-B (middle-stratum) reduction-chain; the chart construction is the open piece.
  ...
  deepest_le_of_optimal_middle_stratum,
...
-- reviewer + lean-formaliser §QA). The §SEL win: chart-DATA (hF/hQ0/hcmp) removed from the hyp list.
  rlctAtOn_quasiSplit_ge_of_contDiff_residual,
  deepest_le_of_optimal_of_iftResidual,
...
-- two named-open gates #44/hDeepest + hInterface/R1 — NOT wired into Skeleton:1172 (stays a conditional component).
  exists_secondPeel_minor,
  rlctAt_deepest_le_of_optimal_L2,
...
-- use-site instantiates with the §SEL selected-minor Φ + the H_indep block-invertible f'.
  exists_boundedUnit_chart_of_contDiffAt,
  rlctAtOn_eq_of_contDiff_chart,
...
-- verify-first gate verdict (reviewer + Codex): the second peel is BOUNDED, off the L≥3 wall.
  secondPeel_hchart_residual,
  deepest_le_of_optimal_secondPeel_discharged,
...
-- J-blocks J00/J01/J11) + the coupling identity `eihd_hD`. The cov-field input to cover_ge_div / NodeAchieverChart.
  eihd_hD,
  interiorDet_leaf_headline_eihd,
...
-- here so a stale-olean `sorryAx` regression on the D1-runway foundation is caught on every build.
  schur_product_ldu_rec,
  blockSchur_partProd_succ,
...
-- stale-olean regression on the diffeo-derivative runway is caught on every build.
  hasStrictFDerivAt_deepestPsiCoreShear,
...
-- regression on the diffeo-smoothness runway is caught on every build.
  contDiff_deepestPsiCoreShear,
  deepestPsiCoreShear_isLocalDiffeoAt,
...
-- BULK — pieces 4 + 5a/5b — NOT closed here; the `hstep2` sorry is untouched.)
  hasStrictFDerivAt_coreShearHomeo_zero,
  contDiff_deepestGConjFlat,
  hasStrictFDerivAt_deepestGConjFlat,
  deepestGConjFlat_fixpoint,
  deepest_diffeo_bridge_gen_impl,
...
-- regression on the SJ carrier is caught on every build across the multi-tide R1-UPPER close.
  ...
  sjLoss_prependColumn_one,
  SJLinGenState.loss_ofMatrix_product,
  SJLinGenState.loss_radialStep,
  SJLinGenState.gen_rowMix_const,
...
-- regression on the reusable joint-move invariants is caught across the multi-tide D1 close.
  blockSchur_movedC,
  prodSchurCore_eq_blockSchur_partProd,
  topRow_movedC,
...
-- Quot.sound] (no `sorryAx`, no `monomial_rlct`). Axiom-gated across the multi-tide R1-UPPER close.
  chartInner_blockReindex_eq,
  chartInner_blockReindex_eq_of_emb,
...
-- Classical.choice, Quot.sound] (no `sorryAx`, no `monomial_rlct`).
  frobSq_schur_split_inv,
  chartInner_schurWeld_eq_of_emb,
...
-- axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`, no `monomial_rlct`).
  gammaPeelIntegral_schurShearFree_eq,
  freedSchurLoss_inner_peel_lt_top,
  freedSchurLoss_inner_bounded_lt_top,
...
-- Classical.choice, Quot.sound] (no `sorryAx`, no `monomial_rlct`, no `cited_aoyagi_dln`).
  ...
  half_minAdm_sub_half_peelCharge_le,
  exists_binding_cut,
...
-- Quot.sound] (no `sorryAx`).
  fromBlocks_lowerFrame_mul_forcedDecode,
  fromBlocks_rightUpper_mul_forcedDecode,
  blockSchur_lowerFrame_left,
  blockSchur_rightUpper_right,
...
-- axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  psiSplitRawGen_deepestChain_interior,
...
-- `cited_aoyagi_dln`).
  gammaPeelIntegral_le_boxIntegral,
  sjJointResolution_of_boxThresholdFinite,
  SJDecoration.rowMix_decLoss,
...
-- `monomial_rlct`, no `cited_aoyagi_dln`) — CONFIRMS it is genuinely S2-free.
  iInf_axisRatio_le_monomialThreshold,
...
-- Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  psiSplitRawGen_deepestChain_hmove,
...
-- remains. Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  prod_deepestM_eq_schur_ldu_readback_gen,
...
-- Classical.choice, Quot.sound] (no `sorryAx`, no `monomial_rlct`, no `cited_aoyagi_dln`).
  SJDecoration.radialAttach_integral,
  radialAttachFactor_lt_top,
  SJDecoration.measurable_integrand,
...
-- Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  forcedDecodeLeft_lowerFrame_mul,
  movedC_eq_self_of_toBlocks₂₁_zero,
  framedLayer_zero,
...
-- axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  hcd_psiSplitRawGen,
  contDiffAt_psiSplitDeltaGen_at,
  eventually_psiInvBundle,
...
-- Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  deepestCoreF_coreAbsorbConj_psiSplitRawGen_eq_score_at_chart,
...
-- Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  eventually_isUnit_deepestChain_decode_toBlocks₁₁,
  eventually_isUnit_partProd_deepestChain_decode_toBlocks₁₁,
  eventually_isUnit_prod_decode_pivot_toBlocks₁₁,
-- + the bundled `∀ᶠ x, ∀ k, IsUnit …` forms (drop-in for the assembly's finite-intersection step).
  eventually_all_isUnit_deepestChain_decode_toBlocks₁₁,
  eventually_all_isUnit_partProd_deepestChain_decode_toBlocks₁₁,
...
-- keystone✓ + piece(2)✓ + hC✓. Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound].
  coreAbsorbConj_reindex_eq_blockSchur_movedC_decode,
...
-- forced `#print axioms` = [propext, Classical.choice, Quot.sound] (no `sorryAx`).
  reindex_prodAux_eq_partProd,
  reindex_prod_eq_partProd,
  reindexECol_regBlocks_eq_of_chain_movedC,
...
-- `monomial_rlct`).
  ...
  aoyagi_learning_coefficient_frontPivot,
...
-- Classical.choice, Quot.sound].
  d1ge_deepestPoint_of_explicit_chart_genL,
  exists_common_pivot_gen,
  reindex_prod_eq_genPartProd,
...
-- rebuilds the product). Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound].
  recoverProductGen_schurChartRawGen,
...
-- ContDiff + germ remain. Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound].
  schurChartRawInvGen_schurChartRawGen,
...
-- Cite-NOTHING: forced `#print axioms` = [propext, Classical.choice, Quot.sound].
  derivEquiv_of_left_inverse,
  contDiffAt_schurChartRawGen_entry,
...
-- non-vacuous hypothesis, NOT monomial_rlct/cited_aoyagi_dln). Unconditional once `#72` lands.
  deepest_gauge_construction_ofBundle,
  deepest_regular_core_reduces_frontPivot_front,
  aoyagi_learning_coefficient_gen,
...
-- MUST be [propext, Classical.choice, Quot.sound] — no sorryAx, no monomial_rlct.
  routeM_rlctAtOn_le_iInf,
  r1_resolution_general_le,
  aoyagi_deepest_reduction_gen,
  aoyagi_learning_coefficient_gen_le,
...
-- Corner endpoint (RouteMSJRadialPolar / RouteMSJSlice334): opaque-`n` cube integral + (3,3,4).
  corner_block_cube_lintegral_lt_top,
  sjSlice334_corner_lintegral_lt_top_of_lt_half_minAdm,
...
-- §8 gate as compactness positive-min + the squared-injective-linear endpoint (RouteMSJCornerGate).
  corner_block_cube_lintegral_lt_top_of_pos,
...
-- ½·(p*q+t*h), via the linear+MP flatten `twoMatFlatL` → `corner_block_cube_lintegral_lt_top_of_pos`).
  twoMatBox_injectiveLinear_lintegral_lt_top,
  sjGoodMap_loss_matBox_lt_top,
...
-- finiteness on the refined cover) is the mountain feeding `sjJointResolution`.
  frobSq_schur_eq_sjGoodMap,
  chartInner_eq_outerShearFree,
...
-- `freedSchurLoss_eq_sjGoodMap_mul` (product form, reads `W,v` off `Q̃=Ã₁·A₂`).
  freedSchurLoss_eq_sjGoodMap,
  freedSchurLoss_eq_sjGoodMap_mul,
...
-- `gammaPeelIntegral_sjGoodMap_eq` (the equality gammaPeelIntegral = ∫∫∫ sjGoodChartLoss^{−c'}).
  sjTail_factor,
  gammaPeelIntegral_sjGoodMap_eq,
...
-- `corner_block_lt_top_of_pos` (ball finiteness from pointwise positivity alone).
  exists_uniform_sphere_lb,
  corner_block_lt_top_of_pos,
...
-- measure-preserving translation U↦v=U+P⁻¹B₁₂W + assembleFront reconstruction).
  sjGoodChartLoss_pivotRows_translate_eq,
...
-- Pi-split form, deep factor A'0-independent). Plumbing, not analytic content.
  rowSplit_lintegral_eq,
  gammaPeelIntegral_piSplit_eq,
...
-- IH can carry at the zero-slack binding cut). The `(□)` chain is RE-POINTED onto the decorated route.
  decoratedBoxThresholdFinite_of_decoratedStep,
  routeMBoxThresholdFinite_of_decoratedDescent,
  routeMBoxThresholdFinite_of_decoratedPeel,
  gammaPeelIntegral_lt_top_of_decoratedPeel,
...
-- catIrev + p2rev PASS (both forced fresh-olean clean-three).
  ...
  integrableOn_norm_rpow_neg_ball,
...
-- c'<½·min(a+1,D+1)) + `frobSq_rmatMul_corank_one` (the fidelity link: frobSq(Γ·Q_b)=‖γ‖²‖z‖² at b=1).
  freeBilinear_box_lt_top,
  frobSq_rmatMul_corank_one,
...
-- box-finiteness to per-chart finiteness via the exact least-index-argmax partition, over arbitrary μ.
  ...
  setLIntegral_lt_top_of_detMinorCover,
...
-- the matrix Rayleigh lower bound + PSD det bound (the reusable spectral bedrock for (b)+(a)).
  minStretch_comp_ge,
  gram_rayleigh_lb,
  posSemidef_det_le_iInf_mul_trace_pow,
...
-- deeper-strata σ-min product-tube majorant ∫⁻ σ_min(A₀·A₁)^{−α} < ⊤ (0≤α<1). Reviewer SURVIVED + Codex.
  sjProductTube_params_lintegral_lt_top,
...
-- fsreview PASS + Codex xhigh. Feeds the crux on the top stratum.
  frobSq_mul_eq_sum_eigenvalues,
  sigMin_sq_eq_iInf_eigenvalues,
  frobSq_ge_twoBlock_of_sector,
  lintegral_comp_rmatMul_orth,
...
-- corner334-review PASS + controller-review. Complement = joint/Tonelli obligation (deferred).
  sjCorner334_sector_slice_lt_top,
  cornerSlice334_eq_top_of_unit1_zero,
...
-- Clean-coords model (literal-A₂ casting = a det-M≠0 CoV, follow-on).
  onePeel334_cleanCoords_lt_top,
  onePeelIntegral_lt_top,
...
-- the reshape core. Review-PASS (du/dv fidelity Codex-confirmed). sjSector hyp = step-6 (#130 (r−1)-minor).
  frontFirst_g_le_of_sector,
  frontBox_twoBlock_le,
  measurePreserving_colSplitReshape,
...
-- (q-ary AM-GM min→sum = ADD-not-MIN). Statement soundness PASS (#133, decorrelated). Crux via banked fin_nat_prod.
  qPeelIntegral_lt_top,
  qCornerSliceAtUnits_le,
...
-- descent integrates. The OUTER (A',x) integral (the (S,J) double induction) stays the isolated skeleton sorry.
  volume_genBox_lt_top,
  measure_shearbox_lt_top,
  freedSchurLoss_inner_bounded_le,
  freedSchurLoss_inner_bounded_shear_lt_top,
...
-- corank-width corollary (`_corankWidth`) — the minAdm-arithmetic half of `adm ⟹ ∀η∈Crit p_η=0`. Pure ℕ.
  ...
  minAdm_redChain_succ_ge_corankWidth,
...
-- witness), making C1's (P)⇏(T) a theorem (guards the monomial terminal against divisorwise-p=0-alone acceptance).
  pDivisorwise_not_imp_pSimultaneous,
...
-- NATIVE (banked front-peel #117 on redChain, NO Core.CTheta). The combinatorial ρ≥b; geometric bridge = #2.
  minAdm_backPeel_cominimizer_ge,
  minAdm_backPeel_cominimizer_ge_corankWidth,
  exists_minAdm_backPeel_cominimizer_corankWidth,
...
-- (`exists_pos_smul_one_le_of_posDef`, dodges the spectral-isDefEq hazard). Feeds LeafFinite's hZ.
  exists_gram_sub_smul_one_posSemidef_of_rank_eq,
  posDef_gram_of_rank_eq,
  exists_pos_smul_one_le_of_posDef,
...
-- REJECTED = banked suppFreshTwo_not_pSimultaneous). genuineCarrier=(i): D.Z ≃ᵐ Params M, tail derived.
  adm_trivial,
  admValuation_iff_pSimultaneous,
  regression_rank1_admitted,
  regression_freshTwo_rejected,
...
-- single level-0 atom bound obviates the flag (a≤M₂−b+1 at every b>1 binding cut = a theorem).
  corankOffSector_bpos_le,
  corankWeight_bpos_lt_top,
...
-- With Obl-1/Obl-2, all corank regimes of the off-sector are closed.
  corankOffSector_borderline_atBorder_le,
  corankOffSector_borderline_le,
  enn_geom_interp,
...
-- corrected corner-shrink is a deeper-cut re-peel + PSD-monotonicity, not these. Clean-three regardless.
  DLNFibre.DLN.RLCT.borderedGram_det,
  DLNFibre.DLN.RLCT.borderedGramSchur_nonneg,
  DLNFibre.DLN.RLCT.lintegral_le_sum_finCover,
  DLNFibre.DLN.RLCT.singularShell_iUnion,
  DLNFibre.DLN.RLCT.gramDet_eq_prod,
...
-- (orthogonal extension of U_s) + the (iii) helpers. OWED-2 reduction + OWED-3 build on these.
  DLNFibre.DLN.RLCT.uniformWenn_proj_le,
  DLNFibre.DLN.RLCT.exists_ortho_ext,
  DLNFibre.DLN.RLCT.detGram_eq_prod_rows,
  DLNFibre.DLN.RLCT.offSector_cover_le,
...
-- dims (column-Fubini, NO bordered-Gram) ∘ OWED-1 on the shell + the w-uniform per-shell interior bound.
  DLNFibre.DLN.RLCT.strongBlock_lintegral_lt_top,
  DLNFibre.DLN.RLCT.deeperCut_shell_corankWeight_lt_top,
  DLNFibre.DLN.RLCT.shell_corankOffSector_le,
...
-- assembly). Tracked here so the day F+D land and the module goes clean-three is visible (std-decision-7).
  DLNFibre.DLN.RLCT.deeperFlag_shell_core_le,
  DLNFibre.DLN.RLCT.deeperFlagCore_decLoss_pos_ae,
  DLNFibre.DLN.RLCT.strongBlock_lintegral_le_unif,
  DLNFibre.DLN.RLCT.shellCorankWeight_le_unif,
  DLNFibre.DLN.RLCT.strongBlock_unif_const_lt_top,
  DLNFibre.DLN.RLCT.deeperFlagUnifConst_lt_top,
  DLNFibre.DLN.RLCT.shell_corankOffSector_le_unif,
...
-- ∫_cube (∑(Lx)²)^{−c'} < ⊤`; the s1-Chle §3 correction of the injective `twoMatBox` to rank-r). Clean-three.
  DLNFibre.DLN.RLCT.lintegral_cube_frobSq_neg_of_finrank_range,
  DLNFibre.DLN.RLCT.twoMatBox_rankR_lintegral_lt_top,
...
-- (Hermitian H + Rayleigh-≥c subspace U ⟹ finrank U + #{eigenvalues<c} ≤ k) + `rayleigh_expansion`. Clean-three.
  DLNFibre.DLN.RLCT.finrank_add_weakCount_le,
  DLNFibre.DLN.RLCT.rayleigh_expansion,
...
-- pivot block — exact equality, ordinary unit sphere, honest Jacobian `r^{u·M₁−1}`, CoV only. Clean-three.
  DLNFibre.DLN.RLCT.pivotBlock_radial_blowup,
  DLNFibre.DLN.RLCT.lintegral_matrix_radial_polar_factor,
...
-- ⟹ `weakEigCount ε' Z_deep ≤ M₂−m`, on the banked D-C core `finrank_add_weakCount_le`. Clean-three.
  DLNFibre.DLN.RLCT.shell_subset_goodSet,
...
-- Hermitian family are measurable in the parameter (Vieta-eval + Lusin–Souslin). Clean-three.
  DLNFibre.DLN.RLCT.measurableEigenvalues₀,
...
-- + F1 `exists_headSplitFrame_impl` (the measurable piecewise m-frame selector). Both clean-three.
  DLNFibre.DLN.RLCT.measurableEigendecomp,
  DLNFibre.DLN.RLCT.exists_headSplitFrame_impl,
...
-- the unproven skeleton rungs; tracked here so the day one goes clean-three is visible).
