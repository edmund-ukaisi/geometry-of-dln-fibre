...
* **Brick B (interface, named — LR Lemma 4.5 + Lemma 4.6).** The passage `Σ̄^r ⤳ mult⁻¹(B)` (for `B`
  ...
  the `RlctRealInterface` bounds + the transfer `hT`, lives in `DLN.BundleShiftDischarge`.

**R2-general** (`rlct_lossDLN_eq_half_cCodim_add_shift_via_aoyagi`) is pure transport: the analytic
equality `rlct = ½·codim_ℝ` (the two Cited bounds) plus the transfer `hT : codim_ℝ = codim_K` gives
`rlct = ½·codim mult⁻¹(B)` (over `K`); Brick B rewrites the fibre codim as `codim Σ̄^r + shift`;
Brick A rewrites `codim Σ̄^r = cCodim d r`. The analytic interface `I`, the Cited transfer `hT`, and
the shift interface `J` are all explicit in the type; `via_aoyagi` names the rlct source. This is
**not** an unconditional `rlct = (C + shift)/2`. The cited lower bound, the transfer, and the shift
are guarded by `0 < N` — the scope (a genuine deep network, N ≥ 1) where Lemma 4.6 and Aoyagi hold;
at `N = 0` the "product" `mult` is the empty product and the shift identity is false.
...
DLN square-Frobenius loss `K^DLN_B` equals `(cCodim d r + r(d_0+d_N−r))/2`: the combinatorial `C/2`
plus the half-shift. Proof: the analytic equality + `hT`
(`rlct_lossDLN_eq_half_codimFibre_of_transfer`) gives `rlct = ½·codim mult⁻¹(B)` (over `K`);
`J.cited_bundle_shift` rewrites the fibre codim as `codim Σ̄^r + shift`; Brick A
(`Core.codimRepCanonical_productRankLocusLE_eq_cCodim`) rewrites `codim Σ̄^r = cCodim d r`. `I`, `hT`,
`J` are all explicit in the type (every dependency visible); `via_aoyagi` names the rlct source.
This is **not** an unconditional `rlct = (C + shift)/2`. The `0 < N` guard is the scope of the cited
lower bound and the shift. `[IsAlgClosed K] [CharZero K]` (where `C` is the geometric codimension). -/
...
through the two Cited analytic bounds `I`, the transfer `hT` (`codim_ℝ = codim_ℂ` at `(2,2,2)`, the
given rank-`1` `B`), and the Lemma-4.5/4.6 bundle-shift interface `J` (now discharged from Core by
`DLN.BundleShiftDischarge.bundleShift_of_core`; the discharged witness is
`DLN.BundleShiftDischarge.rlct_lossDLN_d222_one_eq_two`). The combinatorial `C = cCodim d222 1 = 1`
and the shift `1·(2+2−1) = 3` give `rlct = (1+3)/2 = 2`: the `(2,2,2)` rank-`1` DLN is mildly
singular. `(2,2,2)` has `N = 2 > 0`, so the `0 < N` scope guard is met. `I`, `hT`, `J` are the
explicit hypotheses; `hT` is the Cited real-vs-complex transfer. -/
theorem rlct_lossDLN_d222_one_eq_two_via_aoyagi
  ...
  rw [rlct_lossDLN_eq_half_cCodim_add_shift_via_aoyagi (K := ℂ) (ι := Complex.ofRealHom) I J
    (by norm_num) hB hr Core.kostantPartitions_d222_one_nonempty hT, Core.cCodim_d222_one]
