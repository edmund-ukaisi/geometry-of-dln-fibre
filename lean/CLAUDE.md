...
- **Matrix-inverse MEASURABILITY: two `MeasurableSpace (Matrix …)` instance traps.** (1) There is no direct `MeasurableSpace (Matrix m n ℝ)` instance to resolve entrywise measurability against; (2) ascribing the Pi type (`m → n → ℝ`) to summon one SILENTLY flips `⁻¹` from `Matrix.inv` (the matrix inverse) to `Pi.instInv` (pointwise reciprocal) — a semantic change, not a cast. So `fun_prop`/`measurability` deep-recurse or SIGABRT on any `(Matrix.of P)⁻¹`-containing integrand (the pivot Schur-complement `freedSchurLoss`). **Working pattern: prove entrywise measurability via the banked `DLNFibre.Core.measurable_matrix_of_inv_apply`** (`Core/MeasureTheory/MatrixInverseMeasurable.lean`, network-free): `Matrix.inv_def` + `Ring.inverse_eq_inv'` + `Continuous.matrix_adjugate`/`matrix_det` — the inverse is Borel-measurable everywhere incl. `{det=0}` (via `Ring.inverse`), plus entrywise `measurable_matrix_mul_entry`/`_add_entry` composition helpers. Found closing the d1-a<u α-LOW clean-slice (`outerDom_lintegral_prod`'s `AEMeasurable` side-condition, 2026-07-18); recurs in any lintegral over an inverse/Schur-complement integrand. Adjacent to the `⁻¹` DecidableEq/Inv instance-diamond note above.
