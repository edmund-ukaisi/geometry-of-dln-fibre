...
## Recon (opening wave) — CLOSED 2026-06-27

| # | item | status | outcome |
|---|------|--------|---------|
| R0 | **Interface design** — the cited analytic theorems + the precise geometric hypothesis | **closed (survived)** | Seam = CITED C1+C2+C3 (≈ banked `monomial_rlct`) / PROVED R2+T+R3. Lower bound is a **log-canonical / ideal** condition (`lct(I)=c`), NOT "strata smooth" (false). Transfer T (real↔complex) is PROVABLE via the rational `realizerD` + is the hidden wall. |
