...
| R2b | build (analysis) | pp-zeta-cert → formaliser | 🔄 pen-and-paper | **DECISION: A — zeta-pole `(λ,m)`** (operator, 2026-07-02: bite the citation bullet; future-proofing; "back it up with pen-and-paper cross check"). `pp-zeta-cert` certifying the definition + the exact cited-continuation statement + the pole=threshold=½codim soundness chain; a formaliser then builds `Core/Analysis/RLCT/{Zeta,Cited}` against the certificate. |
