...
| R2a | build (analysis) | r2a-threshold | ✅ LANDED `c06a6f54` (rlct-r2) — **rev-r2a CLEARED, merge queued** | cite-free `RLCT.integrabilityThreshold` + `|·|→1` witness; axiom-clean (rev-r2a independent `#print axioms` on all 8 decls ✓). Fidelity note to land at merge: threshold = classical RLCT iff `{K=0}` null (**HOLDS for DLN** — fibre `mult⁻¹(B)` is a null subvariety) → add to Card 1. Merge + gate-coverage (first-party namespace allowlist) queued for calmer box / operator bullet-ack. |
