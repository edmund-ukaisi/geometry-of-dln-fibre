...
| R2b | build (analysis) | r2b-zeta | ✅ LANDED `e96fff81` (rlct-r2b) — **fidelity checkpoint** | Zeta-pole object built: `Core/Analysis/RLCT/{Zeta,Cited,Pair}` — ζ + convergence (cite-free) + the ONE bundled continuation cite + `RLCTPair(λ=−s₀>0, poleOrder)` + Link 1 (`λ = R2a integrabilityThreshold`). Gates green (`scripts/cited` = `UNACCOUNTED=0 CITED=4 LOCATION=0`; `#print axioms` = std-3 + 1 cite; sorries 0). Codex (xhigh) fixed 3 gaps (locality/maximality/pole-regime). **Controller-review flags 1 fidelity subtlety** (`s₀ = −integrabilityThreshold K U`: the φ-weighted ζ sees only `supp φ`, the threshold sees all `U` — over-general unless `U` = `supp φ` region / x₀ worst in U). `rev-r2b` auditing it + verbatim cite surfaced to operator. **NOT merged** — pending review + operator sight; aggregator wiring (`import …RLCT.Pair`) at merge. |
