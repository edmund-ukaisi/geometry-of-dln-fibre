...
- **The source-fidelity discipline (fidelity arc, 2026-07-23 — the deepest drift). Two composed gates:
  ...
  already has.
- **General-d cite-free — FULL-HEADLINE SATISFIED (de-risked to labor, NOT built; from the Standing-counsel fidelity tripwire; elder re-promote 2026-07-26).** (A) is MONUMENT-FREE at FULL GENERALITY — both monuments bypassed by the SAME "one scalar pivot per node" principle: the VALUE via the pivot-cross single-entry survivor (`Pmat[0][0]=E`, corank-insensitive; F13), the COVER via the single-scalar-pivot triangular det-1 block-shear (no rational k×k inverse; `LeafCoverTiling.covers_subset` general-D sorry-free → `blockShear_covers_scaled` + `flatCube_subset_leafPathImages`; two decorrelated derivations). The general-d cite-free FULL headline (`rlct=½·cCodim`) is REACHABLE / DE-RISKED to detail-at-scale — NOT proven, NOT Lean-inhabited. INFERRED-NOT-PROVEN LEDGER (to be BUILT + verified; monument-free ≠ nearly done, ~10-16 tides): (L1) the VALUE born-α fed-form invariant (pivot=E; the ideal-preservation leaves); (L2) the general-d cover completion — the W1 cover-value unification rides ONE object: the per-leaf native born-α carried DIRECTLY in the fan (FanStep.shear = σ_p per-pivot, INNER/source; blockBlowupMap outer — the block-blowup-outer/shear-inner engine, cert-psi-mix-clean, born-native never Transport334), so g_c = blockBlowupMap∘σ_p carries BOTH hcover (engine covers_fanOfSteps ∀R LANDED, ρ(R)=R exactly, + a per-pivot box-containment reusing blockShear_covers_scaled general-D) AND hsandwich. REMAINING = the per-pivot box-containment brick (bounded, engine-native). Supersedes reparam_image (Path-A implementation refined via the engine's per-pivot support; source-inner principle unchanged); Path B (target) DEAD for W1 (over-vanishes on the value). The value sandwich is GLOBAL (survivor = entry_0 = E exactly, ρ_leaf=∞) ⟹ no value shrink; only the cover's ρ constrains; (L3) the RESOLUTION's next-center-coordinate-block realization condition (shares LEAVES with L1, related-but-DISTINCT — the exact L1=L3 identity is itself a to-ADJUDICATE item for a seat; keep both). L1 AND L3 are BOTH load-bearing (a snag in EITHER hits the headline) = the build's PRIMARY audit target. Build guardrails (neither new): no-clearing-compression (one clearing/node, faithful to `buildTree`); g_c = born fan leaves (F12-B, never Transport334). (3,3,4) stays FULLY cite-free (value de-risked + cover closed clean route-b).
