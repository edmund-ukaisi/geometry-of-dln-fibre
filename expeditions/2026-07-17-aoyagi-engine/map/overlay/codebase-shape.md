...
Tool total at this pass = **35** (post statement-layer-close `0ce9c68b9`) = 10 on-cone + 25 fossil.
...
- **LIVE EDGE** (the expedition's working front): **THE STATEMENT LAYER IS CLOSED** (ruling-B baked
  ...
  unchanged throughout). The live cone is now **pure PROOF + assembly** (10 on-cone sorries): the WALL
  `realBranch_boostReady_case11` (`Case1Wire.lean:386`, seat-L4), the descent `realBranch_multiAffine_step`
  (`MonumentAtlas.lean:1116`, seat-L3T2 — reshaped from the SUSPECT node-form to a carried-invariant
  STEP-form), `lastLayer_clear_preserves`, the **L5 assembly** (`L5FoldSpec`, the longest link), **L7
  commissioning** (startable), and the **Assembly increments** (`MonumentAssembly`, arch-C, GO on
  increment 2). 5/6 `realBranch_*` derived stubs already PROVED clean-three; L6+L8 PROVED (L8's external
  geometry now in `LeafGeometryWire`, on-tree). The last library sorry replaces at the final wiring swap
  (`LearningCoefficient.lean`).
...
- **F1 — CENSUS is the canonical tool.** The meter is `cd lean && scripts/sorries` = **35**
  proof-position sorries post statement-layer-close `0ce9c68b9` (a raw line-grep OVER-counts docstring +
  `⛔ DO NOT FILL` banner mentions AND UNDER-counts inline `by sorry` — trust the tool total, not the
  awk). Tool-verified split: **DLN/Aoyagi 10 ON-CONE** (MonumentAtlas 8 + LearningCoefficient 1 +
  Case1Wire 1 — the WALL boostReady) + **25 fossil** (DLN/RLCT/Validate 18, DLN/RLCT/Engine 4
  [GeoAlphaGauge/ClearableReify/CanonicalWitness224/GeoAtlasTransfer, all RETIRED], DLN/RLCT/Skeleton 3).
  The 41→35 drop = 5/6 `realBranch_*` stubs PROVED (MonumentAtlas 14→8) + `Case2Delta0` DELETED (−1,
  node-form retired in the step-form bake) + `Case1Wire` NEW (+1). Fossils unchanged. The on-cone-vs-
  fossil recount at the M-HYGIENE prune is the navigator's gate-verify.
