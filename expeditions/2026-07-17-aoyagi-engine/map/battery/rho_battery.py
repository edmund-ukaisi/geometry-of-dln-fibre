#!/usr/bin/env python3
# guards: e-p61-band-arithmetic, e-p62-count-identification
# config: L in {2,3,4}, 993 distinct cores; rho = max-crossing over t~=0 global-minAdm divisors = a(l-a)+1; kill-set [2,2,2,2,2] incidence + [2,2,5] t~=0 + 5 ground truths
# provenance: threads/41-rho-count-object/certificate.md (pnp P6.2 adjudication; controller re-ran EXIT 0)
"""
...
# graduated into map/battery/ — the edgespec helper rides alongside as `_edgespec_traversal_334.py`
# (leading `_` so the battery runner skips it as a helper, not a standalone script). Fallbacks kept.
EDGE = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                    "_edgespec_traversal_334.py")
