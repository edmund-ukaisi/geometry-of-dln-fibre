#!/usr/bin/env python3
# guards: e-p62-count-identification
# config: 1018 cores; confirms ρ = max CHAIN of binding minimisers under componentwise <= (chainHeight) = a(l-a)+1; CORRECTS the antichain speculation (Dilworth dual)
# provenance: threads/41-rho-count-object/chain_confirmation.md + certificate.md (pnp, elder-mandated)
"""
...
# graduated into map/battery/ — prefer the sibling `_edgespec_traversal_334.py` helper (shared with
# rho_battery.py; leading `_` so the runner skips it), then the threads/37 absolute fallback.
EDGE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_edgespec_traversal_334.py")
