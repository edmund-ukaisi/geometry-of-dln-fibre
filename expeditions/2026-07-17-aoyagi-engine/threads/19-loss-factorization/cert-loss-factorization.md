...
## VERDICT: **HOLDS-WITH-CONDITIONS** — equivalently, **FALSE for the present pure-β `chartMap`, TRUE for the incidence/Q,P-normalized composite.**
...
- **Next construction step this points to:** fork-15's `geoChartMap` diagonal-normalization swap (task
  #35), with the source-gauge composition made coherent with `srcBox`/`resCoord` — after which
  LeafPullback is provable as stated. No paper adjudication is owed before that build.
