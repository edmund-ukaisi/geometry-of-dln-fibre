...
> (B) the FULL continuous space of monomial weights in the incidence+shear resolution charts of the
> minimizing rank-1 (and the corank-2 for (3,3,4)) branch, with Jacobian-corrected log discrepancy; (B2)
> alternative pivots; (B3, subsumed) weighted centers; and (C) the tracked-leaf Newton-LP values — found NO
> divisorial valuation with ratio `(h+1)/(2k) < ½minAdm`. `codim V(I) = minAdm` was independently confirmed
> by two methods (exact Jacobian rank + Singular Gröbner dimension), so Watanabe's `rlct ≤ ½minAdm` is
> tightly targeted and the achiever divisors `ord_{S(t)}` realise `½minAdm` exactly. This is scoped
> corroboration of the coverage ≥-leg, NOT a proof.

**NOT reached (residual scope, honestly stated):**
- (i) higher-corank / non-incidence-type coordinate changes;
...
- **Firmest:** the value `rlct = ½minAdm` is corroborated by FIVE decorrelated legs (Tier A vacuity, two
  ...
  sharpest D3 concern.
- **Most likely to break it (if anything):** an `L≥3` divisor centered at a DEEPER stratum / mixing a shared
  deep factor `C^{(s)}` across layers that a layer-by-layer incidence peel does not expose as monomial —
  residual scope (ii)/(iii). This is exactly the coupled-sharing regime the engine is built for, so it is
  also where a transcription error would bite.
- **Next construction to settle the open part:** build the exact minimizing-branch chart for (2,3,2,2)
  `t=(2,1,0)` and a truncated corank-2 chart for (4,4,4,4) `t=(2,1,0)`, and run the same exact LP; then push
  the second-blow-up NC check to the (3,3,4)/(2,2,2,2) leaves. The exhaustiveness ARGUMENT (every exceptional
  prime created at a definite `(S,J)` stage — Codex-Q3 in cert-d3) remains the proof; this hunt is its
  decorrelated stress-test and it did not break it.
