...
**HONEST LADDER: R0 ✓ · R1 ✓ · R2-cover-INFRASTRUCTURE ✓ → R3 = a BUILD (hchart de-risk #177 GO'd, decorrelated — R3 = WIRE the priced pieces, NOT a monument; destination stays (A); hchart = the R>0 SANDWICH + survivor-entry cover + cheap hole-nullity, so (A) DROPS binv entirely [binv = (B)-fallback only]). R3 WARNINGS held: W1 D_p-SAME-chart (cover+ideal on the SAME fam l, else R1 re-opens); W2 G1 survivor-ENTRY fan at the wire + G2 additivity-wireable; W3 [SHARP] BORN the fan from StepConstructor.bornSiblings, NOT the banked K-ORBIT skeleton Corank2Chart334:122-123 (= retired O1 transport, WALL F9 = DEAD ROUTE). First-brick LANDED [#178 @a1dad113]: node_cover_334 = the real-(3,3,4) NODE cover (born-fan over the KEPT center {0,1,2,3,20}, 4..7 spectators), reviewer-SURVIVED, clean-three 3 ways, aggregator-wired. NOW [#183, reroute-R2build]: the conOracle(![3,3,4]) COVER-CENTER trace = the fold gate + the fidelity SENSOR — the cover is CENTER-AGNOSTIC (green ≠ geometry), so #183 (NOT the green build) settles center fidelity; the value-side is already DISCHARGED (atlasRealizesExponents_334, buildTree≡gWrap exponents) + shearH shared. Then the 3-node fold (over buildTree, gated on #183 MATCH) + the leaf/value side (SurvivorFanCover sandwich + the reused MvPolynomial.volume_zeroSet_eq_zero + the #172 {R=0} recursion).**
