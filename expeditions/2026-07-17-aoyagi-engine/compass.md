...
**F10. The lower bound needs R>0, NOT the two-sided principality — P-vs-V collapses; residual = recursion-on-{R=0} + the survivor-entry tube-fan; objects-only OFF; B1 open (2026-07-25; #167 ideal-probe + #169 tube-probe + #168 elder adjudication; decorrelated exact-algebra + real Gröbner/ℚ + Codex xhigh, converged; SAME (3,3,3,2,2) t=(2,2,1,0) instance ⟹ the probes compose).** The per-step ideal MECHANISMS are all proven green (composition both ways, reads-only-kept, region=univ-under-unit≡1 — corank-agnostic; `Corank2AdmissibleLeaf222`). #167 VERIFIED-BUT-LOCALIZES: at separated-depth deep-mixed, `I_terminal=⟨b₁⟩` LOCALLY (Gröbner) + `det=±monomial` EXACTLY, but `nbhd=univ` is single-peel-special (the (3,3,4) `chart334_nbhd_univ` STANDS) — separated-depth benignly localizes to `{unit≠0}⊊univ`. So route-P-as-scoped (full two-sided hideal atlas) CONSUMES the general-d terminal single-chain principality (inferred, instance-verified, not proven; worked.tex:651-664). **#169 tube-cover WITNESS + THE CORRECTION (elder-scrutinized SOUND, the load-bearing result):** the LOWER bound needs `loss ≍ (wy)²` where `R>0` — a POINTWISE SANDWICH from the kept-survivor `R(0)=1` (loss = (wy)²·R, R a sum-of-squares) — NOT the two-sided principality. `R ≥ (1+pd)²` ⟹ the `{1+pd≈0}` tube is a two-sided-PRINCIPALITY-PROOF-TECHNIQUE artifact, not a lower-bound obstacle; loss degenerates only at `{R=0}` (codim 4). This DROPS the two-sided principality for the lower bound and VINDICATES the from-below content the charter's "weak reverse" always pointed at (refines F8: the lower bound is neither route-P-as-scoped's over-built two-sided principality NOR the forward-only mirage — it is the genuine from-below `R>0` bound). **P-vs-V COLLAPSES into one corrected R>0 route; V DEMOTED (#168):** V shares the family-cover (rlct global; `b_{k₀}` post-resolution) — not the retired mirage in kind, but no longer a distinct route. **The ONE remaining arbitrary-depth inference:** the recursion on `{R=0}` (codim 4) terminating with no new divisor ratio `< ½·minAdm` (minAdm-minimality; instance-verified thread-28 `rlct=2=½minAdm`; must be PERFORMED general-L, NOT assumed). Detail-at-scale (verified-exact at the instance, general-d = inductive): the kept-survivor `R(0)=1` (thread-28 backbone) + the **survivor-entry fan** — a HARD spec: radial-pivot-only covers **0%** of the tube, the survivor-entry fan (each residual-ideal generator) covers **100% at boxR≥1**, common-uncovered `={X=0}` codim≥2 (MC 0.0000%/2M). **#145 SPLIT + B1 OPEN:** #145's ~2% = (B2) the `{unit≈0}` tube [RESOLVED here, detail-at-scale] + (B1) the sector-count ESCAPE CONE `{max_W>C·max_V}` [SEPARATE, shear-slot/order-induced, LIKELY a Cartesian-fan artifact per the compass's prior full-fan/#86(B)-column-orbit ruling, but NOT CLOSED → its own decorrelated probe; do NOT conflate with B2]. Fatal 1/monomial structurally excluded (r≥1 ⟹ kept survivor ⟹ unit(0)≠0). **STANDING (elder rank): (A) fully-cite-free LEADS** (the R>0 route: kept-survivor sandwich + survivor-entry fan + recursion-on-{R=0}; the one inference = the minAdm-ratio, PERFORM it) **> (B) P-with-the-small-cite** (cite the recursion-ratio only if its general-L proof is a mountain) **> (C) objects-only OFF unless B1 walls.** Destination = operator (wait-for-go). Witnesses: `threads/reroute-R3-idealside/{deeper-mixed-probe,ideal-side-elaboration,codex-answer}.md`; `threads/reroute-R2-tubecover/tube-cover-probe.md`; `Corank2AdmissibleLeaf222.lean`; #145.
