...
**F10. Ideal-side de-risk — VERIFIED-BUT-LOCALIZES; the residual is the TUBE-COVER, not the ideal; V DEMOTED (2026-07-25, #167 probe + #168 elder adjudication; decorrelated exact-algebra + real Gröbner/ℚ + Codex xhigh, converged).** The R3 ideal-side de-risk found the per-step ideal MECHANISMS all proven green (composition both ways, region=univ under unit≡1, reads-only-kept — corank-agnostic, coupling-independent, via `Corank2AdmissibleLeaf222`), BUT route-P's unified fold AS-SCOPED consumes the general-d terminal single-chain principality (the tracked coupled monument; worked.tex:651-664; instance-verified, NOT proven). The #167 deeper-mixed probe (SEPARATED deep corank-2 + shallow shared factor, (3,3,3,2,2)-class, beyond every banked witness): (i) `I_terminal = ⟨b₁⟩` LOCALLY (Gröbner-exact, Rabinowitsch); (ii) `det Dg = ±monomial` EXACTLY; (iii) corner-join NOT localization-free — the kept pivot quotient is a LOCAL UNIT (1+cross-terms) ⟹ `nbhd = {unit≠0} ⊊ univ`. BENIGN: `terminal_bezout` is built for exactly this shrink, `hcover` is already up-to-null, {unit=0} is measure-zero. **CORRECTION: `nbhd=univ` was SINGLE-PEEL-special** — the (3,3,4) `chart334_nbhd_univ` STANDS; only the general-d extrapolation was over-clean (separated-depth mixing localizes). **RE-LOCATION:** the ideal conjunct is now CLEAN one rung deeper for BOTH arms (no denominator / no cleared-coord wall); the true residual MOVED to R2 = a family-indexed, up-to-null `hcover` SUBORDINATE to the per-leaf unit-loci {u≠0} — the {unit≈0} tubes (POSITIVE-measure ⟹ nullity-alone insufficient; sibling charts must cover them). This IS the #145/#146 cover reconciliation (the ~2% sector-count gap = the {unit≈0} tubes), now precisely characterized. Fatal 1/monomial STRUCTURALLY EXCLUDED (binding branch rank r≥1 ⟹ kept survivor row ⟹ unit(0)≠0; strong structural inference, not proven arbitrary-depth). **V DEMOTED (elder #168):** V SHARES the family-cover (rlct is a GLOBAL invariant; the lower bound `∫_U = Σ_charts` needs the up-to-null cover, and V's `b_{k₀}` is a POST-resolution monomial so V needs `g`, hence the cover). V's monument-free edge shrank to avoiding only the LOCAL principality — cheap, verified-one-rung, r≥1-robust — and V faces the SAME arbitrary-depth generality gap (one-sided ⊆ vs two-sided =, both hold locally with identical structure). NOT the retired mirage in kind (ideal-level one-sided containment + Object A ≠ value-level forward-only info-loss; F8/§3), but no longer worth the value-engine re-wire. V-test EXHIBITED it (germ-at-0 containment + monument-free pointwise `L ≥ b_{k₀}²·unit(0)`, coupling-assist = SOS extra mass). **STANDING (elder rank): P-build > objects-only > V.** Fully-cite-free = TWO arbitrary-depth inferences (local principality + tube-cover); the tube-cover is DETAIL-AT-SCALE IN KIND (a cover-tiling, the L7-cover class) but UNPRICED general-d — the #145 object → a DECORRELATED tube-cover probe (born-sibling fan covers the {unit≈0} tubes up-to-null at a deeper-mixed instance) prices it before "just build," and prices the whole destination (A fully-cite-free / B P-with-the-small-local-principality-cite / C objects-only #94). Destination = operator (wait-for-go). Witnesses: `threads/reroute-R3-idealside/{deeper-mixed-probe,ideal-side-elaboration,codex-answer}.md`; `Corank2AdmissibleLeaf222.lean`; #145.
