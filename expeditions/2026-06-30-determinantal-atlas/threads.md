...
| **P2.d** | formaliser | (dispatched) | 🔄 CRUX (BUILD) | **bespoke `IsZariskiLocallyTrivialAffineProduct` over the rank-`r` open** (NOT Mathlib `FiberBundle`, topological-only). All fields now available: cover (`iSup_pivot_basicOpen_eq_rankROpen`+`FibreRankBridge`, every scheme-point) · per-chart `StandardFibreChart` (P2.a) · `overlapTransition` (P2.b) · 2-fold cocycle `overlapTransition_trans_symm` (P2.c). Build the abstract Core predicate + exhibit DLN `RankROpenOverBaseLocalProduct`+`pivotAtlasChart` as the non-vacuous instance. Decorrelated review (predicate fidelity = the crux risk) |
