...
| rung | seat | teammate | status | notes |
|------|------|----------|--------|-------|
| **P2.h** | formaliser | p2h-product | 🔄 (dispatched) | (1) sweep STALE docs now-contradictory (P2.c/f/g landed): `AtlasTransition` top-Scope + `tripleElt` para, `FibreTargetOverlap`, `DLNFibre.lean` import comments, `FibreZariskiLocalTriviality`, `LocalTriviality` "cocycle" prose. (2) narrow "cocycle compatibility" prose to the precise landed theorems. (3) **STRENGTHEN the product API** [substantive]: tie `trivK` to `fibreModel.triv` (force `M = BaseLoc ⊗ Fibre`, `trivK = fibreModel.triv.restrictScalars k`) so the cocycle is genuinely of the OVER-BASE PRODUCT, not a decoupled bare-`k` model — verified CLEAN (DLN `chartDsigAt_schurLocTensorEquiv` is literally `ofRingEquiv chartDsigAt_tensorEquiv`, same map; `M = SchurLoc ⊗ sweepFibreRing` exactly). Guard-first; fall back to narrow-name if the tie isn't clean. Decorrelated Codex |
