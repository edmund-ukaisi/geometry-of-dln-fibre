...
| **P2.g** | formaliser | (dispatched) | 🔄 CRUX (BUILD) — completes operator's (iv) | **naturality**: `restrict(overlapTransition C D to triple) = tripleTransition C D E` (via `Away.mul'` refinement `Away(d·e) ≃ (Away d) away e` + localization initiality), then derive the cocycle of the ACTUAL restricted 2-fold transitions (operator's literal `g_jk∘g_ij=g_ik after restriction`). A LOCAL compatibility — NOT the global-gluing R1. Guard-first; report if genuine wall. Decorrelated review |
