...
| **P2.f** (was P2.c′) | formaliser | (dispatched) | 🔄 CRUX (BUILD) — **operator-requested via PR #21** | **target-side triple cocycle** `g_{jk} ∘ g_{ij} = g_{ik}` on common triple overlaps — completing the standard atlas coherence package (cover · local product · pairwise inverse · triple cocycle). Operator override of the earlier "roadmap to R1" call (PR #21 comment): the atlas API should not stop at the pairwise inverse law. Shape (i)-(iv) (Codex-confirmed, = operator's requested shape): (i) target triple-overlap rings, (ii) triple `awayCongr'`, (iii) restriction maps 2-fold→triple, (iv) naturality `restrict_overlapTransition = tripleTransition`, then the cocycle by localization initiality (transport the LANDED base-side `awayTriple_cocycle` P1.a). Transitions stay THEOREMS not fields. Decorrelated review |
