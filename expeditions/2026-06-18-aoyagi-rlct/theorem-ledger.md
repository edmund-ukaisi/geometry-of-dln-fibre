...
| Blow-up inductive statement | A4 | PDF pp. 14-15 | reduced product coordinates | draft + failed check plus repair: `threads/04-blow-up-certificate/reproduction-draft.md`, `threads/04-blow-up-certificate/reproduction-check.md`, `threads/04-blow-up-certificate/reproduction-repair-a4.md`; source uses actual widths `M^{(s)}` and prefix minima `M(S)` simultaneously; page image also shows the inductive comparability range undercounts labels relative to the Jacobian ranges | invariant theorem TBD; blocked | failed check; xhigh source and pen-and-paper rechecks completed |
| Blow-up Case 1 | A4 | PDF pp. 15-18 | reduced product coordinates, inductive statement | repair report corrects the row-strip center and exponent increments to use actual active column width `M^{(S+1)}-J`; selected-variable chart and displayed pivot chart are understood only locally; full pivot-chart family or permutation reduction still missing; `P` quotient divisibility reduced to recurrence arithmetic; normalized `P` row algebra checked | monomial recurrence divisibility and normalized `P` block row-operation theorem proved in `BlowupArithmetic.lean`; transition lemma TBD/blocked | repair note updated; statement cards `threads/04-blow-up-certificate/statement-card-a4-monomial-recurrence-divisibility.md`, `threads/04-blow-up-certificate/statement-card-a4-normalized-p-row-operation.md` |
| Blow-up Case 2 | A4 | PDF pp. 19-22 | reduced product coordinates, inductive statement | page image confirms printed vector `t_{S,J+1}^{(i)}=M^{(i+1)}` for `i<S` and printed increment `(M(S)-J)(M^{(S+1)}-J)`; substituting that vector into the terminal formula gives `(M^{(S)}-J)(M^{(S+1)}-J)` unless `M(S)=M^{(S)}`; prefix-minimum vector repairs the arithmetic but is not printed; pivot charts and full `b'_i` bookkeeping remain unresolved; `P` quotient divisibility and normalized `P` row algebra are proved narrowly | arithmetic split, monomial recurrence divisibility, and normalized `P` block theorem proved in `lean/DLNFibre/DLN/Aoyagi/BlowupArithmetic.lean`; no source-faithful transition theorem accepted | xhigh rechecks confirm source typo/gap, recurrence-divisibility reduction, and normalized row algebra; statement cards `threads/04-blow-up-certificate/statement-card-a4-terminal-exponent-split.md`, `threads/04-blow-up-certificate/statement-card-a4-monomial-recurrence-divisibility.md`, `threads/04-blow-up-certificate/statement-card-a4-normalized-p-row-operation.md` |
