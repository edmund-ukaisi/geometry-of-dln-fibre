...
|---|---|---|---|---|---|---|
| Retained-passive canonical local-source formal/product-density change of variables | A2 canonical retained-passive measure socket consumes computed raw-order density without an external realization map | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; measure identity uses Mathlib finite-dimensional Jacobian COV already packaged upstream | realized retained-passive local-source formal/product-density theorem; canonical retained-passive p.13 source chart; raw-order inverse on the source-recursive determinant chart; source-chart continuity on the determinant-chart subtype | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-canonical-local-formal-density-cov.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-canonical-local-formal-density-cov.md` | `retainedPassiveP13CanonicalSourceChart_aemeasurable`, `retainedPassiveP13CanonicalSourceChart_realize`, `measure_map_restrict_retainedPassiveP13CanonicalLocalSource_eq_map_comp_topologyTupleEdgeRawOrder_withDensity_formalRawOrderAbsDet`, and `measure_map_restrict_retainedPassiveP13CanonicalLocalSource_eq_map_comp_topologyTupleEdgeRawOrder_withDensity_formalProductAbsDet` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveLocalJacobianMeasure.lean`; focused local build passed; top-level `DLNFibre` build passed with pre-existing warning noise; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed | reviewed by xhigh `Euclid the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-canonical-local-formal-density-cov.md`; PASS; checked canonical source-chart shape, source-side density orientation, realized-wrapper call, and nonclaim boundary: no original source prior, signed-box source-density identification, normal crossings, pole order, or RLCT |
| Retained-passive realized local-source formal/product-density change of variables | A2 local retained-passive measure socket consumes computed raw-order density | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; measure identity uses Mathlib finite-dimensional Jacobian COV already packaged upstream | retained-passive formal/product raw-order COV; existing realized retained-passive local-source restriction theorem; a.e.-measurable downstream source chart; realization hypothesis into retained-passive local source | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-local-formal-density-cov.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-local-formal-density-cov.md` | `map_topologyTupleEdgeRawOrder_withDensity_formalRawOrderAbsDet_eq_restrict_rawSourceChart`, `map_topologyTupleEdgeRawOrder_withDensity_formalProductAbsDet_eq_restrict_rawSourceChart`, `map_comp_topologyTupleEdgeRawOrder_withDensity_formalRawOrderAbsDet_eq_map_restrict_rawSourceChart`, and `map_comp_topologyTupleEdgeRawOrder_withDensity_formalProductAbsDet_eq_map_restrict_rawSourceChart` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobianMeasure.lean`; `measure_map_restrict_retainedPassiveP13LocalSource_eq_map_comp_topologyTupleEdgeRawOrder_withDensity_of_realization_of_cov`, `measure_map_restrict_retainedPassiveP13LocalSource_eq_map_comp_topologyTupleEdgeRawOrder_withDensity_formalRawOrderAbsDet_of_realization`, and `measure_map_restrict_retainedPassiveP13LocalSource_eq_map_comp_topologyTupleEdgeRawOrder_withDensity_formalProductAbsDet_of_realization` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveLocalJacobianMeasure.lean`; focused coordinate and local builds passed; top-level `DLNFibre` build passed with pre-existing warning noise; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed | reviewed by xhigh `Hypatia the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-local-formal-density-cov.md`; PASS; checked density orientation, raw-order composition through `sourceChart`, local-source restriction via `hrealize`, index shape, and nonclaim boundary: no original source prior, signed-box source-density identification, normal crossings, pole order, or RLCT |
| Retained-passive formal/product-density change of variables | A2 target-normalizer density transport support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; measure identity uses Mathlib finite-dimensional Jacobian change of variables already packaged in `RetainedPassiveCoordinatesMeasure` | retained-passive raw-order measure theorem with abstract Frechet determinant density; full target normaliser determinant bridge; formal determinant product formula | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-formal-density-cov.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-formal-density-cov.md` | `retainedPassiveFormalRawOrderJacobianProductAbsDetAt`, `map_topologyTupleEdgeRawOrder_withDensity_formalRawOrderAbsDet_eq_restrict_rawSourceChart_posTail`, `map_topologyTupleEdgeRawOrder_withDensity_formalRawOrderAbsDet_eq_restrict_rawSourceChart_zeroTail`, `map_topologyTupleEdgeRawOrder_withDensity_formalProductAbsDet_eq_restrict_rawSourceChart_posTail`, and `map_topologyTupleEdgeRawOrder_withDensity_formalProductAbsDet_eq_restrict_rawSourceChart_zeroTail` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobianMeasure.lean`; focused new-module build passed; top-level `DLNFibre` build passed with pre-existing warning noise; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed | reviewed by xhigh `Volta the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-formal-density-cov.md`; PASS; checked a.e. restriction scope, zero/positive-tail indexing, product density formula, reused COV theorem, and nonclaim boundary: no original-source prior, signed-box source-density identification, normal crossings, pole order, or RLCT |
| Retained-passive full target normaliser and raw-order determinant product bridge | A2 target-normalizer full tuple and determinant support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant transport is local finite-dimensional linear algebra through a determinant-one target `LinearEquiv` | edge-pair, `A1passive`, `Ctop`, zero-tail `F3`, and positive-tail `F3` component bridges; determinant-one composed zero-tail and positive-tail target maps; conditional determinant bridge `topologyTupleEdgeRawOrderFDerivAbsDet_eq_formalRawOrderAbsDetAt_of_target_linearEquiv` and product version | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-full-target-normaliser-determinant-bridge.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-full-target-normaliser-determinant-bridge.md` | `retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_A3passive`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShear_fderiv_eq_formalRawOrderJacobianAt`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShear_fderiv_eq_formalRawOrderJacobianAt`, `topologyTupleEdgeRawOrderFDerivAbsDet_eq_formalRawOrderAbsDetAt_posTail`, `topologyTupleEdgeRawOrderFDerivAbsDet_eq_formalRawOrderAbsDetAt_zeroTail`, `topologyTupleEdgeRawOrderFDerivAbsDet_product_eq_posTail`, and `topologyTupleEdgeRawOrderFDerivAbsDet_product_eq_zeroTail` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused Jacobian build passed; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed | reviewed by xhigh `Sartre the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-full-target-normaliser-determinant-bridge.md`; PASS; checked component coverage, `A3passive` preservation scope, zero/positive-tail indexing, determinant bridge inputs, and nonclaim boundary: no measure transport, normal crossings, pole order, or RLCT, and no single all-`M` theorem |
| Post-`Ctop` retained-passive `F3` positive-tail raw-tuple shear and component bridge | A2 target-normalizer positive-tail support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant computation is local finite-dimensional linear algebra | edge-pair then `A1passive` then `Ctop` component bridge; post-`Ctop` recursive `dEarly` comparison; target-only positive-tail `F3` theorem; raw tuple regrouping as `F3 × rest` | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-post-ctop-f3-shear.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-post-ctop-f3-positive-shear.md` | `retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt`, `retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_apply`, `retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_det_eq_one`, `retainedPassivePostCtopF3PosShearRawTupleLinearEquivAt_abs_det_eq_one`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt_apply`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShearRawTupleLinearEquivAt_abs_det_eq_one`, and `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3PosShear_fderiv_F3_eq_formalRawOrderJacobianAt` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused Jacobian build passed; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed | reviewed by xhigh `Tesla the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-post-ctop-f3-positive-shear.md`; PASS; checked sign, `Earlyfun z`, terminal `A1` indices, `dEarly_postC` use, determinant scope, and nonclaim boundary: no single full raw-tuple equality, no actual raw-order Frechet determinant equality, no measure transport, normal crossings, pole order, or RLCT |
| Post-`Ctop` retained-passive `F3` zero-tail raw-tuple shear and component bridge | A2 target-normalizer zero-tail support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant computation is local finite-dimensional linear algebra | edge-pair then `A1passive` then `Ctop` component bridge; zero-tail `Ctop` target-staged theorem; zero-tail `F3` target-staged theorem; raw tuple regrouping as `F3 × rest` | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-post-ctop-f3-shear.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-post-ctop-f3-zero-shear.md` | `retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt`, `retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_apply`, `retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_det_eq_one`, `retainedPassivePostCtopF3ZeroShearRawTupleLinearEquivAt_abs_det_eq_one`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt_apply`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShearRawTupleLinearEquivAt_abs_det_eq_one`, and `retainedPassiveTargetEdgePairThenA1passiveThenCtopThenF3ZeroShear_fderiv_F3_eq_formalRawOrderJacobianAt` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused Jacobian and full `DLNFibre` builds passed, the full build with pre-existing warning noise; `scripts/sorries`, `git diff --check`, and direct axiom audit passed; axiom footprint `[propext, Classical.choice, Quot.sound]` | reviewed by xhigh `Nietzsche the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-post-ctop-f3-zero-shear.md`; PASS; checked `F3` focus tuple order, zero-tail shear formula, determinant-one scope, component bridge, and nonclaim boundary: positive-tail `F3` remains open, no full raw-tuple equality, no actual raw-order Frechet determinant equality, no measure transport, normal crossings, pole order, or RLCT |
| Edge-pair then post-edge-pair `A1passive` then post-`A1passive` `Ctop` component bridge | A2 target-normalizer component bridge | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant computation is local finite-dimensional linear algebra | edge-pair then `A1passive` component bridge; post-`A1passive` passive-tail derivative from staged `A1passive`; formal `(F2,C)` inverse recovery from edge-pair-normalised data; recursive target-staged `Ctop` theorem; raw lower-left readout preservation | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-post-a1passive-ctop-shear.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-edge-pair-a1passive-ctop-bridge.md` | `retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt`, `retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_apply`, `retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_det_eq_one`, `retainedPassivePostA1passiveCtopShearRawTupleLinearEquivAt_abs_det_eq_one`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_apply`, `retainedPassiveTargetEdgePairThenA1passiveThenCtopShearRawTupleLinearEquivAt_abs_det_eq_one`, and `retainedPassiveTargetEdgePairThenA1passiveThenCtopShear_fderiv_Ctop_eq_formalRawOrderJacobianAt` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused Jacobian and full `DLNFibre` builds passed, the full build with pre-existing warning noise; `scripts/sorries`, `git diff --check`, code-only forbidden-marker search, and direct axiom audit passed; axiom footprint `[propext, Classical.choice, Quot.sound]` | reviewed by xhigh `Popper the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-edge-pair-a1passive-ctop-bridge.md`; PASS; checked `Ctop` formula, determinant scope, component bridge, and nonclaim boundary: no `F3` agreement, full tuple equality, actual raw-order Frechet determinant equality, measure transport, normal crossings, pole order, or RLCT |
| Edge-pair then post-edge-pair `A1passive` component bridge | A2 target-normalizer component bridge | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant computation is local finite-dimensional linear algebra | target edge-pair raw-tuple equivalence/determinant; post-edge-pair `A1passive` raw-tuple shear/determinant; formal `(F2,C)` inverse recovery from edge-pair-normalised data; existing target-staged `A1passive` theorem; raw lower-left derivative bridge | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-edge-pair-then-a1passive-bridge.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-edge-pair-then-a1passive-bridge.md` | `retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt`, `retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt_apply`, `retainedPassiveTargetEdgePairThenA1passiveShearRawTupleLinearEquivAt_abs_det_eq_one`, `retainedPassiveTargetEdgePairThenA1passiveShear_fderiv_A1passive_eq_formalRawOrderJacobianAt`, `retainedPassiveTargetEdgePairThenA1passiveShear_fderiv_F2C_eq_formalRawOrderJacobianAt`, and `retainedPassiveTargetEdgePairThenA1passiveShear_fderiv_A3passive_eq_formalRawOrderJacobianAt` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused Jacobian build passed; `scripts/sorries`, `git diff --check`, code-only forbidden-marker search, and direct axiom audit passed; axiom footprint `[propext, Classical.choice, Quot.sound]` | reviewed by xhigh `Pasteur the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-edge-pair-then-a1passive-bridge.md`; PASS; `LinearEquiv.trans` order, determinant scope, component bridges, and nonclaim boundary checked; no `Ctop`/`F3` agreement, full tuple equality, actual raw-order Frechet derivative determinant equality, measure transport, normal crossings, pole order, or RLCT |
| Post-edge-pair retained-passive `A1passive` raw-tuple shear determinant one | A2 target-normalizer determinant support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant computation is local finite-dimensional linear algebra | target edge-pair raw-tuple determinant/equivalence; formal `(F2,C)` inverse; raw tuple product order `A1passive × rest`; generic upper product shear determinant lemma | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-post-edge-pair-a1passive-raw-tuple-shear.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-post-edge-pair-a1passive-raw-tuple-shear.md` | `linearEquivUpperShear_symm_apply` proved in `lean/DLNFibre/DLN/Aoyagi/MatrixLinearDeterminant.lean`; `retainedPassivePostEdgePairA1passiveShearRawTupleLinearEquivAt`, `retainedPassivePostEdgePairA1passiveShearRawTupleLinearEquivAt_apply`, `retainedPassivePostEdgePairA1passiveShearRawTupleLinearEquivAt_symm_apply`, `retainedPassivePostEdgePairA1passiveShearRawTupleLinearEquivAt_det_eq_one`, and `retainedPassivePostEdgePairA1passiveShearRawTupleLinearEquivAt_abs_det_eq_one` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused builds of both modules and full `DLNFibre` build passed; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed; direct axiom audit reported `[propext, Classical.choice, Quot.sound]` for the new public post-edge-pair shear theorems and `[propext, Quot.sound]` for `linearEquivUpperShear_symm_apply` | reviewed by xhigh `James the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-post-edge-pair-a1passive-raw-tuple-shear.md`; PASS; no composed target-normalizer theorem, no actual raw-order Frechet derivative determinant equality, no measure transport, normal crossings, pole order, or RLCT |
| Retained-passive target edge-pair raw-tuple determinant one | A2 target-normalizer determinant support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; determinant computation is local finite-dimensional linear algebra | raw-tuple edge-pair linear equivalence; edge-block regrouping `(A1_q,A3_q,F_q,C_q)`; formal inverse first-component formula; generic successor-triangular determinant lemma; same-edge lower-triangular block determinant lemma | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-target-edge-pair-raw-tuple-determinant.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-target-edge-pair-raw-tuple-determinant.md` | `linearMap_det_finSuccUpperTriangular_eq_prod`, `linearMap_det_finSuccUpperUnitriangular_eq_one`, and `finSuccUpperUnitriangularLinearMap_det_eq_one` proved in `lean/DLNFibre/DLN/Aoyagi/MatrixLinearDeterminant.lean`; `retainedPassiveTargetEdgePairShearRawTupleInverseLinearMapAt_det_eq_one`, `retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt_symm_det_eq_one`, `retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt_det_eq_one`, and `retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt_abs_det_eq_one` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused builds of both modules passed; `scripts/sorries`, `git diff --check`, and code-only forbidden-marker search passed | reviewed by xhigh `Lorentz the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-target-edge-pair-raw-tuple-determinant.md`; PASS; reviewer axiom audit for the seven determinant theorems reported `[propext, Classical.choice, Quot.sound]`; no determinant-one claim for `retainedPassiveFormalRawF2CLinearEquivAt`; no actual raw-order Frechet derivative determinant equality, measure transport, normal crossings, pole order, or RLCT |
| Retained-passive target edge-pair raw-tuple linear equivalence package | A2 target-normalizer support | PDF pp. 11-13 as retained-passive product-reduction coordinate algebra; equivalence package is local Lean algebra around the target edge-pair shear | target edge-pair linear map package; formal raw `(F2,C)` inverse; raw side-field projections; backward target-recovered `F2` recurrence | reproduced at `threads/03-block-product-reduction/reproduction-a2-retained-passive-target-edge-pair-raw-tuple-equivalence.md`; statement card `threads/03-block-product-reduction/statement-card-a2-retained-passive-target-edge-pair-raw-tuple-equivalence.md` | `retainedPassiveTargetEdgePairShearRawTupleLinearMapAt`, `retainedPassiveTargetEdgePairShearRawTupleLinearMapAt_apply`, `retainedPassiveTargetEdgePairShearRawTupleInverseLinearMapAt`, `retainedPassiveTargetEdgePairShearRawTupleInverseLinearMapAt_apply`, `retainedPassiveFormalRawF2CLinearEquivAt_symm_targetEdgePairShearAt_fst`, `retainedPassiveTargetEdgePairShearRawTupleInverseLinearMapAt_leftInverse`, `retainedPassiveTargetRecoveredF2At_rawTupleInverse`, `retainedPassiveTargetEdgePairShearRawTupleLinearMapAt_rightInverse`, `retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt`, `retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt_apply`, and `retainedPassiveTargetEdgePairShearRawTupleLinearEquivAt_symm_apply` proved in `lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCoordinatesJacobian.lean`; focused Jacobian build passed; `scripts/sorries`, `git diff --check`, code-only forbidden-marker search, and direct axiom audits passed with footprint `[propext, Classical.choice, Quot.sound]`; determinant-one theorem recorded in the separate determinant row above | reviewed by xhigh `Mendel the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-target-edge-pair-raw-tuple-linear-inverse.md`; PASS for inverse algebra and no-overclaim boundary; xhigh proof audit by `Pascal the 2nd` supplied the staged recurrence proof route; final equivalence review by xhigh `Gauss the 2nd` in `threads/03-block-product-reduction/review-a2-retained-passive-target-edge-pair-raw-tuple-linear-equivalence.md` passed; formal inverse second component is not a raw `C` recovery for arbitrary tuples; no determinant equality, measure transport, normal crossings, pole order, or RLCT |
