...
| Single-edge product-coordinate family constructor | A2 finite product-coordinate constructor support | PDF p. 13 | regular coordinate-vector block reconstruction; residual matrix coordinate reconstruction; raw single-edge p.13 matrix readout; fixed-base prescribed edge realisation; suffix residual-product bridge | reproduced at `threads/03-block-product-reduction/reproduction-a2-single-edge-product-coordinate-family-constructor.md` and `threads/03-block-product-reduction/reproduction-a2-single-edge-residual-product-realisation.md`; statement cards `threads/03-block-product-reduction/statement-card-a2-single-edge-product-coordinate-family-constructor.md` and `threads/03-block-product-reduction/statement-card-a2-single-edge-residual-product-realisation.md` | `AoyagiResidualBlockCoordinateIndex.matrix`, `AoyagiResidualBlockCoordinateIndex.value_matrix`, `AoyagiResidualBlockCoordinateIndex.exists_value_eq`, `AoyagiProductDifferenceCoordinateIndex.value_euclideanCtopMatrixCoordinateMatrices_residual`, `ChartLocalSuffixState.residualProduct_productCoordinateSingleEdge_eq`, `paperEndpointFixedBaseSingleEdgeProductCoordinateMatrixOfEuclidean`, `paperEndpointFixedBaseSingleEdgeProductCoordinateMatrixOfEuclidean_residualProduct_eq`, and `paperEndpointFixedBaseProductDifferenceCoordinateMap_eq_singleEdgeProductCoordinateEuclidean` proved in `lean/DLNFibre/DLN/Aoyagi/ProductReduction.lean` and `lean/DLNFibre/DLN/Aoyagi/RegularSuspensionCoordinates.lean`; for a Euclidean regular vector `u` and residual matrix `D`, the raw one-edge p.13 matrix `[Ctop,-Ctop F2; -F3 Ctop, D + F3 Ctop F2]` has cleaned product-difference readout `u` on regular coordinates and `value D` on residual coordinates, and its transformed suffix residual product is `D`, under `IsUnit Ctop.det` | controller review recorded in `threads/03-block-product-reduction/review-a2-single-edge-product-coordinate-family-constructor.md`; xhigh/controller review recorded in `threads/03-block-product-reduction/review-a2-single-edge-residual-product-realisation.md`; no base-source choice of `D`, no multi-edge arbitrary-terminal-residual theorem, no parameter-continuity, product chart, density/Jacobian transport, normal crossings, pole order, or RLCT |
