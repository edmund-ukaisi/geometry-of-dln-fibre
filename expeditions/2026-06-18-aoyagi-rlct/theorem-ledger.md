...
| Lemma 3 minimisation | A5 | PDF p. 24 | exponent vector definitions | full draft reproduction still blocked by terminal-variable restriction and feasibility; isolated endpoint arithmetic reproduced at `threads/05-arithmetic-tail/reproduction-lemma3-endpoint-arithmetic-a5.md` | isolated integer numerator identity and endpoint-corrected constrained minimum proved in `lean/DLNFibre/DLN/Aoyagi/ArithmeticTail.lean` as `aoyagiLemma3A_eq_min_add` and `aoyagiLemma3A_isLeast_image_Icc`; full Lemma 3/candidate theorem still blocked | isolated sub-slice xhigh checked by `Lorentz the 5th`, review `threads/05-arithmetic-tail/review-lemma3-endpoint-arithmetic-a5.md`; full A5 review still blocked |
