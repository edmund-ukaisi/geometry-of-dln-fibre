...
| Blow-up Case 1 | A4 | PDF pp. 15-18 | reduced product coordinates, inductive statement | repair report corrects the row-strip center and exponent increments to use actual active column width `M^{(S+1)}-J`; selected-variable chart and displayed pivot chart are understood only locally; full pivot-chart family or permutation reduction still missing; `P` quotient divisibility reduced to recurrence arithmetic | monomial recurrence divisibility proved in `BlowupArithmetic.lean`; transition lemma TBD/blocked | repair note updated; statement card `threads/04-blow-up-certificate/statement-card-a4-monomial-recurrence-divisibility.md` |
| Blow-up Case 2 | A4 | PDF pp. 19-22 | reduced product coordinates, inductive statement | page image confirms printed vector `t_{S,J+1}^{(i)}=M^{(i+1)}` for `i<S` and printed increment `(M(S)-J)(M^{(S+1)}-J)`; substituting that vector into the terminal formula gives `(M^{(S)}-J)(M^{(S+1)}-J)` unless `M(S)=M^{(S)}`; prefix-minimum vector repairs the arithmetic but is not printed; pivot charts and `b'_i` bookkeeping remain unresolved; `P` quotient divisibility reduced to recurrence arithmetic | arithmetic split and monomial recurrence divisibility proved in `lean/DLNFibre/DLN/Aoyagi/BlowupArithmetic.lean`; no source-faithful transition theorem accepted | xhigh rechecks confirm source typo/gap and recurrence-divisibility reduction; statement cards `threads/04-blow-up-certificate/statement-card-a4-terminal-exponent-split.md`, `threads/04-blow-up-certificate/statement-card-a4-monomial-recurrence-divisibility.md` |
