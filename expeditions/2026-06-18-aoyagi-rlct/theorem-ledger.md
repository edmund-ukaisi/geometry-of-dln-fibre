...
| Regular square-suspension integrability target | A2 analytic target, not yet Lean | PDF p. 13 as motivation for regular variables | product-coordinate comparability `F ~ |u|^2 + g(y)`; bounded positive density; Fubini; polar integral estimates | target statement at `threads/03-block-product-reduction/reproduction-a2-regular-square-suspension-integrability-target.md` | not formalised; proposed future module should prove local power-integrability threshold shift by `k/2` without invoking normal-crossing extraction | xhigh `Tesla the 4th` source scout recommended this as the next genuine analytic theorem; no Aoyagi p.13 chart/density construction yet |
