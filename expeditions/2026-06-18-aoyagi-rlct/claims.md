...
- **Lean target.**
  ...
  `lean/DLNFibre/DLN/Aoyagi/ProductReduction.lean`; entry-ideal transport lemmas
  in `lean/DLNFibre/DLN/Aoyagi/EntryIdeal.lean`; full Theorem 3 target blocked.
- **Proved.** one chart-local algebraic induction-step identity over a
  ...
  entry ideal generated by `X`, `F2`, `F3`, and `D`.
...
- **Deferred.** post-Theorem-3 RLCT reduction and regular-coordinate additivity
  ...
  local analytic/certificate transport remain open.
