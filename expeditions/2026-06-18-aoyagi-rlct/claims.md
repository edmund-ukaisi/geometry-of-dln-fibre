...
- **Proved.** terminal-exponent split API:
  ...
  packaging the same conditional bridge through named recurrence data.
- **Assumed.** finite dimension/rank hypotheses; no transition invariant is
  ...
  and row-weight expression sprawl, but still assume the Nat-valued `level` map
  is Aoyagi's current `tilde_t`, the variable map is the recurrence variable
  assignment, the introduced-label Case 2 gap holds, and row weights are
  generated by this recurrence. This displayed bridge keeps residual rows
