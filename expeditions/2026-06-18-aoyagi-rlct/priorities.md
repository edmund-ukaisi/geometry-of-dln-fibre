...
5. Repair A4 blow-up reproduction. Keep actual layer widths and prefix minima
   ...
   residual row weights are flat and the source-substitution `Q/P` theorem
	   ...
	   source-substitution theorem applies. The finite-domain bridge is now also
	   Lean-proved for Lean's `introducedLabelFinset`; it still assumes the level
	   and variable maps are the source recurrence data, the introduced-label gap
	   holds, and row weights are generated by that recurrence. Keep the bridge's
	   row and column domains separate: residual rows are `J+1..mu_S`, residual
	   columns are actual-width `J+1..n_(S+1)`, and the selected variable is
	   counted once in the updated weights `u*b_i`. Next A4 target: package the
	   recurrence-state invariant tying `level`, `var`, and row weights to the
	   introduced-label certificates, or move to arbitrary-pivot chart transport if
	   that invariant bridge remains blocked.
