...
This theorem proves that `ell=1` source data exists iff the three source-range
reduced widths are all positive and at least two repeat.  The complete finite
`L=2` classifier now calls this theorem in its `ell=1` forward branch.
...
Focused direct elaboration, focused module build, full local `lake build
DLNFibre`, no-sorry audit, diff check, touched Lean-file forbidden-marker
scan, direct axiom probe, and xhigh independent review passed.  The new
theorem and refactored complete classifier report only
`[propext, Classical.choice, Quot.sound]`.
...
If, on the returned local Case 2 shrink,
...
then the p.13 formal-product chart measure restricted to a p.13 chart piece is
dominated by `D • sourceRef`, and the original edge-family volume restricted
to a measurable p.13 chart piece is dominated by
`(cHaar^{-1} * D) • sourceRef`, where
`sourceRef := Measure.map sourceChart (thetaReference.restrict V)`.

This is the right direction for the original-volume/readback consumers, but
the reverse raw-source domination is still a supplied hypothesis.  Aoyagi
pp. 10-13 support the block/raw-order/source-chart algebra used in the
handoff; they do not, by themselves, prove this reverse raw-source comparison.
Xhigh scout `Mendel` passed the theorem-shape audit and identified the same
API caveat handled in Lean: a.e. measurability of the p.13 raw chart for the
raw image follows from raw-image support in `rawSourceSet`.

Focused elaboration, focused local Lake builds, full local
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
theorem axiom audit passed.  Both new theorems report only
`[propext, Classical.choice, Quot.sound]`.  Focused builds for
`RetainedPassiveCase2PassiveThetaFormalProductSourceReference` and
`RetainedPassiveCase2PassiveThetaOriginalVolumeBridge` had only pre-existing
imported warning noise.  Nonclaims: no determinant-chart Haar
transport, exact raw-Haar pushforward, raw-source reverse domination proof,
source coverage, source-rank coverage, original source-prior transport, normal
crossings, pole order, or RLCT extraction.

## Latest controller decision - 2026-07-01, Case 2 passive-theta endpoint reference
...
and proves the support theorems:
...
If a measurable local set `Omega` lies inside
`case2PassiveThetaDetSector`, then the endpoint topology-tuple pushforward of
the concrete reference source restricted to `Omega` is unchanged by
restriction to `topologyTupleDetChartSet`.  If a passive-field measure is
dominated by `d` times the coordinate passive-field reference, then its
passive-product endpoint pushforward is dominated by `d` times the named
endpoint reference image measure.

This is deliberately only a reference-measure/support layer.  A direct Lean
probe showed bare `volume` on the nested passive-field product has the wrong
measurable-space shape, so the reference is defined as an explicit product of
entrywise Lebesgue measures.  Xhigh scout review found that unrestricted full
determinant-chart Haar domination is obstructed for this theta domain: the
endpoint image fills the full `C` family through selected-entry residual data,
so it is an image slice inside the determinant chart.  No endpoint
determinant-chart Haar domination, Jacobian formula for `Y`, exact Haar
transport, raw-Haar pushforward, original source-prior transport,
source-image/source-rank coverage, normal crossings, pole order, or RLCT
extraction is claimed.  Focused elaboration, focused module build, full local
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
axiom probe passed.  The axiom footprint is
`[propext, Classical.choice, Quot.sound]`.

## Latest controller decision - 2026-07-01, Case 2 determinant-sector determinant-chart support

Lean now proves the support theorem:

```text
measure_map_case2PassiveThetaEndpointTopologyTuple_restrict_detChartSet_eq_self_of_subset_detSector
```

If a measurable local full passive-theta set `Omega` is contained in
`case2PassiveThetaDetSector`, then the endpoint topology-tuple pushforward of
any restricted theta measure is unchanged by restriction to
`topologyTupleDetChartSet`.

This is deliberately below the missing passive-reference COV theorem.  It
proves determinant-chart support only; it does not construct a passive
Haar/Lebesgue reference measure, compute the Jacobian of `Y`, or prove
domination by determinant-chart Haar.

Focused direct elaboration, focused module build, dependent `RawImageHandoff`
elaboration, full local `lake build DLNFibre`, `lean/scripts/sorries`,
`git diff --check`, and direct axiom probe passed.  The axiom footprint is
`[propext, Classical.choice, Quot.sound]`.  Xhigh reviewer `Locke the 3rd`
passed the Lean/documentation boundary check.

## Latest controller decision - 2026-07-01, Case 2 passive-reference determinant-chart transfer

Lean now proves the adapter:
...
The Case 2 theorem says that if
...
This does not prove the determinant-chart comparison for the reference
source.  It prepares the exact adapter needed once a concrete passive
Haar/Lebesgue reference source is constructed.

Focused `LocalMeasureHandoff` build and direct `RawImageHandoff` elaboration
passed.  Full local `lake build DLNFibre`, `lean/scripts/sorries`,
`git diff --check`, the direct axiom probe, and independent xhigh review by
`Kierkegaard` also passed.  The axiom footprint is
`[propext, Classical.choice, Quot.sound]`.

## Latest controller decision - 2026-07-01, Case 2 baseJ raw-Haar domination from determinant Haar

Lean now proves the same-scalar globally Jacobian-weighted raw-Haar socket:
...
It returns an open `V subset G`.  Under the same explicit unweighted
...
Here `baseJ = passiveSource.withDensity jacobianDensity`, and
`jacobianDensity z` is exactly the retained-passive formal raw-order
determinant factor evaluated at `Y z`.  The proof weights the determinant-chart
domination by that formal-product density and then uses
...
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
axiom probe passed.  The theorem and its two generic measure lemmas report
only `[propext, Classical.choice, Quot.sound]`.  Xhigh reviewer `Newton the
3rd` passed the documentation/Lean fidelity check.

This still does not prove the determinant-chart domination hypothesis.  The
next real frontier remains a concrete Haar/Lebesgue passive-source transport
or domination theorem for `Y`; arbitrary `passiveMeasure` is too general.

## Latest controller decision - 2026-07-01, Case 2 baseJ raw-Haar inverse-Jacobian domination

Lean now proves the corresponding globally Jacobian-weighted raw-Haar socket:

```text
exists_pos_open_subset_measure_map_case2PassiveTheta_rawMap_baseJ_restrict_le_smul_rawHaar_withDensity_inverseJacobian_of_endpointTopologyTuple_restrict_le_smul_detHaar
```

It returns a real constant `K > 0` and an open `V subset G`.  Under the same
explicit unweighted determinant-chart hypothesis
...
Here `baseJ = passiveSource.withDensity jacobianDensity`.  The scalar `ofReal K`
comes from the local upper side of the retained-passive forward-Jacobian
sandwich; `c : ℝ≥0∞` is not assumed finite.

Focused direct elaboration, focused module build, full local
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
axiom probe passed; the axiom footprint is `[propext, Classical.choice,
Quot.sound]`.  Xhigh scout `Laplace the 3rd` returned PASS.

This still does not prove the determinant-chart domination hypothesis.  It
only packages the additional forward-Jacobian upper bound needed for `baseJ`.

## Latest controller decision - 2026-07-01, Case 2 passive-source raw-Haar inverse-Jacobian domination

Lean now proves the conditional determinant-Haar to raw-order inverse-Jacobian
domination socket:
...
On the returned local determinant/pivot sector, the theorem assumes
...
This uses the retained-passive raw-order inverse-Jacobian change-of-variables
theorem and the new generic composed-map domination lemma
`map_comp_le_smul_of_map_le_smul_of_map_ref_eq`.
The Lean statement also returns `AEMeasurable rawMap (passiveSource.restrict V)`
for downstream map-domination consumers.

Focused direct elaboration, focused module build, full local
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
axiom probe passed; the axiom footprint is `[propext, Classical.choice,
Quot.sound]`.  Xhigh scouts `James the 3rd` and `Popper the 3rd` both
confirmed the boundary.

This is a socket, not a raw-Haar transport theorem.  The determinant-chart
domination hypothesis remains the next mathematical frontier.  With arbitrary
`passiveMeasure`, exact raw-Haar transport is false in general, since singular
passive measures can push to lower-dimensional or atomic raw images.

## Latest controller decision - 2026-07-01, Case 2 baseJ raw-image inverse-readback density

Lean now proves the concrete inverse-readback specialization of the Case 2
raw-image density handoff:
...
On the returned local determinant/pivot sector,
`topologyTupleEdgeRawOrderInverse (rawMap z) = Y z`, so
`rawDensity (rawMap z) = jacobianDensity z`.  The raw image is a.e. supported
on the raw-order source-recursive determinant chart, where the raw-order
inverse is continuous; this supplies the required a.e. measurability.  The
conclusion is
...
probe passed; the axiom footprint is `[propext, Classical.choice,
Quot.sound]`.  Xhigh Lean/math scout `Pasteur the 3rd` returned PASS.

This density is not the target-side inverse Jacobian density.  It is the
source-side retained-passive Jacobian product read back through the raw-order
inverse.  Nonclaims: no raw-Haar pushforward, no determinant-chart Haar
transport from `passiveSource`, no raw Haar normalization, no original
source-prior transport, no source-image coverage, no source-rank coverage, no
normal crossings, pole order, or RLCT extraction.

Next frontier remains the harder transport theorem: identify or dominate
`Measure.map rawMap (passiveSource.restrict V)` by the correct raw Haar
restriction with support and normalization under control.

## Latest controller decision - 2026-07-01, Case 2 baseJ raw-image density handoff

Lean now proves the exact raw-image density handoff for the globally
Jacobian-weighted passive-product theta reference:
...
On the local Case 2 determinant/pivot sector, with

```text
baseJ = passiveSource.withDensity jacobianDensity,
```

if `jacobianDensity` factors a.e. through the raw-order map as an
a.e.-measurable `rawDensity`, then
...
This is the strongest exact theorem presently available without proving
passive-product determinant-chart Haar transport.  The conclusion deliberately
keeps the actual raw image
`Measure.map rawMap (passiveSource.restrict V)` visible.

Focused direct elaboration, focused module build, full local
`lake build DLNFibre`, `scripts/sorries`, `git diff --check`, and direct axiom
probe passed; the axiom footprint is `[propext, Classical.choice,
Quot.sound]`.  Xhigh Lean/API reviewer `Anscombe the 3rd` and xhigh
math/source-boundary reviewer `Schrodinger the 3rd` returned PASS.

Nonclaims: no raw-Haar pushforward, no determinant-chart Haar transport from
`passiveSource`, no raw Haar normalization, no original source-prior
transport, no source-image coverage, no source-rank coverage, no normal
crossings, pole order, or RLCT extraction.

Next frontier: either prove the a.e. raw-density factorization for the
retained-passive determinant product, or prove a separate determinant-chart
transport theorem identifying the unweighted raw image with the correct raw
Haar restriction up to the necessary support and normalization data.

## Latest controller decision - 2026-07-01, Case 2 formal-product source-reference bridge from raw pushforward

Lean now proves the conditional formal-product/source-reference bridge for the
concrete Case 2 passive-theta endpoint source chart:
...
If the explicit raw-pushforward hypothesis
...
holds, then on every `chartPiece subset p13SourceSet` the p.13 formal-product
chart measure restricts to the chart-produced source reference on that piece.
On measurable chart pieces, this is also packaged with constant density `1`.
The proof uses the local two-stage
`rawMap`-then-`rawChart` measure identity and the p.13 formal-product raw-order
theorem directly; it no longer routes through the original-volume scalar
bridge.

This is a conditional consumer.  It does not prove the raw-pushforward
hypothesis, full raw Haar transport, original source-prior transport,
source-image coverage, source-rank coverage, scalar normalization, normal
crossings, pole order, or RLCT extraction.  Xhigh source-boundary and Lean/API
reviewers passed; the Lean/API suggestion to expose the direct restricted
equality as a separate theorem has been applied.

## Latest controller decision - 2026-07-01, Case 2 source-chart original-volume bridge from raw pushforward

Lean now proves the conditional original-volume bridge for the concrete Case 2
...
The first theorem says that if
...
then `Measure.map sourceChart (thetaReference.restrict V)` is the restricted
original edge-family volume on the named p.13 source set, multiplied by the
existing tuple-side Haar scalar.  The second theorem restricts to any
measurable `chartPiece subset p13SourceSet` and rewrites original volume as a
constant inverse-Haar-density perturbation of the chart-produced source
reference.

This is a conditional consumer.  It does not prove the raw-pushforward
hypothesis, full raw Haar transport, original source-prior transport,
source-image coverage, source-rank coverage, normal crossings, pole order, or
RLCT extraction.  Xhigh Lean/fidelity and mathematical-boundary reviewers both
passed the bridge after wording guardrails were applied.
...
The concrete source-image finite-integral wrappers no longer ask the final
chart-piece handler for a separate p.13 source-set containment hypothesis:

```text
chartPiece subset p13SourceSet
```

The strengthened local `V` is now produced using the p.13 source-set support
package, so the wrappers derive the old containment internally from:
...
This strengthening applies to both the source-image reference domination wrapper
and the bounded-density source-image wrapper:
...
`lean/DLNFibre/DLN/Aoyagi/RetainedPassiveCase2PassiveThetaJacobianMeasure.lean`.
It combines the concrete `Case2PassiveTheta` raw-order Jacobian sandwich with
the existing concrete theta local-domination residual-source socket.
...
The theorem first chooses an open Jacobian-unit neighborhood `U` of a passive
determinant-sector base point and defines
...
The upper side of the Jacobian sandwich gives finite scalar domination of this
`sourceMeasure` by `passiveSource`.  The residual-source socket then supplies a
second open neighborhood `V`; after restricting by `V` and mapping through
`case2PassiveThetaEndpointSourceChart`, the resulting source measure is
supported on the retained-passive p.13 local source and satisfies a.e.
residual square-sum positivity plus `residualNegPowerIntegrableOn`.

This is local domination bookkeeping only.  It does not prove
...
Lean defines the finite branch-state wrapper
`AoyagiIntroducedLabelBranchState` and proves:
...
The measure is the number of actual source labels not yet introduced:

```text
remaining L n s =
  #(actualWidthLabelFinset L n) - #(introducedLabelFinset L n s.S s.J)
```

Strict support growth is well-founded by descent in this finite count.  The
displayed Case 2 same-stage advance `(S,J) -> (S,J+1)` is a progress step under
the actual-width bound `J+1 <= n(S+1)`, and hence under the prefix-minimum
continuation bound via `prefixMinNat_le_width`.

Focused local `lake build`, direct `lake env lean -E warning`, full local
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
axiom probes passed; the four public declarations report only `[propext,
Classical.choice, Quot.sound]`.  Xhigh source/scope reviewer `Pauli the 4th`
and xhigh Lean/API reviewers `Chandrasekhar the 4th` and `Rawls the 4th`
returned PASS, recorded in
`threads/04-blow-up-certificate/review-a4-introduced-label-progress-kernel.md`.

This is not branch termination.  The next frontier is to connect real
branch-source-production payloads and branch guards to this progress relation,
then fill `SelectedEntryAtlasProducedBranchData` and
`SelectedEntryBranchTerminationData` without using fake `SourceProductionObligation`
wrappers.

## Latest controller decision - 2026-06-29, A2 all-pivot producer shell

The all-pivot selected-entry finite chart family now has an honest supplied
producer shell:
...
The shell fills the proved analytic fields over the same shared context
`selectedEntryAllPivotAnalyticAtlasContext hcenter chartEquiv`:
...
It takes the two true frontier fields as explicit inputs:

```text
sourceProduction : SelectedEntryAtlasProducedBranchData ctx BranchState
termination : SelectedEntryBranchTerminationData C BranchState
```

This is not a fake full producer: it does not manufacture branch guards,
source-production payloads, or a termination relation.

Focused local `lake build`, direct `lake env lean -E warning`, full local
`lake build DLNFibre`, `lean/scripts/sorries`, `git diff --check`, and direct
axiom probe passed; the declaration reports only `[propext, Classical.choice,
Quot.sound]`.  Xhigh source/scope reviewer `Darwin the 4th` and xhigh Lean/API
reviewer `James the 4th` returned PASS, recorded in
...
instead of adding wrappers around `SourceProductionObligation`.

## Latest controller decision - 2026-06-29, A2 all-pivot transition regular data

The all-pivot selected-entry finite chart family now has transition regularity
data over the same shared universal-domain context used by source coverage,
regularity, and Jacobian/volume records:
...
on the normalized target-coordinate nonzero overlap
`{x | allPivotTransitionDenom chartEquiv source target x != 0}`.  The map sends
`(u, residual)` to `(u * denom, normalized / denom)`, is continuous on that
overlap, lands in the target universal chart domain, and preserves the
represented center point.
...
returned PASS after documentation attribution was repaired; xhigh Lean/API
reviewer `Confucius the 4th` returned PASS, recorded in
`threads/03-block-product-reduction/review-a2-selected-entry-all-pivot-transition-regular-data.md`.

This proves no source production, branch termination, source-prior transport,
determinant-chart Haar transport, full supplied analytic atlas producer,
normal-crossing extraction, pole order, or RLCT.

## Latest controller decision - 2026-06-29, A2 all-pivot Jacobian/volume data

The all-pivot selected-entry finite chart family now has Jacobian/volume data
over the same shared universal-domain context used by the all-pivot
source-coverage and regularity records:
...
The proof is chart-by-chart.  For chart index `c`, it applies the one-pivot
chart-point pushforward at `chartEquiv c`.  The chart measure is
...
and xhigh Lean/API reviewer `Mendel the 4th` returned PASS, recorded in
`threads/03-block-product-reduction/review-a2-selected-entry-all-pivot-jacobian-volume-data.md`.

This proves no transition regularity between distinct selected-entry pivots,
source production, branch termination, source-prior transport,
determinant-chart Haar transport, full supplied analytic atlas producer,
normal-crossing extraction, pole order, or RLCT.

## Latest controller decision - 2026-06-29, A2 Case 2 passive-sector source-stratum-bounds finite-integral handoff

The passive-sector finite-integral theorem now has a source-stratum-bound sibling
in:
...
This theorem keeps the chart-produced passive/selected-entry measure but states
the local loss lower bound and density bounds over `nhdsWithin base
sourceStratum`, not over `nhdsWithin base localSource`.  The proof reuses the
passive-product residual-source handoff and then applies the boundary-explicit
consumer
...
The only bridge from the source-rank stratum to the retained-passive local source
is the self-base local inclusion

```text
Ulocal ∩ sourceStratum ⊆ Ulocal ∩ localSource.
```

Do not describe this as source-rank coverage.
...
This proves no determinant-chart Haar transport, source-prior transport,
...
## Latest controller decision - 2026-06-29, A2 Case 2 passive-sector finite-integral handoff

The passive selected-entry punctured-sector residual-source theorem now feeds
directly into the retained-passive p.13 local finite-integral consumer in:
...
The theorem returns two neighborhoods.  `V` is an open set in the passive
selected-entry coordinate domain, and for
`μ = Measure.map sourceChart (sourceMeasure.restrict V)` the theorem returns
an open edge-family neighborhood `U` of the fixed-base source family.  The
finite-integral conclusion is over `(μ.restrict (U ∩ sourceStratum)).prod ν`.

The residual-source handoff supplies `μ.restrict localSource = μ`, residual
square-sum positivity on `μ.restrict localSource`, and
`residualNegPowerIntegrableOn localSource μ t`.  The retained-passive local
consumer then uses the explicit local loss lower bound and density bounds on
`nhdsWithin base localSource`.  The theorem assumes `0 < t`, passes
`le_of_lt ht` to the residual-source handoff, and passes `ht` to the local
finite-integral consumer.
...
`Fermat the 4th` returned PASS after a documentation wording repair from
...
## Latest controller decision - 2026-06-29, A2 all-pivot regular data

The all-pivot selected-entry finite chart family now has chart and unit
regularity data over the same shared universal-domain context used by the
all-pivot source-coverage theorem:
...
The proof is chart-by-chart.  For chart index `c`, it applies the one-pivot
continuity and unit facts at the selected pivot `chartEquiv c`.  This records
continuity of the chart map `x_p = u`, `x_i = u r_i`, continuity of the
normal-crossing coordinate `u`, continuity of the loss unit
`1 + sum r_i^2`, and continuity/unitness of the formal Jacobian/prior unit
`1`.
...
## Latest controller decision - 2026-06-29, A2 Case 2 selected-entry Schur cleanup

The Case 2 selected-entry Schur cleanup from Aoyagi pp. 19-22 is now recorded
as source-facing finite matrix algebra in:
...
These are thin wrappers around the existing selected-entry and pivot API.  They
record `D = u E`, `E Q = [[1,0],[c,Z-ca]]`, and the displayed Aoyagi product
`D_chart * Q` as the post-`Q` Schur block.  The existing weighted row-operation
API already supplies the left cleanup, including the corrected old-diagonal
theorem
`weightedPivotBlockRowOp_mul_oldDiagonal_mul_smul_pivotPreQBlock_mul_pivotQ`.

The source-side convention is: if the old residual block is `D = u E`, absorb
`u` into the row weights `B' = u B_old`; after this absorption there is no
additional global `u`.
...
reviewer `Franklin the 4th` returned PASS, recorded in
`threads/03-block-product-reduction/review-a2-case2-selected-entry-schur-cleanup.md`.

This proves no analytic atlas coverage, chart-domain regularity, transition
...
## Latest controller decision - 2026-06-29, A2 Case 2 source chart-point coverage

The Case 2 residual-block certificate now has source-facing chart-point
coverage wrappers in:
...
These specialize the generic all-pivot selected-entry theorem
`selectedEntryCenterSqFormalJacobianChartFamilyCertificate.exists_sourceChartPoint_chartMap_eq_value`
to the residual-block Case 2 certificate, then add the coordinate readout
...
expedition-built finite coordinate bookkeeping only.

The full `DLNFibre` build passed via local `lake build` fallback; the
environment rejected escalated `scripts/lb` access to `$HOME/.lake-shared`, but
the shared package symlink was already in place.  After a docstring wording
repair, the focused `SelectedEntryNormalCrossing` build passed.  `scripts/sorries`,
...
the 4th` returned PASS, recorded in
`threads/03-block-product-reduction/review-a2-case2-source-chart-point-coverage.md`.

This proves no analytic atlas coverage, transition regularity, source
...
## Latest controller decision - 2026-06-29, A2 all-pivot source coverage data

The all-pivot selected-entry finite chart family now has a shared
universal-domain source-coverage record in:
...
The proof reuses the existing finite theorem
`selectedEntryCenterSqFormalJacobianChartFamilyCertificate.exists_chartPoint_chartMap_eq_value`.
For any center value, either the value is zero and any pivot chart maps
`u = 0` to it, or a nonzero coordinate is chosen as pivot and all other
coordinates are divided by that pivot.

Aoyagi prints a displayed top-left chart in the selected-entry calculation,
not this all-pivot analytic atlas as a source theorem.  Treat this theorem as
expedition-built finite coordinate coverage for the all-pivot certificate
only.

Focused and full builds passed; `scripts/sorries`, `git diff --check`, touched
Lean-file forbidden-marker scan, and direct axiom probes passed with
`[propext, Classical.choice, Quot.sound]`.  Xhigh source/scope reviewer
`Wegener the 4th` returned PASS after documentation repairs; xhigh Lean/API
reviewer `Nash the 4th` returned PASS.

This proves no chart regularity, transition regularity, unit regularity,
Jacobian/volume compatibility, full supplied analytic atlas producer, source
production, branch termination, original/source-prior transport,
determinant-chart Haar transport, normal-crossing extraction, pole order, or
RLCT.

## Latest controller decision - 2026-06-29, A2 one-chart source-coverage obstruction

The exact one-chart selected-entry context with `sourceDomain = Set.univ` now
has a formal source-coverage obstruction in:
...
If the center has a non-pivot coordinate `q`, the source-domain point with
pivot coordinate `0` and `q` coordinate `1` lies in `Set.univ` but cannot lie
in the one-chart image.  The obstruction is exactly that the selected-entry
chart map `(u, r) |-> (x_p = u, x_i = u r_i)` sends every pivot-zero chart
point to the origin.

This is an obstruction only for `selectedEntryOneChartAnalyticAtlasContext
pivot`.  It does not rule out source coverage for a smaller source domain, a
multi-pivot atlas, or a supplied analytic atlas producer, and it proves no
source production, branch termination, source-prior transport,
determinant-chart Haar transport, source-rank coverage, normal-crossing
extraction, pole order, or RLCT.
...
Classical.choice, Quot.sound]`.  Xhigh source/scope reviewer `Epicurus the
4th` and xhigh Lean/API reviewer `Nietzsche the 4th` returned PASS, recorded in
`threads/03-block-product-reduction/review-a2-selected-entry-one-chart-source-coverage-obstruction.md`.

## Latest controller decision - 2026-06-29, A2 one-chart analytic predicate data

The selected-entry one-chart records now inhabit the forgetful analytic
predicates in:
...
Each proof is the predicate-wrapper step
`<selectedEntryOneChartAnalyticAtlasContext pivot, <data>>`, using the
previously proved one-chart chart-regular, transition-regular, unit-regular,
and Jacobian/volume data records.  The Jacobian/volume-compatible predicate
inherits positive radii from `selectedEntryOneChartAnalyticJacobianVolumeData`;
the other predicates do not need radii.

This is useful API exposure for the supplied analytic-atlas interface.  It is
not source-domain coverage, not a full `SelectedEntrySuppliedAnalyticAtlasProducer`,
not source production, not branch termination, and it proves no source-prior
transport, determinant-chart Haar transport, source-rank coverage,
normal-crossing extraction, pole order, or RLCT.

Focused and full builds passed; `scripts/sorries`, `git diff --check`, touched
Lean-file forbidden-marker scan, and direct axiom probe passed with `[propext,
Classical.choice, Quot.sound]`.  Xhigh source/scope reviewer `Lorentz the 4th`
and xhigh Lean/API reviewer `Harvey the 4th` returned PASS, recorded in
`threads/03-block-product-reduction/review-a2-selected-entry-one-chart-analytic-predicate-data.md`.

## Latest controller decision - 2026-06-29, A2 one-chart regular data

The selected-entry one-chart context now has chart, identity-transition, and
unit regularity data in:

```text
lean/DLNFibre/DLN/Aoyagi/SelectedEntryOneChartRegularData.lean
```

Lean proves the helper continuity lemmas:
...
The chart map regularity uses the already-proved componentwise continuity of
`formalChartMap`; the unique certificate coordinate is the pivot coordinate.
The transition data is the identity transition on the single chart with
universal transition domain.  The loss unit is the continuous finite function
`1 + sum r_i^2`, and the Jacobian/prior unit is constant `1`.  No positive
radii hypothesis is needed.

Focused and full builds passed; `scripts/sorries`, `git diff --check`, touched
Lean-file forbidden-marker scan, and direct axiom probe passed with `[propext,
Classical.choice, Quot.sound]`.  Xhigh route scout `Dirac the 4th` accepted the
target at this narrow scope, and xhigh implementation reviewer `Mill the 4th`
returned PASS after documentation repairs recorded in
`threads/03-block-product-reduction/review-a2-selected-entry-one-chart-regular-data.md`.

This is one-chart regularity for the finite selected-entry microcertificate.
It is not source-domain coverage, not multi-pivot analytic transition
regularity, not a full analytic atlas producer, and it proves no source
production, branch termination, original/source-prior transport,
determinant-chart Haar transport, source-rank coverage, normal-crossing
extraction, pole order, or RLCT.

## Latest controller decision - 2026-06-29, A2 one-chart Jacobian/volume data

The selected-entry chart-point volume-form pushforward is now packaged as one
`SelectedEntryAnalyticJacobianVolumeData` record in:
...
Lean proves the positivity-free chart-point pushforward:

```text
map_formalChartMap_chartPointProductMeasure_withDensity_eq_restrict_image
```

and, for positive radii `forall i, 0 < R i`, defines:
...
`sourceDomain = Set.univ` and `chartDomain = Set.univ`.  The Jacobian/volume
data uses `chartPointProductMeasure pivot R`, density
`ofReal (chartPointDensity pivot x)`, source measure `volume`, and target
`chartMap pivot '' signedBoxSet R`.  Positive radii are used only for target
nonemptiness and nonzero restricted source measure fields; the pushforward
equality itself remains radius-degenerate-safe.

This is a finite selected-entry one-chart measure/Jacobian compatibility
package.  It is not a full analytic atlas producer, and it proves no source
coverage, chart/transition/unit regularity as full analytic data, source
production, branch termination, original/source-prior transport,
determinant-chart Haar transport, source-rank coverage, normal-crossing
extraction, pole order, or RLCT.
...
It works over an arbitrary coordinate-domain measure on
`eta x (center -> R)`, restricted to the open determinant-and-pivot-nonzero
sector.  The chart-produced source pushforward is supported on the
retained-passive p.13 local source, and the inverse residual readout
pushes it back to the residual-coordinate marginal `Measure.map Prod.snd`.

This removes no determinant-chart Haar or source-prior field.  It is useful
because it is the honest measure-level consequence of the pointwise
punctured-sector readout and makes the chart-produced target explicit.  The
next source-measure frontier remains one of:

```text
concrete passive-sector Haar/Jacobian transport,
external/original source-prior comparison by explicit density or mutual AC,
or a downstream consumer genuinely stated over this chart-produced sector measure.
```
...
supported finite-integral wrapper for endpoint-basis original `lossDLN`:
...
It composes the endpoint-basis original-loss source-stratum theorem with the
existing Case 2 support theorem
...
Under explicit rank equations `hprod`, `hr0`, and `hr1`, the final product
measure is rewritten from `(mu.restrict (U inter sourceStratum)).prod nu` to
...
`[propext, Classical.choice, Quot.sound]`.  Locke the 3rd xhigh review
passed.

Nonclaims remain strict: the rank equations are hypotheses, not source-rank
...
a chart-produced measure wrapper:
...
pointwise chart landing in the retained-passive p.13 local source.  It then
calls the already banked source-stratum-bound theorem.  Residual readout,
residual boundedness over the chart-produced measure restricted to
`localSource`, and all four source-stratum two-sided loss/density bounds remain
explicit.

Nonclaims remain strict: no chart landing proof, no selected-entry critical
inequality, no positive signed-box radius hypothesis, no source-density
upper/nonnegativity hypotheses, no residual integrability proof, no signed-box
source/image equality or source-rank coverage proof, no external source-prior/
...
It applies the local-source signed-box two-sided iff to
`source := Ulocal ∩ paperEndpointFixedBaseSourceRankStratum`, using the
filter identity
`nhdsWithin x0 (Ulocal ∩ sourceStratum) = nhdsWithin x0 sourceStratum` for
open `Ulocal` containing `x0`.  The signed-box pushforward hypothesis is for
this exact local piece.  This is intentional: a pushforward identity for a
larger chart source is not treated as automatically restricting to an arbitrary
smaller source.

Nonclaims remain strict: no proof of signed-box chart construction, no proof
that larger-source pushforwards restrict to open pieces, no source-stratum
...
the ambient retained-passive `TopologyTuple`:
...
The proof uses the ambient source-edge-family readout identity to rewrite
fixed-base edge matrices of the direct chart to `(ofTopologyTuple z).edgeMatrix`;
the latter is Borel-measurable by finite matrix operations and global real
matrix inverse measurability.  The residual-coordinate measurability API then
supplies the square-sum positive set.

This removes the explicit target positive-set measurability hypothesis from:
...
The determinant-chart pushforward identity remains explicit.  The finite-
integral theorem still assumes the local loss lower bound and density bounds.
This is not residual positivity, residual integrability, chart coverage,
original source-prior transport, source-rank coverage, normal crossings, pole
order, or RLCT.

Reproduction, statement card, and review:
`threads/03-block-product-reduction/reproduction-a2-retained-passive-direct-chart-positive-set-measurability.md`,
`threads/03-block-product-reduction/statement-card-a2-retained-passive-direct-chart-positive-set-measurability.md`,
and
`threads/03-block-product-reduction/review-a2-retained-passive-direct-chart-positive-set-measurability.md`.
...
After the direct-chart positive-set measurability hardening, this removes the
chart-side residual hypotheses from the Case 2 raw-order inverse-Jacobian
sockets while assuming only the determinant-chart pushforward identity, and,
for the finite-integral theorem, local loss and density bounds.  It does not
prove chart coverage, original external source-prior transport, source-rank
coverage, normal crossings, pole order, or RLCT.
...
The theorem still keeps the determinant-chart pushforward identity explicit.
Direct-chart target positive-set measurability is discharged by
`measurableSet_residualSquareSum_pos_retainedPassiveP13Canonical_directChart`,
and the theorem concludes a.e. positivity and finite negative-power integral
for the retained-passive determinant-chart residual.
...
handoff now has a continuous-positive-density wrapper:
...
nonnegativity, and eventual density boundedness as inputs.  Instead, it assumes
`0 < Rmax`, `ContinuousAt density (base,0)`, and
...
Focused build of `DLNFibre.DLN.Aoyagi.RetainedPassiveLocalMeasure` passed.
`scripts/sorries`, `git diff --check`, touched Lean-file forbidden-marker
search, direct axiom probe, and xhigh review passed; the declaration reports
only `[propext, Classical.choice, Quot.sound]`.  This is still a
chart-produced source measure theorem: it does not construct endpoint
equivalences, prove endpoint provenance, identify an original prior or
...
Next high-value A2 move from Pauli's scout: prove the retained-passive
source-edge-family measure comparison between direct determinant-chart
coordinates and the raw-order source chart with inverse-Jacobian density in
`RetainedPassiveLocalJacobianMeasure.lean`.  That is a real source-prior /
Jacobian transport step for the retained-passive chart layer, but still not
the original external DLN prior.
...
The theorem states that, for staged index `m <= M+1`, the Frechet derivative
of `retainedPassiveLowerLeftProductTailSum` agrees with
`retainedPassiveLowerLeftProductTailTargetStagedFDerivAt`.  The actual tail
sum has the widened bound `m <= M+2`, reflecting the zeroed-final slot.  The
base is `m = M+1` with value zero; the zero-current step uses the solved
top-left tangent; successor-current steps use `v.1 s.castSucc`; the induction
substitutes only the recursive successor derivative into the step-core
`dNext` argument.

Focused build of `DLNFibre.DLN.Aoyagi.RetainedPassiveCoordinatesDerivative`
passed, and the full `DLNFibre` build passed with only pre-existing warning
noise.  `scripts/sorries` reported zero forbidden markers, `git diff --check`
passed, forbidden-marker search on the touched file was clean, and the new
theorem axiom audit reported only `[propext, Classical.choice, Quot.sound]`.
Xhigh review passed in
...
order, or RLCT.  The recovery companion composes this exact staged equality
with `retainedPassiveFormalRawOrderJacobianAt_recovers_F3` and right-multiplies
by `(-(coord.solvedA1 (Fin.last ((M+1)+1))))^-1`; it performs no further
...
PASS by xhigh `Dirac` for the equality theorem and xhigh `Jason` for the
recovery companion.
...
clean, and the recovery theorem axiom audit reported only
`[propext, Classical.choice, Quot.sound]`.
...
`min(a,1)*(1+||x||^2) <= a+||x||^2`.  The stronger sharp scaling theorem below
is now the preferred fixed-parameter estimate.

A2 sharp positive-parameter fiber scaling has now landed:
`threads/03-block-product-reduction/reproduction-a2-sharp-positive-parameter-fiber-scaling.md`.
Lean proves the exact Haar-scaling identity
`∫ (a+||u||^2)^(-s) = a^(dim/2-s) * ∫ (1+||v||^2)^(-s)` for `a>0`,
the ball-restricted bound, and the supercritical finite ball corollary.  The
...
The solved-`A3` determinant-chart continuity layer is now landed/in progress
in `RetainedPassiveCoordinatesTopology.lean`; see
...
It proves continuity of the finite early lower-left product tail and every
solved full `A3` component on `{data // data.detChart}`.

Next retained-passive target after review/banking: prove continuity of the
nonredundant `edgeMatrix` source map on the determinant-chart subtype, using
the already banked continuity of `F2full`, `C`, `solvedA1`, and `solvedA3`.
Do not assert image openness, source-rank coverage, source/image equality,
measure transport, density/Jacobian accounting, normal crossings, pole order,
or RLCT.
...
A2 Case 2 product source-chart support in the named p.13 source set has landed
locally in `RetainedPassiveCase2PassiveThetaSourceImage.lean`:
...
This closes a naming/interface gap: the small-ball product chart was already
supported on the retained-passive local source, and that local source is the
identity preimage of the named p.13 source edge-family set.  The new theorems
expose pointwise and pushed-forward-measure support directly in the named set.

Next priority remains the harder source-image frontier: prove a genuine local
product-chart readback/image theorem or a density/Jacobian transport theorem
for a specified original/source measure.  Do not treat support in the named
source set as source-image coverage, source-rank coverage, original prior
transport, normal crossings, pole order, or RLCT.

A2 Case 2 product source-chart source-rank preservation has landed locally in
`RetainedPassiveCase2PassiveThetaSourceImage.lean`:
...
This closes a one-way image-membership gap: once the passive-theta base point
satisfies the explicit Case 2 rank equations, small p.13 regular coordinates
preserve membership in the same source-rank stratum.  Do not upgrade this to
source-rank coverage or image equality.

A2 passive-theta source-reference readback has also landed locally:

```text
exists_open_subset_measure_map_case2PassiveThetaEndpointSourceChart_map_readback_sourceReference_eq_self
```

This packages the chart-produced source reference
`Measure.map sourceChart (thetaReference.restrict V)` with readback
a.e.-measurability and exact pullback to `thetaReference.restrict V`.

Next priority: a concrete chart-piece bounded-density handoff or original
source-prior density theorem.  The clean wrapper still available is the
chart-piece version of the bounded-density readback domination theorem for the
concrete source chart.  The genuinely hard frontier remains proving that an
original/source measure satisfies the required density identity or domination
on the chart image.

A2 passive-theta chart-piece bounded-density readback domination has landed
locally:
...
This closes the concrete wrapper around the generic chart-piece handoff: a
supplied bounded-density identity on a source-side chart piece now gives
readback a.e. measurability and domination by `c • thetaReference.restrict W`
for any theta superset `W` containing the local source-chart domain.

Next priority: stop adding pure handoff wrappers unless a named downstream
consumer asks for a slightly different shape.  The substantive frontier is now
to prove a real source-measure density identity or domination theorem for the
original/source prior, or to build the local product-chart inverse/image
compatibility needed to make such a density statement natural.

A2 Case 2 raw-order source-chart density transport has landed locally:
...
This is the first precise bridge from the existing local raw-order p.13
two-stage equality to a weighted source-side measure identity.  It keeps the
raw reference equal to the chart-produced measure
`Measure.map rawMap (thetaReference.restrict V)`.

Next priority: prove a genuine raw/Haar or original-volume density identity
feeding this theorem, or explicitly decide that the next frontier should be
the local product-chart inverse/image compatibility needed to state that
identity cleanly.  Do not treat this bridge as determinant-chart Haar
transport, raw-order Haar transport, original source-prior transport,
source-image coverage, source-rank coverage, normal crossings, pole order, or
RLCT extraction.

A2 passive-theta raw-order reference image has landed locally:
...
This supplies the correct named raw-order image target for the concrete
passive-theta reference source.  It records support on the raw-order
source-recursive determinant chart and pushes passive-field domination through
`rawMap`, while staying strictly at the actual image-measure level.

Next priority: build the endpoint-image composition and source-chart handoff
wrappers recommended by the xhigh scouts, or feed this raw-order image measure
into the existing conditional raw-density transport bridge.  Do not replace
the named image measure by unrestricted raw Haar.

A2 endpoint-reference raw-order/source-chart handoff has landed locally:

```text
exists_open_subset_case2PassiveThetaEndpointReferenceImage_rawOrder_sourceChart_handoff
```

This closes the bookkeeping gap between the named endpoint image, named
raw-order image, and direct p.13 source-chart reference image.  The theorem is
only a local functorial image-measure identity; it does not identify any image
with Haar, original volume, or an original/source prior.

Next priority: feed this handoff into the raw-density transport bridge for the
endpoint reference, or begin the harder original/source-prior density identity
needed downstream.

A2 raw-order reference density transport has landed locally:
...
`case2PassiveThetaRawOrderReferenceImageMeasure`.  A supplied raw density on
that image measure now gives the corresponding source-chart image of the
theta-domain reference source with composed density.

Next priority: use this named bridge as the receiving socket for a genuine
original/source-prior density statement, or prove boundedness/integrability
for a supplied raw density on a concrete local chart piece.  Do not treat the
named raw-order image as raw Haar.

A2 raw-order reference same-shrink package has landed locally:

```text
exists_open_subset_case2PassiveThetaRawOrderReferenceImage_same_shrink_package
```

This packages the raw-order reference support/domination theorem, endpoint to
raw image handoff, raw to source-chart pushforward, and raw-density transport
on one common local `V`.  It closes the interface risk that downstream
consumers might accidentally combine separate existential shrinks.

Next priority: stop adding raw-order reference plumbing unless a concrete
consumer needs a different packaging shape.  The substantive frontier remains
a genuine original/source-prior density statement, a boundedness/integrability
theorem for a supplied raw density on a concrete chart piece, or the local
product-chart inverse/image compatibility needed to state such a theorem
cleanly.  Do not treat the named raw-order image as raw Haar.

A2 conditional original-volume/source-image same-shrink socket has landed
locally:
...
This uses one local `V` for source-chart readback/injectivity/measurable-image
data and for the conditional original-volume/source-image bounded-density
identity.  The raw-Haar raw-source pushforward remains an explicit hypothesis.

Next priority: decide whether to prove the separate local
change-of-variables/Jacobian theorem needed to remove that hypothesis, or to
thread this conditional socket into a downstream finite-integral wrapper that
already accepts such a raw-source hypothesis.  Do not present p.13 block
algebra as an unconditional original-volume transport theorem.

A2 conditional original-volume readback domination has landed locally:
...
This threads the conditional original-volume/source-image socket into the
direct readback-domination adapter on the same local `V`.  It avoids the
same-witness trap in the older bounded source-image finite-integral theorem.
The raw-Haar raw-source pushforward remains an explicit chart-piece
hypothesis.

Next priority: either build the downstream finite-integral wrapper that uses
this readback-domination adapter directly, or attack the genuinely missing
local change-of-variables/Jacobian theorem needed to remove the raw-pushforward
hypothesis.  Do not claim the latter from Aoyagi p.13 block algebra alone.

A2 conditional original-volume finite-integral wrapper has landed locally:
...
This is now the strongest Case 2 original-volume finite-integral statement in
the passive-theta branch.  It uses the same `V` for source-chart image data,
raw-source pushforward, readback domination, p.13 source-set containment, and
right-inverse data.  The raw-Haar raw-source pushforward remains explicit.

Next priority: the substantive blocker is the local change-of-variables /
Jacobian theorem that would prove the raw-pushforward or an equivalent
bounded-density comparison for `coordinateSourceMeasure`.  Short of that, only
minor packaging wrappers remain; do not confuse them with progress toward
removing the cited/assumed measure-comparison boundary.

A2 source-image forward domination from raw domination has landed locally:
...
This records the strongest honest consequence of a one-way raw-Haar upper
bound: `sourceImage <= constant * originalVolume`.  It deliberately does not
feed the original-volume readback finite-integral socket, which needs the
opposite direction or an equality on each chart piece.

Verification for this checkpoint passed through focused elaboration, focused
module build, full local `lake build DLNFibre`, `lean/scripts/sorries`,
`git diff --check`, and direct theorem axiom audits.  Both new declarations
report only `[propext, Classical.choice, Quot.sound]`.

Next priority: stop trying to weaken the raw-pushforward equality by forward
raw-Haar domination.  The next source-moving target is a formal-product /
source-image comparison on actual chart pieces, ideally a bounded-density
...
or an equality with a locally bounded density.  This is the bridge identified
by the xhigh Lean/API and source-boundary scouts as the smallest useful theorem
that can feed the existing finite-integral front ends.

A2 original-volume/source-image domination from formal-product domination has
landed locally:
...
This theorem turns the hypothesized useful comparison
...
```text
originalVolume.restrict chartPiece <= (cHaar^{-1} * D) *
  Measure.map sourceChart (thetaReference.restrict V).
```

Verification passed through focused warning-clean elaboration, focused module
build, dependent readback build, full local `lake build DLNFibre`,
`lean/scripts/sorries`, `git diff --check`, and direct theorem axiom audit.
The axiom footprint is `[propext, Classical.choice, Quot.sound]`.

Next priority: attack the comparison now isolated by this bridge.  The useful
target is not another finite-integral wrapper; it is a genuine local
formal-product/source-image domination or bounded-density equality on actual
chart pieces.  Xhigh scouts `Aquinas` and `Descartes` both reported that
Aoyagi pp.10-13 do not supply this unconditionally; it needs local
coverage/injectivity plus a bounded Jacobian or lower-density argument.

A2 reverse raw-source readback handoff has landed and passed full local
verification:
...
This means the useful reverse-domination socket now reaches the
original-volume readback conclusion directly.  The remaining high-value target
is proving, or explicitly citing/assuming, the reverse raw-source domination
for the concrete `coordinateSourceMeasure` on an actual chart piece:
...
If this can be supplied with finite `D`, the finite-integral wrapper can be
weakened from raw-pushforward equality to domination.  Until that theorem is
proved, keep the reverse domination as an explicit hypothesis.

The finite-integral wrapper has now been weakened and fully locally verified:
...
Verification passed through focused warning-clean elaboration, focused module
build, full local `lake build DLNFibre`, `lean/scripts/sorries`,
`git diff --check`, direct axiom audit, and xhigh read-only review.  The
