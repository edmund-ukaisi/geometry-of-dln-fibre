...
5. Repair A4 blow-up reproduction. Keep actual layer widths and prefix minima
   ...
   Lean-proved conditionally: if `step k=1` for `J+1<=k<mu_S`, then displayed
	   ...
	   displayed residual-row weight flatness. It still assumes the level and
	   variable maps are the source recurrence data, the introduced-label gap
	   holds, and row weights are generated by that recurrence. Keep the bridge's row and column
	   domains separate: residual rows are `J+1..mu_S`, residual columns are
	   actual-width `J+1..n_(S+1)`, and the selected variable is counted once in
	   the updated weights `u*b_i`. Next A4 target: use this interface to repair
	   post-pivot recurrence-state updates, or move to arbitrary-pivot chart
	   transport if that remains blocked.
