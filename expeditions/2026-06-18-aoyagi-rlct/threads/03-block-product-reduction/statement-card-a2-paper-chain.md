...
>   product splits as paper prefix followed by paper suffix.
> - **Assumed.** Field-vector-space chain and paper-order linear maps. No rank,
>   chart, topology, or analytic hypotheses.
> - **Cited.** None.
> - **Deferred.** Relating the reversed source-to-target `chainMap` to
>   `paperChainMap`; transferring the finite adapted-basis block corollaries
>   into paper-order notation; determinant/open-chart wrappers; the product
