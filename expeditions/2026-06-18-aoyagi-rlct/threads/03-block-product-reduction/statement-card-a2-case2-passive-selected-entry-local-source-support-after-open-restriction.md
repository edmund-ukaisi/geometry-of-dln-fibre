# Statement Card - A2 Case 2 Passive Selected-Entry Local Source Support After Open Restriction
...
domain `U`.  For any source-domain measure, the pushforward of its restriction
to `U` along the fixed-base p.13 source chart is supported on the
...
- The source chart is continuous on `U`: the proof packages `retainedData z`
  as determinant-chart subtype data on `U` and composes with
  `continuous_paperEndpointFixedBaseRetainedPassiveP13SourceChart`.
- `ContinuousOn.aemeasurable₀` gives a.e. measurability of the source chart
  for `sourceMeasure.restrict U`.
...
Independent xhigh reviews by `Singer the 3rd` and `Descartes the 3rd` passed;
see
`review-a2-case2-passive-selected-entry-local-source-support-after-open-restriction.md`.

## Nonclaims

This card does not prove source-image equality, source-rank support or
coverage, determinant-chart Haar pushforward, raw-Haar transport, source-prior
