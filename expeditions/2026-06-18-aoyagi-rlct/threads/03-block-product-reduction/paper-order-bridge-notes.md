...
`A^(1) ... A^(S)` without this reversal.

## Chart hypotheses

On PDF p. 11 Aoyagi assumes each layer matrix is near its base value, has rank
`r^(s)`, and satisfies `r^(s) >= r`, where `r` is the product rank. The proof
also works inside determinant charts:
...
The remaining elementary step is to record that determinant nonvanishing at
the base point gives a local open chart.

## Needed elementary bridge statements

1. A paper-order reindexing lemma: the reversed Lean chain has total composite
   equal to Aoyagi's `A^(1) ... A^(L)`, and Lean edge `j` corresponds to paper
   layer `s = L-j`.
...
normalization an elementary step; PDF p. 11 invokes Aoyagi Lemma 1 there, which
is outside the current Lean citation boundary. It does not say product rank
alone makes fixed top-left charts invertible. It does not justify the RLCT
shift on PDF p. 13; that remains analytic/certificate work.
