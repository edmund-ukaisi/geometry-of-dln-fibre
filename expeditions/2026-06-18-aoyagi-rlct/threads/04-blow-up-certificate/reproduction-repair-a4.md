...
- The recurrence divisibility and normalized displayed `Q/P` matrix identities,
  ...
  represented as `u * monomialRec step rowLevel`. The finite source-block
  tail lift is also proved: already-proved displayed residual-tail identities
  can be reattached below unchanged top rows by `fromBlocks` and
  `verticalBlock`. The full polynomial-coordinate pivot chart construction,
...
- The introduced-label finite-domain bridge is now proved conditionally:
  Lean's `introducedLabelFinset` enumerates the existing introduced-label
  predicate, and the Case 2 label-product/source-substitution wrappers apply to
  this finite domain. This still assumes the `level` and variable maps are the
  source recurrence data, the introduced-label gap holds, and row weights are
  generated by that recurrence. It does not prove the recurrence-state invariant
  or the source's Case 2 comparability sentence. Review artifact:
  `review-case2-introduced-label-gap-a4.md`.
- The recurrence-state interface is now packaged conditionally:
  `IntroducedLabelRecurrenceState` stores `level` and `var`, derives `step`,
  `weight`, and displayed residual-row weights from `introducedLabelFinset`,
  bridges integer least-value gaps to Nat-valued level gaps through the
  equality-only `IntroducedLabelLevelInvariants`, and feeds the packaged state into the
  displayed Case 2 source-substitution `Q/P` theorem. This still does not prove
  the recursive transition, the Case 2 gap, or source comparability. Review
  artifact: `review-case2-recurrence-state-interface-a4.md`.
...
Full transition formalisation should not start before these blockers are either
proved or split into named assumptions with a narrow certificate target. Narrow
finite algebra and bookkeeping lemmas may continue when their assumptions are
