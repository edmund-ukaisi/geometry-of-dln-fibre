...
| 04-blow-up-certificate | pen-and-paper/formalisation | blocked | A4 repair pass separates actual widths from prefix minima and image-checks the Case 2/terminal formulas. The printed Case 2 vector currently disagrees with the printed numerator increment unless `M(S)=M^{(S)}`; corrected Case 2 new-label certificate is packaged for `(S,J+1)` only; pivot-chart coverage, full vector invariant, `P` divisibility, `b'_i` bookkeeping, termination, and boundary cases remain open. |
