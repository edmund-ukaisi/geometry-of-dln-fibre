...
| 07 | formalisation (tide) | **S4b — the `SchurLoc`-linear (over-base) trivialization** `Total ≃ₐ[SchurLoc] SchurLoc ⊗ Fibre` → unlocks BOTH the S3 `Flat π` payoff (`Module.Flat.of_linearEquiv`) AND genuine S4 local triviality | in-progress (the convergent keystone) |
