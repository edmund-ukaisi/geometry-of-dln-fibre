...
- **Rung 4 (type-A Gabriel) — FULL BUILD chosen by operator; opened:**
  ...
  - **Remaining for the full Cor 2.9 orbit ↔ Kostant bijection:** the explicit normal-form object `∃ g ∈ G_d, g·A = ⊕ M_{ij}^{m}` (assemble `barcodeVertexBasis` → invertible `P_t` → match `intervalDirectSum`'s block encoding — the one heavy "block-encoding bridge" step, no new math). It is needed for the bijection's **injective** direction (same rank pattern ⟹ same orbit). Successor tide (thread 14).
