...
| 14-normal-form-object | formalisation | open (next) | **Cor 2.9 completion:** the explicit `∃ g ∈ G_d, g·A = ⊕ M_{ij}^{m}` (assemble `barcodeVertexBasis` → invertible `P_t` → match `intervalDirectSum`'s block encoding — the one heavy step) → orbit ↔ Kostant bijection (Cor 2.9), incl. the injective direction. + decorrelated audit of thread 13's new content (`Core.Gabriel`). |
