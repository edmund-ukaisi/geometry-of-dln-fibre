...
| 16-orbit-kostant-equiv | formalisation (teammate `normalform`) | in-progress | Package orbit ↔ Kostant as a single `Equiv` (`orbitKostantEquiv`) in a new `Core.OrbitKostant`; content in hand (complete invariant + `cumulDiffEquiv` + `baseChange_normalForm` realizability), quotient/`SuppArray` bookkeeping only. Then → close. |
