...
| 16-orbit-kostant-equiv | formalisation (teammate `normalform`) | closed | **LANDED** (`Core.OrbitKostant`, `ac40439`, green/axiom-clean): `orbitKostantEquiv : Quotient (orbitSetoid d) ≃ RealizableRank d` — the Cor 2.9 bijection (orbits ↔ realizable rank patterns) via the complete invariant + `quotientKerEquivRange`. Honest docstring: the Kostant-multiplicity-array RHS is the established `cumulDiffEquiv` composition (optional). |

**Expedition status: the core quiver engine (Bundle 1–2 spine) is COMPLETE + AUDITED + on `origin`.** Closing:
final synthesis/ROADMAP + the close-phase PR into `dev` (operator-gated). Trivial follow-ups (non-blocking):
the literal orbit↔{Kostant arrays} `Equiv` (one `cumulDiffEquiv` composition); a `longLine` style note in
`OrbitKostant.lean`.
