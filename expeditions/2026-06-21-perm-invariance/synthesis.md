...
## 2026-06-22 — EXPEDITION COMPLETE: full geometric (C,θ) permutation-invariance, zero-cited, all reviewer-SOUND
The central question is ANSWERED and carried to the destination. Full library green (3695 jobs), sorry-free, axiom-clean [propext, Classical.choice, Quot.sound]. THREE independent reviewer passes (fivegon; Thm5.5/Cor5.10; geometric transfer) all SOUND, with decorrelated Codex corroboration + exact-arithmetic numeric checks.

DELIVERABLES (all in DLNFibre, aggregated):
- Thm 5.5 (Poincaré product): `thm55` (QSeriesThm55).
- Cor 5.10 (combinatorial (C,θ) perm-invariance): `cCodim_comp_perm`, `numTop_comp_perm` (CThetaPermInvariance).
- GEOMETRIC transfer (the variety's actual (C,θ) of Σ̄^r, UNCONDITIONAL, over [IsAlgClosed k][CharZero k]): `geomCodim_comp_perm`, `ncard_topComponents_comp_perm` (CThetaGeometricPerm) — via codimRepCanonical_productRankLocusLE_eq_cCodim + numTop_eq_ncard_topComponents composed with the comp_perm.
- Reduce-to-sorted: `cCodim_comp_sort`/`numTop_comp_sort`. Order-reversal (#5, subsumed): `cCodim_rev`/`numTop_rev` (= comp_perm at Fin.revPerm).
ROUTE: fivegon (Thm 5.6 = RWY 2018, reproved via last-column PEEL transfer) → S1'–S4 → Thm 5.5 → Cor 5.10 (M5 Qseries-extraction bridge). Built a zero-cited q-series sub-library (P/Pm/Pmult/Qseries, N=1 Durfee, transferRHS, inverse-Pochhammer orthogonality — Mathlib v4.29 lacks all q-binomial/q-Vandermonde). S3 de-risked: ORTH is a single-var induction on P_mul_one_sub_succ, NO q-library.
