...
| **P2.5b-review** | reviewer | dispatching | 🔄 CRUX-review | **the `k`-rationality sub-crux:** does M3 density genuinely give a smooth `k`-RATIONAL orbit point over non-alg-closed `k` (not just smooth-over-`k̄`)? + B4 honesty/non-vacuity, GAP re-home cleanliness, R6 preserved, re-gate, Codex |
| P2.6 | — | — | ⏳ (held) | squeeze headline `varietyDim = finrank(range δ)` = B1 (`≤`) + B3∘B4 (`≥`) + `le_antisymm`. **Held until P2.5b-review clears** (composes B3∘B4) |
