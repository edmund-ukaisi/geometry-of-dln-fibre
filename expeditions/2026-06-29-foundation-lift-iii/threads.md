...
| **P2.5b** | formaliser | dispatching | 🔄 CRUX | B4 `SmoothCotangentDim` — `finrank(cotangent) = varietyDim` from a smooth `k`-rational point + dense `k`-orbit. Lift R6 onto the carrier; `[PerfectField k][Infinite k]`. **GUARD-first** the smooth-point/density hyps; **controller decorrelated review after** (the "dense `k`-orbit meets smooth locus over non-alg-closed `k`" sub-crux) |
