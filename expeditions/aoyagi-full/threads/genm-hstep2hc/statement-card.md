...
**DELIVERED so far (green, 0 warnings, pushed):**
- step-A `absorbedCoreConj_eq_blockSchur_synthetic`;
- **lemma-1** `deepestChain_toBlocks₁₂/₂₁/₂₂_eq_layer` (off-diagonal/core bridge trio);
- **lemma-2** `deepestPoint_interior_rows_vanish`, `deepBlkZ_interior_zero`, `deepBlkT_interior_zero`
  (interior-vanishing helpers; rows-mirror of the banked cols version).

**Refined remaining structure (KEY insight — one lemma is reused twice):** the two "framed↔decode"
steps both reduce to a single **frame-invisibility (FI)** lemma:
> **lemma-FI** `blockSchur_reindex_decode_eq_framedChain`: for a flat point `w`,
> `blockSchur (reindex (rThr)(rThr) (decode w) s) = blockSchur (deepestChain (framedParamsPivot (split w)) s)`
> (modulo the lemma-1 `finCongr` width relabel). Interior `s`: the two layers are EQUAL (deepest-interior =
> corM, via lemma-2 + `deepestChain_framedParamsPivot_blocks_of_frame_one`). Boundary `s∈{0,L-1}`: frame-
> invisibility of the endpoint frame (`blockSchur_lowerFrame_left`/`_rightUpper_right`, banked; unpack
> `framedParamsPivot` at layer 0 / last).

Then:
3. **step B** = FI at `w = wψ` (`split wψ = psiSplitRawGen q`) ▸ step-A ▸ banked `absorbedCoreConj_eq_schurCore`:
   `absorbedCore(psiSplitRawGen q) s = blockSchur(framedChain(psiSplitRawGen q) s)`.
4. **step 4a** = FI at `w = x`: `blockSchur(decodeChain x s) = blockSchur(framedChain(split x) s)` (base chains).
5. **lemma-Kcoup** (the crux) `Kcoup(framedChain(split x)) s = Kcoup(decodeChain x) s` — `partProd(F) =
   lowerFrame·partProd(D)` (`P11` cancels) + `Kcoup_zero` at k=0 + last-layer pivot.
6. **step D** = `schurTilde F s = schurTilde D s` from 4a + 5 (Invariant B `blockSchur(movedC)=schurTilde`).
7. **assemble hC** = step-B ▸ `psiSplitRawGen_deepestChain_hmove` ▸ step-D + width casts.
