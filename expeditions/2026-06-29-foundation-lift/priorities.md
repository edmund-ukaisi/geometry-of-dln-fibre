...
| P2-R2 | extract the matrix-rank core (`rank_le_iff_forall_submatrix_det_eq_zero` + supports + `rank_map_eq_of_injective`) | `RankLocusClosed.lean:46–166` | `Core/Matrix/RankMinors.lean` (ns `Matrix`) | **done** (`eff28654`, rebased onto p1'; `↔` + `rank_map_eq_of_injective`, `←` kept verbatim at `Fin`/`ℕ` generality [broader `Fintype` index = clean follow-up, not a gap]; DLN remainder local; sibling-clash cleared; green 3820 / axiom-clean) — **CRUX-review in flight** |
| P2-R3 | extract/generalise `GraphIdealHeight` (`height_graphIdeal_eq = Nat.card σ`) onto P2-R1 + #14 `Codimension`; confirm minimal hyps | `GraphIdealHeight.lean` | `Core/MvPolynomial/GraphIdealHeight.lean` (sibling, ns `MvPolynomial`) | **done** (`12fb9faa`; verbatim re-home; min hyps confirmed — height needs `[Finite σ]`+`[Finite τ]`, quotient-dim only `[Finite τ]`; dimension-dep quarantined from upstream-ready `GraphIdeal`; sibling-clash cleared; green 3820) |
