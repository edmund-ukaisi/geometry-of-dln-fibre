...
| 05r | reviewer (crux audit) | P1-R5 per-prime `hper` shape + transport soundness + P1 assembly | **closed** — **PASS** (per-prime `hper` genuine + byte-faithful; non-foldability Codex-corroborated, counterexample `A=k[t]₍ₜ₎×k[u]`; transport sound; assembly coherent). **PHASE 1 SIGNED OFF → PR #15.** Verdict: `threads/05-topdim-transport/review.md` |
| 06 | lean-formaliser (P2-R1) | extract graph-ideal package (`ker_aeval=graphIdeal`, primality) → `Core/.../MvPolynomial/GraphIdeal` | **closed** (`c7287232`) — `Core/MvPolynomial/GraphIdeal.lean` (ns `MvPolynomial`); verbatim re-home, all general; only `GraphIdealHeight` re-pointed; green 3819 / axiom-clean. Card: `threads/06-graph-ideal/statement-card.md` |
| 07 | lean-formaliser (P2-R2, **CRUX**) | extract matrix minor-rank core (`rank_le_iff_forall_submatrix_det_eq_zero` + `rank_map_eq_of_injective`) → `Core/.../Matrix/RankMinors` | **closed** (`eff28654`, rebased) — `Core/Matrix/RankMinors.lean` (ns `Matrix`); the `↔` Mathlib lacks; `←` verbatim at `Fin`/`ℕ` generality; DLN remainder local; green 3820 / axiom-clean. Card: `threads/07-matrix-rank/statement-card.md`. **Pending crux-review (07r).** |
| 07r | reviewer (crux audit) | P2-R2 minor-rank `←` direction + faithful split | **in flight** |
| 08 | lean-formaliser (P2-R3) | extract/generalise `GraphIdealHeight` (`height_graphIdeal_eq = card σ`) onto P2-R1 + #14 `Codimension` | **in flight** (branch `-p2`) |
