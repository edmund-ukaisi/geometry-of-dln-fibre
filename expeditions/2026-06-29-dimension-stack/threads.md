...
| 03 | lean-formaliser (R2) | polynomial-ring catenary + monic positioning → `Core.Dimension.Catenary` | **closed** (`574a9056`) — `height p + dim(R/p) = n` (`@[stacks 00OS]`) + monic-positioning crux (private Mathlib substitution re-derived in-repo, Codex xhigh PASS); 7+aggregator consumers re-pointed (4 transitive, L2); green 3819 / sorry-0 / axiom-clean. Card: `threads/03-catenary/statement-card.md`. **Pending controller re-gate + crux-review (04).** |
| 04 | reviewer (crux audit) | R2 fidelity: verbatim private-substitution copy + "any field" generality | **in flight** |
