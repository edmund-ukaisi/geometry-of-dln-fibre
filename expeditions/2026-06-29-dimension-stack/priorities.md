...
| E1 | étale local-dimension bridge + `Ideal.height_eq_under_of_etale` | `FlatQuasiFiniteHeight`, `SmoothLocalRelativeDimension` | `Core.Dimension.Smooth` | **done** (`614b8838`; verbatim re-home, non-circular local-dim bridge preserved; **no closure hyp at E1** — the `[IsAlgClosed]→[PerfectField]` win is E2's; re-gate green 3819; L3-clean) |
| E2 | smooth point ⟹ regular local ring, with the **`[IsAlgClosed] → [PerfectField]`** generalisation | `SmoothPointRegular` | `Core.Dimension.Regular` (new file) | **DONE** (`b9affa4a`; `smooth_point_isRegularLocalRing [PerfectField k]` `@[stacks 00TV]`; re-gate green 3819; capstone audit **PASS 3/3** [PerfectField sound · 00TV tag honest · re-home faithful]) — **surprise:** `[PerfectField]` was already banked (rlct-bridge `0084b645`), so E2 = verify + re-home + drop vestigial `IsAlgClosed.Basic` import (per-lemma trace: only `FormallySmooth.of_perfectField` is field-theoretic, `[PerfectField]`-only) |
| RF | retrofit DLN consumers (`codimRep`/`codimRepCanonical`/orbit-dim/fibre-codim) to tag to the general core; delete bespoke; green-gate | (consumers) | — | **DONE** (`a0cbcf74`; `codimRep_*` already `[Field k]` since R4; `DeepChartRing` prose + 2 more dead-weight `[IsAlgClosed]` shed; aggregator longLine 105→0; DLN payoff axioms UNCHANGED; re-gate green 3819) |
...
`[IsAlgClosed]`; fix the `DeepChartRing.lean:138` stale prose flagged by R4; the recurring `DLNFibre.lean` longLine — now `:422`, chased at RF not per-rung as line numbers shift).
