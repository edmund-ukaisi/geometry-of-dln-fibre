...
| E2 | smooth point ⟹ regular local ring, with the **`[IsAlgClosed] → [PerfectField]`** generalisation | `SmoothPointRegular` | `Core.Dimension.Regular` (new file) | **DONE** (`b9affa4a`; `smooth_point_isRegularLocalRing [PerfectField k]` `@[stacks 00TV]`; re-gate green 3819; capstone audit in flight) — **surprise:** `[PerfectField]` was already banked (rlct-bridge `0084b645`), so E2 = verify + re-home + drop vestigial `IsAlgClosed.Basic` import (per-lemma trace: only `FormallySmooth.of_perfectField` is field-theoretic, `[PerfectField]`-only) |
