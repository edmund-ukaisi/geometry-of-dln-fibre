...
## Tide progress log — update 12 (`e` descent infra LANDED: seams A+B + zero-tests; chart-eval lemma is next)

**Banked (committed, zero-sorry, axiom-clean):**
- `Core.PrincipalOpenComorphism.away_mk'_eq_zero_iff_exists_pow_mul_eq_zero` (`5c8833a3`): the GENERAL
  away zero-test over any base ring `P` (`mk' x gⁿ = 0 ↔ ∃ m, gᵐ·x = 0`) — the Ψ-target analog of the
  Σ-side primitive (`Away gF` localizes the poly-over-quotient `P = MvPolynomial SchurVar O(F)`).
- `Core.SchurFibreVanishing` (`63c5a6d8`, = **seam B / the route-3 wall**): `evalFibrePt` (point eval
  `O(F) →ₐ[k] k`) + `mvpoly_eq_zero_of_forall_eval_fibre` (`h : MvPolynomial σ O(F)` is `0` if its
  double evaluation vanishes on all `(s, y∈F)` — `MvPolynomial.funext` over infinite `k` + `coeff_map`
  + `Quotient.eq_zero_iff_mem`).
- `Core.ChartPointEval` (`4ba5e195`, = **seam A evaluation maps**): `evalF` (fibre-point eval),
  `evalP` (chart-point eval `P →ₐ[k] k` via `aevalTower`), `evalP_gF` (`= eval s detSchurS`, the unit
  bridge), `evalAway` (the lift `Away gF →ₐ[k] k` via `IsLocalization.liftAlgHom` when `detSchurS s≠0`),
  `evalAway_algebraMap`.

**Route-3 descent verdict (decorrelated Codex `codex/psi-descent-ring-answer.md`):** point-realization
+ target-side `mk'_eq_zero_iff` on `P` (route 1's zero-test, route 3's geometry). Symbolic route 2
does NOT suffice (confirms the =r/≤r catch: `vanishingIdeal Σ` is the full radical of rank-EXACTLY-r,
not symbolically generated by the `multPoly`-relations).

**THE remaining linchpin — the chart-evaluation lemma (seam A's deliverable, not yet built):**
`evalAway s B hs (chartPsiAeval p) = aeval (canonicalCoord (chartGauge(M(s))⁻¹ • B)) p`, where `M(s)`
is the Schur base matrix at `s`. Per-generator: `evalAway (chartPsiSub x) = canonicalCoord(chartGauge(M)⁻¹•B) x`,
chaining `evalAway ∘ chartPsiTower` (= eval the SchurLoc-coeff gauge poly at `s` via `schurToGfib`,
at `B` via `fibCoordT`) through `gaugeSub(endpointGauge⁻¹)`. Once this lands, the seam-C descent
obligation `chartPsi_vanishingIdeal_le` is: for `p ∈ vanishingIdeal Σ`, `chartPsiAeval p` vanishes
under every `evalAway s B` (because `chartGauge(M)⁻¹•B ∈ Σ` by LANDED `mult_chartGauge_inv_smul_fibre`),
so `mvpoly_eq_zero_of_forall_eval_fibre` (seam B) + the target zero-test gives `= 0`.

**Seam-C skeleton exists** (`DLNFibre/Core/ChartPsiDescent.lean`, UNCOMMITTED — created by the
killed `tide-e`/controller, 2 sorries: `chartPsi_vanishingIdeal_le`, `chartPsi_dsig_isUnit`; the
`liftₐ`/`liftAlgHom` chain around them is machine-checked). To discharge: sorry-1 needs the
chart-evaluation lemma + seam B; sorry-2 (`chartPsiQuot dsig` is a unit) needs the symbolic
product-reconstruction (`gaugeEquiv(endpointGauge⁻¹)(ΔPdeep) ↦ unit·gF` via `det` of the gauge
transport — the `chartPsiTower_gaugeEquiv` bridge applied to the pivot minor).
