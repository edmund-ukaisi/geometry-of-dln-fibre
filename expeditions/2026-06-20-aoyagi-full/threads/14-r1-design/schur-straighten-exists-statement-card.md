...
- **Status:** `sorry-free` (awaiting fidelity review). Branch `origin/fm2/crux-recover` @17bcd33
  (Lean @393b9a7). File `lean/DLNFibre/DLN/RLCT/Validate/GeneralR1Recursion.lean`.
- **Seat:** fm2 (formaliser). Build GREEN; axioms clean-three `[propext, Classical.choice, Quot.sound]`.
- **SPECIFY CHANGE — controller adjudication needed.** The handed *abstract* statement was unprovable
  (see below). The `IsSchurStraighten` STRUCTURE is UNCHANGED; the theorem's hypotheses were re-attached.
