...
| **F2 (eigendecomp primitive)** | `RouteMSJMeasurableEigendecomp` (assembly @origin/genm-sj5-brickF2-assembly) | ladder (adjud a3648768 ✅ LABOUR) | 🔄 2 sub-tides | VERDICT LABOUR, no wall — but a ~2200-4500 LoC / **6-module MOUNTAIN** (every spectral-measurability primitive Mathlib-absent; decorrelated tide+Codex). Assembly banked (`frame_diagonalizes` clean-three + 2 recipe'd sorries). **F2a** ✅ LANDED @d6bc1ed4 clean-three (`measurableEigenvalues₀`, Vieta+Lusin–Souslin) — doubly load-bearing (F2 conjunct-i + the plumbing's hGmeas source). **F2b** (#164, aa147a6d): eigenframe via zero-safe Lagrange projector + measurable pivot; Layer A COMPLETE (@16860fc2, blessed trace non-vanishing held), now Layer M/T. On F2b: reconcile the F2 assembly (measurableEigendecomp clean-three) → F1 imports it. discuss#140 |
...
**★ Two more stub refinements (2026-07-13, plumbing findings, certs-backed):** the `headSplit_domination` stub is under-constrained; add (a) `hε'le : ε' ≤ ε/√(M₁·M₂)` (the shell⊆good containment threshold, S1 cert §B.3 — deeperFlag_spineToCore discharges by le_refl at ε'=ε/√(M₁M₂)); (b) `hjr : (j:ℕ) < min(M₀−t)(M₁−t)` STRICT (the saturated shell j=r is a SEPARATE arity-(L+1)-IH branch, final-assembly cert). Controller owns at integration: edit the RouteMSJDeeperFlagCore stub + thread through deeperFlag_spineToCore + **VERIFY the j=r saturated shell has its separate discharge in deeperFlag_shell_le/T-peel (per cert: zero-freed-corner arity-IH) — soundness check, must NOT be dropped.**

**★ GLUE-2 arch corrected (2026-07-13):** my "drop-corank+D-B" finiteness route was UNSOUND (missed the ab/2 corank regularization; threshold undershoot; numeric witness (3,3,3) c'∈(3,3.5)) — the corank is load-bearing, A and B collapse to the irreducible cross-term-drop crux. Corrected: isolate `pivotDom_finiteness : (RHS<⊤)→(LHS<⊤)` (corank KEPT) as the ONE sorry; (b) proves it via the cert mechanism (fresh tide, pending the isolated statement). discuss#142.
