...
- **step2** (a27134b, genm-sj5-step2 @8a82f11b) — head-split crux hole (a) `pivotPeel_domination`. ★ FAVORABLE RECALIBRATION (step2 self-recon): NOT a from-scratch module — it's a COMPOSE of the BANKED front-first two-block-radial toolkit (genm-sj5-schur/#121/#125: `RouteMSJFrontFirst.frontFirst_g_le_of_sector` = the pivot bound; `RouteMSJFrontSpectral.frobSq_mul_eq_sum_eigenvalues` = the Σσ² identity already built; β-CoV; `twoBlock_radial_le`; Ky-Fan; D-B) + 2 GENUINE glue pieces: **(a) sector-from-shell** (discharge frontFirst's held #130 sector hyp for Q_stack on the shell via couplingfin §3-4 shell+Ky-Fan — OBSTACLE B), **(b) corank ½ab reconciliation** (add the S3 corank charge, reconcile front-first's α with c'−½ab + A_cor-coupling — OBSTACLE A). BLESSED: compose the banked side (low-risk) + checkpoint-first on (a)/(b). Scaffold (5 lemmas) banked. **SINGLE-WRITER**.
