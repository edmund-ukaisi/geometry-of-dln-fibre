...
| **RHS≠0 + uzero** (helper) | aa56113 | RouteMSJPivotDom on genm-sj5-pivotdom | pivotDom_RHS_ne_zero DONE axiom-clean (local @58568ab8); pivotDom_uzero assembly complete, ~15min tactic polish (all 6 sub-lemmas build, NOT blocked). Will push genm-sj5-pivotdom + SHA when uzero green+axiom-checked, then L=1 scope-first. |
...
|---|---|---|---|
| **DecoratedStepHyp skeleton + hole-scope** | stepasm a7673edf | genm-sj5-stepasm @a676232d | ★ SKELETON DONE — `decoratedStepHyp_dispatch : DecoratedStepHyp adm` sorry-free, green (8370), **5 named holes** (not 3). Seam report IN. NO j≤r/j<r gap (shells partition {j<r}⊎{j=r}; j=r degeneracy rests on F/D content — CRUX confirms). Waist hole (c) matches waistpin route-(a) (contract item 2 = the commutation). 2 SEAMS: (i) gate mismatch binding-cut hpiv vs M₁<min(tail) vs per-cut hpiv → #172 (waistpin); (ii) hole (d) needs NEW decorated-peel (banked peels trivial-decoration only) → #171. NOW scoping (d)/(e)/(b) labour-vs-new. |
