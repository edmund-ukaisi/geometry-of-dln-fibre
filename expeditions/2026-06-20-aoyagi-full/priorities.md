...
**★ NOW (2026-07-14 WALL-FALSE / RE-SCOPE) — SOLE active priority = the shell-restricted RE-SCOPE + its tractability read.** Strategy (Route A DecoratedDescent → (□) → mint #108) UNCHANGED; but the head-split's hard analytic core was NOT done — a decorrelated obstruction hunt (thresholdhunt) CERTIFIED the "off-shell full-block" wall `frobSqBlockFull_lt_top` FALSE (witness (6,6,6)@u=4 diverges; true λ=minAdm(M)/2, the claimed (minAdm(redChain)+ab)/2 overcharges at non-argmin cuts; the SHELL σ_min(hsQ)≥ε is load-bearing — couplingfin vindicated; hfloor is NOT a substitute). So the full-block arc (t2adjud→pradial→finfin→shearfin→flagpeel) built on a false object; flagpeel killed. RE-SCOPE = restore the shell-restriction → the shell-restricted coupled-residual (= step2's OBSTACLE-A, the hard piece the detour was avoiding; couplingfin-adjudicated bounded but NEVER built). **`rescoperecon` is reading the driver trace + banked shell-restricted state + a fresh LABOUR-vs-WALL tractability read** — that verdict decides: commission the shell-restricted build (labour) OR surface a terminal wall to the operator. Endgame is NOT near the mint. Banked-clean + shell-compatible: brick B (re-examine — feeds the false wall), j=0 sector borderline, (b) arity-IH wiring, facts 1/2. Full detail = synthesis.md WALL-FALSE FRONTIER + endgame-lanes.md thresholdhunt entry + discuss-at-close.
