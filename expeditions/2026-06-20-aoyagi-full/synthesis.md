...
- **IN FLIGHT (a3901f84, worktree):** (1) item-4 → concrete `composeFold_abs_det_leafH`; (2) the RATE of `composeFold fs` — PATH B (direct chain telescope on the factors, may avoid the bridge) first, PATH A (the bridge `composeFold fs = phiFlatStruct` + transport `routeMCore_phiFlatStruct`) fallback; (3) assemble `nodeChartGeneral` → discharge the atom ∀M.
