...
**LIVE (2026-06-27, UPDATE-137):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **(2,2,1) validate-small DONE (`6fb2e15d`)**; checkpoint (b) SETTLED the math (UPDATE-137): pure-radial fails the rate on split-codim → Schur coupling load-bearing ∀M; `e_M` evaporates; order=`u^1` is the banked telescope. **The wall is now a pure Lean-ROUTE decision** (2a re-derive via forward telescope / 2b the persistent bridge / A-revisited fix the decoder det). **IN FLIGHT (`decoder-fix`):** ADJUDICATE + COMMIT to one build-ready route + (3,3,3,3) check → then `option-c-chart` (at rest) builds it → discharge the atom ∀M. Watch: route MUST land; else escalate to operator.
