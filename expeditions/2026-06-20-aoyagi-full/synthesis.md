...
**LIVE (2026-06-28, UPDATE-230):** CHARGING (A), four legs converging — **all three legs volume-not-wall, no open design/soundness risk except 3289**; each leg's prereqs are now bedrock, with ONE final standalone/coupled unit each, all on fresh-context tides. **R1-UPPER carving — ~1 SORRY FROM DONE on the canonical lineage:** `origin/genm-carving` independently built the FULL machinery (cellOfG, bgShiftG, `resolvedShiftRG_le` #136, the full reshape coreJoinG/matToProdG/matToFlatG/gFlatG_cover_sum/matBoxG_outer_flat, stepShearG, a.e.-pos, its own cover) with the assembly skeleton present — ONLY `schurRatioResidGen_mid` (line 1213) is sorry. `genm-assemble` (re-based onto origin/genm-carving) verifies green + CLOSES that single sorry → R1-UPPER math done. (Reconciling genm-carving's lineage into the canonical aggregator — cover/names — is a separate follow-on, genm-firing's role.) **R1-LOWER (1,1):** chart substrate + U-foundation bedrock (rate/MP/connection/embedding/flat-rate `routeMCore_phiSm_offpole`/`frontU`). Remaining = the subBox/divergence assembly (Tonelli factor + `∫|u_p|^{−2c'}=⊤` + the rest-witness containment trick) → fresh tide `genm-subbox` (base genm-smeared-build @ e3a71a0d; Codex design banked; genm-smeared-build on-call). **L2 bridge:** real Ψ + S3 axiom-clean (`8b5645cf`); gating brick pre-exists ⟹ S4/S2/S6 PURE PLUMBING → `genm-l2cle`. **D1** wires on the bridge. Headline = A2 ▸ D1-via-bridge ▸ R1; general-L gated only on **3289** (#120).
