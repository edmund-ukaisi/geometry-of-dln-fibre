...
**LIVE (2026-06-27, UPDATE-161):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, via a 4-branch `NodeAchieverChart` (RATE `leaf_integrand` a.e. + DET/MP `cov`): **INTERIOR ✓ integrated · CLEAN ✓ integrated · SMEARED (cov ✓ via route-(b) MP; atom in flight, `genm-boundary` a223…) · L=1 banked** → then the 3-way+L1 `nodeChartGeneral` assembly (incl. WALL 1 `BoundaryClean ⟹ NoInteriorBothDrop`).
