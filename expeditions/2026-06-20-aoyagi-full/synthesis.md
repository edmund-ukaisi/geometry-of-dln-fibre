...
**LIVE (2026-06-28, UPDATE-246):** CHARGING (A) — all legs in their final unit, no open design/soundness risk except 3289. **R1 ladder now fully mapped + VERIFIED-bounded** (genm-carving's 2 audits + controller grep): the top layer `resolution_charts` is READY-TO-WIRE with ZERO new bricks (`resolution_charts_of_layerCover` + `routeMLayerCover_of_atoms` + transport/bridge are sorry-free; the one real code-sorry in that stack IS the hdiv atom). So R1 = {carve (genm-firing) + `routeMCore_le_matBox` + premise-reduction (genm-n4) = hfin} + {the NodeAchieverChart hdiv atom (genm-subbox #142) = R1-LOWER} + sorry-free wires. No top-layer surprise. **R1-UPPER hfin (`routeMCore_threshold_lt_top`) needs TWO pieces (precision correction, genm-carving audit + controller-verified):** (i) the **carve** `schurRatioResidGen_mid` (the SchurCore inner-finiteness) — every ingredient probe-confirmed on `origin/genm-firing @ e2941aa1`; SOLE remaining = the ~80-100-line capstone wiring (the Fin-index reconcile, cleanest via `obtain ⟨m,rfl⟩:∃m,r=m+1` — sidesteps the cast) → **`genm-firing`** finishing it (gated: forced #print axioms + valve; genm-capstone = fallback); AND (ii) the generic **`routeMCore_le_matBox`** (the ∀M M-shape reduction — exists only for r=3; the "N4 long pole") + the `routeMCore_threshold_lt_top` re-point (RouteMSchur:426, SKELETON sorry) → fresh tide **`genm-n4`** (base-verified e2941aa1, in parallel; gated modulo the carve's schurRecStep_four). So **closing the carve alone does NOT auto-green R1-UPPER hfin** — it needs carve + routeMCore_le_matBox + a 1-line `schurGen_lt_top_modulo_recStep schurRecStep_four` wire + the `schurRecStep4_stub` drop. (`resolution_charts` Skeleton:1234 = the SEPARATE higher RLCT-layer above hfin.) Aggregation PRE-STAGED (#143). **R1-LOWER (1,1):** chart substrate + U-foundation bedrock; SOLE remaining = the subBox/divergence assembly (Tonelli + `∫|u_p|^{−2c'}=⊤` + the rest-witness containment) → `genm-subbox` (#140-142). **L2 bridge:** real Ψ + S3 + the lens-decomposition linchpin + the S4 toolkit banked; the W⁻¹-1 deriv-0 crack is now LANDED (genm-l2cle @5d3184a6 — the one conceptually-new S4 piece); S4 remaining = PURE MECHANICAL ASSEMBLY (~250-400 LoC) → **`genm-l2cle`** continuing S4→S2 (has the latest + full toolkit; gated). genm-l2leaves stood down (was on the stale 41fbce4e — would re-derive the crack; Item-52). **S6** (LDU-energy geometric leaf) = a later dedicated tide. **D1** wires on the bridge. Headline = A2 ▸ D1-via-bridge ▸ R1; general-L gated only on **3289** (#120).

**⚙ UPDATE-246 (2026-06-28, heartbeat) — R1 TOP LAYER verified READY-TO-WIRE (0 new bricks); R1 ladder fully mapped + bounded. Good news (no second surprise). ⚙**
