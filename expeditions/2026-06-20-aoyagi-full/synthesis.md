...
**LIVE (2026-06-27, UPDATE-138):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **(2,2,1) validate-small DONE; ROUTE 2a COMMITTED + build-ready (UPDATE-138, `c891bfb3`):** `φ_det := phiGen(B_det)` — rate via the decoder-agnostic `routeMCore_phiGen` (one line, keystone verified), det via coordinate factorization; 2b retired (false equation). Remaining = `φ_det_eq` (single-decoder boundary-prefix induction, heavy-but-tractable, anchor-templated). **IN FLIGHT (`option-c-chart`, RESUMED):** build 2a, validate-small on (2,2,2) [2-drop multi-boundary] first → ∀M lift → discharge the atom ∀M. decoder-fix idle/on-call.
