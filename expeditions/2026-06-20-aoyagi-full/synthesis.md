...
- **KC1 step-2 architecture DECIDED via the diff-gate = (B)** (genm-l2 proposed, BETTER than my literal (A) framing — accepted, no defense). The alignment-conditional invertible-leading-block fact is a STANDALONE lemma `deepestPoint_leadingBlock_isUnit (htop)`, NOT a conjunct on `IsDeepLayers` and NOT an htop change to `deepestPoint_exists`. Rationale: `IsDeepLayers` is HONESTLY general-B (a deepest point exists for any B); the invertible-leading-block is ALIGNMENT-conditional (the Q1 witness `[[0,0],[1,0]]` shows it's not general) — folding it in would mislabel the predicate (name≠content). (B) keeps the general property general + the conditional fact visibly htop-gated + zero projection churn in DeepestFrame/DeepestGaugeChart + soundness-critical `deepestPoint_exists` signature stable. Proof = the rank argument (top-r-rows(B')=top-r-rows(U)·V, V full-row-rank ⟹ top-r-rows(U) invertible). block_elimination/:337 byte-untouched. genm-l2 building; sends the diff BEFORE commit (STOP if any :337 edit).
