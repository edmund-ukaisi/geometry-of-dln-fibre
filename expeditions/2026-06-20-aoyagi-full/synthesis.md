...
**LIVE (2026-06-27, UPDATE-134):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **IN FLIGHT (`decoder-fix` pen-and-paper, RESUMED): KC1** — pin the achiever path ↔ minAdm correspondence ∀M (`active.card = minAdm`, the Aoyagi-peel → chain-blow-up bridge), the crux blocking the general active-set; + sketch **KC2** (the general full-rank `Q_M` pack). THEN the formaliser builds (C) → `nodeChartGeneral (hpos)` → discharge the atom ∀M. (Kill-flag: KC1 with no uniform ∀M rule = research wall → surface to operator.)
