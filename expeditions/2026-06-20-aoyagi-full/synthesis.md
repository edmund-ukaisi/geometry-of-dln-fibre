...
**LIVE (2026-06-28, UPDATE-229):** CHARGING (A), four legs converging — **all three legs volume-not-wall, no open design/soundness risk except 3289**; each leg's prereqs are now bedrock, with ONE final standalone/coupled unit each, all on fresh-context tides. **R1-UPPER carving:** EVERY isolable brick axiom-clean (cellOfG carve, schurSc_readback, #133 + #134×2 + T-peel; #136 `resolvedShiftRG_le` on genm-carving). Remaining = the COUPLED core (zslotG decoder threading the pivot-perm + zEG MP carve-equiv + Δof_eq_zE + per-z chain + `schurRatioResidGen_mid` assembly, #137) → fresh tide `genm-assemble` (base genm-firing @ f86590cf; reconciles genm-carving's #136; genm-recstep on-call for the pivot-perm). **R1-LOWER (1,1):** chart substrate + U-foundation bedrock (rate/MP/connection/embedding/flat-rate `routeMCore_phiSm_offpole`/`frontU`). Remaining = the subBox/divergence assembly (Tonelli factor + `∫|u_p|^{−2c'}=⊤` + the rest-witness containment trick) → fresh tide `genm-subbox` (base genm-smeared-build @ e3a71a0d; Codex design banked; genm-smeared-build on-call). **L2 bridge:** real Ψ + S3 axiom-clean (`8b5645cf`); gating brick pre-exists ⟹ S4/S2/S6 PURE PLUMBING → `genm-l2cle`. **D1** wires on the bridge. Headline = A2 ▸ D1-via-bridge ▸ R1; general-L gated only on **3289** (#120).
