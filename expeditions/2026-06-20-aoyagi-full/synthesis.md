...
**LIVE (2026-06-26, UPDATE-129):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M` (ONE flat full-ambient map `phi` with BOTH: RATE `routeMCore (φ u) = u²·V` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- PATH B is DEAD (`routeMCore` reads via `paramsEquivFlat.symm`); the bridge is irreducible but blueprinted as a per-layer `funext s` (OPTION-1, Params-conjugated factors — UPDATE-130).
- **IN FLIGHT (`option1-bridge`, fresh worktree tide off `fa6d185c`):** OPTION-1 — (1) per-layer Params-split CLEs; (2) `paramsConjBlockFactor` (reuses `conjBlockFactor` via `paramsEquivFlatCLE`); (3) variable-N `chainVarMap` (det 1); (4) `fs` foldr deepest-first; (5) **the `funext s` bridge** (THE CRUX, validate-small on 334/4422 first); (6) `hdet` assembly; (7) `nodeChartGeneral` → discharge `routeMCore_box_diverges_achiever` ∀M.
