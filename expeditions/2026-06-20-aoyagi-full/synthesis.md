...
**LIVE (2026-06-27, UPDATE-151):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **(2,2,2) MULTI-BOUNDARY VALIDATE-SMALL COMPLETE + INTEGRATED (UPDATE-141, `a6d96d48`, green 8481):** atom discharged for (2,2,2) via the general route-2a machinery; the `φ_det_eq` bridge (`chartParamsGen_eq_chartParams222`) + the cast quirk (banked in lean/CLAUDE.md) both de-risk the ∀M lift; det genuinely Schur-coupled (`|x0|²·|x4|`). **R1-lower atom: 4 anchors discharged + rate ∀M banked; the ∀M chart is the last piece.** ∀M-lift: step 1 (path) + the 2- and 3-boundary validate-smalls ((2,2,2)/(3,3,3,3)) DONE; route (i) committed; the bridge template validated at 2+3 boundaries; (3,3,3,3) det = `RouteM3333Atom` exactly. **BUILD PHASE (UPDATE-148): the achiever path + ALL RATE fields DONE ∀M except ONE witness (`ab7c4238`, green 8486)** — `Umeas` done, `Ubound` done-given-witness, the MvPolynomial encoding banked. **STRUCTURAL RECALIBRATION (UPDATE-151):** the achiever chart `phiFlatStructV` is degenerate for ~19% of M (boundary-only-drop); the rate-side is COMPLETE only for the 285 interior-drop M. Path = OPTION A (3-case ∀M chart). **IN FLIGHT (`genm-witness`):** the exhaustive-validation-gated unified re-design (colPath interior-drop + radial? boundary-drop + L=1 `DeepestBaseL1`) → v2 certificate → build → rate-side ∀M COMPLETE → phase 4 det bridge funext → phase 5 cov+`nodeChartGeneral`+atom ∀M. Honest scope: the ∀M chart is now a 3-case construction (more tides than the prior ≈2 estimate); then R1-UPPER/L2/D1/headline. NEW gate: exhaustive validation, not Codex-assert (the cert was wrong 3×). Build tides merge origin first (Item-36). Others idle on-call.
