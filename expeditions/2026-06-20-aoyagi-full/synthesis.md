...
**LIVE (2026-06-27, UPDATE-131):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **IN FLIGHT (a008af4c, RESUMED off its worktree):** the 4-brick crux (thread.md UPDATE-9) — (a) per-layer Params-split CLEs [hardest, current]; (b) Params-level layer-ops; (c) `funext s` matching; (d) leafH summation. Directives: validate-small on a LAYERED case ((2,2,2)/(3,3,4) — the 4422 anchor is radial-only), dependent-dim cast kernel, keep the genuine spectator exponents. Then `nodeChartGeneral` → discharge the atom ∀M.
