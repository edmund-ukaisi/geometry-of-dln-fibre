...
**LIVE (2026-06-27, UPDATE-136):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **(2,2,1) VALIDATE-SMALL DONE (`6fb2e15d`): option-(C) proven end-to-end through the general machinery on a layered node** — `nodeChart221` (S2-free) → atom discharged (4422 footprint). The approach is validated. **IN FLIGHT (`option-c-chart`, RESUMED): checkpoint (b)** — the ∀M `nodeChartGeneral (hpos)`: opaque-width `e_M : Fin N ≃ FlatIdx M` (hardest, rest-here) + T* argmin + per-M pack + L≥3 spectators → discharge the atom ∀M. decoder-fix on-call for design walls.
