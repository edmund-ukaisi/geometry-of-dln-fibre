...
1. **hGne** (L2+R1) — RESOLVED → **STATED headline non-degeneracy hypothesis** (fm3 verdict (b), FORCED): interior M_s=H_s−r=0 ⟹ prod_M≡0 ⟹ dlnLoss M 0≡0 ⟹ RLCT=⊤, but lambdaCore M finite ⟹ the R1 identity is LITERALLY FALSE there; the domain MUST exclude it (forced by the formula's correctness). **Headline gains `∀ interior s (0<s<L), r<H_s` (interior M_s≥1)** — fenced as **OUR FORMALISATION CARVE-OUT** (fm3 paper-check, main.tex:1874): it refines the paper's realisability `r≤min(d⃗)` (NON-strict ≤) to exclude the prod≡0 case the combinatorial lambdaCore can't see — sound + consistent within Aoyagi's realisable domain, but NOT a verbatim paper assumption (my earlier "Aoyagi-faithful" CORRECTED to "our carve-out"). Strengthens hr:∀s,r≤H_s. **THREADED (crux2 @081cf51): `hMid : ∀ s:Fin L, 0<s.val → r<H s.castSucc` added ONCE to all 5 spine theorems** (aoyagi_learning_coefficient, deepest_point_reduction, product_reduction, deepest_regular_core_normal_form, rlctAt_deepest_le_of_optimal); build green, sorry count unchanged; headline no longer false-as-stated. fm3 threads R1's resolution_charts to CONSUME the shared hMid (not re-add). REMAINING: the small shared discharge `hMid ⟹ hGne` (interior M_s≥1 ⟹ prod_M ≢ 0 ⟹ dlnLoss M 0 ≢ 0 a.e., zero-locus measure-zero) — one lemma serves R1+L2; ownership crux2↔fm3 (crux2's natural — the named obligation is on its deepest_regular_core_reduces + it has the measure infra). A fidelity WIN. NOT an operator surface — a prominent honest caveat on "DLNs mildly singular." (Optional: DLN-preprint deep-read on Aoyagi's genericity.)
...
- g150 gauge chart: g150-fix @50251296 (T̃) → g172 @f8ea2eb (per-layer Schur S_s) → **origin/g174-core-precision @3e817ef** ← canonical (literal core = FULL-product Schur ‖R‖², unit-peeled to ‖∏S_s‖² = dlnLoss M 0; the per-layer-∏ form was itself unit-peeled, not literal)

**RECURRING PRECISION LESSON (unit-peel → unit-squeeze, never exact — hold for future core-object specs):** THREE times the honest loss-core object was reached by peeling a BOUNDED UNIT, and the comparability was a unit-SQUEEZE (c₁<c₂, tight at w0), NEVER an exact germ: (i) #48 "exact germ" → squeeze (cross-term couples reg+core); (ii) g150 per-layer UNIT T̃_s=T(I−VY)⁻¹ → per-layer SCHUR S_s; (iii) g172 ∏S_s → FULL-product ‖R‖² (R=∏S_s·g, g a bounded unit). PATTERN: don't write the unit-peeled reduced form as the LITERAL core — the literal core carries a bounded unit, peeled by `rlctAtOn_unit_invariant_aux` (the NON-MP machinery), comparability c₁<c₂. ⟹ `ofExactGerm` (c₁=c₂=1) is UN-DISCHARGEABLE in raw coords (exact only via a Morse c-o-v, not what's built) — a TRAP; use the genuine squeeze constructor.
