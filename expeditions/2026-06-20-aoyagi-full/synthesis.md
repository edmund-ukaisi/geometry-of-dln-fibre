...
**LIVE (2026-06-27, UPDATE-139):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **ROUTE 2a RATE leg LANDED on (2,2,2) (UPDATE-139): the certificate's headline CONFIRMED in Lean** (one-line `routeMCore_phiGen` at full-rank `B_det222`, multi-boundary, S2-free). Det leg = a mechanical `(P) i j` cast residual (not a wall). **IN FLIGHT (`option-c-chart`, RESUMED):** dedupe the `t222` clash + full green-gate; Codex the cast residual → `φ_det_eq` → `cov` → `nodeChart222` → atom for (2,2,2); rest → ∀M lift. decoder-fix idle/on-call.
