...
**LIVE (2026-06-27, UPDATE-133):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **Brick (a) DONE** (ARCH-1, `ebccb9a1`): the alignment dissolved by CLE cancellation; the bridge is a Params-level `funext s`. **WALL surfaced at bricks b/c/d (UPDATE-133):** the decoder's det isn't the radial blow-up `|x p|^{minAdm−1}` (latent gap — the decoder's det was never exercised). **IN FLIGHT (`decoder-fix` pen-and-paper):** adjudicate the decoder fix (A redefine / B separate radial factor) + certify the corrected chart (rate + det) on the (2,2,2) layered anchor. Then the formaliser builds (c)/(d) against the fixed decoder → `nodeChartGeneral (hpos)` → atom ∀M. The ARCH-1 spine + role engine + rate + funext-s technique all SURVIVE the fix.
