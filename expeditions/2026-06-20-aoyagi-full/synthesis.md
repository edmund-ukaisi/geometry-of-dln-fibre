...
**LIVE (2026-06-28, UPDATE-219):** CHARGING (A), sea-rising on TWO fronts. **(1) R1-LOWER atom `routeMCore_box_diverges_achiever` ∀M** — 4-branch `NodeAchieverChart`: INTERIOR ✓ · CLEAN ✓ · SMEARED (cov ✓ route-(b) MP; atom in flight, `genm-boundary` a223…) · L=1 banked → then the 3-way+L1 assembly (WALL 1 `BoundaryClean ⟹ NoInteriorBothDrop`). **(2) R1-UPPER ladder `routeMCore_threshold_lt_top` ∀M** (`genm-upper`, just commissioned) — N1/N2a/N3a/N3b ✓; N2b Schur-split first (de-risked, vacuity-gated), then the (3,3,4) depth-2 recStep, then ∀M N4. R1 closes when both legs land (`rlct = ½·minAdm`).
