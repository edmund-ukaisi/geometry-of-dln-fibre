...
**LIVE (2026-06-28, UPDATE-240):** CHARGING (A) — all three legs in their FINAL unit, each on a fresh-context tide, no open design/soundness risk except 3289. **R1-UPPER carving:** EVERY ingredient proven + probe-confirmed on `origin/genm-firing @ e2941aa1` (green, canonical cover, the zEG carve + all readbacks + the Sc de-shift identity `= matOf M − bgShiftG v`). SOLE remaining = the `schurRatioResidGen_mid` capstone (~80-100-line PURE WIRING; one intricacy = the N2b↔stepShearG row-0 expansion) → fresh tide **`genm-assemble`** (genm-firing depth-checkpointed at the maximal de-risked state + on-call). Aggregation PRE-STAGED (#143: e2→e2flatD2 + 15-module order). Closing the capstone = R1-UPPER math done. **R1-LOWER (1,1):** chart substrate + U-foundation bedrock; SOLE remaining = the subBox/divergence assembly (Tonelli + `∫|u_p|^{−2c'}=⊤` + the rest-witness containment) → `genm-subbox` (#140-142). **L2 bridge:** real Ψ + S3 + the lens-decomposition linchpin + the S4 toolkit banked; SOLE remaining = the 3 leaves S4→S2→S6 (the W⁻¹ deriv-0 eventuallyEq + the joint-radius bump re-key [approved] + the S6 LDU-energy geometric leaf) → fresh tide **`genm-l2leaves`** (genm-l2cle depth-checkpointed + on-call). **D1** wires on the bridge. Headline = A2 ▸ D1-via-bridge ▸ R1; general-L gated only on **3289** (#120).
