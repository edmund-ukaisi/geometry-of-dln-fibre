...
**LIVE (2026-06-26, UPDATE-128):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M` (a flat full-ambient map: RATE `routeMCore (φ u) = u²·V` + DET `|det Dφ| = ∏_j |u_j|^{leafH j}`).
...
- **IN FLIGHT (a3901f84, worktree):** item-3 = the map equality `composeFold fs = phiFlatStruct` (the factor list from the role-split CLEs + the entry-wise `chainA`-layer stage-induction over opaque widths — the one genuinely-deep build left) → item-4 leafH summation → **`phiFlat_abs_det`** (det side COMPLETE ∀M).
