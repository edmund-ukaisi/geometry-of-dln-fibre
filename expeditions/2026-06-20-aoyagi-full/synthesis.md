...
- **KC1 step-2 architecture = (A)** [CORRECTED same tick from an initial (B) — the reversal is MATH-FORCED, verified in-code]. The invertible-leading-block conjunct goes ON `IsDeepLayers` (appended LAST — existing projections stable, only the 3 final-conjunct sites shift) + an `htop` alignment hyp on `deepestPoint_exists`/`deepestPoint`, proven in the r>0 branch via the rank argument (top-r-rows(B')=top-r-rows(U)·V, V full-row-rank ⟹ top-r-rows(U) invertible). **Why (A) not (B):** the proof NEEDS U,V (`P⁻¹·embM`/`projM·Q⁻¹`), which are PROOF-LOCAL to `deepestPoint_exists` (Skeleton:936-7) — the opaque `deepestPoint` can't reach them, so a standalone (B) lemma is dead. My (B) "don't mislabel IsDeepLayers" objection DISSOLVED on inspection: `IsDeepLayers` has NO general-B characterization usage — it's ONLY ever the constructed deepest-point's spec (`deepestPoint_isDeep`/`IsDeepLayers.2.x`), so strengthening it mislabels nothing. And the producer ALREADY plans this exact route (`DeepestGaugeConstruction:2906-2911`: "strengthen IsDeepLayers/deepestPoint_exists … bundle-wide"). genm-l2 was right; updated. **Soundness watch (in diff):** producer 2908 calls it "always achievable (a11_check.py)" but it's htop-CONDITIONAL (Q1 witness) — reconcile that a11_check meant POST-WLOG, and verify the htop thread is consistent end-to-end (headline WLOG → deepestPoint_exists htop → conjunct → 2915). block_elimination/:337 byte-untouched. genm-l2 building; diff BEFORE commit (STOP on any hSigma/:337 edit).
