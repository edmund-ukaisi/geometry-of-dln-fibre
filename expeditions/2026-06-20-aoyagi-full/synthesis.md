...
**LIVE (2026-06-28, UPDATE-244):** CHARGING (A) — all three legs in their FINAL unit, each on a fresh-context tide, no open design/soundness risk except 3289. **R1-UPPER carving:** EVERY ingredient proven + probe-confirmed on `origin/genm-firing @ e2941aa1` (green, canonical cover, the zEG carve + all readbacks + the Sc de-shift identity `= matOf M − bgShiftG v`). SOLE remaining = the `schurRatioResidGen_mid` capstone (~80-100-line PURE WIRING; the live intricacy = the Fin-index reconcile `⟨1+a⟩`[N2b Sc] vs `⟨a+1⟩`[carve readbacks]) → **`genm-firing`** finishing it (RESUMED — holds the full readback context + identified that intricacy; gated: MANDATORY forced #print axioms per increment + valve). `genm-capstone` = fallback if the valve trips; genm-assemble stood down. Aggregation PRE-STAGED (#143: e2→e2flatD2 + 15-module order). Closing the capstone = R1-UPPER math done. **R1-LOWER (1,1):** chart substrate + U-foundation bedrock; SOLE remaining = the subBox/divergence assembly (Tonelli + `∫|u_p|^{−2c'}=⊤` + the rest-witness containment) → `genm-subbox` (#140-142). **L2 bridge:** real Ψ + S3 + the lens-decomposition linchpin + the S4 toolkit banked; the W⁻¹-1 deriv-0 crack is now LANDED (genm-l2cle @5d3184a6 — the one conceptually-new S4 piece); S4 remaining = PURE MECHANICAL ASSEMBLY (~250-400 LoC) → **`genm-l2cle`** continuing S4→S2 (has the latest + full toolkit; gated). genm-l2leaves stood down (was on the stale 41fbce4e — would re-derive the crack; Item-52). **S6** (LDU-energy geometric leaf) = a later dedicated tide. **D1** wires on the bridge. Headline = A2 ▸ D1-via-bridge ▸ R1; general-L gated only on **3289** (#120).
