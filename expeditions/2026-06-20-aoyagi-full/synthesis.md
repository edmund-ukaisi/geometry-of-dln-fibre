...
**LIVE (2026-06-28, UPDATE-232):** CHARGING (A), four legs converging — **all three legs volume-not-wall, no open design/soundness risk except 3289**; each leg's prereqs are now bedrock, with ONE final standalone/coupled unit each, all on fresh-context tides. **R1-UPPER carving — TRIPLE-LINEAGE RESOLVED, ~1 SORRY FROM DONE on ONE canonical branch:** genm-firing CONSOLIDATED genm-carving's full machinery onto its branch — `origin/genm-firing @ e3dfe9cf` now has ALL of it (resolvedShiftRG_le, cellOfG, bgShiftG, stepShearG, RmatGnorm, the reshape matToProdG/coreJoinG, a.e.-pos, slotMatG_spec) **+ the canonical cover**, builds GREEN, ONE real sorry (`schurRatioResidGen_mid` @1249) — verified. `genm-assemble` (re-based onto e3dfe9cf) CLOSES that single sorry via the banked Codex recipe (`genM-final-assembly-answer.md`; K=max 1(r·T) radius fix; thread the Fin((r-1)+1)-vs-r boundary). Canonical cover already on the branch ⟹ NO separate cover-reconcile. → closing it = R1-UPPER math done. **R1-LOWER (1,1):** chart substrate + U-foundation bedrock (rate/MP/connection/embedding/flat-rate `routeMCore_phiSm_offpole`/`frontU`). Remaining = the subBox/divergence assembly (Tonelli factor + `∫|u_p|^{−2c'}=⊤` + the rest-witness containment trick) → fresh tide `genm-subbox` (base genm-smeared-build @ e3a71a0d; Codex design banked; genm-smeared-build on-call). **L2 bridge:** real Ψ + S3 axiom-clean (`8b5645cf`); linchpin `psiSplitDeltaL2Core_eq_payload` landed (reduces S2/S4 to payload-smoothness through the CLEs); re-key `cutoffBumpSplit` to a joint-unit radius (3 dets ≠ 0) APPROVED → S2 → S4 → S6 (`genm-l2cle`). **D1** wires on the bridge. Headline = A2 ▸ D1-via-bridge ▸ R1; general-L gated only on **3289** (#120).
