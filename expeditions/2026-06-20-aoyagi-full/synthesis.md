...
1. **hGne** (L2+R1) — RESOLVED → **STATED headline non-degeneracy hypothesis** (fm3 verdict (b), FORCED): interior M_s=H_s−r=0 ⟹ prod_M≡0 ⟹ dlnLoss M 0≡0 ⟹ RLCT=⊤, but lambdaCore M finite ⟹ the R1 identity is LITERALLY FALSE there; the domain MUST exclude it (forced by the formula's correctness). **Headline gains `∀ interior s (0<s<L), r<H_s` (interior M_s≥1)** — fenced as **OUR FORMALISATION CARVE-OUT** (fm3 paper-check, main.tex:1874): it refines the paper's realisability `r≤min(d⃗)` (NON-strict ≤) to exclude the prod≡0 case the combinatorial lambdaCore can't see — sound + consistent within Aoyagi's realisable domain, but NOT a verbatim paper assumption (my earlier "Aoyagi-faithful" CORRECTED to "our carve-out"). Strengthens hr:∀s,r≤H_s; threaded through aoyagi_learning_coefficient + deepest_point_reduction + L2 (crux2) + R1 (fm3). Endpoints s=0,L may have H=r. A fidelity WIN (headline-as-stated was false for interior H_s=r). NOT an operator surface — but a prominent, honestly-framed caveat on "DLNs mildly singular" in the final deliverable. (Optional, not a gate: deep-read the DLN preprint to pin whether Aoyagi's genericity implicitly excludes it.)
2. **D1(a) deepest=min-rlct** — **CORE PROVEN IN LEAN** (crux2 @152ef0d, zero sorries, clean-three): L1-a `rlctAtOn_ray_scaling_invariant` + L1-b `rlctAtOn_lsc_at_origin` + `deepest_le_of_homogeneous_core`. So "deepest = min-core" is LEAN-REAL (proven, NOT cert-claimed), value-free, NO Aoyagi Thm 2, NO heavy primitive — **the critical citable-constraint risk is fully dissolved + GREEN; only-S2 HOLDS.** (P1/L1-b was ~10 lines from the team's own rlctAt def; g160's "heavy P1" overestimate fully corrected.) Keying = ⨅ (faithful, Skeleton:1505). REMAINING (NOT small — crux2's honest trace resolved the non-rank-exact-v question): the full-B bridge core→`rlctAt_deepest_le_of_optimal` NEEDS **L2-at-GENERAL-v** (rlctAt(v)=n_v/2+rlctAtOn(core_v) for general v) — the non-homogeneous full-B loss can't be ray-dominated directly (L1-a is core/B=0-only), so reduce EACH v to its core via L2 then apply core-P1; n_v=n on the fibre (r constant). L2-at-NON-rank-exact-v = the **broader homogeneous-residual split** (Morse-leading, #112) = task **#62** (design-first via pp2 → extends cobuild-sub34's deepest chart to arbitrary v; OR a cheaper direct lower-bound rlctAt(v)≥rlctAt(deepest) if pp2 finds one). NOT a 2nd citation / new primitive, but substantial. crux2 building the bridge SKELETON (core-P1 + constant-shift + L2-at-general-v named obligation). The critical core-P1 (deepest=min) is PROVEN; #62 lifts it to full-B.
...
- g150 gauge chart: origin/g150-gauge-chart-fix @50251296 (T̃ form) → **per-layer-Schur correction origin/g172-perlayer-schur-fix @f8ea2eb** ← canonical (per-layer core = Schur S_s, NOT the (I−VY)⁻¹ unit gloss)
