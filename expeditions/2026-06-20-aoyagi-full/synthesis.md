...
**LIVE (2026-06-27, UPDATE-135):** CHARGING (A), sea-rising. **Critical path = the R1-lower atom `routeMCore_box_diverges_achiever` ∀M**, reduced M-agnostically to building `NodeAchieverChart M (hpos : 1 ≤ minAdm M)` (ONE chart `phi := phiFlatStructV` with BOTH: RATE `routeMCore (φ u) = (x p)²·U` [`leaf_integrand`] + DET `|det Dφ| = ∏_j |u_j|^{leafH j}` [`cov`]).
...
- **KC1 RESOLVED + verified (UPDATE-135): `active.card = minAdm` is DEFINITIONAL** (`minAdm = Mval = ∑ r_j c_j`); NOT a wall. KC2's pack banked-reusable. The (C) chart is design-complete. **IN FLIGHT (`option-c-chart`, fresh worktree):** build `φ_M = Q_M ∘ pivotBlowupOn(active,p) ∘ layer-ops` — validate-small on (2,2,1) first, then `nodeChartGeneral (hpos)` → discharge `routeMCore_box_diverges_achiever` ∀M. Traps: Aoyagi AFTER-width for actives; verify `e_M` is a genuine Equiv.
