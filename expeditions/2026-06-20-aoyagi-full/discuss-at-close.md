...
- **Status:** cornrev is verifying "elementary route genuinely fails ⟹ Aoyagi §5 needed" (both-conclusions-withheld). If it REFUTES (finds an elementary route), there's no decision — we build it. If it CONFIRMS, your call on (A) vs (B). I'll surface the crisp decision with cornrev's verdict; no action needed from you yet unless you want to pre-state a preference. My lean: given the charter's build-default but the genuine scale of Aoyagi §5's resolution, (B) cite-with-a-precisely-scoped-interface may be the honest+proportionate call — but it's yours.
