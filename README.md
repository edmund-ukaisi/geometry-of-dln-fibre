...
cordon-accounted `@[cited]` monuments (`scripts/cordon` + the batched `#assert_banked_clean_batch` root
gate — run both via `scripts/cordon-all`: `CITED=3` — Watanabe-upper, Aoyagi-lower, and the local ζ-continuation). Legend: ✅ Proved · 🟡 Proved (scoped beyond the paper) · 🔵 Cited (an explicit
