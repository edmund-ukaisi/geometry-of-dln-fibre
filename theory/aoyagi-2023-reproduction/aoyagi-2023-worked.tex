...
$\rlct_{w^\ast}(G_1^2+\cdots+G_m^2)\le\rlct_{w^\ast}(F_1^2+\cdots+F_n^2)$.
...
% =====================================================================
\begin{enumerate}
\item[(R-1)] \textbf{Lemma 1 direction --- reproduction typo, corrected 2026-07-23 (front-matter
image pass).} The reproduction printed $\rlct(\sum G^2)\ge\rlct(\sum F^2)$ for $G\in J$; the source
(Aoyagi Lemma~1, p.5) and the correct direction are $\le$ (pointwise $\sum G^2\le c\sum F^2$
$\Rightarrow$ the more-singular $G$-side has the \emph{smaller} threshold; e.g.\
$\rlct(x^4)=\tfrac14\le\rlct(x^2)=\tfrac12$). The Lean object was already correct
(\texttt{rlctAt\_sumSqFam\_le\_of\_germRepresents}); only the reproduction statement line (166) was
flipped, and its own fnote (line 174) already carried the correct domination. \emph{Status note
(supersedes a header edit --- anchor-freeze):} with this front-matter pass (Def~1 / Lemma~1
(corrected) / Def~2 / monomial rule / Thm~1 / Def~3 / Thm~2 / Lemma~2 / Thm~3 / Thm~4, pp.5--14,
2026-07-23) the \textbf{full paper pp.1--31 is image-cross-checked}; the prior passes covered
pp.8, 14--22, 24--26.
